...
function renderGroupedPLCF(which, cv) {
 ...
 function zeroArr() { return new Array(PM()).fill(0); }
 function sumRows(rows) {
   const out = zeroArr();
   rows.filter(Boolean).forEach(arr => {
 ...
 function signedAdjArrCF(ac) {
   const sign = ac.adj && ac.adj.sign === 'add' ? 1 : -1;
   return (ac.arrCF || ac.arr || zeroArr()).map(v => Math.abs(Number(v) || 0));
 }
 function customArrsFor(sections, onlyCF) {
   return customLines
 ...
 const cfInCustomRows = customArrsFor(['income_section','cf_after_inflows'], true);
 const cfOutCustomRows = customArrsFor(['cogs_section','cf_after_outflows'], true);
 const cfAdjIncomeRows = cv.adjIncomeRows.map(ac => ({ ac, arr: (ac.arrCF || ac.arr || zeroArr()).map(v => (ac.adj.sign === 'add' ? 1 : -1) * (Number(v)||0)) }));
 const cfAdjCostRows = cv.adjCostRows.map(ac => ({ ac, arr: (ac.arrCF || ac.arr || zeroArr()).map(v => Math.abs(Number(v)||0)) }));
 ...
 const vatNet = (cv.vatIn || zeroArr()).map((v, i) => v - ((cv.vatOut || zeroArr())[i] || 0));
 ...
 const plCustom = (section) => customLinesFor(section, cv, mkRow);
 ...
 function plSeg(id) {
   switch(id) {
     ...
     case 'pl_adj_inc': return (cv.adjIncomeRows || []).map(ac => mkRow((ac.adj.name||'Adj')+' ('+ac.adj.pct+'%)', signedAdjArr(ac), {ind:true,color:'amber'})).join('');
     case 'pl_total_income': return plCustom('income_section') + mkRow('Total Income', cv.totalIncome || zeroArr(), {bold:true, color:'teal'});
     case 'pl_cogs_sec': return mkSec('COGS', 'var(--th-cogs)');
     case 'pl_cost_cats': return (cv.userCats || []).map(cat => mkRow(cat, ((cv.costCat || {})[cat] || zeroArr()).map(v=>-v), {ind:true})).join('');
     case 'pl_contingency': return mkRow('Contingency ('+CFG.con+'%)', ((cv.costCat || {})['Contingency'] || zeroArr()).map(v=>-v), {ind:true});
     case 'pl_adj_cost': return (cv.adjCostRows || []).map(ac => mkRow((ac.adj.name||'Adj')+' ('+ac.adj.pct+'%)', (ac.arr || zeroArr()).map(v=>-(ac.adj.sign==='add'?v:-v)), {ind:true,color:'amber'})).join('');
     case 'pl_total_cogs': return plCustom('cogs_section') + mkRow('Total COGS', (cv.totalCogsAdj || zeroArr()).map(v=>-v), {bold:true});
     case 'pl_gross_profit': return mkRow('Gross Profit', cv.grossProfit || zeroArr(), {bold:true, color:'blue'}) + plCustom('after_gross_profit');
     case 'pl_bond_sec': return mkSec('Bond Fees', 'var(--th-bond)');
     case 'pl_cb_fee': return mkRow('Compliance Bond Fee', (cv.finExpCB || zeroArr()).map(v=>-v), {ind:true, color:'amber'});
     case 'pl_ub_fee': return mkRow('Upfront Bond Fee', (cv.finExpUB || zeroArr()).map(v=>-v), {ind:true, color:'amber'});
     case 'pl_profit_bopex': return mkRow('Profit before OPEX / Interest', cv.profitBeforeOpex || zeroArr(), {bold:true, color:'blue'}) + plCustom('after_profit_b_opex');
     case 'pl_opex_sec': return mkSec('OPEX & Interest', 'var(--th-opex)');
     case 'pl_opex_buf': return mkRow('OPEX Buffer ('+(CFG.opxbuf??30)+'% of Revenue)', (cv.opexBuffer || zeroArr()).map(v=>-v), {ind:true, color:'amber'});
     case 'pl_hedging': return mkRow('Hedging Adjustment (monthly)', (cv.hedgingArr || zeroArr()).map(v=>-v), {ind:true});
     case 'pl_fin_cost': return mkRow('Financing Cost (on neg. CF)', (cv.finCostArr || zeroArr()).map(v=>-v), {ind:true, color:'amber'});
     case 'pl_ebit': return plCustom('after_ebit') + mkRow('EBIT', cv.ebit || zeroArr(), {bold:true, color:'blue'});
     case 'pl_ebitda': return mkRow('EBITDA', cv.ebitda || zeroArr(), {bold:true, color:'teal'});
     default: return '';
   }
 }
 ...
 function cfSeg(id) {
   switch(id) {
     ...
     case 'cf_adj_inc': return cfAdjIncomeRows.map(({ac,arr}) => mkRow((ac.adj.name||'Adj')+' ('+ac.adj.pct+'%)', arr, {ind:true,color:'amber'})).join('');
     case 'cf_total_in': return cfInCustomRows.map(({line,arr}) => mkRow(line.name||'Custom', arr, {ind:true,color:line.color||'amber'})).join('') + mkRow('Total Inflows', totalInflowArr, {bold:true});
     ...
     case 'cf_adj_cost': return cfAdjCostRows.map(({ac,arr}) => mkRow((ac.adj.name||'Adj')+' ('+ac.adj.pct+'%)', arr, {ind:true,color:'amber'})).join('');
     ...
     case 'cf_total_out': return cfOutCustomRows.map(({line,arr}) => mkRow(line.name||'Custom', arr, {ind:true,color:line.color||'amber'})).join('') + mkRow('Total Outflows', totalOutflowArr, {bold:true});
     case 'cf_net_sec': return mkSec('NET CASH FLOW FROM OPERATIONS', 'var(--th-bond)');
     case 'cf_net_cf': return mkRow('NET CASH FLOW From OPERATIONS', netCFArr, {bold:true,color:'blue'});
     case 'cf_accum': return mkRow('Accumulated cash flow', accumCFArr, {bold:true,color:'purple', acc:true});
...
function renderDashboard() {
 ...
 const dashNetCF = dashInflow.map((v,i)=>v-dashOutflow[i]);
 const dashAccumCF = (() => { let a=0; return dashNetCF.map(v=>{ a+=v; return a; }); })();
