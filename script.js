...
function getCFRevArr(item) {
 ...
 // If no collection schedule defined  revenue CF = accrual (same timing)
 if (!payKeys.length) return accrual;
...
function fmt(v, blankZero) {
 ...
 if ((blankZero !== false) && Math.abs(v) < 0.005) return '';
...
function renderFormulaPanel() {
 ...
 function liveVal(id) {
 if (!cv) return null;
 const f2 = v => isNaN(v)||!isFinite(v) ? '' : '$' + fmt(v);
 switch(id) {
...
async function loadPortfolio() {
 try {
  ...
  portfolio = (data || []).map(normalizeProjectFromRow);
...
async function saveProjectToSupabase(project) {
 const row = projectToSupabaseRow(project);
 let result;
 ...
 return normalizeProjectFromRow(result.data);
}
...
renderPortfolio();
/* =====================================================================
   STABILITY HOTFIX — formulas, AI indicators, cost summary, UX
   ===================================================================== */
(function(){
  // ---------- Generic formula engine with human-friendly aliases ----------
  function _fmEscapeRegExp(s){ return String(s).replace(/[.*+?^${}()|[\]\\]/g, '\\$&'); }
  function _fmAddAlias(vars, name, value){
    if (!name) return;
    ...
    const forms = new Set([
      ...
      raw.replace(/\s+/g,'_'), raw.toLowerCase().replace(/\s+/g,'_'),
      raw.replace(/\s+/g,''), raw.toLowerCase().replace(/\s+/g,'')
    ]);
    forms.forEach(k => { vars[k] = v; });
  }
  function _fmValueContext(cv, mi){
    const vars = {};
    const z = Z();
    const add = (n, v) => _fmAddAlias(vars, n, v);

    // Core P&L / CF line names and common spellings
    add('totalIncome', (cv.totalIncome||z)[mi]); add('Total Income', (cv.totalIncome||z)[mi]);
    add('revenue', (cv.totalIncome||z)[mi]); add('income', (cv.totalIncome||z)[mi]);
    add('totalCogs', (cv.totalCogsAdj||z)[mi]); add('Total COGS', (cv.totalCogsAdj||z)[mi]); add('COGS', (cv.totalCogsAdj||z)[mi]);
    add('grossProfit', (cv.grossProfit||z)[mi]); add('Gross Profit', (cv.grossProfit||z)[mi]); add('GP', (cv.grossProfit||z)[mi]);
    add('profitBeforeOpex', (cv.profitBeforeOpex||z)[mi]); add('Profit before OPEX', (cv.profitBeforeOpex||z)[mi]); add('Profit before OPEX Interest', (cv.profitBeforeOpex||z)[mi]);
    add('ebit', (cv.ebit||z)[mi]); add('EBIT', (cv.ebit||z)[mi]);
    add('ebitda', (cv.ebitda||z)[mi]); add('EBITDA', (cv.ebitda||z)[mi]);
    add('opexBuffer', (cv.opexBuffer||z)[mi]); add('OPEX Buffer', (cv.opexBuffer||z)[mi]);
    add('hedging', (cv.hedgingArr||z)[mi]); add('Hedging', (cv.hedgingArr||z)[mi]); add('Hedging Adjustment', (cv.hedgingArr||z)[mi]);
    add('finCostArr', (cv.finCostArr||z)[mi]); add('financingCost', (cv.finCostArr||z)[mi]); add('Financing Cost', (cv.finCostArr||z)[mi]);
    add('accumCF', (cv.accumCF||z)[mi]); add('Accumulated CF', (cv.accumCF||z)[mi]); add('accumulatedCashFlow', (cv.accumCF||z)[mi]);
    add('netCF', (cv.netCF||z)[mi]); add('Net Cash Flow', (cv.netCF||z)[mi]);
    add('totalIncomeCF', (cv.totalIncomeCF||z)[mi]); add('Total Inflows', (cv.totalIncomeCF||z)[mi]);
    add('penalties', (cv.penArr||z)[mi]); add('Penalties', (cv.penArr||z)[mi]);
    add('contingency', ((cv.costCat||{}).Contingency||z)[mi]); add('Contingency', ((cv.costCat||{}).Contingency||z)[mi]);
    add('complianceBondFee', (cv.finExpCB||z)[mi]); add('Compliance Bond Fee', (cv.finExpCB||z)[mi]);
    add('upfrontBondFee', (cv.finExpUB||z)[mi]); add('Upfront Bond Fee', (cv.finExpUB||z)[mi]);

    // Revenue/cost categories
    Object.entries(cv.revCats || {}).forEach(([cat, arr]) => add(cat, (arr||z)[mi]));
    Object.entries(cv.costCat || {}).forEach(([cat, arr]) => add(cat, (arr||z)[mi]));

    // Individual revenue/cost items by name
    (revenues || []).forEach(it => { if (it.name) add(it.name, (getDistArr(it)||z)[mi]); });
    ([...(capex||[]), ...(opex||[])]).forEach(it => { if (it.name) add(it.name, (getDistArr(it)||z)[mi]); });

    // Project parameters
    add('fx', CFG.fx); add('FX', CFG.fx); add('pm', PM()); add('months', PM());
    add('igv', IGV); add('IGV', IGV); add('con', CFG.con); add('opxbuf', CFG.opxbuf || 0);
    add('finRate', (CFG.finRate || 0)/100); add('finMonthlyRate', (CFG.finRate || 0)/100);
    add('m', mi); add('month', mi + 1);
    return vars;
  }
  function _fmEvalExpression(rawFormula, cv, mi){
    if (!rawFormula || !String(rawFormula).trim()) return 0;
    const vars = _fmValueContext(cv, mi);
    let expr = String(rawFormula).trim().replace(/^=/,'');

    // Spanish/English friendly syntax:
    // "10% of EBIT" / "10% de Gross Profit" -> (10/100)*(EBIT)
    expr = expr.replace(/(\d+(?:\.\d+)?)\s*%\s*(?:of|de)\s*([A-Za-zÁÉÍÓÚÜÑáéíóúüñ][A-Za-zÁÉÍÓÚÜÑáéíóúüñ0-9 _&/\-]*?)(?=\s*(?:[+\-*/),]|$))/gi, function(_, pct, name){
      return '(' + (parseFloat(pct)/100) + ')*(' + name.trim() + ')';
    });
    // "EBIT x 10%" / "EBIT * 10%" -> EBIT * (10/100)
    expr = expr.replace(/(\d+(?:\.\d+)?)\s*%/g, function(_, pct){ return '(' + (parseFloat(pct)/100) + ')'; });

    Object.keys(vars).sort((a,b)=>b.length-a.length).forEach(name => {
      const safe = _fmEscapeRegExp(name);
      // Accept variable names with spaces as full phrases; avoid replacing inside words.
      expr = expr.replace(new RegExp('(?<![A-Za-z0-9_])' + safe + '(?![A-Za-z0-9_])', 'g'), String(vars[name]));
    });
    expr = expr.replace(/\bIF\s*\(/gi, 'IF(');
    try {
      const v = Function('"use strict"; const IF=(c,a,b)=>c?a:b; const ABS=Math.abs; const MAX=Math.max; const MIN=Math.min; const ROUND=Math.round; return (' + expr + ')')();
      return isFinite(v) ? v : 0;
    } catch(e) {
      console.warn('Formula error:', rawFormula, '=>', expr, e.message);
      return 0;
    }
  }

  // Override custom line formula evaluator: supports EBIT, Total COGS, "10% of ebit", Spanish/English syntax.
  window.evalCustomFormula = evalCustomFormula = function(formula, cv, monthIdx){
    return _fmEvalExpression(formula, cv || compute(), monthIdx || 0);
  };
  window.evalCustomLineArr = evalCustomLineArr = function(line, cv){
    const c = cv || compute();
    return AMs().map((_, i) => {
      const raw = _fmEvalExpression(line.formula, c, i);
      return line.sign === 'subtract' ? -Math.abs(raw) : Math.abs(raw);
    });
  };
  window._evalFormulaForId = _evalFormulaForId = function(fid, expr, cv){
    const c = cv || compute();
    return AMs().map((_, i) => _fmEvalExpression(expr, c, i));
  };

  // Debounced live preview: keeps typing fluid in formula fields.
  let _fmPreviewTimer = null;
  window._livePreviewFormula = _livePreviewFormula = function(fid, expr){
    const prev = document.getElementById('fprev-' + fid);
    if (!prev) return;
    clearTimeout(_fmPreviewTimer);
    _fmPreviewTimer = setTimeout(() => {
      if (!String(expr||'').trim()) { prev.textContent = 'Enter a formula above…'; return; }
      try {
        const results = _evalFormulaForId(fid, expr, compute());
        const samples = results.slice(0, Math.min(6, PM())).map((v,i)=>`M${i+1}: $${fmt(v,false)}`).join('  ·  ');
        prev.innerHTML = `<span style="color:var(--teal);font-weight:700">${samples}</span> <span style="color:var(--navy);margin-left:8px">Total: $${fmt(rSum(results),false)}</span>`;
      } catch(e) {
        prev.innerHTML = `<span style="color:var(--red)">Formula error: ${e.message}</span>`;
      }
    }, 120);
  ...
  // Custom line modal preview: same engine + examples that match user typing.
  window._clPreview = _clPreview = function(){
    const formula = document.getElementById('cl-m-formula')?.value?.trim() || '';
    const prev = document.getElementById('cl-m-preview');
    if (!prev) return;
    if (!formula) { prev.textContent = 'Enter a formula to preview…'; prev.style.color = 'var(--sub)'; return; }
    try {
      const arr = evalCustomLineArr({ formula, sign:'add' }, compute());
      const samples = arr.slice(0, Math.min(6, PM())).map((v,i)=>`<span style="margin-right:10px"><em style="color:var(--muted)">M${i+1}</em> <strong>$${fmt(v,false)}</strong></span>`).join('');
      prev.innerHTML = `${samples}<span style="margin-left:8px;color:var(--teal);font-weight:700">Total: $${fmt(rSum(arr),false)}</span>`;
      prev.style.color = 'var(--text)';
    } catch(e) { prev.innerHTML = `<span style="color:var(--red)">Formula error: ${e.message}</span>`; }
  };

  // ---------- Modern Cost Summary ----------
  window.renderCostSummary = renderCostSummary = function(){
    const cv = compute();
    const wrap = document.getElementById('costs-content');
    if (!wrap) return;
    const capexTotal = capex.reduce((s,it)=>s+Math.abs(getBase(it)),0);
    const opexTotal = opex.reduce((s,it)=>s+Math.abs(getBase(it)),0);
    const contingency = Math.abs(rSum((cv.costCat||{}).Contingency || Z()));
    const opexBuffer = Math.abs(rSum(cv.opexBuffer || Z()));
    const hedging = Math.abs(rSum(cv.hedgingArr || Z()));
    const fin = Math.abs(rSum(cv.finCostArr || Z())) + Math.abs(rSum(cv.finExpCB || Z())) + Math.abs(rSum(cv.finExpUB || Z()));
    const total = capexTotal + opexTotal + contingency + opexBuffer + hedging + fin;
    const groups = [
      ['CAPEX', capexTotal, '#0176D3', capex], ['OPEX', opexTotal, '#5A2D82', opex],
      ['Contingency', contingency, '#A56105', []], ['OPEX Buffer', opexBuffer, '#06A59A', []],
      ['Hedging', hedging, '#8E44AD', []], ['Financial / Bond Fees', fin, '#BA0517', []]
    ];
    const pct = v => total ? (v/total*100) : 0;
    wrap.innerHTML = `
      <style>
        .cs-modern-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px;margin-bottom:16px}
        .cs-modern-card{background:#fff;border:1px solid #e4e8ee;border-radius:14px;padding:14px 16px;box-shadow:0 8px 22px rgba(3,45,96,.06)}
        .cs-modern-card .k{font-size:10px;text-transform:uppercase;letter-spacing:.08em;color:#667085;font-weight:800;margin-bottom:6px}
        .cs-modern-card .v{font-size:20px;font-weight:900;color:#102033}
        .cs-bar{height:8px;background:#eef2f6;border-radius:99px;overflow:hidden;margin-top:10px}.cs-fill{height:100%;border-radius:99px}
        .cs-modern-table{width:100%;border-collapse:separate;border-spacing:0;background:#fff;border:1px solid #e4e8ee;border-radius:14px;overflow:hidden;box-shadow:0 8px 22px rgba(3,45,96,.06)}
        .cs-modern-table th{background:#0f2235;color:#fff;font-size:10px;letter-spacing:.07em;text-transform:uppercase;text-align:left;padding:11px 12px}.cs-modern-table td{padding:10px 12px;border-bottom:1px solid #edf0f4;font-size:12px}.cs-modern-table tr:last-child td{border-bottom:none}.cs-num{text-align:right;font-variant-numeric:tabular-nums}.cs-pill{font-size:10px;font-weight:800;border-radius:99px;padding:3px 8px;display:inline-block}
      </style>
      <div style="display:flex;justify-content:space-between;align-items:flex-end;gap:12px;margin-bottom:14px;flex-wrap:wrap">
        <div><div style="font-size:18px;font-weight:900;color:#0f2235">Cost Summary</div><div style="font-size:12px;color:#667085">Breakdown ejecutivo por componente de costo, peso relativo y referencias.</div></div>
        <div style="background:#0f2235;color:#fff;border-radius:12px;padding:10px 14px"><div style="font-size:10px;opacity:.7;text-transform:uppercase">Total Cost Base</div><div style="font-size:20px;font-weight:900">$${fmt(total,false)}</div></div>
      </div>
      <div class="cs-modern-grid">${groups.map(([k,v,c])=>`<div class="cs-modern-card"><div class="k">${k}</div><div class="v" style="color:${c}">$${fmt(v,false)}</div><div style="font-size:11px;color:#667085;margin-top:2px">${pct(v).toFixed(1)}% del total</div><div class="cs-bar"><div class="cs-fill" style="width:${Math.min(pct(v),100)}%;background:${c}"></div></div></div>`).join('')}</div>
      <table class="cs-modern-table"><thead><tr><th>Tipo</th><th>Descripción</th><th class="cs-num">Monto USD</th><th class="cs-num">Peso</th><th>Referencia</th></tr></thead><tbody>
        ${[...capex.map(it=>['CAPEX',it]), ...opex.map(it=>['OPEX',it])].map(([type,it])=>{ const v=Math.abs(getBase(it)); const color=type==='CAPEX'?'#0176D3':'#5A2D82'; return `<tr><td><span class="cs-pill" style="background:${color}18;color:${color}">${type}</span></td><td><strong>${esc(it.name||'Unnamed')}</strong><div style="font-size:10px;color:#667085">${esc(it.cat||'Other')} · ${it.cur||'USD'} ${it.igv?'· Incl. IGV':''}</div></td><td class="cs-num"><strong>$${fmt(v,false)}</strong></td><td class="cs-num">${pct(v).toFixed(1)}%</td><td>${it.refUrl?`<a href="${esc(it.refUrl)}" target="_blank">Open ref</a>`:esc(it.refNote||'—')}</td></tr>`; }).join('')}
        ${groups.slice(2).map(([k,v,c])=>`<tr><td><span class="cs-pill" style="background:${c}18;color:${c}">Auto</span></td><td><strong>${k}</strong><div style="font-size:10px;color:#667085">Calculated from model settings</div></td><td class="cs-num"><strong>$${fmt(v,false)}</strong></td><td class="cs-num">${pct(v).toFixed(1)}%</td><td>Formula-based</td></tr>`).join('')}
      </tbody></table>`;
  };

  // ---------- AI tab indicators ----------
  window.renderAIPreview = function(){
    const output = document.getElementById('ai-output');
    const result = document.getElementById('ai-result');
    const loading = document.getElementById('ai-loading');
    ...
    try {
      const snap = buildFinancialSnapshot();
      const kpis = [
        ['Total Income', '$'+fmtK(snap.totalIncome), snap.totalIncome>0?'good':'info'],
        ['Gross Margin', (snap.grossMargin||0).toFixed(1)+'%', snap.grossMargin>=20?'good':snap.grossMargin>=0?'warn':'bad'],
        ['EBIT Margin', (snap.ebitMargin||0).toFixed(1)+'%', snap.ebitMargin>=10?'good':snap.ebitMargin>=0?'warn':'bad'],
        ['Net CF', '$'+fmtK(snap.netCF), snap.netCF>=0?'good':'bad'],
        ['Min Cash Need', '$'+fmtK(snap.minCashNeed), snap.minCashNeed>0?'warn':'good'],
        ['Break-even', snap.breakEvenMo?'M'+snap.breakEvenMo:'No reached', snap.breakEvenMo?'good':'warn']
      ];
      const color = s => s==='good'?'#2E844A':s==='warn'?'#A56105':s==='bad'?'#BA0517':'#0176D3';
      output.style.display = 'block'; if (loading) loading.style.display='none';
      result.style.display = 'block';
      result.innerHTML = `<div style="background:linear-gradient(135deg,#032D60,#0176D3);border-radius:14px;padding:20px;color:#fff;margin-bottom:16px"><div style="font-size:11px;text-transform:uppercase;letter-spacing:.09em;opacity:.72;font-weight:800">AI Financial Analysis</div><div style="font-size:22px;font-weight:900;margin-top:4px">Indicadores clave del proyecto</div><div style="font-size:12px;opacity:.75;margin-top:4px">Vista automática basada en P&L y Cash Flow. Presiona “Generate Analysis” para el reporte completo.</div></div><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:12px;margin-bottom:16px">${kpis.map(([l,v,s])=>`<div class="ai-card" style="border-left:4px solid ${color(s)}"><div style="font-size:10px;text-transform:uppercase;letter-spacing:.06em;color:var(--muted);font-weight:800">${l}</div><div style="font-size:22px;font-weight:900;color:${color(s)};margin-top:4px">${v}</div></div>`).join('')}</div>`;
    } catch(e) {
      result.innerHTML = `<div class="ai-card bad"><h4>No se pudo leer el modelo</h4><p>${e.message}</p></div>`;
      result.style.display = 'block'; output.style.display = 'block'; if (loading) loading.style.display='none';
    }
  };

  // Override tab router so AI tab actually paints indicators.
  window.renderTab = renderTab = function(t){
    if (t === 'dashboard') renderDashboard();
    else if (t === 'settings') renderSettings();
    else if (t === 'revenues') renderList('revenues');
    else if (t === 'capex') renderList('capex');
    ...
    else if (t === 'ai') renderAIPreview();
  };

  // Re-bind custom line input once DOM is ready in case previous handler was attached before override.
  setTimeout(() => {
    const f = document.getElementById('cl-m-formula');
    if (f) f.addEventListener('input', _clPreview);
  }, 0);
})();

/* =====================================================================
   FINAL FINANCIAL LOGIC HOTFIX — P&L / CF totals, custom lines, formulas
   ===================================================================== */
(function(){
  const _originalCompute = compute;

  function arrClone(a){ return (a || Z()).slice(); }
  function arrAdd(a,b){ return a.map((v,i)=> (Number(v)||0) + (Number((b||[])[i])||0)); }
  function arrSub(a,b){ return a.map((v,i)=> (Number(v)||0) - (Number((b||[])[i])||0)); }
  function arrNeg(a){ return (a||Z()).map(v => -(Number(v)||0)); }
  function arrAbs(a){ return (a||Z()).map(v => Math.abs(Number(v)||0)); }
  function arrZero(){ return Z(); }
  function sumMany(arrs){ return arrs.reduce((acc,a)=>arrAdd(acc,a||Z()), Z()); }
  function money(v, blankZero){ return fmt(v, blankZero === undefined ? false : blankZero); }

  function signedAdjustmentArr(ac, cf){
    const source = cf ? (ac.arrCF || ac.arr || Z()) : (ac.arr || Z());
    return (ac.adj && ac.adj.sign === 'add') ? arrClone(source) : arrNeg(source);
  }

  function normalizeCv(cv){
    if (!cv) return cv;
    const z = Z();
    cv.userCats = Array.isArray(cv.userCats) ? cv.userCats : Object.keys(cv.costCat || {}).filter(k => k !== 'Contingency' && !String(k).startsWith('Financial'));

    const costBase = z.slice();
    cv.userCats.forEach(cat => { const a = (cv.costCat || {})[cat] || z; a.forEach((v,i)=> costBase[i] += Number(v)||0); });
    ((cv.costCat || {}).Contingency || z).forEach((v,i)=> costBase[i] += Number(v)||0);
    (cv.adjCostRows || []).forEach(ac => {
      const s = signedAdjustmentArr(ac, false);
      // Cost adjustment in P&L cost base: add = more cost, subtract = less cost
      s.forEach((v,i)=> costBase[i] += (ac.adj && ac.adj.sign === 'add') ? Math.abs(Number(v)||0) : -Math.abs(Number(v)||0));
    });
    cv.totalCogsAdj = costBase;

    const cfCostBase = z.slice();
    cv.userCats.forEach(cat => { const a = (cv.costCatCF || {})[cat] || z; a.forEach((v,i)=> cfCostBase[i] += Number(v)||0); });
    ((cv.costCatCF || {}).Contingency || z).forEach((v,i)=> cfCostBase[i] += Number(v)||0);
    (cv.adjCostRows || []).forEach(ac => {
      const source = ac.arrCF || ac.arr || z;
      source.forEach((v,i)=> cfCostBase[i] += (ac.adj && ac.adj.sign === 'add') ? Math.abs(Number(v)||0) : -Math.abs(Number(v)||0));
    });
    cv.totalCogsCFAdj = cfCostBase;

    cv.grossProfit = arrSub(cv.totalIncome || z, cv.totalCogsAdj || z);
    cv.bondFees = sumMany([cv.finExpCB || z, cv.finExpUB || z]);
    cv.profitBeforeOpex = arrSub(cv.grossProfit || z, cv.bondFees || z);
    cv.ebit = (cv.profitBeforeOpex || z).map((v,i)=>
      (Number(v)||0) - (Number((cv.opexBuffer||z)[i])||0) - (Number((cv.hedgingArr||z)[i])||0) - (Number((cv.finCostArr||z)[i])||0)
    );
    cv.ebitda = arrClone(cv.ebit); // Same structure as Excel: EBITDA references EBIT line.

    const totalOut = (cv.totalCogsCFAdj || z).map((v,i)=>
      (Number(v)||0) + (Number((cv.opexBufferCF||z)[i])||0) + (Number((cv.finExpCB||z)[i])||0) +
      (Number((cv.finExpUB||z)[i])||0) + (Number((cv.hedgingArr||z)[i])||0) + (Number((cv.finCostArr||z)[i])||0) +
      Math.max(0, Number((cv.vatToPay||z)[i])||0)
    );
    cv._totalOutflows = totalOut;
    cv.netCF = (cv.totalIncomeCF || z).map((v,i)=> (Number(v)||0) - (Number(totalOut[i])||0));
    let ac = 0; cv.accumCF = cv.netCF.map(v => { ac += Number(v)||0; return ac; });
    return cv;
  }

  window.compute = compute = function(){ return normalizeCv(_originalCompute()); };
  ...
  function getYears(){
    ...
    AMs().forEach(m => {
      const inf = mInfo(m);
      if (!seen.has(inf.year)) { seen.set(inf.year, { yi: years.length, year: inf.year, yms: [] }); years.push(seen.get(inf.year)); }
      seen.get(inf.year).yms.push(m);
    });
    return years;
  }
  function yearAgg(arr, yms, accumulated){
    if (accumulated) return arr[(yms[yms.length-1]||1)-1] || 0;
    return yms.reduce((s,m)=>s+(Number((arr||[])[m-1])||0),0);
  }
  function cellFor(arr, col, accumulated){
    if (col.type === 'month') return (arr||[])[col.m-1] || 0;
    if (col.type === 'year') return yearAgg(arr, col.yms, accumulated);
    return accumulated ? ((arr||[])[(arr||[]).length-1] || 0) : rSum(arr||[]);
  }
  function customArr(line, cv){
    try { return evalCustomLineArr(line, cv); } catch(e){ console.warn('Custom line error', line && line.name, e); return Z(); }
  }
  function customRows(section, cv, onlyCF){
    return (customLines || [])
      .filter(l => l && l.section === section && String(l.formula||'').trim() && (!onlyCF || l.showInCF))
      .map(l => ({
        type:'row', id:'cl_'+l.id, label: (l.name || 'Custom line'), arr: customArr(l, cv), ind:true, color:l.color||'amber', custom:l
      }));
  }
  function editButton(line){
    return line ? ` <button class="cl-edit-inline" data-line-id="${line.id}" title="Edit line" style="margin-left:6px;border:1px solid var(--bdr2);background:var(--alt);color:var(--accent);border-radius:4px;font-size:9px;padding:1px 6px;cursor:pointer">Edit</button>` : '';
  }
  function addLineRow(section, colCount){
    return `<tr class="add-line-row"><td colspan="${colCount+1}" style="padding:4px 8px;background:#fbfcfe;border-top:1px dashed #d7dde6;border-bottom:1px dashed #d7dde6"><button class="add-line-btn" data-section="${section}" style="width:100%;text-align:left;border:0;background:transparent;color:var(--accent);font-size:11px;font-weight:700;cursor:pointer;padding:4px 8px">+ Add calculated line here</button></td></tr>`;
  }

  function renderMatrix(tblId, which, rows){
    const yearOpen = which === 'pl' ? plYearOpen : cfYearOpen;
    const years = getYears();
    years.forEach(y => { if (yearOpen[y.yi] === undefined) yearOpen[y.yi] = (y.yi === 0); });
    const cols = [];
    years.forEach(y => { if (yearOpen[y.yi]) y.yms.forEach(m => cols.push({type:'month', m, yi:y.yi, year:y.year, yms:y.yms})); cols.push({type:'year', yi:y.yi, year:y.year, yms:y.yms, open:!!yearOpen[y.yi]}); });
    cols.push({type:'total'});
    let html = '<thead><tr><th style="min-width:230px;text-align:left;position:sticky;left:0;z-index:3;background:var(--navy)">Category</th>';
    cols.forEach(col => {
      if (col.type === 'month') { const mi=mInfo(col.m); html += `<th style="min-width:82px;background:var(--navy)"><div style="font-size:10px">${mi.short.toUpperCase()} ${String(mi.year).slice(2)}</div><div style="color:#5ba4f5;font-size:9px;font-weight:800">(M${col.m})</div></th>`; }
      else if (col.type === 'year') html += `<th class="yr-col-hdr" data-yi="${col.yi}" style="min-width:92px;background:#1a4a80;cursor:pointer"><div style="font-size:12px;font-weight:900">${col.year}</div><div style="font-size:8px;letter-spacing:.08em;opacity:.75;text-transform:uppercase">${col.open?'collapse':'expand'}</div></th>`;
      else html += '<th style="min-width:92px;background:#0a2a50"><div style="font-size:10px">TOTAL</div></th>';
    });
    html += '</tr></thead><tbody>';
    rows.forEach(row => {
      if (row.type === 'section') { html += `<tr class="sec-row"><td colspan="999" style="background:${row.color||'#032D60'}">${row.label}</td></tr>`; return; }
      if (row.type === 'add') { html += addLineRow(row.section, cols.length); return; }
      const bold = !!row.bold; const ind = !!row.ind; const color = row.color ? ' c-'+row.color : ''; const arr = row.arr || Z(); const acc = !!row.acc;
      const stickyBg = bold ? 'var(--alt)' : 'var(--surface)';
      html += `<tr class="${bold?'bold-row':''}"><td class="lbl${ind?' ind':''}${bold?' bold':''}" style="position:sticky;left:0;z-index:1;background:${stickyBg}">${row.label}${editButton(row.custom)}</td>`;
      cols.forEach(col => { const v = cellFor(arr,col,acc); const bg = col.type==='year'?'background:#f0f4f8;font-weight:700;':col.type==='total'?'background:var(--alt);':''; html += `<td class="num${bold?' bold':''}${v<0?' c-red':''}${color}" style="${bg}">${money(v,false)}</td>`; });
      html += '</tr>';
    });
    html += '</tbody>';
    const tbl = document.getElementById(tblId); if (!tbl) return; tbl.innerHTML = html;
    tbl.querySelectorAll('.yr-col-hdr').forEach(th => th.addEventListener('click',()=>{ const yi=parseInt(th.dataset.yi); if (which==='pl') plYearOpen[yi]=!plYearOpen[yi]; else cfYearOpen[yi]=!cfYearOpen[yi]; which==='pl'?renderPL():renderCF(); }));
    tbl.querySelectorAll('.cl-edit-inline').forEach(btn => btn.addEventListener('click', e => { e.stopPropagation(); const line=(customLines||[]).find(l=>String(l.id)===String(btn.dataset.lineId)); if (line) openEditLineModal(line); }));
  }

  function plRows(cv){
    const z=Z();
    const cogsChildren=[];
    (cv.userCats||[]).forEach(cat => cogsChildren.push({type:'row', label:cat, arr:arrNeg((cv.costCat||{})[cat]||z), ind:true}));
    cogsChildren.push({type:'row', label:`Contingency (${CFG.con}%)`, arr:arrNeg((cv.costCat||{}).Contingency||z), ind:true});
    (cv.adjCostRows||[]).forEach(ac=>{ const a=ac.arr||z; cogsChildren.push({type:'row', label:`${ac.adj.name||'Adj'} (${ac.adj.pct||0}%)`, arr: ac.adj.sign==='add'?arrNeg(a):arrAbs(a), ind:true, color:'amber'}); });
    cogsChildren.push(...customRows('cogs_section', cv, false));
    const totalCogsDisplay = sumMany(cogsChildren.map(r=>r.arr));

    const incomeChildren=[];
    Object.entries(cv.revCats||{}).forEach(([cat,arr])=>incomeChildren.push({type:'row', label:cat, arr, ind:true}));
    incomeChildren.push({type:'row', label:'Penalties', arr:cv.penArr||z, ind:true, color:'amber'});
    (cv.adjIncomeRows||[]).forEach(ac=>incomeChildren.push({type:'row', label:`${ac.adj.name||'Adj'} (${ac.adj.pct||0}%)`, arr:signedAdjustmentArr(ac,false), ind:true, color:'amber'}));
    incomeChildren.push(...customRows('income_section', cv, false));
    const totalIncome = sumMany(incomeChildren.map(r=>r.arr));
    const grossProfit = arrAdd(totalIncome,totalCogsDisplay);

    const afterGP = customRows('after_gross_profit', cv, false);
    const bondChildren=[
      {type:'row', label:'Compliance Bond Fee', arr:arrNeg(cv.finExpCB||z), ind:true, color:'amber'},
      {type:'row', label:'Upfront Bond Fee', arr:arrNeg(cv.finExpUB||z), ind:true, color:'amber'},
      ...afterGP
    ];
    const profitBefore = arrAdd(grossProfit, sumMany(bondChildren.map(r=>r.arr)));
    const afterProfit = customRows('after_profit_b_opex', cv, false);
    const opexChildren=[
      {type:'row', label:`OPEX Buffer (${CFG.opxbuf??30}% of Revenue)`, arr:arrNeg(cv.opexBuffer||z), ind:true, color:'amber'},
      {type:'row', label:'Hedging Adjustment (monthly)', arr:arrNeg(cv.hedgingArr||z), ind:true},
      {type:'row', label:'Financing Cost (on neg. CF)', arr:arrNeg(cv.finCostArr||z), ind:true, color:'amber'},
      ...afterProfit
    ];
    const ebit = arrAdd(profitBefore, sumMany(opexChildren.map(r=>r.arr)));
    const afterEbit = customRows('after_ebit', cv, false);
    const ebitda = ebit.slice();

    return [
      {type:'section',label:'INCOME',color:'var(--th-income)'}, ...incomeChildren, {type:'add',section:'income_section'}, {type:'row',label:'Total Income',arr:totalIncome,bold:true,color:'teal'},
      {type:'section',label:'COGS',color:'var(--th-cogs)'}, ...cogsChildren, {type:'add',section:'cogs_section'}, {type:'row',label:'Total COGS',arr:totalCogsDisplay,bold:true},
      {type:'row',label:'Gross Profit',arr:grossProfit,bold:true,color:'blue'}, {type:'add',section:'after_gross_profit'},
      {type:'section',label:'BOND FEES',color:'var(--th-bond)'}, ...bondChildren.filter(r=>!afterGP.includes(r)), ...afterGP, {type:'row',label:'Profit before OPEX / Interest',arr:profitBefore,bold:true,color:'blue'}, {type:'add',section:'after_profit_b_opex'},
      {type:'section',label:'OPEX & INTEREST',color:'var(--th-opex)'}, ...opexChildren, {type:'row',label:'EBIT',arr:ebit,bold:true,color:'blue'}, ...afterEbit, {type:'add',section:'after_ebit'}, {type:'row',label:'EBITDA',arr:ebitda,bold:true,color:'teal'}
    ];
  }
  ...
  function cfRows(cv){
    ...
    Object.entries(cv.revCatsCF||{}).forEach(([cat,arr])=>inflowChildren.push({type:'row',label:cat,arr,ind:true}));
    inflowChildren.push({type:'row',label:'Penalties',arr:cv.penArrCF||z,ind:true,color:'amber'});
    inflowChildren.push({type:'row',label:'VAT Inflow',arr:cv.vatIn||z,ind:true,color:'teal'});
    (cv.adjIncomeRows||[]).forEach(ac=>inflowChildren.push({type:'row', label:`${ac.adj.name||'Adj'} (${ac.adj.pct||0}%)`, arr:signedAdjustmentArr(ac,true), ind:true, color:'amber'}));
    inflowChildren.push(...customRows('income_section', cv, true), ...customRows('cf_after_inflows', cv, true));
    const totalIn = sumMany(inflowChildren.map(r=>r.arr));

    const outflowChildren=[];
    (cv.userCats||[]).forEach(cat=>outflowChildren.push({type:'row',label:cat,arr:(cv.costCatCF||{})[cat]||z,ind:true}));
    outflowChildren.push({type:'row',label:'Contingency',arr:(cv.costCatCF||{}).Contingency||z,ind:true});
    (cv.adjCostRows||[]).forEach(ac=>{ const a=ac.arrCF||ac.arr||z; outflowChildren.push({type:'row',label:`${ac.adj.name||'Adj'} (${ac.adj.pct||0}%)`,arr: ac.adj.sign==='add'?arrAbs(a):arrNeg(a),ind:true,color:'amber'}); });
    outflowChildren.push(...customRows('cogs_section', cv, true));
    outflowChildren.push({type:'row',label:`OPEX Buffer (${CFG.opxbuf??30}%)`,arr:cv.opexBufferCF||z,ind:true,color:'amber'});
    outflowChildren.push({type:'row',label:'Compliance Bond Fee',arr:cv.finExpCB||z,ind:true,color:'amber'});
    outflowChildren.push({type:'row',label:'Upfront Bond Fee',arr:cv.finExpUB||z,ind:true,color:'amber'});
    outflowChildren.push({type:'row',label:'Hedging',arr:cv.hedgingArr||z,ind:true,color:'purple'});
    outflowChildren.push({type:'row',label:'Financing Cost',arr:cv.finCostArr||z,ind:true,color:'amber'});
    outflowChildren.push({type:'row',label:'VAT Outflow',arr:cv.vatOut||z,ind:true,color:'red'});
    outflowChildren.push({type:'row',label:'VAT NET to pay to SUNAT',arr:cv.vatToPay||z,ind:true});
    outflowChildren.push(...customRows('cf_after_outflows', cv, true));
    const totalOut = sumMany(outflowChildren.map(r=>r.arr));
    const net = totalIn.map((v,i)=>(Number(v)||0)-(Number(totalOut[i])||0));
    let ac=0; const accum=net.map(v=>{ac+=(Number(v)||0); return ac;});
    const netCustom = [...customRows('after_gross_profit', cv, true), ...customRows('after_profit_b_opex', cv, true), ...customRows('after_ebit', cv, true)];

    return [
      {type:'section',label:'INFLOWS',color:'var(--th-cf-in)'}, ...inflowChildren.filter(r=>!customRows('cf_after_inflows',cv,true).includes(r)), {type:'add',section:'cf_after_inflows'}, {type:'row',label:'Total Inflows',arr:totalIn,bold:true},
      {type:'section',label:'OUTFLOWS',color:'var(--th-cf-out)'}, ...outflowChildren, {type:'add',section:'cf_after_outflows'}, {type:'row',label:'Total Outflows',arr:totalOut,bold:true},
      {type:'section',label:'NET CASH FLOW FROM OPERATIONS',color:'var(--th-bond)'}, ...netCustom, {type:'row',label:'NET CASH FLOW From OPERATIONS',arr:net,bold:true,color:'blue'}, {type:'row',label:'Accumulated cash flow',arr:accum,bold:true,color:'purple',acc:true}
    ];
  }

  window.renderPL = renderPL = function(){
    const togWrap = document.getElementById('pl-view-tog'); if (togWrap){ togWrap.innerHTML=''; const tog=buildToggle(plView, v=>{plView=v;renderPL();}); if(tog) togWrap.appendChild(tog); }
    const cv = compute();
    if (plView === 'grouped') return renderMatrix('pl-tbl','pl',plRows(cv));
    renderGroupedPLCF('pl', cv); // safe fallback
  };
  window.renderCF = renderCF = function(){
    const togWrap = document.getElementById('cf-view-tog'); if (togWrap){ togWrap.innerHTML=''; const tog=buildToggle(cfView, v=>{cfView=v;renderCF();}); if(tog) togWrap.appendChild(tog); }
    const cv = compute();
    if (cfView === 'grouped') return renderMatrix('cf-tbl','cf',cfRows(cv));
    renderGroupedPLCF('cf', cv);
  };

  // Better formula references using corrected P&L/CF lines + custom lines by name.
  function evalAliasFormula(rawFormula, cv, mi){
    if (!rawFormula || !String(rawFormula).trim()) return 0;
    const rows = [...plRows(cv||compute()), ...cfRows(cv||compute())].filter(r=>r.type==='row');
    const vars = {};
    function add(name,value){
      if(!name) return; const v=Number(value)||0; const raw=String(name).replace(/<[^>]*>/g,'').trim(); if(!raw) return;
      [raw, raw.toLowerCase(), raw.toUpperCase(), raw.replace(/\s+/g,'_'), raw.toLowerCase().replace(/\s+/g,'_'), raw.replace(/\s+/g,''), raw.toLowerCase().replace(/\s+/g,'')].forEach(k=>vars[k]=v);
    }
    rows.forEach(r=>add(r.label, (r.arr||Z())[mi]));
    add('month',mi+1); add('m',mi+1); add('fx',CFG.fx); add('IGV',IGV); add('igv',IGV); add('months',PM());
    let expr=String(rawFormula).trim().replace(/^=/,'');
    expr=expr.replace(/(\d+(?:\.\d+)?)\s*%\s*(?:of|de)\s*([A-Za-zÁÉÍÓÚÜÑáéíóúüñ][A-Za-zÁÉÍÓÚÜÑáéíóúüñ0-9 _&/().\-]*?)(?=\s*(?:[+\-*/),]|$))/gi, (_,pct,name)=>`(${parseFloat(pct)/100})*(${name.trim()})`);
    expr=expr.replace(/(\d+(?:\.\d+)?)\s*%/g,(_,pct)=>`(${parseFloat(pct)/100})`);
    Object.keys(vars).sort((a,b)=>b.length-a.length).forEach(k=>{ expr=expr.replace(new RegExp('(?<![A-Za-z0-9_])'+k.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')+'(?![A-Za-z0-9_])','g'), String(vars[k])); });
    try { const v=Function('"use strict"; const IF=(c,a,b)=>c?a:b; const ABS=Math.abs; const MAX=Math.max; const MIN=Math.min; const ROUND=Math.round; return ('+expr+')')(); return isFinite(v)?v:0; }
    catch(e){ console.warn('Formula error:', rawFormula, '=>', expr, e.message); return 0; }
  }
  window.evalCustomLineArr = evalCustomLineArr = function(line, cv){ const c=cv||compute(); return AMs().map((_,i)=>{ const raw=evalAliasFormula(line.formula,c,i); return line.sign==='subtract' ? -Math.abs(raw) : Math.abs(raw); }); };
  window.evalCustomFormula = evalCustomFormula = function(formula, cv, monthIdx){ return evalAliasFormula(formula, cv||compute(), monthIdx||0); };

  // Smooth formula input: replace the input node to remove older aggressive listeners, then attach one debounced preview.
  setTimeout(()=>{
    const old = document.getElementById('cl-m-formula');
    if (old && !old.dataset.finalHotfix){ const clone=old.cloneNode(true); clone.dataset.finalHotfix='1'; old.parentNode.replaceChild(clone, old); clone.addEventListener('input',()=>{ clearTimeout(window.__clPrevTimer); window.__clPrevTimer=setTimeout(()=>{ try{ _clPreview(); }catch(e){} },100); }); }
  },0);

  // If the current tab is visible, repaint once with corrected logic.
  try { if (document.getElementById('panel-pl')?.classList.contains('show')) renderPL(); if (document.getElementById('panel-cf')?.classList.contains('show')) renderCF(); if (document.getElementById('panel-dashboard')?.classList.contains('show')) renderDashboard(); } catch(e) { console.warn('Final repaint warning', e); }
})();

/* Formula context recursion fix — keep custom line formulas stable */
(function(){
  function _escRe(s){ return String(s).replace(/[.*+?^${}()|[\]\\]/g,'\\$&'); }
  function _addAlias(vars,name,value){
    if(!name) return; const v=Number(value)||0; const raw=String(name).replace(/<[^>]*>/g,'').trim(); if(!raw) return;
    [raw, raw.toLowerCase(), raw.toUpperCase(), raw.replace(/\s+/g,'_'), raw.toLowerCase().replace(/\s+/g,'_'), raw.replace(/\s+/g,''), raw.toLowerCase().replace(/\s+/g,'')].forEach(k=>vars[k]=v);
  }
  function _ctx(cv, mi){
    const z=Z(); const vars={}; const add=(n,v)=>_addAlias(vars,n,v);
    const totalCogsDisplay = arrNeg(cv.totalCogsAdj||z);
    const totalOut = cv._totalOutflows || (cv.totalCogsCFAdj||z).map((v,i)=>(Number(v)||0)+(Number((cv.opexBufferCF||z)[i])||0)+(Number((cv.finExpCB||z)[i])||0)+(Number((cv.finExpUB||z)[i])||0)+(Number((cv.hedgingArr||z)[i])||0)+(Number((cv.finCostArr||z)[i])||0));
    add('Revenue',(cv.totalIncome||z)[mi]); add('Income',(cv.totalIncome||z)[mi]); add('Total Income',(cv.totalIncome||z)[mi]); add('totalIncome',(cv.totalIncome||z)[mi]);
    add('Penalties',(cv.penArr||z)[mi]);
    add('Total COGS',totalCogsDisplay[mi]); add('totalCogs',totalCogsDisplay[mi]); add('COGS',totalCogsDisplay[mi]);
    add('Gross Profit',(cv.grossProfit||z)[mi]); add('grossProfit',(cv.grossProfit||z)[mi]); add('GP',(cv.grossProfit||z)[mi]);
    add('Compliance Bond Fee',-(cv.finExpCB||z)[mi]); add('Upfront Bond Fee',-(cv.finExpUB||z)[mi]); add('Bond Fees',-((cv.finExpCB||z)[mi]||0)-((cv.finExpUB||z)[mi]||0));
    add('Profit before OPEX / Interest',(cv.profitBeforeOpex||z)[mi]); add('profitBeforeOpex',(cv.profitBeforeOpex||z)[mi]);
    add('OPEX Buffer',-(cv.opexBuffer||z)[mi]); add('opexBuffer',-(cv.opexBuffer||z)[mi]);
    add('Hedging',-(cv.hedgingArr||z)[mi]); add('Hedging Adjustment',-(cv.hedgingArr||z)[mi]);
    add('Financing Cost',-(cv.finCostArr||z)[mi]); add('financingCost',-(cv.finCostArr||z)[mi]);
    add('EBIT',(cv.ebit||z)[mi]); add('ebit',(cv.ebit||z)[mi]); add('EBITDA',(cv.ebitda||z)[mi]); add('ebitda',(cv.ebitda||z)[mi]);
    add('Total Inflows',(cv.totalIncomeCF||z)[mi]); add('totalInflows',(cv.totalIncomeCF||z)[mi]);
    add('Total Outflows',totalOut[mi]); add('totalOutflows',totalOut[mi]);
    add('Net Cash Flow',(cv.netCF||z)[mi]); add('NET CF from Operations',(cv.netCF||z)[mi]); add('netCF',(cv.netCF||z)[mi]);
    add('Accumulated CF',(cv.accumCF||z)[mi]); add('Accumulated cash flow',(cv.accumCF||z)[mi]); add('accumCF',(cv.accumCF||z)[mi]);
    Object.entries(cv.revCats||{}).forEach(([k,a])=>add(k,(a||z)[mi]));
    Object.entries(cv.costCat||{}).forEach(([k,a])=>{ add(k,-((a||z)[mi]||0)); });
    (revenues||[]).forEach(it=>{ if(it.name) add(it.name,(getDistArr(it)||z)[mi]); });
    ([...(capex||[]),...(opex||[])]).forEach(it=>{ if(it.name) add(it.name,-((getDistArr(it)||z)[mi]||0)); });
    add('fx',CFG.fx); add('FX',CFG.fx); add('month',mi+1); add('m',mi+1); add('months',PM()); add('IGV',IGV); add('igv',IGV);
    return vars;
  }
  function _eval(rawFormula, cv, mi){
    if(!rawFormula || !String(rawFormula).trim()) return 0;
    const vars=_ctx(cv||compute(), mi||0);
    let expr=String(rawFormula).trim().replace(/^=/,'');
    expr=expr.replace(/(\d+(?:\.\d+)?)\s*%\s*(?:of|de)\s*([A-Za-zÁÉÍÓÚÜÑáéíóúüñ][A-Za-zÁÉÍÓÚÜÑáéíóúüñ0-9 _&/().\-]*?)(?=\s*(?:[+\-*/),]|$))/gi,(_,pct,name)=>`(${parseFloat(pct)/100})*(${name.trim()})`);
    expr=expr.replace(/(\d+(?:\.\d+)?)\s*%/g,(_,pct)=>`(${parseFloat(pct)/100})`);
    Object.keys(vars).sort((a,b)=>b.length-a.length).forEach(k=>{ expr=expr.replace(new RegExp('(?<![A-Za-z0-9_])'+_escRe(k)+'(?![A-Za-z0-9_])','g'), String(vars[k])); });
    try{ const v=Function('"use strict"; const IF=(c,a,b)=>c?a:b; const ABS=Math.abs; const MAX=Math.max; const MIN=Math.min; const ROUND=Math.round; return ('+expr+')')(); return isFinite(v)?v:0; }
    catch(e){ console.warn('Formula error:', rawFormula, '=>', expr, e.message); return 0; }
  }
  window.evalCustomFormula = evalCustomFormula = function(formula, cv, monthIdx){ return _eval(formula, cv||compute(), monthIdx||0); };
  window.evalCustomLineArr = evalCustomLineArr = function(line, cv){ const c=cv||compute(); return AMs().map((_,i)=>{ const raw=_eval(line.formula,c,i); return line.sign==='subtract' ? -Math.abs(raw) : Math.abs(raw); }); };
})();
