...
async function loadPortfolio() {
  // Always load from localStorage immediately (instant)
  portfolio = lsLoad();
  portfolioLoaded = true;

  // Try Supabase sync in background
  try {
    const session = supabaseClient.auth && (await supabaseClient.auth.getSession())?.data?.session;
    if (!session) return; // not logged in — localStorage only
    ...
    // Merge: remote takes priority for shared IDs, keep local-only entries
    const remoteIds = new Set(remote.map(p => p.id));
    const localOnly = portfolio.filter(p => p.id && p.id.startsWith('local_') && !remoteIds.has(p.id));
    portfolio = [...remote, ...localOnly];
    lsSave(portfolio);
  } catch(e) {
    // Supabase unavailable — use localStorage silently
    console.info('[Portfolio] Supabase unavailable, using local storage:', e.message || e);
  }
}
...
async function saveProjectToSupabase(project) {
  ...
  try {
    const session = supabaseClient.auth && (await supabaseClient.auth.getSession())?.data?.session;
    if (!session) return project; // no auth — local only
    ...
    if (!isLocal) {
      result = await supabaseClient.from(PROJECTS_TABLE).update(row).eq('id', project.id)
        .select('id,name,category,description,status,created_at,updated_at,snapshot,data').single();
    } else {
      result = await supabaseClient.from(PROJECTS_TABLE).insert([row])
        .select('id,name,category,description,status,created_at,updated_at,snapshot,data').single();
    }
    if (!result.error && result.data) {
      const remote = normalizeProjectFromRow(result.data);
      // Replace local entry with remote (has real UUID now)
      const ls = lsLoad();
    ...
    console.info('[Portfolio] Supabase save skipped:', e.message || e);
  }
  return project;
}
...
async function deleteProjectFromSupabase(id) {
  ...
  try {
    if (!String(id).startsWith('local_')) {
      ...
      if (error) console.warn('[Portfolio] Supabase delete error:', error.message);
    }
  } catch(e) {}
}
