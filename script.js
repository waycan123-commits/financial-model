...
function _initSupabase() {
  try {
    if (typeof supabase === 'undefined') { _sbStatus.error = 'Supabase JS not loaded'; return; }
    if (!SUPABASE_URL || !SUPABASE_KEY || SUPABASE_KEY.startsWith('sb_publishable')) {
      _sbStatus.error = 'Invalid API key — get your "anon public" key from Supabase Dashboard → Settings → API';
      return;
    }
    supabaseClient = supabase.createClient(SUPABASE_URL, SUPABASE_KEY);
    _sbStatus.error = '';
  } catch(e) {
    _sbStatus.error = 'createClient failed: ' + e.message;
  }
}
_initSupabase();

// Test the connection — called from Settings panel
async function testSupabaseConnection() {
  if (!supabaseClient) return { ok: false, error: _sbStatus.error || 'Client not initialized' };
  try {
    const { data, error } = await supabaseClient.from('projects').select('id').limit(1);
    if (error) {
      // Typical RLS error: "new row violates row-level security policy"
      // or "permission denied for table projects"
      _sbStatus.ok = false;
      _sbStatus.error = error.message;
      _sbStatus.noRLS = error.message.includes('row-level security') || error.code === '42501';
    } else {
      _sbStatus.ok = true;
      _sbStatus.error = '';
      _sbStatus.noRLS = false;
    }
  } catch(e) {
    _sbStatus.ok = false;
    _sbStatus.error = e.message;
  }
  _sbStatus.tested = true;
  return _sbStatus;
}
...
function getBase(item) {
 ...
 const b = item.igv ? a / (1 + IGV) : a;
 return item.cur === 'PEN' ? b / CFG.fx : b;
}
...
function getDistArr(item) {
 ...
 // ── Linked-cost recognition (revenue P&L follows a CAPEX/OPEX item's curve) ──
 if (!item.progMode && item.linkedCostId) {
  const linkedList = item.linkedCostType === 'capex' ? (capex||[]) : (opex||[]);
  const linked = linkedList.find(c => c.id === item.linkedCostId);
  if (linked) {
   // Get the linked item's own distribution (use a temporary unlinked copy to avoid circularity)
   const tmpItem = Object.assign({}, linked, { linkedCostId: null });
   const costDist = getDistArr(tmpItem);
   const totalCost = costDist.reduce((s, v) => s + Math.abs(v), 0);
   if (totalCost > 0) {
    const revTotal = getBase(item);
    return costDist.map(v => revTotal * Math.abs(v) / totalCost);
   }
   // Linked cost has no distribution yet — spread revenue evenly over all months
   const revTotal = getBase(item);
   return arr.map(() => revTotal / Math.max(pm, 1));
  }
  // Linked cost not found — fall through to manual distribution
 }
...
function getCFRevArr(item) {
 ...
 const IGV_RATE = 0.18;
 const igvMult = item.igv ? (1 + IGV_RATE) : 1;
 const mode = item.cfScheduleMode || 'terms';

 // ── Mode 1: Lag schedule — percentages distributed with month lag ──
 if (mode === 'lag' && Array.isArray(item.lagSchedule) && item.lagSchedule.length) {
  const arr = new Array(pm).fill(0);
  item.lagSchedule.forEach(({ lag = 0, pct = 0 }) => {
   accrual.forEach((v, i) => {
    const ti = Math.min(i + (parseInt(lag) || 0), pm - 1);
    arr[ti] += v * (pct / 100) * igvMult;
   });
  });
  return arr;
 }

 // ── Mode 2: Month map — explicit accrual-month → pay-month mapping ──
 if (mode === 'map') {
  const cmap = item.collectionMap || {};
  const payKeys = Object.keys(cmap)
   .filter(k => !k.startsWith('delay_') && /^\d+$/.test(k))
   .map(Number).filter(m => m >= 1 && m <= pm);
  if (payKeys.length) {
   const arr = new Array(pm).fill(0);
   ...
   payKeys.forEach(payMo => {
    ...
    const cashMo = Math.min(payMo - 1 + Math.round(delayDays / 30), pm - 1);
    accrualMos.forEach(am => {
     if (am >= 1 && am <= pm) { arr[cashMo] += accrual[am - 1] * igvMult; mapped.add(am); }
    });
   });
   accrual.forEach((v, i) => { if (!mapped.has(i + 1)) arr[i] += v * igvMult; });
   return arr;
  }
 }

 // ── Mode 3: Payment terms (days) — shift by payCond ──
 const payDays = parseInt(item.payCond) || 0;
 if (payDays > 0) {
  const arr = new Array(pm).fill(0);
  const shift = Math.round(payDays / 30);
  accrual.forEach((v, i) => { arr[Math.min(i + shift, pm - 1)] += v * igvMult; });
  return arr;
 }

 // Default: same timing as accrual, apply IGV if applicable
 return accrual.map(v => v * igvMult);
}
...
function getCFDistArr(item) {
 ...
 if (days <= 0) return accrual;
 const shift = Math.round(days / 30);
 const arr = new Array(pm).fill(0);
 accrual.forEach((v, i) => { arr[Math.min(i + shift, pm-1)] += v; });
 return arr;
}
...
function compute() {
 ...
 let finExpCB = Z(); // updated after totalIncome known; NOT in costCat (avoids double-count below EBITDA)
 const fubTotal = rSum(baseIncome) * (CFG.fub / 100);
 const finExpUB = new Array(pm).fill(pm > 0 ? fubTotal / pm : 0);
 // intentionally NOT added to costCat — they appear below EBITDA as financial items
 ...
 // intentionally NOT added to costCatCF either
 ...
 // finExpCB/UB stay out of costCat — they are financial items below EBITDA

 // COGS = user cost cats + contingency + adj (does NOT include bond fees)
 const totalCogs = Z();
 Object.values(costCat).forEach(a => a.forEach((v, i) => { totalCogs[i] += v; }));
 const totalCogsAdj = totalCogs.map((v, i) => v + adjCostDelta[i]);

 // Gross Profit = Total Income − Total COGS (no bond fees here)
 const grossProfit = totalIncome.map((v, i) => v - totalCogsAdj[i]);

 // Bond fees (financial items, below EBITDA)
 const bondFees = finExpCB.map((v, i) => v + finExpUB[i]);

 // Profit before OPEX — kept for backward compat with any exports/snapshots
 const profitBeforeOpex = grossProfit.map((v, i) => v - bondFees[i]);

 // Hedging: net PEN exposure / FX / months
 // revTotalPEN = all revenue converted to PEN (USD×FX, PEN stays as-is)
 // costTotalPEN = only PEN-denominated costs (they naturally offset PEN revenue)
 // Net PEN to hedge = revTotalPEN - costTotalPEN
 // USD equivalent per month = netPEN / FX / pm
 const revTotalPEN = revenues.reduce((s, r) => {
  const amt = parseFloat(r.amount) || 0;
  return s + (r.cur === 'PEN' ? amt : amt * CFG.fx);
 }, 0);
 const costTotalPEN = [...capex, ...opex].reduce((s, it) => {
  const amt = parseFloat(it.amount) || 0;
  return s + (it.cur === 'PEN' ? amt : 0);
 }, 0);
 const hedgingMonthly = pm > 0 ? Math.max(0, (revTotalPEN - costTotalPEN) / CFG.fx / pm) : 0;
 const hedgingArr = (() => {
 ...
 const ovr = (typeof formulaOverrides!=='undefined') && formulaOverrides['expr_opex_buffer'];
 if (ovr) return evalFormulaOverride(ovr, (month, mi) => ({
 totalIncome_m: totalRevBaseCF[mi]||0, totalIncome: totalRevBaseCF[mi]||0,
 opxBufPct, opxbuf: CFG.opxbuf||30
 })).map(v => Math.abs(v));
 return totalRevBaseCF.map(v => Math.abs(v) * opxBufPct);
 })();
 const totalCogsCF = Z();
 ...
 // Pass 1: netCF without financing cost (finExpCB/UB deducted once here, not in totalCogsCFAdj)
 const netCFbase = totalIncomeCF.map((v, i) =>
  v - totalCogsCFAdj[i] - opexBufferCF[i] - finExpCB[i] - finExpUB[i] - hedgingArr[i]
 );
 let acBase = 0;
 ...
 // EBITDA = Gross Profit − OPEX Buffer  (no D&A in this model → EBITDA is the operating profit)
 const ebitda = grossProfit.map((v, i) => v - opexBuffer[i]);
 // EBIT = EBITDA − Financial items (Bond Fees + Hedging + Financing Cost)
 const ebit = ebitda.map((v, i) => v - bondFees[i] - hedgingArr[i] - finCostArr[i]);
...
function renderSettings() {
 ...
 renderSSum();
 _renderSupabasePanel();
}

function _renderSupabasePanel() {
 let panel = document.getElementById('sb-config-panel');
 if (!panel) {
  const settingsEl = document.getElementById('panel-settings');
  if (!settingsEl) return;
  panel = document.createElement('div');
  panel.id = 'sb-config-panel';
  panel.style.cssText = 'margin-top:22px;border:1.5px solid #e2e8f0;border-radius:12px;overflow:hidden';
  settingsEl.appendChild(panel);
 }

 const keyInvalid = !SUPABASE_KEY || SUPABASE_KEY.startsWith('sb_publishable');
 const urlOk = SUPABASE_URL && SUPABASE_URL.includes('supabase.co');
 const { ok, tested, error: sbErr, noRLS } = _sbStatus;

 let statusBadge, statusColor, statusBg;
 if (keyInvalid)         { statusBadge = '🔑 Invalid API Key'; statusColor = '#BA0517'; statusBg = '#fef1f1'; }
 else if (!tested)       { statusBadge = '⏳ Not tested yet'; statusColor = '#A56105'; statusBg = '#fff8e8'; }
 else if (ok)            { statusBadge = '✅ Connected'; statusColor = '#2E844A'; statusBg = '#eafaea'; }
 else                    { statusBadge = '❌ Connection error'; statusColor = '#BA0517'; statusBg = '#fef1f1'; }
 ...
 panel.innerHTML = `
  <div style="display:flex;align-items:center;gap:10px;padding:12px 16px;background:linear-gradient(90deg,#1c2b3a,#2d4a6b);color:#fff">
   ...
   <div style="flex:1">
    <div style="font-size:12px;font-weight:700">Supabase Database</div>
    <div style="font-size:10px;opacity:.7;margin-top:1px">Cloud storage for your projects</div>
   </div>
   <span style="font-size:10px;font-weight:700;padding:3px 10px;border-radius:12px;background:${statusBg};color:${statusColor}">${statusBadge}</span>
  </div>

  <div style="padding:16px;display:grid;gap:14px;background:#fff">

   ${keyInvalid ? `
   <div style="background:#fff8e8;border:1.5px solid #A56105;border-radius:8px;padding:12px 14px">
    <div style="font-size:12px;font-weight:700;color:#A56105;margin-bottom:8px">⚠️ API Key needs to be updated</div>
    <div style="font-size:11px;color:#374151;line-height:1.7">
     Your current key <code style="background:#f1f5f9;padding:1px 5px;border-radius:3px;font-size:10px">${SUPABASE_KEY.slice(0,30)}...</code> is not valid.<br>
     A valid Supabase anon key starts with <code style="background:#f1f5f9;padding:1px 5px;border-radius:3px">eyJhbGciOiJIUzI1...</code><br><br>
     <strong>How to get the correct key:</strong><br>
     1. Go to <a href="https://supabase.com/dashboard" target="_blank" style="color:#0176D3">supabase.com/dashboard</a><br>
     2. Open your project <strong>qzqkqxpwkbvxuddlvmku</strong><br>
     3. Click <strong>Project Settings → API</strong><br>
     4. Copy the <strong>"anon public"</strong> key (not the service_role key)<br>
     5. Paste it in <code style="background:#f1f5f9;padding:1px 5px;border-radius:3px">script.js</code> line 2 — <code>const SUPABASE_KEY = "eyJ..."</code>
    </div>
   </div>` : ''}

   ${tested && !ok ? `
   <div style="background:#fef1f1;border:1.5px solid #BA0517;border-radius:8px;padding:12px 14px">
    <div style="font-size:12px;font-weight:700;color:#BA0517;margin-bottom:6px">Connection error</div>
    <code style="font-size:10px;color:#BA0517;word-break:break-all">${sbErr}</code>
    ${noRLS || (sbErr && (sbErr.includes('row-level security') || sbErr.includes('permission'))) ? `
    <div style="margin-top:10px;font-size:11px;color:#374151;line-height:1.7">
     <strong>Fix: Run this SQL in Supabase Dashboard → SQL Editor:</strong>
     <pre style="background:#1c2b3a;color:#7dd3fc;padding:10px;border-radius:6px;font-size:10px;margin-top:6px;overflow-x:auto">-- Option 1: Allow access without auth (single-user app)
ALTER TABLE projects ALTER COLUMN user_id DROP NOT NULL;
DROP POLICY IF EXISTS "Users see own projects" ON projects;
CREATE POLICY "Allow all" ON projects FOR ALL USING (true) WITH CHECK (true);

-- Option 2: Disable RLS entirely (simplest)
ALTER TABLE projects DISABLE ROW LEVEL SECURITY;</pre>
    </div>` : ''}
   </div>` : ''}

   ${tested && ok ? `
   <div style="background:#eafaea;border:1.5px solid #2E844A;border-radius:8px;padding:10px 14px;font-size:11px;color:#064A0C">
    ✅ Connected to <strong>${SUPABASE_URL.replace('https://','').split('.')[0]}</strong>. Projects are syncing to Supabase automatically.
   </div>` : ''}

   <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">
    <button id="sb-test-btn" class="btn" style="background:#1c2b3a;color:#fff;border:none;font-size:12px;padding:7px 16px">
     🔌 Test Connection
    </button>
    <button id="sb-sync-btn" class="btn" style="font-size:12px;padding:7px 16px">
     🔄 Sync now
    </button>
    <span id="sb-test-result" style="font-size:11px;color:var(--sub)"></span>
   </div>

   <details style="font-size:11px">
    <summary style="cursor:pointer;font-weight:600;color:#0176D3">📋 Required Supabase table schema</summary>
    <pre style="background:#1c2b3a;color:#7dd3fc;padding:12px;border-radius:8px;font-size:10px;margin-top:8px;overflow-x:auto">-- Run once in Supabase → SQL Editor
CREATE TABLE IF NOT EXISTS projects (
  id uuid DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id uuid,  -- nullable if not using auth
  name text,
  category text,
  description text,
  status text,
...
-- For single-user without auth:
ALTER TABLE projects DISABLE ROW LEVEL SECURITY;

-- For multi-user with auth:
-- ALTER TABLE projects ADD CONSTRAINT fk_user FOREIGN KEY (user_id) REFERENCES auth.users;
-- ALTER TABLE projects ENABLE ROW LEVEL SECURITY;
-- CREATE POLICY "own" ON projects FOR ALL USING (auth.uid() = user_id);</pre>
   </details>
  </div>`;

 document.getElementById('sb-test-btn').onclick = async () => {
  ...
  res.textContent = status.ok ? '✅ Connected!' : '❌ ' + (status.error || 'Failed');
  res.style.color = status.ok ? '#2E844A' : '#BA0517';
  _renderSupabasePanel();
 };
 document.getElementById('sb-sync-btn').onclick = async () => {
  const btn = document.getElementById('sb-sync-btn');
  btn.textContent = '⏳ Syncing…'; btn.disabled = true;
  await loadPortfolio();
  btn.textContent = '🔄 Sync now'; btn.disabled = false;
  _sbToast('Sync complete — ' + portfolio.length + ' projects loaded');
 };
}
...
const mkRev = () => ({ id: uid(), name:'', amount:'', cur:'USD', igv:false, cat:'', mos:{},
  adv:'', advMo:1, payCond:'', refNote:'', refUrl:'',
  billingMode:'pattern', billingCuts:[], billingDelay:0, billingFirst:1, billingEvery:1,
  progMode:false, progMonthly:'', progCatRef:'',
  penOverride:'', penEnabled:false,
  // P&L recognition link — follows a CAPEX/OPEX item's distribution
  linkedCostId: null, linkedCostType: null,
  // CF collection schedule
  cfScheduleMode: 'terms',    // 'lag' | 'map' | 'terms'
  lagSchedule: [{lag:0, pct:100}], // e.g. [{lag:0,pct:50},{lag:1,pct:30},{lag:2,pct:20}]
  collectionMap: {},
});
...
function buildItemCard(item, idx, type) {
 ...
 igvCkL.appendChild(igvChk); igvCkL.appendChild(document.createTextNode(' Includes IGV'));
 ...
 if (!isCost) {
  ...
  // ── P&L RECOGNITION ─────────────────────────────────────────────────────
  const isLinkedMode = !!item.linkedCostId;

  // Build a single flat list of all cost items (CAPEX + OPEX) with prefixed labels
  function allCostOptions() {
    const opts = [];
    (capex||[]).forEach(c => opts.push({ id: c.id, type:'capex', label:'[CAPEX] '+(c.name||'Unnamed')+(c.cat?' — '+c.cat:'') }));
    (opex||[]).forEach(c  => opts.push({ id: c.id, type:'opex',  label:'[OPEX] ' +(c.name||'Unnamed')+(c.cat?' — '+c.cat:'') }));
    return opts;
  }

  const recSec = document.createElement('div');
  recSec.style.cssText = 'margin-top:14px;border:1.5px solid #b8d4f0;border-radius:10px;overflow:hidden';
  recSec.innerHTML = '<div style="display:flex;align-items:center;gap:8px;padding:10px 14px;background:linear-gradient(90deg,#eaf3fd,#f5f9ff)">'
    + '<span style="font-size:15px">📊</span>'
    + '<div style="flex:1"><div style="font-size:12px;font-weight:700;color:#032D60">P&L Recognition</div>'
    + '<div style="font-size:10px;color:#5a7fa0;margin-top:1px">When is this revenue recognized in the income statement?</div></div></div>';
  const recBody = document.createElement('div');
  recBody.style.cssText = 'padding:12px 14px;display:grid;gap:10px';

  // Mode pills
  const modePills = document.createElement('div');
  modePills.style.cssText = 'display:grid;grid-template-columns:1fr 1fr;gap:8px';
  function makePill(label, sublabel, val, active) {
    const p = document.createElement('div');
    p.dataset.pillmode = val;
    p.style.cssText = 'padding:10px 12px;border-radius:8px;cursor:pointer;border:2px solid '
      + (active ? '#0176D3;background:#e8f4fd' : '#e2e8f0;background:#fff') + ';transition:all .15s';
    p.innerHTML = '<div style="font-size:11px;font-weight:700;color:'+(active?'#0176D3':'#374151')+'">'+label+'</div>'
      + '<div style="font-size:9px;color:'+(active?'#5a9fd4':'#9ca3af')+';margin-top:2px;line-height:1.4">'+sublabel+'</div>';
    return p;
  }
  const pillManual = makePill('✏️ Manual', 'Set % per month yourself in the grid below', 'manual', !isLinkedMode);
  const pillLinked = makePill('🔗 Follow a Cost Item', 'Auto: recognized as % of a CAPEX/OPEX curve', 'linked', isLinkedMode);
  modePills.appendChild(pillManual);
  modePills.appendChild(pillLinked);
  recBody.appendChild(modePills);

  // Manual hint
  const manualHint = document.createElement('div');
  manualHint.style.cssText = 'font-size:10px;color:#64748b;background:#f8fafc;padding:8px 10px;border-radius:6px;border-left:3px solid #94a3b8;display:' + (isLinkedMode?'none':'block');
  manualHint.innerHTML = '📝 Fill in <strong>% per month</strong> in the distribution grid below. Total must be 100%.';
  recBody.appendChild(manualHint);

  // Linked panel — single combined dropdown
  const linkedPanel = document.createElement('div');
  linkedPanel.style.cssText = 'display:' + (isLinkedMode?'grid':'none') + ';gap:8px';

  const lFld = document.createElement('div'); lFld.className = 'fld'; lFld.style.margin = '0';
  const lLbl = document.createElement('label');
  lLbl.style.cssText = 'font-size:10px;font-weight:700;color:#374151;display:block;margin-bottom:4px';
  lLbl.textContent = 'Select the CAPEX or OPEX item to follow:';
  const lSel = document.createElement('select');
  lSel.style.cssText = 'font-weight:600;width:100%';

  function rebuildCostDropdown() {
    lSel.innerHTML = '<option value="">— pick a cost item —</option>';
    const opts = allCostOptions();
    if (!opts.length) {
      const o = document.createElement('option'); o.value = ''; o.textContent = 'No CAPEX/OPEX items yet — add them first';
      lSel.appendChild(o); return;
    }
    // Group header: CAPEX
    const capexItems = opts.filter(o => o.type === 'capex');
    const opexItems  = opts.filter(o => o.type === 'opex');
    if (capexItems.length) {
      const g = document.createElement('optgroup'); g.label = '── CAPEX Items ──';
      capexItems.forEach(o => {
        const el = document.createElement('option'); el.value = o.id + '|capex'; el.textContent = o.label;
        if (o.id === item.linkedCostId) el.selected = true;
        g.appendChild(el);
      });
      lSel.appendChild(g);
    }
    if (opexItems.length) {
      const g = document.createElement('optgroup'); g.label = '── OPEX Items ──';
      opexItems.forEach(o => {
        const el = document.createElement('option'); el.value = o.id + '|opex'; el.textContent = o.label;
        if (o.id === item.linkedCostId) el.selected = true;
        g.appendChild(el);
      });
      lSel.appendChild(g);
  ...
  rebuildCostDropdown();
  lFld.appendChild(lLbl); lFld.appendChild(lSel);
  linkedPanel.appendChild(lFld);

  // Preview banner
  const lPreview = document.createElement('div');
  lPreview.style.cssText = 'font-size:10px;border-radius:6px;overflow:hidden;display:none';

  function updatePreview() {
    if (!item.linkedCostId) { lPreview.style.display = 'none'; return; }
    const list = item.linkedCostType === 'capex' ? (capex||[]) : (opex||[]);
    const linked = list.find(c => c.id === item.linkedCostId);
    if (!linked) { lPreview.style.display = 'none'; return; }
    const hasDist = linked.progMode || Object.values(linked.mos||{}).some(v => v && parseFloat(v) > 0);
    lPreview.style.display = 'block';
    lPreview.innerHTML = hasDist
      ? '<div style="background:#eafaea;padding:8px 12px;border-left:3px solid #2E844A;color:#064A0C;line-height:1.6">'
        + '✅ <strong>Following: ' + (linked.name||'Unnamed') + '</strong>'
        + '<span style="color:#2E844A;margin-left:8px;font-size:9px">['+(item.linkedCostType||'').toUpperCase()+']</span>'
        + '<div style="margin-top:3px;font-size:9px;opacity:.85">Revenue recognized proportionally to this item\'s monthly distribution. The % grid below is auto-driven.</div></div>'
      : '<div style="background:#fff8e8;padding:8px 12px;border-left:3px solid #A56105;color:#7a5000;line-height:1.6">'
        + '⚠️ <strong>' + (linked.name||'Unnamed') + '</strong> has no monthly distribution set yet.'
        + '<div style="margin-top:3px;font-size:9px">Go to '+(item.linkedCostType||'cost').toUpperCase()+' tab and set the monthly % for that item first. Until then, revenue is spread evenly.</div></div>';
  }
  updatePreview();
  linkedPanel.appendChild(lPreview);
  recBody.appendChild(linkedPanel);
  recSec.appendChild(recBody);
  body.appendChild(recSec);

  // ── Wire mode pills ──────────────────────────────────────────────────────
  function setPillActive(mode) {
    [pillManual, pillLinked].forEach(p => {
      const a = p.dataset.pillmode === mode;
      p.style.borderColor  = a ? '#0176D3' : '#e2e8f0';
      p.style.background   = a ? '#e8f4fd' : '#fff';
      p.querySelector('div').style.color = a ? '#0176D3' : '#374151';
    });
    linkedPanel.style.display = mode === 'linked' ? 'grid' : 'none';
    manualHint.style.display  = mode === 'manual'  ? 'block' : 'none';
  }
  pillManual.addEventListener('click', () => {
    item.linkedCostId = null; item.linkedCostType = null;
    setPillActive('manual');
    lPreview.style.display = 'none';
    triggerGlobalRefresh();
  });
  pillLinked.addEventListener('click', () => {
    setPillActive('linked');
    rebuildCostDropdown();  // refresh list in case items were added
  });

  // ── Wire cost dropdown ───────────────────────────────────────────────────
  lSel.addEventListener('change', () => {
    const val = lSel.value; // format: "uuid|capex" or "uuid|opex"
    if (!val) { item.linkedCostId = null; item.linkedCostType = null; lPreview.style.display = 'none'; return; }
    const parts = val.split('|');
    item.linkedCostId   = parts[0] || null;
    item.linkedCostType = parts[1] || 'capex';
    updatePreview();
    triggerGlobalRefresh();
  });
  ...
  cfSec.style.cssText = 'margin-top:10px;border:1.5px solid #b8d4c0;border-radius:10px;overflow:hidden';
  cfSec.innerHTML = '<div style="display:flex;align-items:center;gap:8px;padding:10px 14px;background:linear-gradient(90deg,#eafaea,#f5fff5)">'
    + '<span style="font-size:15px">💵</span>'
    + '<div style="flex:1"><div style="font-size:12px;font-weight:700;color:#064A0C">Cash Flow Collection</div>'
    + '<div style="font-size:10px;color:#4a7a4a;margin-top:1px">When does cash actually land in your account?</div></div></div>';
  const cfBody2 = document.createElement('div');
  cfBody2.style.cssText = 'padding:12px 14px;display:grid;gap:10px';

  const cfModeWrap = document.createElement('div');
  cfModeWrap.style.cssText = 'display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px';
  const cfModes = [{v:'terms',icon:'📆',label:'Payment Terms',desc:'Credit days'},{v:'lag',icon:'📅',label:'Lag Schedule',desc:'Split % by lag'},{v:'map',icon:'🗓',label:'Month Map',desc:'Explicit mapping'}];
  const curMode = item.cfScheduleMode || 'terms';
  cfModes.forEach(({v,icon,label,desc}) => {
    const btn = document.createElement('div'); btn.dataset.cfmode = v;
    const a = curMode === v;
    btn.style.cssText = 'padding:8px;border-radius:8px;cursor:pointer;border:2px solid '+(a?'#2E844A;background:#eafaea':'#e2e8f0;background:#fff')+';text-align:center;transition:all .15s';
    btn.innerHTML = '<div style="font-size:14px">'+icon+'</div><div style="font-size:10px;font-weight:700;color:'+(a?'#064A0C':'#374151')+'">'+label+'</div><div style="font-size:9px;color:#9ca3af">'+desc+'</div>';
    cfModeWrap.appendChild(btn);
  });
  cfBody2.appendChild(cfModeWrap);

  // Terms panel
  const termsPanel = document.createElement('div');
  termsPanel.style.cssText = 'display:'+(curMode==='terms'?'grid':'none')+';grid-template-columns:1fr 1fr;gap:8px';
  function mkF(l,h,inp){ const d=document.createElement('div');d.className='fld';d.style.margin='0';const lb=document.createElement('label');lb.style.cssText='font-size:10px;font-weight:700;display:block';lb.innerHTML=l+'<span style="font-weight:400;color:#9ca3af;display:block;margin-top:1px">'+h+'</span>';d.appendChild(lb);d.appendChild(inp);return d;}
  const t1=document.createElement('input');t1.type='number';t1.min='0';t1.placeholder='0 = same month';t1.value=item.payCond||'';
  const t2=document.createElement('input');t2.type='number';t2.min='0';t2.max='100';t2.placeholder='0 = no advance';t2.value=item.adv||'';
  termsPanel.appendChild(mkF('Credit Days','30=1mo · 60=2mo · 90=3mo',t1));
  termsPanel.appendChild(mkF('Advance %','Paid upfront on month 1',t2));
  t1.addEventListener('input',()=>{item.payCond=t1.value;});
  t2.addEventListener('input',()=>{item.adv=t2.value;});
  cfBody2.appendChild(termsPanel);

  // Lag panel
  const lagPanel = document.createElement('div');
  lagPanel.style.cssText = 'display:'+(curMode==='lag'?'block':'none');
  function buildLag(){
    lagPanel.innerHTML='';
    const sch = item.lagSchedule && item.lagSchedule.length ? item.lagSchedule : [{lag:0,pct:100}];
    item.lagSchedule = sch;
    const hd=document.createElement('div');hd.style.cssText='display:grid;grid-template-columns:1fr 1fr 32px;gap:6px;margin-bottom:6px';
    hd.innerHTML='<span style="font-size:9px;font-weight:700;color:#64748b;text-transform:uppercase">Lag (months after recognition)</span><span style="font-size:9px;font-weight:700;color:#64748b;text-transform:uppercase">% of amount</span><span></span>';
    lagPanel.appendChild(hd);
    sch.forEach((e2,ei)=>{
      const row=document.createElement('div');row.style.cssText='display:grid;grid-template-columns:1fr 1fr 32px;gap:6px;align-items:center;margin-bottom:4px';
      const lI=document.createElement('input');lI.type='number';lI.min='0';lI.max='24';lI.value=e2.lag??0;lI.style.cssText='text-align:center;font-weight:600';lI.placeholder='0=same';
      const pI=document.createElement('input');pI.type='number';pI.min='0';pI.max='100';pI.value=e2.pct??0;pI.style.cssText='text-align:center;font-weight:600';pI.placeholder='%';
      const dl=document.createElement('button');dl.textContent='✕';dl.style.cssText='background:none;border:none;color:#BA0517;cursor:pointer;font-size:13px;padding:2px';
      lI.addEventListener('input',()=>{e2.lag=parseInt(lI.value)||0;checkTotal();});
      pI.addEventListener('input',()=>{e2.pct=parseFloat(pI.value)||0;checkTotal();});
      dl.addEventListener('click',()=>{sch.splice(ei,1);buildLag();});
      row.appendChild(lI);row.appendChild(pI);row.appendChild(dl);lagPanel.appendChild(row);
    });
    const tot=sch.reduce((s,e2)=>s+(parseFloat(e2.pct)||0),0);
    const ok2=Math.abs(tot-100)<0.01;
    const tb=document.createElement('div');tb.id='lt-'+item.id;
    tb.style.cssText='padding:5px 10px;border-radius:5px;font-size:10px;font-weight:700;margin-top:4px;background:'+(ok2?'#eafaea':'#fef1f1')+';color:'+(ok2?'#2E844A':'#BA0517');
    tb.textContent='Total: '+tot.toFixed(1)+'% '+(ok2?'✓':'← must equal 100%');
    lagPanel.appendChild(tb);
    const addR=document.createElement('button');addR.textContent='+ Add row';addR.style.cssText='font-size:10px;margin-top:8px;color:#0176D3;background:none;border:1px dashed #0176D3;border-radius:4px;padding:3px 12px;cursor:pointer';
    addR.addEventListener('click',()=>{sch.push({lag:sch.length,pct:0});buildLag();});
    lagPanel.appendChild(addR);
    const ex2=document.createElement('div');ex2.style.cssText='margin-top:8px;font-size:10px;color:#64748b;background:#f8fafc;padding:8px;border-radius:6px;line-height:1.6';
    ex2.innerHTML='💡 <b>Example:</b> lag=0→50%, lag=1→30%, lag=2→20%<br>Month 4: collects 50% of M4 rev + 30% of M3 + 20% of M2.';
    lagPanel.appendChild(ex2);
  }
  function checkTotal(){const el=document.getElementById('lt-'+item.id);if(!el)return;const t=(item.lagSchedule||[]).reduce((s,e2)=>s+(parseFloat(e2.pct)||0),0);const ok2=Math.abs(t-100)<0.01;el.textContent='Total: '+t.toFixed(1)+'% '+(ok2?'✓':'← must equal 100%');el.style.background=ok2?'#eafaea':'#fef1f1';el.style.color=ok2?'#2E844A':'#BA0517';}
  buildLag();
  cfBody2.appendChild(lagPanel);

  const mapNote=document.createElement('div');
  mapNote.style.cssText='display:'+(curMode==='map'?'block':'none')+';font-size:10px;background:#f8fafc;padding:8px;border-radius:6px;border-left:3px solid #94a3b8;color:#374151;line-height:1.6';
  mapNote.innerHTML='<b>🗓 Month Map</b>: Use the collection schedule grid below to map which accrual months get paid on each payment event.';
  cfBody2.appendChild(mapNote);

  const igvNote2=document.createElement('div');
  igvNote2.style.cssText='font-size:10px;padding:7px 10px;border-radius:6px;background:#fff8e8;border-left:3px solid #A56105;color:#7a5000;line-height:1.5;display:'+(item.igv?'block':'none');
  igvNote2.innerHTML='⚠️ <b>IGV on CF:</b> Cash received × 1.18. P&L stays ex-VAT.';
  cfBody2.appendChild(igvNote2);
  cfSec.appendChild(cfBody2); body.appendChild(cfSec);

  // wire CF mode buttons
  cfModeWrap.querySelectorAll('[data-cfmode]').forEach(btn => {
    btn.addEventListener('click',()=>{
      item.cfScheduleMode = btn.dataset.cfmode;
      cfModeWrap.querySelectorAll('[data-cfmode]').forEach(b=>{const a2=b.dataset.cfmode===item.cfScheduleMode;b.style.borderColor=a2?'#2E844A':'#e2e8f0';b.style.background=a2?'#eafaea':'#fff';b.querySelector('div+div').style.color=a2?'#064A0C':'#374151';});
      termsPanel.style.display = item.cfScheduleMode==='terms'?'grid':'none';
      lagPanel.style.display   = item.cfScheduleMode==='lag'?'block':'none';
      mapNote.style.display    = item.cfScheduleMode==='map'?'block':'none';
    });
  });
  igvChk.addEventListener('change',()=>{igvNote2.style.display=igvChk.checked?'block':'none';});
 ...
 // ── If revenue is linked to a cost item, show a read-only banner instead of the manual grid ──
 let gridWrapper; // will hold grid so we can hide it
 if (!isCost && item.linkedCostId) {
  const linkedList = item.linkedCostType==='capex'?capex:opex;
  const linkedItem = (linkedList||[]).find(c=>c.id===item.linkedCostId);
  const linkBanner = document.createElement('div');
  linkBanner.style.cssText='padding:12px 14px;border-radius:8px;background:linear-gradient(135deg,#e8f4fd,#eafaea);border:1.5px solid #0176D380;margin-bottom:10px';
  linkBanner.innerHTML='<div style="font-size:11px;font-weight:700;color:#0176D3;margin-bottom:6px">🔗 Recognition linked to: '+(linkedItem?linkedItem.name||'Unnamed':'(not found)')+'</div>'
   +'<div style="font-size:10px;color:#374151;line-height:1.6">The monthly distribution below is <strong>driven by the linked cost item</strong>. '
   +'You do not need to fill in percentages — they are computed automatically from that item\'s curve. '
   +'<br><a href="#" style="color:#0176D3" onclick="event.preventDefault();'+
   'var el=document.getElementById(\'dist-grid-'+item.id+'\');if(el)el.style.display=el.style.display===\'none\'?\'\':\' none\'">Show/hide preview grid</a></div>';
  body.appendChild(linkBanner);
 }
 ...
 if (item.progMode && item.progMonthly) {
  pc = '#2E844A'; borderColor = '#DDDBDA';
  distLbl.innerHTML = 'Phase 1 Advance — <span style="color:' + pc + '">' + p.toFixed(1).replace(/\.?0+$/, '') + '% progress</span>' + statusMsg;
 } else if (!isCost && item.linkedCostId) {
  // Linked to cost item — distribution is driven automatically
  const lList = item.linkedCostType === 'capex' ? (capex||[]) : (opex||[]);
  const lFound = lList.find(c => c.id === item.linkedCostId);
  pc = '#0176D3'; borderColor = '#b8d4f0';
  distLbl.innerHTML = '🔗 Recognition linked to: <strong style="color:#0176D3">'
   + (lFound ? lFound.name || 'Unnamed' : '(item not found)') + '</strong>'
   + ' <span style="font-size:9px;font-weight:400;color:#5a7fa0">[' + (item.linkedCostType||'?').toUpperCase() + ']</span>'
   + (!lFound ? ' <span style="color:#BA0517">— item deleted, switch to Manual</span>' : '');
 } else {
 ...
 ? (item.cur + ' ' + parseFloat(item.amount).toLocaleString() + (item.igv ? ' (incl. IGV)' : ''))
 : '';

 // Update collapsed summary line
 const sumParts = [];
 if (item.amount) sumParts.push(item.cur + ' ' + parseFloat(item.amount).toLocaleString());
...
function _evalFormulaForId(fid, expr, cv) {
 ...
 const costTotalPEN = [...capex, ...opex].filter(it=>it.cur==='PEN').reduce((s,it)=>s+(parseFloat(it.amount)||0),0);
 const finMonthlyRate = (CFG.finRate||0)/100;
...
const PL_DEFAULT_ROWS = [
 ...
 // OPEX (operating, before EBITDA)
 { id:'pl_opex_sec', type:'section', label:'OPEX', section:'opex' },
 ...
 // FINANCIAL ITEMS (below EBITDA)
 { id:'pl_bond_sec', type:'section', label:'Financial & Interest', section:'bond' },
 ...
 { id:'pl_hedging', type:'builtin', label:'Hedging Adjustment', section:'bond', formula:'(revTotalPEN - costTotalPEN) / CFG.fx / PM()', editable:false },
 { id:'pl_fin_cost', type:'builtin', label:'Financing Cost', section:'bond', formula:'|accumCF[m-1]| * finRate%', editable:false },
 // EBIT
 { id:'pl_ebit', type:'total', label:'EBIT', section:'bond' },
];
...
// Interactive formula variable chip builder
function _buildFormulaChips() {
 const chipWrap = document.getElementById('cl-formula-chips');
 if (!chipWrap) return;

 // Gather all available variables
 const cv = (() => { try { return compute(); } catch(e) { return {}; } })();
 const groups = [
   {
     label: '📊 P&L Totals', color: '#0176D3', vars: [
       'totalIncome','totalCogs','grossProfit','profitBeforeOpex','ebit','ebitda',
       'opexBuffer','hedging','penArr','finCostArr'
     ]
   },
   {
     label: '💵 Cash Flow', color: '#2E844A', vars: [
       'totalIncomeCF','accumCF','netCF'
     ]
   },
   {
     label: '📁 Revenue Categories', color: '#06A59A',
     vars: Object.keys(cv.revCats || {})
   },
   {
     label: '📁 Cost Categories', color: '#BA0517',
     vars: Object.keys(cv.costCat || {}).filter(k => !k.startsWith('Financial'))
   },
   {
     label: '🔹 Revenue Items', color: '#1B96FF',
     vars: (revenues||[]).map(r => r.name).filter(Boolean)
   },
   {
     label: '🔸 Cost Items', color: '#A56105',
     vars: [...(capex||[]),...(opex||[])].map(r => r.name).filter(Boolean)
   },
   {
     label: '⚙️ Operators', color: '#5A2D82', isOp: true, vars: [
       '+', '-', '*', '/', '(', ')', 'IF(', 'ABS(', 'MAX(', 'MIN(', '0.01', '%'
     ]
   }
 ];

 chipWrap.innerHTML = groups.filter(g => g.vars.length).map(g => `
   <div style="margin-bottom:8px">
     <div style="font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:${g.color};margin-bottom:4px;opacity:.9">${g.label}</div>
     <div style="display:flex;flex-wrap:wrap;gap:4px">
       ${g.vars.slice(0,20).map(v => `
         <button class="fml-chip" data-val="${esc(v)}"
           style="font-size:10px;padding:2px 8px;border-radius:12px;border:1.5px solid ${g.color}30;
                  background:${g.color}12;color:${g.color};cursor:pointer;font-weight:600;
                  transition:all .12s;white-space:nowrap;font-family:monospace"
           onmouseover="this.style.background='${g.color}28';this.style.transform='translateY(-1px)'"
           onmouseout="this.style.background='${g.color}12';this.style.transform='none'"
           title="Click to insert: ${esc(v)}">${esc(v)}</button>
       `).join('')}
     </div>
   </div>
 `).join('');

 // Wire chip clicks: insert at cursor position
 chipWrap.querySelectorAll('.fml-chip').forEach(btn => {
   btn.addEventListener('click', () => {
     const inp = document.getElementById('cl-m-formula');
     const val = btn.dataset.val;
     const start = inp.selectionStart, end = inp.selectionEnd;
     const before = inp.value.slice(0, start);
     const after = inp.value.slice(end);
     const insert = (before && !before.match(/[+\-*/(\s,]$/) && !val.match(/^[+\-*/),]/) ) ? ' ' + val : val;
...
function openEditLineModal(line) {
 ...
 setTimeout(() => { document.getElementById('cl-m-formula').focus(); _buildFormulaChips(); }, 50);
}
...
function openCLModal(section) {
 ...
 document.getElementById('cl-modal').style.display = 'flex';
 setTimeout(() => { document.getElementById('cl-m-name').focus(); _buildFormulaChips(); }, 50);
}
...
function renderGroupedPLCF(which, cv) {
 ...
 body += `<tr><td colspan="999" style="padding:2px 8px 4px;border:none;background:transparent"><button class="add-line-btn" data-section="income_section" style="font-size:10px;color:var(--accent);background:none;border:1px dashed var(--bdr2);border-radius:4px;padding:3px 14px;cursor:pointer;width:100%;text-align:left;opacity:.55;transition:opacity .15s" onmouseover="this.style.opacity=1" onmouseout="this.style.opacity=.55">+ Agregar línea calculada en Ingresos</button></td></tr>`;
 ...
 body += `<tr><td colspan="999" style="padding:2px 8px 4px;border:none;background:transparent"><button class="add-line-btn" data-section="cogs_section" style="font-size:10px;color:var(--accent);background:none;border:1px dashed var(--bdr2);border-radius:4px;padding:3px 14px;cursor:pointer;width:100%;text-align:left;opacity:.55;transition:opacity .15s" onmouseover="this.style.opacity=1" onmouseout="this.style.opacity=.55">+ Agregar línea calculada en COGS</button></td></tr>`;

 //  GROSS PROFIT 
 body += mkRow('Gross Profit', cv.grossProfit, {bold:true, color:'blue'});
 body += customLinesFor('after_gross_profit', cv, mkRow);
 body += `<tr><td colspan="999" style="padding:2px 8px 4px;border:none;background:transparent"><button class="add-line-btn" data-section="after_gross_profit" style="font-size:10px;color:var(--accent);background:none;border:1px dashed var(--bdr2);border-radius:4px;padding:3px 14px;cursor:pointer;width:100%;text-align:left;opacity:.55;transition:opacity .15s" onmouseover="this.style.opacity=1" onmouseout="this.style.opacity=.55">+ Agregar línea después de Gross Profit</button></td></tr>`;

 //  OPEX (Operating Expenses — subtracted before EBITDA) 
 body += mkSec('OPEX', 'var(--th-opex)');
 ...
 body += `<tr><td colspan="999" style="padding:2px 8px 4px;border:none;background:transparent"><button class="add-line-btn" data-section="after_profit_b_opex" style="font-size:10px;color:var(--accent);background:none;border:1px dashed var(--bdr2);border-radius:4px;padding:3px 14px;cursor:pointer;width:100%;text-align:left;opacity:.55;transition:opacity .15s" onmouseover="this.style.opacity=1" onmouseout="this.style.opacity=.55">+ Agregar línea en OPEX</button></td></tr>`;

 //  EBITDA 
 body += mkRow('EBITDA', cv.ebitda, {bold:true, color:'teal'});

 //  FINANCIAL ITEMS (Bond Fees + Hedging + Financing — below EBITDA) 
 body += mkSec('Financial & Interest', 'var(--th-bond)');
 ...
 body += `<tr><td colspan="999" style="padding:2px 8px 4px;border:none;background:transparent"><button class="add-line-btn" data-section="after_ebit" style="font-size:10px;color:var(--accent);background:none;border:1px dashed var(--bdr2);border-radius:4px;padding:3px 14px;cursor:pointer;width:100%;text-align:left;opacity:.55;transition:opacity .15s" onmouseover="this.style.opacity=1" onmouseout="this.style.opacity=.55">+ Agregar línea después de EBIT</button></td></tr>`;
 body += mkRow('EBIT', cv.ebit, {bold:true, color:'blue'});
   ...
   // ── INFLOW total: sum exactly the visible rows ──
   const totInflowArr = Z();
   Object.values(cv.revCatsCF).forEach(arr => arr.forEach((v,i) => { totInflowArr[i] += (v||0); }));
   cv.penArrCF.forEach((v,i) => { totInflowArr[i] += (v||0); });
   cv.adjIncomeRows.forEach(ac => {
     const s = ac.adj.sign === 'add' ? 1 : -1;
     (ac.arrCF||Z()).forEach((v,i) => { totInflowArr[i] += s*(v||0); });
   });
   customLines.filter(l => l.showInCF && (l.section==='income_section'||l.section==='cf_after_inflows') && l.formula.trim())
     .forEach(l => evalCustomLineArr(l, cv).forEach((v,i) => { totInflowArr[i] += (v||0); }));

   // ── OUTFLOW total: sum exactly the visible rows (all positive, then negate) ──
   const _outRaw = Z();
   cv.userCats.forEach(c => (cv.costCatCF[c]||Z()).forEach((v,i) => { _outRaw[i] += Math.abs(v||0); }));
   (cv.costCatCF['Contingency']||Z()).forEach((v,i) => { _outRaw[i] += Math.abs(v||0); });
   cv.adjCostRows.forEach(ac => (ac.arrCF||Z()).forEach((v,i) => { _outRaw[i] += Math.abs(v||0); }));
   cv.opexBufferCF.forEach((v,i) => { _outRaw[i] += Math.abs(v||0); });
   cv.finExpCB.forEach((v,i) => { _outRaw[i] += Math.abs(v||0); });
   cv.finExpUB.forEach((v,i) => { _outRaw[i] += Math.abs(v||0); });
   cv.hedgingArr.forEach((v,i) => { _outRaw[i] += Math.abs(v||0); });
   cv.finCostArr.forEach((v,i) => { _outRaw[i] += Math.abs(v||0); });
   customLines.filter(l => l.showInCF && (l.section==='cogs_section'||l.section==='cf_after_outflows') && l.formula.trim())
     .forEach(l => evalCustomLineArr(l, cv).forEach((v,i) => { _outRaw[i] += Math.abs(v||0); }));
   const totOutflowNeg = _outRaw.map(v => -v);

   // NET CF = inflows + outflows(neg), ACCUMULATED = running sum
   const netCFdisp = totInflowArr.map((v,i) => v + totOutflowNeg[i]);
   let _a=0; const accumCFdisp = netCFdisp.map(v=>{_a+=v;return _a;});
   ...
   cv.adjIncomeRows.forEach(ac => {
     const s = ac.adj.sign === 'add' ? 1 : -1;
     body += mkRow((ac.adj.name||'Adj')+' ('+ac.adj.pct+'%)', (ac.arrCF||Z()).map(v=>s*v), {ind:true,color:'amber'});
   });
   customLines.filter(l=>l.showInCF&&(l.section==='income_section'||l.section==='cf_after_inflows')&&l.formula.trim())
     .forEach(l=>body+=mkRow(l.name||'Custom', evalCustomLineArr(l,cv), {ind:true,color:'amber'}));
   body += mkRow('Total Inflows', totInflowArr, {bold:true, color:'teal'});

   body += mkSec('OUTFLOWS', 'var(--th-cf-out)');
   cv.userCats.forEach(c => body += mkRow(c, (cv.costCatCF[c]||Z()).map(v=>-v), {ind:true}));
   body += mkRow('Contingency', (cv.costCatCF['Contingency']||Z()).map(v=>-v), {ind:true});
   cv.adjCostRows.forEach(ac => body += mkRow((ac.adj.name||'Adj')+' ('+ac.adj.pct+'%)', ac.arrCF.map(v=>-Math.abs(v)), {ind:true,color:'amber'}));
   customLines.filter(l=>l.showInCF&&(l.section==='cogs_section'||l.section==='cf_after_outflows')&&l.formula.trim())
     .forEach(l=>body+=mkRow(l.name||'Custom', evalCustomLineArr(l,cv).map(v=>-Math.abs(v)), {ind:true,color:'amber'}));
   body += mkRow('OPEX Buffer ('+(CFG.opxbuf??30)+'%)', cv.opexBufferCF.map(v=>-v), {ind:true,color:'amber'});
   body += mkRow('Financial Exp - CB', cv.finExpCB.map(v=>-v), {ind:true,color:'amber'});
   body += mkRow('Financial Exp - UB', cv.finExpUB.map(v=>-v), {ind:true,color:'amber'});
   body += mkRow('Hedging', cv.hedgingArr.map(v=>-v), {ind:true,color:'purple'});
   body += mkRow('Financing Cost (on neg. CF)', cv.finCostArr.map(v=>-v), {ind:true,color:'amber'});
   body += mkRow('Total Outflows', totOutflowNeg, {bold:true, color:'red'});

   body += mkSec('NET CASH FLOW', 'var(--th-bond)');
   body += mkRow('Net Cash Flow', netCFdisp, {bold:true,color:'blue'});
   body += mkRow('Accumulated CF', accumCFdisp, {bold:true,color:'purple'});
 }
...
function renderPL() {
 ...
 case 'pl_opex_sec': return secRow('OPEX', 'var(--th-opex)');
 case 'pl_opex_buf':
 ...
 case 'pl_bond_sec': return secRow('Financial & Interest', 'var(--th-bond)');
 case 'pl_cb_fee':
 ...
 + sumRow('EBIT', cv.ebit, 'var(--blue)', cv.totalIncome);
 default: return '';
 }
 }
...
function renderCF() {
 ...
 const totInflowArr = Z();
 Object.values(cv.revCatsCF).forEach(arr => arr.forEach((v,i) => { totInflowArr[i] += (v||0); }));
 cv.penArrCF.forEach((v,i) => { totInflowArr[i] += (v||0); }); // negative
 cv.adjIncomeRows.forEach(ac => {
   const s = ac.adj.sign === 'add' ? 1 : -1;
   (ac.arrCF||Z()).forEach((v,i) => { totInflowArr[i] += s*(v||0); });
 });
 ...
 // ── OUTFLOW total: sum exactly the visible rows (all positive, then negate) ──
 const _outflowRaw = Z();
 cv.userCats.forEach(c => (cv.costCatCF[c]||Z()).forEach((v,i) => { _outflowRaw[i] += Math.abs(v||0); }));
 (cv.costCatCF['Contingency']||Z()).forEach((v,i) => { _outflowRaw[i] += Math.abs(v||0); });
 cv.adjCostRows.forEach(ac => (ac.arrCF||Z()).forEach((v,i) => { _outflowRaw[i] += Math.abs(v||0); }));
 cv.opexBufferCF.forEach((v,i) => { _outflowRaw[i] += Math.abs(v||0); });
 cv.finExpCB.forEach((v,i) => { _outflowRaw[i] += Math.abs(v||0); });
 cv.finExpUB.forEach((v,i) => { _outflowRaw[i] += Math.abs(v||0); });
 cv.hedgingArr.forEach((v,i) => { _outflowRaw[i] += Math.abs(v||0); });
 finCost.forEach((v,i) => { _outflowRaw[i] += Math.abs(v||0); });
 customLines.filter(l => l.showInCF && (l.section==='cogs_section'||l.section==='cf_after_outflows') && l.formula.trim())
   .forEach(l => evalCustomLineArr(l, cv).forEach((v,i) => { _outflowRaw[i] += Math.abs(v||0); }));
 const totOutflowNeg = _outflowRaw.map(v => -v);

 // NET CF = inflows + outflows(neg), ACCUMULATED = running sum
 const netCFdisp = totInflowArr.map((v,i) => v + totOutflowNeg[i]);
 let _acCF = 0;
 const accumCFdisp = netCFdisp.map(v => { _acCF += v; return _acCF; });
 ...
 cv.adjIncomeRows.forEach(ac => {
   const s = ac.adj.sign === 'add' ? 1 : -1;
   h += cfDataRow((ac.adj.name||'Adj') + ' (' + (ac.adj.pct||0) + '%)', (ac.arrCF||Z()).map(v => s*v), {ind:true, color:'amber'});
 });
 ...
 h += cfDataRow('Total Inflows', totInflowArr, {bold:true, color:'teal'});
 h += addLineRow('cf_after_inflows', nc);

 //  OUTFLOWS  (displayed as negative)
 h += secRow('OUTFLOWS', 'var(--th-cf-out)');
 cv.userCats.forEach(c => h += cfDataRow(c, (cv.costCatCF[c] || Z()).map(v => -v), {ind:true}));
 h += cfDataRow('Contingency (' + CFG.con + '%)', (cv.costCatCF['Contingency'] || Z()).map(v => -v), {ind:true});
 cv.adjCostRows.forEach(ac => {
   h += cfDataRow((ac.adj.name||'Adj') + ' (' + (ac.adj.pct||0) + '%)', ac.arrCF.map(v => -Math.abs(v)), {ind:true, color:'amber'});
 });
 customLines.filter(l => l.showInCF && l.section === 'cogs_section' && l.formula.trim()).forEach(l => {
   h += cfDataRow(l.name||'Custom', evalCustomLineArr(l, cv).map(v => -Math.abs(v)), {ind:true, color:'amber'});
 });
 h += cfDataRow('OPEX Buffer (' + (CFG.opxbuf ?? 30) + '%)', cv.opexBufferCF.map(v => -v), {ind:true, color:'amber'});
 h += cfDataRow('Compliance Bond Fee', cv.finExpCB.map(v => -v), {ind:true, color:'amber'});
 h += cfDataRow('Upfront Bond Fee', cv.finExpUB.map(v => -v), {ind:true, color:'amber'});
 h += cfDataRow('Hedging (monthly)', cv.hedgingArr.map(v => -v), {ind:true});
 if (rSum(finCost) !== 0) h += cfDataRow('Financing Cost', finCost.map(v => -v), {ind:true, color:'amber'});
 customLines.filter(l => l.showInCF && l.section === 'cf_after_outflows' && l.formula.trim()).forEach(l => {
   h += cfDataRow(l.name||'Custom', evalCustomLineArr(l, cv).map(v => -Math.abs(v)), {ind:true, color:'amber'});
 });
 h += cfDataRow('Total Outflows', totOutflowNeg, {bold:true, color:'red'});
 h += addLineRow('cf_after_outflows', nc);

 //  NET CF 
 h += secRow('NET CASH FLOW', 'var(--th-bond)');
 h += cfDataRow('Net Cash Flow', netCFdisp, {bold:true, color:'blue'});
 h += cfDataRow('Accumulated CF', accumCFdisp, {bold:true, acc:true, color:'purple'});
...
// Global refresh called from inside item cards after linked-cost changes
function triggerGlobalRefresh() {
  try {
    if (typeof currentTab === 'undefined') return;
    if      (currentTab === 'pl')        { if (typeof renderPL          === 'function') renderPL(); }
    else if (currentTab === 'cf')        { if (typeof renderCF          === 'function') renderCF(); }
    else if (currentTab === 'dashboard') { if (typeof renderDashboard   === 'function') renderDashboard(); }
    else if (currentTab === 'costs')     { if (typeof renderCostSummary === 'function') renderCostSummary(); }
...
function renderKnowHow() {
 ...
 const sections = [
  {
   id:'overview', icon:'🗺', title:'Model Overview', color:'#0176D3',
   content: `
    <p>This is a <strong>project finance model</strong> — it helps you forecast the economics of a project over time. It has two financial statements:</p>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin:14px 0">
     <div style="padding:14px;border-radius:8px;background:#e8f4fd;border-left:3px solid #0176D3">
      <div style="font-weight:700;color:#032D60;margin-bottom:6px">📊 P&L (Profit & Loss)</div>
      <div style="font-size:11px;line-height:1.7;color:#374151">Shows <strong>what you earn and spend</strong> on an accrual basis — revenue is recognized when services are delivered, costs when they are incurred. Gives you <strong>Gross Profit, EBITDA, EBIT</strong>.</div>
     </div>
     <div style="padding:14px;border-radius:8px;background:#eafaea;border-left:3px solid #2E844A">
      <div style="font-weight:700;color:#064A0C;margin-bottom:6px">💵 Cash Flow</div>
      <div style="font-size:11px;line-height:1.7;color:#374151">Shows <strong>when cash actually moves</strong>. A project can be profitable on P&L but run out of cash if collections are delayed. Shows <strong>Net CF and Accumulated CF</strong>.</div>
     </div>
    </div>
    <div style="background:#fff8e8;padding:12px 14px;border-radius:8px;border-left:3px solid #A56105;font-size:11px;line-height:1.7">
     💡 <strong>The key insight:</strong> Revenue might be recognized in month 1 (P&L), but the client pays in month 3 (CF). The model tracks both separately so you know exactly when you need cash vs when you show profit.
    </div>`
  },
  {
   id:'settings', icon:'⚙️', title:'Settings', color:'#5A2D82',
   content: `
    <table style="width:100%;border-collapse:collapse;font-size:11px">
     <tr style="background:#f1f5f9"><th style="padding:8px 10px;text-align:left;border-bottom:2px solid #e2e8f0">Field</th><th style="padding:8px 10px;text-align:left;border-bottom:2px solid #e2e8f0">What it means</th><th style="padding:8px 10px;text-align:left;border-bottom:2px solid #e2e8f0">Typical value</th></tr>
     ${[
      ['FX Rate (FX)', 'How many PEN = 1 USD. Used to convert PEN costs to USD and for hedging.', '3.75 – 3.85'],
      ['Penalty %', 'Global penalty applied to all revenue if a target is missed. Can be overridden per item.', '2% – 5%'],
      ['Contingency %', 'Extra % added to COGS as a buffer for unexpected cost overruns.', '5% – 10%'],
      ['Discount Rate %', 'Annual rate used to calculate NPV of future cash flows. Usually your cost of capital.', '10% – 15%'],
      ['Months', 'Project duration in months. Determines how many columns appear.', '12, 24, 36, 60'],
      ['Start Month / Year', 'When the project kicks off.', 'Month 1 of concession'],
      ['OPEX Buffer %', 'Reserve for operating expenses. Applied as % of total monthly revenue.', '20% – 40%'],
      ['Financing Rate %', 'Monthly interest on negative accumulated CF (when you\'re in the red).', '0.5% – 1.5%/mo'],
      ['Compliance Bond (CB)', 'Bond paid as % of total contract value to guarantee performance.', '1% – 3%'],
      ['Upfront Bond (UB)', 'One-time bond paid at start. Spread evenly over all months.', '0.5% – 2%'],
     ].map(([f,d,v])=>`<tr style="border-bottom:1px solid #f1f5f9"><td style="padding:8px 10px;font-weight:600;color:#0176D3;white-space:nowrap">${f}</td><td style="padding:8px 10px;color:#374151">${d}</td><td style="padding:8px 10px;color:#2E844A;white-space:nowrap">${v}</td></tr>`).join('')}
    </table>`
  },
  {
   id:'revenue', icon:'📈', title:'Revenue', color:'#06A59A',
   content: `
    <p style="margin-bottom:14px">Each revenue item represents a stream of income. You configure <strong>two things independently</strong>: when it's recognized (P&L) and when cash is collected (CF).</p>
    <div style="display:grid;gap:10px">
     ${[
      ['Name', '📛', 'A descriptive name for this revenue stream.', 'e.g. "Monthly Service Fee", "Component Supply DAHUA"'],
      ['Amount (USD)', '💲', 'Total contract value in USD, <strong>ex-VAT</strong>. Always enter without IGV.', 'e.g. 500,000'],
      ['Currency (USD/PEN)', '💱', 'Whether this revenue is denominated in USD or PEN. Used for hedging calculation.', 'USD = client pays in dollars · PEN = client pays in soles'],
      ['IGV', '🧾', 'Check if the client pays IGV (18%). P&L stays ex-VAT; CF amounts are multiplied × 1.18.', 'B2B usually: No IGV · B2C or formal invoicing: Yes'],
      ['Category', '🗂', 'Groups revenue lines together in the P&L. Same category = same row in the income section.', 'e.g. "Components", "Services", "Maintenance"'],
      ['P&L Recognition', '📊', '<strong>Manual:</strong> you set % per month in the grid. <strong>Linked:</strong> follows a CAPEX/OPEX item\'s curve automatically (% completion method).', 'Linked is ideal for revenue tied to physical installation milestones'],
      ['CF Collection', '💵', '<strong>Payment Terms:</strong> X days credit (30=1mo, 60=2mo). <strong>Lag Schedule:</strong> split % by lag months. <strong>Month Map:</strong> explicit mapping.', 'Lag Schedule: 50% month 0, 30% month 1, 20% month 2'],
     ].map(([f,i,d,ex])=>`
      <div style="display:grid;grid-template-columns:24px 140px 1fr;gap:8px;padding:10px;border-radius:8px;background:#f8fafc;border:1px solid #e2e8f0;align-items:start">
       <span style="font-size:16px">${i}</span>
       <div style="font-weight:700;font-size:11px;color:#374151;padding-top:1px">${f}</div>
       <div style="font-size:11px;color:#374151;line-height:1.6">${d}<br><span style="color:#06A59A;font-style:italic">Example: ${ex}</span></div>
      </div>`).join('')}
    </div>
    <div style="margin-top:14px;background:#fff8e8;padding:12px;border-radius:8px;border-left:3px solid #A56105;font-size:11px;line-height:1.7">
     💡 <strong>Distribution grid:</strong> Each month column shows a cell where you enter a % (e.g. 25 for 25%). The cells must sum to 100% for the full revenue to be recognized. You can also paste data from Excel with Ctrl+V.
    </div>`
  },
  {
   id:'capex', icon:'🔧', title:'CAPEX', color:'#1B96FF',
   content: `
    <p>CAPEX (Capital Expenditure) = <strong>one-time investments</strong> in equipment, infrastructure, licenses, installations.</p>
    <div style="background:#f0f9ff;padding:14px;border-radius:8px;margin:12px 0;font-size:11px;line-height:1.8">
     <strong>When to use CAPEX vs OPEX:</strong><br>
     → CAPEX: buying equipment, installation, civil works, software licenses (one-time)<br>
     → OPEX: monthly services, salaries, rent, maintenance (recurring)<br><br>
     <strong>Fields are the same as OPEX:</strong> Name, Amount, Currency, IGV, Category, Distribution %, Payment Terms.
    </div>
    <div style="display:grid;gap:8px">
     ${[
      ['Payment Condition (days)','💳','How many days after invoicing does the supplier get paid. 0 = paid immediately, 30 = paid next month.','30 days (net-30) is standard for most suppliers'],
      ['Advance %','🤝','Percentage paid upfront before the work begins. Common for equipment orders.','30% advance on large equipment orders'],
      ['Category','🗂','Groups costs in P&L COGS section. Multiple CAPEX items in the same category are shown as one row.','e.g. "Equipos Dahua", "Instalación Civil", "Licencias"'],
      ['Distribution %','📅','How the total cost is spread across months. Enter % per month. Must sum to 100%.','Month 1: 30%, Month 2: 40%, Month 3: 30%'],
      ['Negative (rebate)','↩️','Check if this is a cost credit or rebate (reduces COGS).','Supplier discount, government subsidy'],
     ].map(([f,i,d,ex])=>`
      <div style="padding:10px;border-radius:8px;background:#f0f9ff;border-left:3px solid #1B96FF;font-size:11px">
       <span style="font-size:14px">${i}</span> <strong>${f}:</strong> ${d}<br>
       <span style="color:#0176D3;font-style:italic;margin-left:22px">Example: ${ex}</span>
      </div>`).join('')}
    </div>`
  },
  {
   id:'opex', icon:'🔄', title:'OPEX', color:'#5A2D82',
   content: `
    <p>OPEX (Operating Expenditure) = <strong>recurring costs</strong> to keep the project running month after month.</p>
    <div style="display:grid;gap:8px;margin-top:12px">
     ${[
      ['Recurring cost example','Monthly staffing, maintenance contracts, software subscriptions, rent, utilities'],
      ['Non-recurring OPEX','One-time training, audits, certifications — put these in CAPEX if significant'],
      ['Tip: Category naming','Use the same category name across multiple items to group them in P&L. The P&L shows one row per category.'],
      ['Tip: Currency','If you pay a supplier in PEN, mark it as PEN. This affects the hedging calculation.'],
      ['Tip: Progressive mode','Use progressive mode for contracts where you pay a fixed monthly amount × % completion (great for subcontractors).'],
     ].map(([t,d])=>`
      <div style="padding:10px 12px;border-radius:8px;background:#f5f3ff;border-left:3px solid #5A2D82;font-size:11px;line-height:1.6">
       <strong style="color:#5A2D82">${t}:</strong> ${d}
      </div>`).join('')}
    </div>`
  },
  {
   id:'pl', icon:'📊', title:'P&L Structure', color:'#0176D3',
   content: `
    <div style="font-size:11px">
     <p>The P&L is built automatically from your inputs. Here's what each line means:</p>
     <table style="width:100%;border-collapse:collapse;margin-top:12px">
      ${[
       ['Revenue (by category)','Sum of all revenue items in each category, by month','green'],
       ['Penalties','Global penalty % applied to revenue (e.g. KPI shortfall deduction)','amber'],
       ['Total Income','Revenue + Adjustments','blue bold'],
       ['COGS (by category)','Sum of all CAPEX + OPEX items in each cost category','red'],
       ['Contingency','Extra % of COGS as cost buffer (set in Settings)','amber'],
       ['Total COGS','All direct costs combined','red bold'],
       ['Gross Profit','Total Income − Total COGS','blue bold'],
       ['OPEX Buffer','% of Total Income reserved for operating expenses','amber'],
       ['EBITDA','Gross Profit − OPEX Buffer. <strong>Earnings before financial items</strong>.','purple bold'],
       ['Compliance Bond Fee','Performance bond cost spread over project months','gray'],
       ['Upfront Bond Fee','One-time bond spread over all months','gray'],
       ['Hedging','Cost to hedge PEN/USD exposure (auto-calculated from your currency flags)','gray'],
       ['Financing Cost','Interest on negative Accumulated CF (cost of being cash-negative)','gray'],
       ['EBIT','EBITDA − all financial items. Net operating result.','blue bold'],
      ].map(([n,d,c])=>`<tr style="border-bottom:1px solid #f1f5f9">
       <td style="padding:7px 10px;font-weight:600;font-size:11px;color:#0176D3;white-space:nowrap">${n}</td>
       <td style="padding:7px 10px;font-size:11px;color:#374151">${d}</td>
      </tr>`).join('')}
     </table>
    </div>`
  },
  {
   id:'cf', icon:'💵', title:'Cash Flow Logic', color:'#2E844A',
   content: `
    <p style="margin-bottom:14px">The Cash Flow shows <strong>actual cash movements</strong>. Unlike P&L (accrual), CF tracks when money physically moves.</p>
    <div style="display:grid;gap:10px">
     <div style="padding:12px;border-radius:8px;background:#eafaea;border-left:3px solid #2E844A;font-size:11px;line-height:1.7">
      <strong>Inflows</strong> = cash collected from clients (same as revenue but shifted by payment terms + IGV if applicable)
     </div>
     <div style="padding:12px;border-radius:8px;background:#fef1f1;border-left:3px solid #BA0517;font-size:11px;line-height:1.7">
      <strong>Outflows</strong> = cash paid to suppliers (shown as negative; shifted by payment conditions)
     </div>
     <div style="padding:12px;border-radius:8px;background:#e8f4fd;border-left:3px solid #0176D3;font-size:11px;line-height:1.7">
      <strong>Net CF</strong> = Inflows + Outflows (outflows are negative, so this is Inflows − |Outflows|)
     </div>
     <div style="padding:12px;border-radius:8px;background:#f5f3ff;border-left:3px solid #5A2D82;font-size:11px;line-height:1.7">
      <strong>Accumulated CF</strong> = running sum of Net CF month by month. The moment it crosses zero = Break-even. If it goes negative, you need financing.
     </div>
    </div>
    <div style="margin-top:12px;background:#fff8e8;padding:12px;border-radius:8px;border-left:3px solid #A56105;font-size:11px;line-height:1.7">
     💡 <strong>Practical example:</strong> You recognize $100K revenue in month 1 (P&L). Client pays on 60-day terms → CF shows +$100K in month 3. In between, you still need to pay suppliers → Accumulated CF goes negative → Financing Cost kicks in.
    </div>`
  },
  {
   id:'tips', icon:'💡', title:'Common Mistakes', color:'#BA0517',
   content: `
    <div style="display:grid;gap:10px">
     ${[
      ['Distributions don\'t sum to 100%','Every revenue/cost item\'s monthly % grid must sum to exactly 100%, otherwise your totals will be wrong. Watch the colored progress bar below each grid.'],
      ['Currency confusion','Always enter amounts in USD (ex-VAT). If a cost is in PEN, mark it as PEN — the model will use the FX rate for hedging. Do NOT convert to USD manually.'],
      ['IGV in P&L','P&L always shows ex-VAT amounts. Only CF is affected by IGV (if you check the IGV box, CF cash flows are multiplied by 1.18).'],
      ['Linked recognition with no cost distribution','If you link a revenue to a CAPEX/OPEX item but that cost item has no monthly distribution set, the revenue will be spread evenly. Set up the cost distribution first.'],
      ['EBITDA vs EBIT','EBITDA = Gross Profit − OPEX Buffer (operating result). EBIT = EBITDA − Bond Fees − Hedging − Financing Cost (after all financial items). They will differ when you have bonds or hedging costs.'],
      ['Negative Accumulated CF is normal','It means you need working capital / financing. The model automatically calculates financing cost on the negative balance. Check the Break-even point in the dashboard.'],
      ['Custom lines formulas','Use variable names exactly as shown in the Formula Builder chip panel. Wrong name = 0. Always preview the formula before saving.'],
     ].map(([t,d])=>`
      <div style="padding:12px 14px;border-radius:8px;background:#fff5f5;border-left:3px solid #BA0517;font-size:11px;line-height:1.7">
       <div style="font-weight:700;color:#BA0517;margin-bottom:4px">⚠️ ${t}</div>
       <div style="color:#374151">${d}</div>
      </div>`).join('')}
    </div>`
  },
 ];
 ...
 function render() {
  root.innerHTML = '';
  const wrap = document.createElement('div');
  wrap.style.cssText = 'display:grid;grid-template-columns:220px 1fr;gap:0;min-height:600px;border:1px solid #e2e8f0;border-radius:14px;overflow:hidden;background:#fff';

  // Sidebar
  const sidebar = document.createElement('div');
  sidebar.style.cssText = 'background:#f8fafc;border-right:1px solid #e2e8f0;padding:16px 0';
  const sideTitle = document.createElement('div');
  sideTitle.style.cssText = 'font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:.08em;color:#94a3b8;padding:0 16px 12px';
  sideTitle.textContent = '📚 Know How Guide';
  sidebar.appendChild(sideTitle);

  sections.forEach(sec => {
   const btn = document.createElement('button');
   const a = sec.id === activeSection;
   btn.style.cssText = 'display:flex;align-items:center;gap:10px;width:100%;padding:10px 16px;border:none;cursor:pointer;text-align:left;transition:all .12s;border-left:3px solid '
    + (a?sec.color+';background:'+sec.color+'12;':'transparent;background:transparent;');
   btn.innerHTML = '<span style="font-size:16px">' + sec.icon + '</span>'
    + '<span style="font-size:12px;font-weight:'+(a?'700':'500')+';color:'+(a?sec.color:'#374151')+'">' + sec.title + '</span>';
   btn.addEventListener('click', () => { activeSection = sec.id; render(); });
  ...
  content.style.cssText = 'padding:28px 32px;overflow-y:auto;max-height:80vh';
  const sec = sections.find(s => s.id === activeSection);
  if (sec) {
   content.innerHTML = '<div style="display:flex;align-items:center;gap:10px;margin-bottom:20px;padding-bottom:16px;border-bottom:2px solid '+sec.color+'20">'
    + '<span style="font-size:28px">' + sec.icon + '</span>'
    + '<h2 style="margin:0;font-size:18px;font-weight:800;color:' + sec.color + '">' + sec.title + '</h2>'
    + '</div>'
    + '<div style="font-size:12px;line-height:1.75;color:#374151">' + sec.content + '</div>';
  }
...
document.addEventListener('DOMContentLoaded', () => {
 const khTab = document.querySelector('[data-tab="knowhow"]');
 if (khTab) khTab.addEventListener('click', renderKnowHow);
});
function _mkChart(id, config) {
  ...
  try { _charts[id] = new Chart(canvas.getContext('2d'), config); }
  catch(e) { console.warn('Chart error ['+id+']:', e); }
  return _charts[id] || null;
}

// Gradient plugin — creates gradients after canvas is sized
const _gradPlugin = {
  id: 'gradFill',
...
if (Chart && Chart.register) Chart.register(_gradPlugin);

function renderDashboard() {
  if (typeof Chart === 'undefined') { setTimeout(renderDashboard, 150); return; }
  const cv = compute();
  const tR  = rSum(cv.totalIncome),   tC  = rSum(cv.totalCogsAdj);
  const tG  = rSum(cv.grossProfit),   tEB = rSum(cv.ebitda);
  const tE  = rSum(cv.ebit),          tN  = rSum(cv.netCF);
  const mg  = tR ? (tG /tR*100).toFixed(1) : '0.0';
  const ebm = tR ? (tEB/tR*100).toFixed(1) : '0.0';
  const em  = tR ? (tE /tR*100).toFixed(1) : '0.0';

  // ── KPI Cards ──
  document.getElementById('kpi-grid').innerHTML = [
    { l:'Revenue',      v:'$'+fmtK(tR),       c:'#06A59A', sub:'Total Income',           icon:'📈' },
    { l:'COGS',         v:'$'+fmtK(tC),        c:'#E05252', sub:'Direct Costs',           icon:'📦' },
    { l:'Gross Profit', v:'$'+fmtK(tG),        c:'#0176D3', sub:'GP Margin '+mg+'%',      icon:'💰' },
    { l:'EBITDA',       v:'$'+fmtK(tEB),       c:'#5A2D82', sub:'EBITDA Margin '+ebm+'%', icon:'⚡' },
    { l:'EBIT',         v:'$'+fmtK(tE),        c:tE>=0?'#2E844A':'#BA0517', sub:'EBIT Margin '+em+'%', icon:'🎯' },
    { l:'Net CF',       v:'$'+fmtK(tN),        c:tN>=0?'#2E844A':'#BA0517', sub:'Cash Generated',      icon:'💵' },
    { l:'NPV',          v:'$'+fmtK(cv.npvCF),  c:'#1B96FF', sub:'At '+CFG.dis+'% discount',            icon:'📊' },
  ].map(({l,v,c,sub,icon})=>`
    <div class="kpi" style="border-left:3px solid ${c};background:linear-gradient(135deg,#fff 70%,${c}0a);position:relative;overflow:hidden">
      <div style="position:absolute;top:-4px;right:4px;font-size:28px;opacity:.1">${icon}</div>
      <div style="font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#94a3b8;margin-bottom:4px">${icon} ${l}</div>
      <div class="kpi-val" style="color:${c};font-size:clamp(14px,2vw,20px);font-weight:800;letter-spacing:-.02em">${v}</div>
      <div style="font-size:9px;color:${c};opacity:.75;margin-top:2px;font-weight:600">${sub}</div>
    </div>`).join('');

  // View toggle
  const ctWrap = document.getElementById('chart-view-tog');
  ctWrap.innerHTML = '<span style="font-size:11px;color:var(--sub);font-weight:600;margin-right:4px">View:</span>';
  ...
  const dG  = isA ? aggY(cv.grossProfit)  : cv.grossProfit;
  const dEB = isA ? aggY(cv.ebitda)       : cv.ebitda;
  const dE  = isA ? aggY(cv.ebit)         : cv.ebit;
  ...
  document.getElementById('rc-chart-title').textContent =
    (isA?'Annual':'Monthly') + ' · Revenue, COGS & Gross Profit';

  // ── Shared Chart.js config pieces ──
  const F  = { family:"'Inter','Segoe UI',system-ui,sans-serif", size:10 };
  const TC = '#94a3b8';
  const GC = 'rgba(148,163,184,0.1)';
  const TT = {
    backgroundColor:'rgba(15,23,42,0.93)', titleColor:'#f1f5f9',
    bodyColor:'#94a3b8', padding:12, cornerRadius:10,
    titleFont:{...F, weight:'700', size:11},
    borderColor:'rgba(255,255,255,.08)', borderWidth:1
  };
  const xAx = () => ({
    grid:{display:false}, border:{display:false},
    ticks:{color:TC, font:{...F,size:9}, maxRotation:isA?0:40, minRotation:isA?0:40}
  });
  const yAx = (extra={}) => ({
    grid:{color:GC, drawBorder:false}, border:{display:false, dash:[4,3]},
    ticks:{color:TC, font:F, callback:v=>fmtK(v)}, ...extra
  });
  const yAxPct = () => yAx({ticks:{color:TC, font:F, callback:v=>v.toFixed(0)+'%'}});
  const legOpts = {display:true, position:'bottom',
    labels:{font:F, color:TC, boxWidth:10, padding:16, usePointStyle:true}};

  // ── Chart 1: Revenue vs COGS + Gross Profit line ──
  _mkChart('chart-rc', {
    type: 'bar',
    data: {
      labels: dL,
      datasets: [
        { label:'Revenue', data:dI,
          backgroundColor:'rgba(6,165,154,0.75)', borderColor:'#06A59A',
          borderWidth:1.5, borderRadius:4, borderSkipped:false, order:2 },
        { label:'COGS', data:dC.map(Math.abs),
          backgroundColor:'rgba(224,82,82,0.7)', borderColor:'#E05252',
          borderWidth:1.5, borderRadius:4, borderSkipped:false, order:3 },
        { label:'Gross Profit', data:dG, type:'line',
          borderColor:'#0176D3', borderWidth:2.5,
          backgroundColor:'rgba(1,118,211,0.12)', fill:true, tension:.4, order:1,
          pointRadius:isA?5:2, pointHoverRadius:7,
          pointBackgroundColor:'#fff', pointBorderColor:'#0176D3', pointBorderWidth:2 }
      ]
    },
    options: {
      ...
      plugins:{legend:legOpts, tooltip:{...TT,
  ...
  // ── Chart 2: Margin % ──
  const gpM  = dI.map((v,i)=>v?(dG[i]/v*100):0);
  const ebdM = dI.map((v,i)=>v?(dEB[i]/v*100):0);
  const ebiM = dI.map((v,i)=>v?(dE[i]/v*100):0);
  _mkChart('chart-margins', {
    type:'line',
    data:{
      labels:dL,
      datasets:[
        { label:'GP Margin %', data:gpM,
          borderColor:'#0176D3', borderWidth:2.5,
          backgroundColor:'rgba(1,118,211,0.15)', fill:true, tension:.45,
          pointRadius:isA?5:1.5, pointHoverRadius:7,
          pointBackgroundColor:'#fff', pointBorderColor:'#0176D3', pointBorderWidth:2 },
        { label:'EBITDA Margin %', data:ebdM,
          borderColor:'#5A2D82', borderWidth:2,
          backgroundColor:'rgba(90,45,130,0.1)', fill:true, tension:.45,
          pointRadius:isA?4:1.5, pointHoverRadius:6,
          pointBackgroundColor:'#fff', pointBorderColor:'#5A2D82', pointBorderWidth:1.5 },
        { label:'EBIT Margin %', data:ebiM,
          borderColor:'#2E844A', borderWidth:1.8, borderDash:[5,4],
          fill:false, tension:.45,
          pointRadius:isA?4:1, pointHoverRadius:6,
          pointBackgroundColor:'#fff', pointBorderColor:'#2E844A', pointBorderWidth:1.5 }
      ]
    },
    options:{
      responsive:true, maintainAspectRatio:false,
      ...
      scales:{x:xAx(), y:yAxPct()}
    }
  });

  // ── Chart 3: Net CF signed bars ──
  const ncfPos = dN.map(v=>v>=0?'rgba(46,132,74,0.8)':'rgba(186,5,23,0.8)');
  const ncfBdr = dN.map(v=>v>=0?'#2E844A':'#BA0517');
  _mkChart('chart-ncf', {
    type:'bar',
    data:{
      labels:dL,
      datasets:[{
        label:'Net CF', data:dN,
        backgroundColor:ncfPos, borderColor:ncfBdr,
    ...
    options:{
      ...
      plugins:{legend:{display:false}, tooltip:{...TT,
        callbacks:{label:ctx=>'  Net CF:  $'+fmt(ctx.raw)+(ctx.raw>=0?'  ✓':'  ✗')}}},
      scales:{
        x:xAx(),
        y:{...yAx(), grid:{color:ctx=>ctx.tick.value===0?'rgba(0,0,0,.2)':GC,
            lineWidth:ctx=>ctx.tick.value===0?2:1, drawBorder:false}}
      }
    }
  });

  // ── Chart 4: Accumulated CF dual-area + break-even ──
  const beIdx = dA.findIndex(v=>v>=0);
  const posD  = dA.map(v=>v>=0?v:null);
  const negD  = dA.map(v=>v<0?v:null);
  const ptR   = dA.map((_,i)=>i===beIdx?8:(isA?4:1.5));
  const ptBg  = dA.map((_,i)=>i===beIdx?'#FFB300':(dA[i]>=0?'#2E844A':'#BA0517'));
  const ptBdr = dA.map((_,i)=>i===beIdx?'#fff':'transparent');
  const ptBW  = dA.map((_,i)=>i===beIdx?3:0);
  _mkChart('chart-accum', {
    type:'line',
    data:{
      labels:dL,
      datasets:[
        { data:posD, borderWidth:0,
          backgroundColor:'rgba(46,132,74,0.18)', fill:true,
          tension:.4, pointRadius:0, spanGaps:false },
        { data:negD, borderWidth:0,
          backgroundColor:'rgba(186,5,23,0.18)', fill:true,
          tension:.4, pointRadius:0, spanGaps:false },
        { label:'Accumulated CF', data:dA,
          borderColor:'#1c2b3a', borderWidth:2.8, fill:false, tension:.4,
          pointRadius:ptR, pointHoverRadius:9,
          pointBackgroundColor:ptBg, pointBorderColor:ptBdr, pointBorderWidth:ptBW }
      ]
    },
    options:{
      responsive:true, maintainAspectRatio:false,
      ...
      plugins:{
        ...
        tooltip:{...TT, callbacks:{label:ctx=>{
          ...
          const be = ctx.dataIndex===beIdx ? '  ★ Break-even' : '';
          return '  Accumulated CF:  $'+fmt(ctx.raw)+be;
        }}}
      },
      scales:{
        x:xAx(),
        y:{...yAx(), grid:{color:ctx=>ctx.tick.value===0?'rgba(0,0,0,.22)':GC,
           lineWidth:ctx=>ctx.tick.value===0?2:1, drawBorder:false}}
      }
    }
  });
  ...
  const wfRev  = rSum(cv.totalIncome);
  const wfCogs = Math.abs(rSum(cv.totalCogsAdj));
  const wfGP   = rSum(cv.grossProfit);
  const wfOPX  = rSum(cv.opexBuffer);
  const wfEB   = rSum(cv.ebitda);
  const wfFin  = rSum(cv.bondFees) + rSum(cv.hedgingArr) + rSum(cv.finCostArr);
  const wfEBIT = rSum(cv.ebit);

  // Floating bar data: [low, high]
  const wfData = [
    [0,        wfRev ],    // Revenue
    [wfGP,     wfRev - wfCogs], // COGS deduction (top of GP = Rev-Cogs)
    [0,        wfGP  ],    // Gross Profit
    [wfEB,     wfGP - wfOPX],   // OPEX deduction
    [0,        wfEB  ],    // EBITDA
    [wfEBIT,   wfEB - wfFin],   // Financial deductions
    [0,        wfEBIT],   // EBIT
  ];
  const wfLabels = ['Revenue','− COGS','Gross Profit','− OPEX','EBITDA','− Financial','EBIT'];
  const isTotalBar = [true,false,true,false,true,false,true];
  const wfBg = wfData.map(([a,b],i) => {
    if (!isTotalBar[i]) return 'rgba(224,82,82,0.65)';
    if (i===4) return b>=0?'rgba(90,45,130,0.8)':'rgba(186,5,23,0.8)';
    if (i===6) return b>=0?'rgba(46,132,74,0.8)':'rgba(186,5,23,0.8)';
    return b>=0?'rgba(1,118,211,0.8)':'rgba(186,5,23,0.8)';
  });
  const wfBdr = wfData.map(([a,b],i) => {
    if (!isTotalBar[i]) return '#E05252';
    if (i===4) return b>=0?'#5A2D82':'#BA0517';
    if (i===6) return b>=0?'#2E844A':'#BA0517';
    return b>=0?'#0176D3':'#BA0517';
  });

  // Inline label plugin for waterfall
  const wfLabelPlugin = {
    id:'wfLabels',
    afterDatasetsDraw(chart) {
      const {ctx:c, scales:{x,y}} = chart;
      chart.getDatasetMeta(0).data.forEach((bar,i) => {
        const [lo,hi] = wfData[i];
        const v = hi - lo;
        ...
        c.fillStyle = bdrC;
        c.textAlign = 'center';
        const yPos = hi>=lo ? bar.y - 6 : bar.y + bar.height + 14;
        c.fillText(lbl, bar.x, yPos);
        c.restore();
      });
    }
  };

  _mkChart('chart-waterfall', {
    type:'bar',
    ...
    options:{
      ...
      plugins:{
        ...
        tooltip:{...TT, callbacks:{label:ctx=>{
          const [lo,hi]=ctx.raw;
          return '  '+wfLabels[ctx.dataIndex]+':  $'+fmt(hi)+(lo!==0?'  (delta: $'+fmt(hi-lo)+')':'');
        }}}
      },
      scales:{
        x:{grid:{display:false}, border:{display:false},
           ticks:{font:{...F,size:10,weight:'600'}, color:TC}},
        y:yAx()
      }
    }
  });
...
// ── Portfolio storage: localStorage-first, Supabase optional sync ──
const LS_KEY = 'fm_portfolio_v1';
...
async function loadPortfolio() {
  // 1. Always load from localStorage immediately
  portfolio = lsLoad();
  portfolioLoaded = true;

  // 2. Try Supabase (with or without auth)
  if (!supabaseClient) { _renderPortfolioIfOpen(); return; }
  try {
    // Try with session first (if auth is set up)
    let userId = null;
    try {
    ...
    // If auth → filter by user; if no auth + RLS disabled → get all (single-user mode)
    ...
    const localOnly = portfolio.filter(p => p.id && String(p.id).startsWith('local_') && !remoteIds.has(p.id));
    portfolio = [...remote, ...localOnly];
    lsSave(portfolio);
    _sbStatus.ok = true; _sbStatus.tested = true; _sbStatus.error = '';
  } catch(e) {
    _sbStatus.ok = false; _sbStatus.tested = true; _sbStatus.error = e.message || String(e);
    console.warn('[Supabase] loadPortfolio failed:', _sbStatus.error);
  }
  _renderPortfolioIfOpen();
}

function _renderPortfolioIfOpen() {
...
async function saveProjectToSupabase(project) {
  ...
  if (!project.id) project.id = lsGenId();
  ...
  // Try Supabase
  if (!supabaseClient) return project;
  try {
    let userId = null;
    try {
      const { data: sd } = await supabaseClient.auth.getSession();
    ...
    // Only include user_id if we have auth (avoids NOT NULL violation)
    ...
    // Replace local entry with remote UUID
    const ls = lsLoad();
    const li = ls.findIndex(p => p.id === project.id);
    if (li >= 0) ls[li] = remote; else ls.unshift(remote);
    lsSave(ls);
    _sbStatus.ok = true; _sbStatus.error = '';
    return remote;
  } catch(e) {
    _sbStatus.ok = false; _sbStatus.error = e.message || String(e);
    console.warn('[Supabase] saveProject failed:', _sbStatus.error);
    // Show toast to user
    _sbToast('⚠️ Saved locally. Supabase error: ' + (_sbStatus.error.length > 80 ? _sbStatus.error.slice(0,80)+'…' : _sbStatus.error));
  }
  return project;
}

async function deleteProjectFromSupabase(id) {
  const ls = lsLoad().filter(p => p.id !== id);
  lsSave(ls);
  if (!supabaseClient || String(id).startsWith('local_')) return;
  try {
...
function _sbToast(msg) {
  ...
  if (!t) {
    t = document.createElement('div'); t.id = 'sb-toast';
    t.style.cssText = 'position:fixed;bottom:80px;left:50%;transform:translateX(-50%);background:rgba(28,43,58,.95);color:#fff;padding:10px 18px;border-radius:8px;font-size:12px;z-index:9999;max-width:420px;text-align:center;box-shadow:0 4px 20px rgba(0,0,0,.3);pointer-events:none';
...
function buildSummarySheet(cv, type, projName) {
 ...
 rows.push([' Total CF Outflows', f2(-(rSum(cv.totalCogsCFAdj)+rSum(cv.opexBufferCF)+rSum(cv.finExpCB)+rSum(cv.finExpUB)+rSum(cv.hedgingArr))), '', '']);
...
// Template columns
const REV_COLS = ['name','amount','currency(USD/PEN)','category','igv(yes/no)','reference_note','reference_url'];
const COST_COLS = ['name','amount','currency(USD/PEN)','category(Equipment/Payroll/Services/Overhead)','igv(yes/no)','payment_cond_days','advance_pct','reference_note','reference_url'];
...
function downloadTemplate(type) {
 ...
 const headers = isRev
 ? ['name', 'amount', 'currency (USD/PEN)', 'category', 'igv (yes/no)', 'reference_note', 'reference_url']
 : ['name', 'amount', 'currency (USD/PEN)', 'category (Equipment/Payroll/Services/Overhead)', 'igv (yes/no)', 'payment_cond_days', 'advance_pct', 'reference_note', 'reference_url'];
 ...
 // Column widths
 ws['!cols'] = headers.map((h, i) => ({ wch: Math.max(h.length + 2, 18) }));

 // Style header row bold (SheetJS CE doesn't support full styles, but sets !ref)
 const wb = XLSX.utils.book_new();
...
function importCSV(type, text) {
 ...
 const amount = getFirst(row, ['amount','total','value','monto']);
 ...
 item.name = name;
 item.amount = amount;
 item.cur = (getFirst(row, ['currency_usd_pen_','currency','moneda']) || 'USD').toUpperCase().includes('PEN') ? 'PEN' : 'USD';
 item.cat = getFirst(row, ['category','categoria','type']);
 item.igv = (getFirst(row, ['igv_yes_no_','igv','incluye_igv']) || '').toLowerCase() === 'yes';
 ...
 item.cat = catMap[catRaw.toLowerCase().trim()] || 'Equipment';
 item.igv = (getFirst(row, ['igv_yes_no_','igv']) || '').toLowerCase() === 'yes';
 ...
 renderList(type);
 alert(' Imported ' + added + ' item' + (added!==1?'s':'') + ' into ' + type.toUpperCase() + '.');
}
...
(function(){
  ...
  // Wrap compute so all views receive reliable totals.
  const _baseCompute = compute;
  compute = window.compute = function(){
    const cv = _baseCompute();
    // totalCogsAdj from compute() is now correct (finExpCB/UB excluded from costCat).
    // grossProfit from compute() is also correct.
    // Recompute bondFees in case it was cleared.
    cv.bondFees = _arrAdd(cv.finExpCB || _arrZero(), cv.finExpUB || _arrZero());
    // EBITDA = Gross Profit − OPEX Buffer  (correct definition; finExp sits below EBITDA)
    cv.ebitda = _arrSub(cv.grossProfit, cv.opexBuffer || _arrZero());
    // EBIT = EBITDA − Bond Fees − Hedging − Financing Cost
    cv.ebit = _sumMany([cv.ebitda, _arrNeg(cv.bondFees), _arrNeg(cv.hedgingArr || _arrZero()), _arrNeg(cv.finCostArr || _arrZero())]);
    cv.profitBeforeOpex = _arrSub(cv.grossProfit, cv.bondFees);
    // Recalculate netCF using visible CF rows (consistent with CF table display).
    const adjIncomeCF = (cv.adjIncomeRows || []).map(ac =>
      (ac.arrCF || ac.arr || _arrZero()).map(v => (ac.adj && ac.adj.sign === 'add') ? (Number(v)||0) : -(Number(v)||0)));
    const adjCostCF = (cv.adjCostRows || []).map(ac =>
      (ac.arrCF || ac.arr || _arrZero()).map(v => (ac.adj && ac.adj.sign === 'add') ? (Number(v)||0) : -(Number(v)||0)));
    const inflows = _sumMany([...Object.values(cv.revCatsCF || {}), cv.penArrCF || _arrZero(), ...adjIncomeCF]);
    const outflows = _sumMany([
      ...(cv.userCats||[]).map(c => (cv.costCatCF||{})[c] || _arrZero()),
      (cv.costCatCF||{}).Contingency || _arrZero(),
      ...adjCostCF,
      cv.opexBufferCF || _arrZero(),
      cv.finExpCB || _arrZero(),
      cv.finExpUB || _arrZero(),
      cv.hedgingArr || _arrZero(),
      cv.finCostArr || _arrZero()
    ]);
  ...
  renderPL = window.renderPL = function(){
    ...
    // COGS display: user cats + contingency + adj (no finExpCB/UB — those go below EBITDA)
    const cogsBase = _cogsRawFromVisible(cv);              // positive
    const totalCogs = _arrNeg(_sumMany([cogsBase, ...cogsCustom.map(x=>x.arr)])); // negative for display
    const grossProfit = _arrAdd(totalIncome, totalCogs);   // income - cogs
    // EBITDA = Gross Profit − OPEX Buffer  (from compute())
    const ebitda = _arrClone(cv.ebitda || _arrSub(grossProfit, cv.opexBuffer || _arrZero()));
    // EBIT = EBITDA − Bond Fees − Hedging − Financing Cost  (from compute())
    const ebit = _arrClone(cv.ebit || _sumMany([ebitda, _arrNeg(cv.bondFees||_arrZero()), _arrNeg(cv.hedgingArr||_arrZero()), _arrNeg(cv.finCostArr||_arrZero())]));
    ...
    h += secRow('FINANCIAL & INTEREST','var(--th-bond)');
    ...
    h += sum('EBIT', ebit, 'var(--blue,#0176D3)', totalIncome);
    h += '</tbody>';
    document.getElementById('pl-tbl').innerHTML = h;
  };
