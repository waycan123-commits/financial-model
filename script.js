...
function renderDashboard() {
 ...
 if (kpiEl) {
  kpiEl.innerHTML = [
   ...
   + '<div class="kpi-val" style="color:' + c + ';text-shadow:0 0 22px ' + c + '55">' + v + '</div></div>'
  ).join('');
 }
 ...
 const labels = isA ? yrLabels() : AMs().map(m => mInfo(m).short);
 const incomeData = isA ? aggY(cv.totalIncome) : cv.totalIncome;
 const cogsData = isA ? aggY(cv.totalCogsAdj) : cv.totalCogsAdj;
 const gpData = isA ? aggY(cv.grossProfit) : cv.grossProfit;
 const ebitdaData = isA ? aggY(cv.ebitda || Z()) : (cv.ebitda || Z());
 const ebitData = isA ? aggY(cv.ebit || Z()) : (cv.ebit || Z());
 const netCFData = isA ? aggY(cv.netCF) : cv.netCF;
 const accumCFData = isA ? aggAccY(cv.accumCF) : cv.accumCF;

 const title = document.getElementById('rc-chart-title');
 if (title) title.textContent = 'Revenue vs COGS  -  ' + (isA ? 'Annual' : 'Monthly');

 // Ensure we always have a real canvas. This fixes blank dashboard cards when the HTML had <div> or <canvas> inconsistently.
 function getCanvas(id, preferredHeight) {
  ...
  parent.style.minHeight = parent.style.minHeight || (preferredHeight + 'px');
  canvas.style.display = 'block';
  canvas.style.width = '100%';
  canvas.style.height = preferredHeight + 'px';

  const rectW = Math.floor((parent.clientWidth || el.clientWidth || 600));
  const cssW = Math.max(320, rectW);
  ...
  if (!ctx) {
   parent.innerHTML = '<div style="font-size:12px;color:#9ca3af;padding:24px">No se pudo renderizar el gráfico.</div>';
   return null;
  }
 ...
 function niceRange(values) {
  ...
  min -= pad; max += pad;
  if (Math.abs(min) < 0.00001) min = 0;
  if (Math.abs(max) < 0.00001) max = 0;
 ...
 function drawAxes(ctx, w, h, pad, min, max, percent) {
  ...
  for (let i = 0; i <= 4; i++) {
   ...
   ctx.fillText(percent ? (v.toFixed(0) + '%') : fmtK(v), pad.l - 8, y);
  }
  const zeroY = toY(0);
 ...
 function drawLegend(ctx, series, x, y) {
  ...
  series.forEach(s => {
   ...
   ctx.fillText(s.label, cx + 14, y);
   cx += ctx.measureText(s.label).width + 42;
  });
  ctx.restore();
 }
 ...
 function drawBarChart(id, series, labels, opts) {
  opts = opts || {};
  const surface = getCanvas(id, opts.height || 180);
  if (!surface) return;
  const { ctx, w, h } = surface;
  const pad = { t: 26, r: 18, b: 42, l: 68 };
  const allVals = series.flatMap(s => s.data || []);
  const { min, max } = niceRange(allVals);
  const { toY, zeroY, plotW } = drawAxes(ctx, w, h, pad, min, max, !!opts.percent);
  const n = Math.max(1, labels.length);
  const groupW = plotW / n;
  const barW = Math.max(5, Math.min(26, (groupW * 0.72) / Math.max(1, series.length)));
  ...
  labels.forEach((lbl, i) => {
   ...
   const labelStep = isA ? 1 : Math.max(1, Math.ceil(labels.length / 8));
   if (i % labelStep === 0 || i === labels.length - 1) {
    ctx.save();
    ctx.fillStyle = '#6B7280';
    ctx.font = isA ? '11px Arial, sans-serif' : '10px Arial, sans-serif';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'top';
    if (!isA && labels.length > 10) {
     ctx.translate(groupX, h - pad.b + 13);
     ctx.rotate(-Math.PI / 7);
     ctx.fillText(lbl, 0, 0);
    } else {
     ctx.fillText(lbl, groupX, h - pad.b + 12);
    }
    ctx.restore();
   }
  });
  drawLegend(ctx, series, pad.l, 12);
 }

 function drawLineChart(id, data, labels, color) {
  const surface = getCanvas(id, 145);
  if (!surface) return;
  const { ctx, w, h } = surface;
  const pad = { t: 20, r: 20, b: 36, l: 68 };
  const { min, max } = niceRange(data);
  const { toY, zeroY, plotW } = drawAxes(ctx, w, h, pad, min, max, false);
  const plotH = h - pad.t - pad.b;
  const n = Math.max(1, data.length);
  const toX = i => pad.l + (n === 1 ? 0 : i / (n - 1)) * plotW;

  // area
  ctx.save();
  ctx.beginPath();
  ctx.moveTo(toX(0), zeroY);
  data.forEach((v, i) => ctx.lineTo(toX(i), toY(v || 0)));
  ...
  data.forEach((v, i) => {
   if (i % step === 0 || i === n - 1) {
    ctx.beginPath(); ctx.arc(toX(i), toY(v || 0), 3, 0, Math.PI * 2);
  ...
  ctx.textBaseline = 'top';
  labels.forEach((lbl, i) => {
   if (i % step === 0 || i === n - 1) ctx.fillText(lbl, toX(i), h - pad.b + 12);
  });
  ctx.restore();
 }
 ...
 try {
  drawBarChart('chart-rc', [
   ...
   { label: 'COGS', data: cogsData.map(v => Math.abs(v)), color: '#BA0517', opacity: 0.85 },
   { label: 'Gross Profit', data: gpData, color: v => v >= 0 ? '#0176D3' : '#A56105', opacity: 0.9 },
  ], labels, { height: 190 });
  ...
  drawBarChart('chart-margins', [
   { label: 'GP %', data: gpMarginData, color: v => v >= 0 ? '#0176D3' : '#BA0517' },
   { label: 'EBITDA %', data: ebitdaMarginData, color: v => v >= 0 ? '#2E844A' : '#BA0517', opacity: 0.85 },
   { label: 'EBIT %', data: ebitMarginData, color: v => v >= 0 ? '#5A2D82' : '#BA0517', opacity: 0.85 },
  ], labels, { height: 190, percent: true });

  drawBarChart('chart-ncf', [
   { label: 'Net CF', data: netCFData, color: v => v >= 0 ? '#0176D3' : '#BA0517' },
  ], labels, { height: 145 });

  drawLineChart('chart-accum', accumCFData, labels, '#5A2D82');

  drawBarChart('chart-waterfall', [
   { label: 'USD', data: [
    totalRevenue,
    -Math.abs(totalCogs),
    grossProfitTotal,
    -Math.abs(rSum(cv.opexBuffer || [])),
    ebitdaTotal,
    ebitTotal,
    netCFTotal,
   ], color: v => v >= 0 ? '#0176D3' : '#BA0517', opacity: 0.9 },
  ], ['Revenue', 'COGS', 'GP', 'OPEX Buf.', 'EBITDA', 'EBIT', 'Net CF'], { height: 205 });
 } catch (e) {
  console.error('[Dashboard charts] render error:', e);
  ['chart-rc','chart-margins','chart-ncf','chart-accum','chart-waterfall'].forEach(id => {
   const el = document.getElementById(id);
   if (el) el.innerHTML = '<div style="font-size:12px;color:#BA0517;padding:18px">Error rendering chart: ' + esc(e.message) + '</div>';
  });
 }
}
