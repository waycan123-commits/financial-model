...
//  DASHBOARD 
let dashChart3D = false; // global 3D toggle
...
function renderDashboard() {
 ...
 // 3D toggle button
 let btn3d = document.getElementById('dash-3d-btn');
 if (!btn3d) {
   btn3d = document.createElement('button');
   btn3d.id = 'dash-3d-btn';
   btn3d.style.cssText = 'margin-left:10px;padding:5px 14px;border-radius:20px;border:1.5px solid var(--accent);background:'+(dashChart3D?'var(--accent)':'transparent')+';color:'+(dashChart3D?'#fff':'var(--accent)')+';font-size:11px;font-weight:700;cursor:pointer;transition:all .2s;letter-spacing:.04em';
   btn3d.textContent = '⬡ 3D';
   btn3d.addEventListener('click', () => { dashChart3D = !dashChart3D; renderDashboard(); });
   ctWrap.appendChild(btn3d);
 } else {
   btn3d.style.background = dashChart3D ? 'var(--accent)' : 'transparent';
   btn3d.style.color = dashChart3D ? '#fff' : 'var(--accent)';
 }
 ...
 function make3DBarChart(container, series, labels, opts) {
   opts = opts || {};
   ...
   const depth = 6; // 3D depth px
   const barW = Math.max(6, Math.min(28, cW / n / (series.length + 0.5) - 2));
   ...
   let svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" style="overflow:visible;filter:drop-shadow(0 4px 12px rgba(0,0,0,0.1))">`;

   // Grid lines
   for (let s = 0; s <= 4; s++) {
     const v = minV + (range / 4) * s;
     const y = toY(v);
     svg += `<line x1="${PAD.l}" y1="${y}" x2="${W-PAD.r}" y2="${y}" stroke="#e2e8f0" stroke-width="1" stroke-dasharray="4,3"/>`;
     svg += `<text x="${PAD.l-5}" y="${y+4}" text-anchor="end" font-size="9" fill="#94a3b8" font-family="system-ui">${fmtK(v)}</text>`;
   }
   // Zero line
   svg += `<line x1="${PAD.l}" y1="${zeroY}" x2="${W-PAD.r}" y2="${zeroY}" stroke="#64748b" stroke-width="1.5"/>`;

   labels.forEach((lbl, i) => {
     const grpX = PAD.l + grpGap + i * (grpW + grpGap);
     series.forEach((ser, si) => {
       const v = ser.data[i] || 0;
       const x = grpX + si * (barW + 2);
       const y1 = Math.min(toY(v), zeroY);
       const bh = Math.max(2, Math.abs(toY(v) - zeroY));
       const col = typeof ser.color === 'function' ? ser.color(v) : ser.color;
       const colDark = col.replace('#', '');

       if (dashChart3D && bh > 3) {
         // 3D top face
         const topY = y1;
         svg += `<polygon points="${x},${topY} ${x+barW},${topY} ${x+barW+depth},${topY-depth/2} ${x+depth},${topY-depth/2}" fill="${col}" opacity="0.95"/>`;
         // 3D right face
         svg += `<polygon points="${x+barW},${topY} ${x+barW},${topY+bh} ${x+barW+depth},${topY+bh-depth/2} ${x+barW+depth},${topY-depth/2}" fill="${col}" opacity="0.6"/>`;
         // Main face
         svg += `<rect x="${x}" y="${topY}" width="${barW}" height="${bh}" fill="${col}" opacity="0.9" rx="1"/>`;
         // Shine
         svg += `<rect x="${x+1}" y="${topY+1}" width="${Math.max(2,barW*0.3)}" height="${Math.max(1,bh*0.4)}" fill="white" opacity="0.2" rx="1"/>`;
       } else {
         // Flat bar with gradient effect
         svg += `<defs><linearGradient id="bg${i}${si}" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="${col}" stop-opacity="1"/><stop offset="100%" stop-color="${col}" stop-opacity="0.7"/></linearGradient></defs>`;
         svg += `<rect x="${x}" y="${y1}" width="${barW}" height="${bh}" fill="url(#bg${i}${si})" rx="2">
           <title>${ser.label}: ${fmt(v)}</title></rect>`;
       }
       if (isA && Math.abs(v) > 0) {
         svg += `<text x="${x+barW/2+(dashChart3D?depth/2:0)}" y="${y1-(dashChart3D?depth/2+4:3)}" text-anchor="middle" font-size="8.5" fill="${col}" font-weight="700" font-family="system-ui">${fmtK(v)}</text>`;
       }
     });
     const lblX = grpX + grpW / 2;
     svg += `<text x="${lblX}" y="${H-PAD.b+14}" text-anchor="middle" font-size="${isA?10:8}" fill="#64748b" font-family="system-ui"
       ${!isA?'transform="rotate(-30,'+lblX+','+(H-PAD.b+14)+')"':''}>${lbl}</text>`;
   });

   // Legend
   const legY = PAD.t - 8;
   series.forEach((ser, si) => {
     const col = typeof ser.color === 'function' ? ser.color(1) : ser.color;
     const legX = PAD.l + si * 90;
     svg += `<rect x="${legX}" y="${legY}" width="10" height="10" fill="${col}" rx="2"/>`;
     svg += `<text x="${legX+14}" y="${legY+8}" font-size="9" fill="#64748b" font-family="system-ui">${ser.label}</text>`;
   });
 ...
 //  3D line / area chart for accumulated CF 
 function make3DLineChart(container, lineData, labels, color) {
   const W = container.clientWidth || 600;
   const H = 115;
   ...
   svg += `<line x1="${PAD.l}" y1="${zeroY}" x2="${W-PAD.r}" y2="${zeroY}" stroke="#94a3b8" stroke-width="1.5"/>`;

   // Fill area
   const pts2 = lineData.map((v,i) => `${toX(i)},${toY(v)}`).join(' ');
   svg += `<polygon points="${PAD.l},${zeroY} ${pts2} ${toX(n-1)},${zeroY}" fill="url(#cfAreaGrad)"/>`;

   // 3D depth lines if enabled
   if (dashChart3D) {
     const shift = 4;
     const pts3d = lineData.map((v,i) => `${toX(i)+shift},${toY(v)+shift}`).join(' ');
     svg += `<polyline points="${pts3d}" fill="none" stroke="${color}" stroke-width="1" opacity="0.3" stroke-dasharray="4,2"/>`;
   }

   // Main line with glow
   svg += `<polyline points="${pts2}" fill="none" stroke="${color}" stroke-width="2.5" stroke-linejoin="round" style="filter:drop-shadow(0 0 4px ${color}66)"/>`;
   ...
   lineData.forEach((v,i) => {
     ...
     svg += `<circle cx="${x}" cy="${y}" r="${dashChart3D?5:3.5}" fill="${v<0?'#BA0517':color}" stroke="#fff" stroke-width="1.5" style="filter:drop-shadow(0 0 3px ${color}88)">
   ...
   if (beIdx > 0) {
     const bx = toX(beIdx), by = toY(lineData[beIdx]);
     svg += `<circle cx="${bx}" cy="${by}" r="6" fill="${color}" stroke="#fff" stroke-width="2" style="filter:drop-shadow(0 0 6px ${color})"/>`;
     svg += `<text x="${bx}" y="${by-10}" text-anchor="middle" font-size="9" fill="${color}" font-weight="800" font-family="system-ui">BE</text>`;
   }

   const lstep = Math.max(1, Math.floor(n/6));
   labels.forEach((lbl, i) => {
     if (i % lstep === 0 || i === n-1)
       svg += `<text x="${toX(i)}" y="${H-PAD.b+12}" text-anchor="middle" font-size="8" fill="#94a3b8" font-family="system-ui">${lbl}</text>`;
   });
   svg += '</svg>';
   container.innerHTML = svg;
 }
 ...
 ], dL, { h: dashChart3D ? 195 : 175 });

 make3DBarChart(chartNCF, [
   { label:'Net CF', data:dN, color: v => v >= 0 ? '#0176D3' : '#BA0517' },
 ], dL, { h: dashChart3D ? 130 : 115 });

 const accContainer = document.getElementById('chart-accum');
 if (accContainer) make3DLineChart(accContainer, dA, dL, '#5A2D82');
}
