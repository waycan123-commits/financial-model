...
function renderGroupedPLCF(which, cv) {
 ...
 body += mkRow('EBITDA', cv.ebitda, {bold:true, color:'teal'});
   ...
   // ── INFLOW total: includes custom inflow lines ──
   const totInflowArr = totalIncomeCF.slice();
   customLines.filter(l => l.showInCF && (l.section === 'income_section' || l.section === 'cf_after_inflows') && l.formula.trim())
     .forEach(l => evalCustomLineArr(l, cv).forEach((v,i) => { totInflowArr[i] += v; }));

   // ── OUTFLOW total: stored positive, displayed NEGATIVE ──
   const _outRaw = cv.totalCogsCFAdj.map((v,i) => v + cv.opexBufferCF[i] + cv.finExpCB[i] + cv.finExpUB[i] + cv.hedgingArr[i] + cv.finCostArr[i]);
   customLines.filter(l => l.showInCF && (l.section === 'cogs_section' || l.section === 'cf_after_outflows') && l.formula.trim())
     .forEach(l => evalCustomLineArr(l, cv).forEach((v,i) => { _outRaw[i] += Math.abs(v); }));
   const totOutflowNeg = _outRaw.map(v => -Math.abs(v));
   ...
   cv.adjIncomeRows.forEach(ac => body += mkRow((ac.adj.name||'Adj')+' ('+ac.adj.pct+'%)', ac.arrCF, {ind:true,color:'amber'}));
   customLines.filter(l=>l.showInCF&&(l.section==='income_section'||l.section==='cf_after_inflows')&&l.formula.trim())
...
function renderCF() {
 ...
 // ── INFLOW total: base CF revenue + custom inflow lines ──
 const totInflowArr = totalIncomeCF.slice();
 const _cfCustomInflow = customLines.filter(l => l.showInCF && (l.section === 'income_section' || l.section === 'cf_after_inflows') && l.formula.trim());
 _cfCustomInflow.forEach(l => { evalCustomLineArr(l, cv).forEach((v,i) => { totInflowArr[i] += v; }); });

 // ── OUTFLOW total: all outflows stored positive, displayed/summed as NEGATIVE ──
 const _outflowRaw = cv.totalCogsCFAdj.map((v, i) =>
   v + cv.opexBufferCF[i] + cv.finExpCB[i] + cv.finExpUB[i] + cv.hedgingArr[i] + finCost[i]
 );
 const _cfCustomOutflow = customLines.filter(l => l.showInCF && (l.section === 'cogs_section' || l.section === 'cf_after_outflows') && l.formula.trim());
 _cfCustomOutflow.forEach(l => { evalCustomLineArr(l, cv).forEach((v,i) => { _outflowRaw[i] += Math.abs(v); }); });
 // Displayed and summed as negative
 const totOutflowNeg = _outflowRaw.map(v => -Math.abs(v));

 // NET CF = Inflows + Outflows (neg)
 const netCFdisp = totInflowArr.map((v, i) => v + totOutflowNeg[i]);
 // ACCUMULATED CF = running sum of Net CF
 let _acCF = 0;
