...
/* =====================================================================
   FINAL TARGETED PATCH — formulas, P&L/CF totals, cost summary UX
   ===================================================================== */
(function(){
  // Show numeric zeros cleanly. This also removes invisible control characters that were showing as blanks.
  fmt = function(v, blankZero) {
  ...
  function arrNeg(a){ return a.map(v=>-(Number(v)||0)); }
  function arrAbs(a){ return a.map(v=>Math.abs(Number(v)||0)); }
  function arrSum(list){ return list.reduce((acc,a)=>arrAdd(acc,a||Z()), Z()); }
  function pctColor(v){ return v < 0 ? ' c-red' : ''; }
  function safeId(s){ return String(s||'').replace(/[^a-zA-Z0-9_-]/g,'_'); }

  // ---------- Formula references / Excel-like engine ----------
  function normName(s){ return String(s||'').toLowerCase().replace(/[^a-z0-9]+/g,''); }
  function addRef(map, label, value){
    if (!label) return;
    const n = Number(value) || 0;
    const raw = String(label).trim();
    const variants = new Set([raw, raw.toLowerCase(), raw.toUpperCase(), raw.replace(/\s+/g,'_'), raw.toLowerCase().replace(/\s+/g,'_'), raw.replace(/\s+/g,''), raw.toLowerCase().replace(/\s+/g,''), normName(raw)]);
    variants.forEach(k=>{ if(k) map[k]=n; });
  }
  function formulaRefs(cv, mi){
    cv = cv || compute(); mi = mi || 0;
    const refs = {};
    const add = (label, arrOrVal) => addRef(refs, label, Array.isArray(arrOrVal) ? (arrOrVal[mi]||0) : arrOrVal);
    add('Revenue', cv.totalIncome); add('Income', cv.totalIncome); add('Total Income', cv.totalIncome); add('totalIncome', cv.totalIncome);
    add('Penalties', cv.penArr); add('Total COGS', arrNeg(cv.totalCogsAdj||Z())); add('totalCogs', arrNeg(cv.totalCogsAdj||Z())); add('COGS', arrNeg(cv.totalCogsAdj||Z()));
    add('Gross Profit', cv.grossProfit); add('grossProfit', cv.grossProfit); add('GP', cv.grossProfit);
    add('Profit before OPEX', cv.profitBeforeOpex); add('Profit before OPEX Interest', cv.profitBeforeOpex); add('profitBeforeOpex', cv.profitBeforeOpex);
    add('EBIT', cv.ebit); add('ebit', cv.ebit); add('EBITDA', cv.ebitda); add('ebitda', cv.ebitda);
    add('OPEX Buffer', arrNeg(cv.opexBuffer||Z())); add('opexBuffer', arrNeg(cv.opexBuffer||Z()));
    add('Hedging', arrNeg(cv.hedgingArr||Z())); add('Hedging Adjustment', arrNeg(cv.hedgingArr||Z()));
    add('Financing Cost', arrNeg(cv.finCostArr||Z())); add('financingCost', arrNeg(cv.finCostArr||Z()));
    add('Compliance Bond Fee', arrNeg(cv.finExpCB||Z())); add('Upfront Bond Fee', arrNeg(cv.finExpUB||Z()));
    add('Total Inflows', cv.totalIncomeCF||Z()); add('totalInflows', cv.totalIncomeCF||Z());
    add('Net Cash Flow', cv.netCF||Z()); add('netCF', cv.netCF||Z()); add('Accumulated Cash Flow', cv.accumCF||Z()); add('accumCF', cv.accumCF||Z());
    Object.entries(cv.revCats||{}).forEach(([k,a])=>add(k,a));
    Object.entries(cv.costCat||{}).forEach(([k,a])=>{ add(k, arrNeg(a||Z())); add(k+' Cost', arrNeg(a||Z())); });
    (revenues||[]).forEach(it=>{ if(it.name) add(it.name, getDistArr(it)); });
    ([...(capex||[]), ...(opex||[])]).forEach(it=>{ if(it.name) add(it.name, arrNeg(getDistArr(it))); });
    // Parameters
    addRef(refs,'fx',CFG.fx); addRef(refs,'FX',CFG.fx); addRef(refs,'IGV',IGV); addRef(refs,'months',PM()); addRef(refs,'month',mi+1); addRef(refs,'m',mi+1);
    return refs;
  }
  function refOptions(){
    let cv; try { cv = compute(); } catch(e) { cv = null; }
    const base = ['Total Income','Revenue','Penalties','Total COGS','Gross Profit','Profit before OPEX','EBIT','EBITDA','OPEX Buffer','Hedging Adjustment','Financing Cost','Compliance Bond Fee','Upfront Bond Fee','Total Inflows','Total Outflows','Net Cash Flow','Accumulated Cash Flow'];
    if (cv) {
      Object.keys(cv.revCats||{}).forEach(k=>base.push(k));
      Object.keys(cv.costCat||{}).forEach(k=>base.push(k));
    }
    (revenues||[]).forEach(it=>{ if(it.name) base.push(it.name); });
    ([...(capex||[]),...(opex||[])]).forEach(it=>{ if(it.name) base.push(it.name); });
    return [...new Set(base.filter(Boolean))].sort((a,b)=>a.localeCompare(b));
  }
  function ensureFormulaDatalist(){
    let dl = document.getElementById('formula-vars-list');
    if (!dl) { dl = document.createElement('datalist'); dl.id = 'formula-vars-list'; document.body.appendChild(dl); }
    dl.innerHTML = refOptions().map(v=>`<option value="${esc(v)}"></option>`).join('');
    ['cl-m-formula', ...Array.from(document.querySelectorAll('[id^="fbar-"]')).map(e=>e.id)].forEach(id=>{ const el=document.getElementById(id); if(el) el.setAttribute('list','formula-vars-list'); });
  }
  function evalFormulaExcel(raw, cv, mi){
    if (!raw || !String(raw).trim()) return 0;
    cv = cv || compute(); mi = mi || 0;
    const refs = formulaRefs(cv, mi);
    let expr = String(raw).trim().replace(/^=/,'');
    // 10% of EBIT / 10% de Total Income
    expr = expr.replace(/(\d+(?:\.\d+)?)\s*%\s*(?:of|de)\s*([A-Za-zÁÉÍÓÚÜÑáéíóúüñ][A-Za-zÁÉÍÓÚÜÑáéíóúüñ0-9 _&/().-]*?)(?=\s*(?:[+\-*/),]|$))/gi, (_,pct,name)=>`(${Number(pct)/100})*(${name.trim()})`);
    expr = expr.replace(/(\d+(?:\.\d+)?)\s*%/g, (_,pct)=>`(${Number(pct)/100})`);
    // Replace references. Use the longest labels first to avoid partial matches.
    Object.keys(refs).sort((a,b)=>b.length-a.length).forEach(name=>{
      const safe = name.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');
      expr = expr.replace(new RegExp('(?<![A-Za-z0-9_])'+safe+'(?![A-Za-z0-9_])','gi'), String(refs[name]));
    });
    // Also resolve squashed names like totalincome, totalcogs
    const squashed = Object.keys(refs).reduce((m,k)=>{ m[normName(k)] = refs[k]; return m; },{});
    Object.keys(squashed).sort((a,b)=>b.length-a.length).forEach(name=>{
      if (!name) return;
      expr = expr.replace(new RegExp('(?<![A-Za-z0-9_])'+name+'(?![A-Za-z0-9_])','gi'), String(squashed[name]));
    });
    try {
      const v = Function('"use strict"; const IF=(c,a,b)=>c?a:b; const ABS=Math.abs; const MAX=Math.max; const MIN=Math.min; const ROUND=Math.round; const SUM=(...x)=>x.reduce((s,v)=>s+(Number(v)||0),0); return ('+expr+');')();
      return isFinite(v) ? Number(v) : 0;
    } catch(e) {
      console.warn('Formula could not be evaluated:', raw, '=>', expr, e.message);
      return 0;
    }
  }
  evalCustomFormula = function(formula, cv, monthIdx){ return evalFormulaExcel(formula, cv, monthIdx||0); };
  evalCustomLineArr = function(line, cv){
    cv = cv || compute();
    return AMs().map((_,i)=>{
      const raw = evalFormulaExcel(line.formula, cv, i);
      return (line.sign === 'add') ? Math.abs(raw) : -Math.abs(raw);
    });
  };
  window.evalCustomFormula = evalCustomFormula;
  window.evalCustomLineArr = evalCustomLineArr;

  let previewTimer = null;
  _clPreview = function(){
    const input = document.getElementById('cl-m-formula');
    const prev = document.getElementById('cl-m-preview');
    if (!input || !prev) return;
    const formula = input.value.trim();
    clearTimeout(previewTimer);
    previewTimer = setTimeout(()=>{
      ensureFormulaDatalist();
      if (!formula) { prev.textContent = 'Enter a formula to preview…'; prev.style.color = 'var(--sub)'; return; }
      const arr = AMs().map((_,i)=>evalFormulaExcel(formula, compute(), i));
  ...
  _livePreviewFormula = function(fid, expr){
    const prev = document.getElementById('fprev-' + fid);
    if (!prev) return;
    clearTimeout(previewTimer);
    previewTimer = setTimeout(()=>{
      ...
      const cv = compute();
      const results = AMs().map((_,i)=>evalFormulaExcel(expr, cv, i));
  ...
  window._clPreview = _clPreview;
  window._livePreviewFormula = _livePreviewFormula;

  // Replace the modal openers to add formula suggestions and avoid focus jumps.
  const oldOpenCLModal = openCLModal;
  openCLModal = function(section){
    oldOpenCLModal(section);
    ensureFormulaDatalist();
    const f = document.getElementById('cl-m-formula');
    if (f) f.setAttribute('list','formula-vars-list');
  };
  const oldOpenEditLineModal = openEditLineModal;
  openEditLineModal = function(line){
    oldOpenEditLineModal(line);
    ensureFormulaDatalist();
    const f = document.getElementById('cl-m-formula');
    if (f) f.setAttribute('list','formula-vars-list');
  };
  ...
  // ---------- P&L / CF grouped tables with validated subtotal logic ----------
  function groupedColumns(){
    const years = getYearRanges();
    years.forEach(({yi})=>{ if (plYearOpen[yi] === undefined) plYearOpen[yi] = (yi === 0); if (cfYearOpen[yi] === undefined) cfYearOpen[yi] = (yi === 0); });
    return years;
  }
  function renderHead(cols, which){
    let h = '<thead><tr><th style="min-width:220px;text-align:left;position:sticky;left:0;z-index:3;background:var(--navy)">Category</th>';
    cols.forEach(col=>{
      if(col.type==='month') { const mi=mInfo(col.m); h += `<th style="min-width:76px;background:var(--navy)"><div style="font-size:10px">${mi.short} ${String(mi.year).slice(2)}</div><div style="color:#5ba4f5;font-size:9px;font-weight:700">(M${col.m})</div></th>`; }
      else if(col.type==='year') h += `<th class="yr-col-hdr" data-which="${which}" data-yi="${col.yi}" style="min-width:90px;background:#1a4a80;cursor:pointer;user-select:none"><div style="font-size:11px;font-weight:800">${col.year}</div><div style="font-size:8px;opacity:.75;margin-top:2px">${col.open?'collapse':'expand'}</div></th>`;
      else h += '<th style="min-width:90px;background:#0a2a50"><div style="font-size:10px">Total</div></th>';
    });
    return h+'</tr></thead>';
  }
  function cellValue(arr, col, acc){
    if(col.type==='month') return arr[col.m-1]||0;
    if(col.type==='year') return acc ? (arr[Math.min(col.yms[col.yms.length-1], arr.length)-1]||0) : col.yms.reduce((s,m)=>s+(arr[m-1]||0),0);
    return acc ? (arr[arr.length-1]||0) : rSum(arr);
  }
  function mkGRow(label, arr, cols, opts){
    opts = opts || {}; const bold=!!opts.bold, ind=!!opts.ind, cc=opts.color||'', acc=!!opts.acc;
    let r = `<tr class="${bold?'bold-row':''}"><td class="lbl${ind?' ind':''}${bold?' bold':''}" style="position:sticky;left:0;z-index:1;background:${bold?'var(--alt)':'var(--surface)'}">${label}</td>`;
    cols.forEach(col=>{ const v=cellValue(arr,col,acc); const bg=col.type==='year'?'background:#f0f4f8;':col.type==='total'?'background:var(--alt);':''; const fw=col.type==='year'?'font-weight:700;':''; r += `<td class="num${bold?' bold':''}${pctColor(v)}${cc?' c-'+cc:''}" style="${bg}${fw}">${fmt(v,false)}</td>`; });
    return r+'</tr>';
  }
  function mkSec2(label,color){ return `<tr class="sec-row"><td colspan="999" style="background:${color||'#032D60'}">${label}</td></tr>`; }
  function customArrs(section, cv, cfOnly){
    return customLines.filter(l=>l.section===section && l.formula && (!cfOnly || l.showInCF)).map(l=>({ line:l, arr:evalCustomLineArr(l,cv) }));
  }
  function customRows(section, cv, cols, opts){
    return customArrs(section, cv, opts&&opts.cfOnly).map(({line,arr})=>{
      const btn = `<button onclick="openEditLineModal(customLines.find(l=>l.id=='${line.id}'))" style="margin-left:8px;font-size:9px;padding:1px 6px;border:1px solid var(--bdr2);border-radius:3px;background:var(--alt);color:var(--accent);cursor:pointer">edit</button>`;
      return mkGRow(esc(line.name||'Custom')+btn, arr, cols, {ind:true,color:'amber'});
    }).join('');
  }
  renderGroupedPLCF = function(which, cv){
    cv = cv || compute();
    const tblId = which === 'pl' ? 'pl-tbl' : 'cf-tbl';
    const yearOpen = which === 'pl' ? plYearOpen : cfYearOpen;
    const years = groupedColumns();
    const cols=[]; years.forEach(({yi,year,yms})=>{ const open = yearOpen[yi] !== false; if(open) yms.forEach(m=>cols.push({type:'month',yi,m,year,yms})); cols.push({type:'year',yi,year,yms,open}); }); cols.push({type:'total'});
    let body='';
    if(which==='pl'){
      const incomeCustom = arrSum(customArrs('income_section',cv).map(x=>x.arr));
      const totalIncome = arrAdd(cv.totalIncome||Z(), incomeCustom);
      const cogsBase = arrNeg(cv.totalCogsAdj||Z());
      const cogsCustom = arrSum(customArrs('cogs_section',cv).map(x=>x.arr));
      const totalCogs = arrAdd(cogsBase, cogsCustom);
      const gross = arrAdd(totalIncome, totalCogs);
      const afterGP = arrSum(customArrs('after_gross_profit',cv).map(x=>x.arr));
      const bondFees = arrAdd(arrNeg(cv.finExpCB||Z()), arrNeg(cv.finExpUB||Z()));
      const profitBefore = arrAdd(arrAdd(gross, afterGP), bondFees);
      const afterProfit = arrSum(customArrs('after_profit_b_opex',cv).map(x=>x.arr));
      const opexInterest = arrSum([arrNeg(cv.opexBuffer||Z()), arrNeg(cv.hedgingArr||Z()), arrNeg(cv.finCostArr||Z())]);
      const afterEbit = arrSum(customArrs('after_ebit',cv).map(x=>x.arr));
      const ebit = arrAdd(arrAdd(arrAdd(profitBefore, afterProfit), opexInterest), afterEbit);
      const ebitda = ebit.slice();
      body += mkSec2('INCOME','var(--th-income)');
      Object.entries(cv.revCats||{}).forEach(([cat,arr])=>body+=mkGRow(esc(cat),arr,cols,{ind:true}));
      body += mkGRow('Penalties',cv.penArr||Z(),cols,{ind:true,color:'amber'});
      cv.adjIncomeRows.forEach(ac=>body+=mkGRow(esc(ac.adj.name||'Adj')+' ('+(ac.adj.pct||0)+'%)', ac.arr.map(v=>ac.adj.sign==='add'?v:-v), cols,{ind:true,color:'amber'}));
      body += customRows('income_section',cv,cols);
      body += addLineRow('income_section', cols.length-1);
      body += mkGRow('Total Income',totalIncome,cols,{bold:true,color:'teal'});
      body += mkSec2('COGS','var(--th-cogs)');
      (cv.userCats||[]).forEach(cat=>body+=mkGRow(esc(cat),arrNeg((cv.costCat||{})[cat]||Z()),cols,{ind:true}));
      body += mkGRow('Contingency ('+(CFG.con||0)+'%)',arrNeg((cv.costCat||{}).Contingency||Z()),cols,{ind:true});
      cv.adjCostRows.forEach(ac=>body+=mkGRow(esc(ac.adj.name||'Adj')+' ('+(ac.adj.pct||0)+'%)', ac.arr.map(v=>-(ac.adj.sign==='add'?v:-v)), cols,{ind:true,color:'amber'}));
      body += customRows('cogs_section',cv,cols);
      body += addLineRow('cogs_section', cols.length-1);
      body += mkGRow('Total COGS',totalCogs,cols,{bold:true,color:'red'});
      body += mkGRow('Gross Profit',gross,cols,{bold:true,color:'blue'});
      body += customRows('after_gross_profit',cv,cols); body += addLineRow('after_gross_profit', cols.length-1);
      body += mkSec2('BOND FEES','var(--th-bond)');
      body += mkGRow('Compliance Bond Fee',arrNeg(cv.finExpCB||Z()),cols,{ind:true,color:'amber'});
      body += mkGRow('Upfront Bond Fee',arrNeg(cv.finExpUB||Z()),cols,{ind:true,color:'amber'});
      body += mkGRow('Profit before OPEX / Interest',profitBefore,cols,{bold:true,color:'blue'});
      body += customRows('after_profit_b_opex',cv,cols); body += addLineRow('after_profit_b_opex', cols.length-1);
      body += mkSec2('OPEX & INTEREST','var(--th-opex)');
      body += mkGRow('OPEX Buffer ('+(CFG.opxbuf??30)+'% of Revenue)',arrNeg(cv.opexBuffer||Z()),cols,{ind:true,color:'amber'});
      body += mkGRow('Hedging Adjustment (monthly)',arrNeg(cv.hedgingArr||Z()),cols,{ind:true});
      body += mkGRow('Financing Cost (on neg. CF)',arrNeg(cv.finCostArr||Z()),cols,{ind:true,color:'amber'});
      body += customRows('after_ebit',cv,cols); body += addLineRow('after_ebit', cols.length-1);
      body += mkGRow('EBIT',ebit,cols,{bold:true,color:'blue'});
      body += mkGRow('EBITDA',ebitda,cols,{bold:true,color:'teal'});
    } else {
      const incomeCustom = arrSum([arrSum(customArrs('income_section',cv,true).map(x=>x.arr)), arrSum(customArrs('cf_after_inflows',cv,true).map(x=>x.arr))]);
      const totalInflows = arrAdd(cv.totalIncomeCF||Z(), incomeCustom);
      const outflowRows = [];
      (cv.userCats||[]).forEach(cat=>outflowRows.push((cv.costCatCF||{})[cat]||Z()));
      outflowRows.push((cv.costCatCF||{}).Contingency||Z(), cv.opexBufferCF||Z(), cv.finExpCB||Z(), cv.finExpUB||Z(), cv.hedgingArr||Z(), cv.finCostArr||Z(), cv.vatOut||Z());
      customArrs('cogs_section',cv,true).forEach(x=>outflowRows.push(arrAbs(x.arr)));
      customArrs('cf_after_outflows',cv,true).forEach(x=>outflowRows.push(arrAbs(x.arr)));
      const totalOutflows = arrSum(outflowRows);
      const net = arrSub(totalInflows,totalOutflows);
      let running=0; const accum = net.map(v=>{running += v; return running;});
      body += mkSec2('INFLOWS','var(--th-cf-in)');
      Object.entries(cv.revCatsCF||{}).forEach(([cat,arr])=>body+=mkGRow(esc(cat),arr,cols,{ind:true}));
      body += mkGRow('Penalties',cv.penArrCF||Z(),cols,{ind:true,color:'amber'});
      body += mkGRow('VAT Inflow',cv.vatIn||Z(),cols,{ind:true,color:'teal'});
      body += customRows('income_section',cv,cols,{cfOnly:true}); body += customRows('cf_after_inflows',cv,cols,{cfOnly:true}); body += addLineRow('cf_after_inflows', cols.length-1);
      body += mkGRow('Total Inflows',totalInflows,cols,{bold:true});
      body += mkSec2('OUTFLOWS','var(--th-cf-out)');
      (cv.userCats||[]).forEach(cat=>body+=mkGRow(esc(cat),(cv.costCatCF||{})[cat]||Z(),cols,{ind:true}));
      body += mkGRow('Contingency',(cv.costCatCF||{}).Contingency||Z(),cols,{ind:true});
      body += customRows('cogs_section',cv,cols,{cfOnly:true});
      body += mkGRow('Compliance Bond Fee',cv.finExpCB||Z(),cols,{ind:true,color:'amber'});
      body += mkGRow('Upfront Bond Fee',cv.finExpUB||Z(),cols,{ind:true,color:'amber'});
      body += mkGRow('OPEX Buffer ('+(CFG.opxbuf??30)+'%)',cv.opexBufferCF||Z(),cols,{ind:true,color:'amber'});
      body += mkGRow('Hedging',cv.hedgingArr||Z(),cols,{ind:true,color:'purple'});
      body += mkGRow('Financing Cost',cv.finCostArr||Z(),cols,{ind:true,color:'amber'});
      body += mkGRow('VAT Outflow',cv.vatOut||Z(),cols,{ind:true,color:'red'});
      body += mkGRow('VAT NET to pay to SUNAT',(cv.vatIn||Z()).map((v,i)=>(v||0)-((cv.vatOut||Z())[i]||0)),cols,{ind:true});
      body += customRows('cf_after_outflows',cv,cols,{cfOnly:true}); body += addLineRow('cf_after_outflows', cols.length-1);
      body += mkGRow('Total Outflows',totalOutflows,cols,{bold:true});
      body += mkSec2('NET CASH FLOW FROM OPERATIONS','var(--th-bond)');
      body += mkGRow('NET CASH FLOW From OPERATIONS',net,cols,{bold:true,color:'blue'});
      body += mkGRow('Accumulated cash flow',accum,cols,{bold:true,color:'purple',acc:true});
    }
    const tbl = document.getElementById(tblId); if(!tbl) return;
    tbl.innerHTML = renderHead(cols, which) + '<tbody>' + body + '</tbody>';
  ...
  // ---------- Cost Summary visual cleanup ----------
  renderCostSummary = function(){
    const cv = compute(); const wrap = document.getElementById('costs-content'); if(!wrap) return;
    const items = [...(capex||[]),(opex||[])];
    const baseCost = items.reduce((s,it)=>s+Math.abs(getBase(it)),0);
    const contingency = Math.abs(rSum((cv.costCat||{}).Contingency||Z()));
    const cb = Math.abs(rSum(cv.finExpCB||Z()));
    const ub = Math.abs(rSum(cv.finExpUB||Z()));
    const opexBuf = Math.abs(rSum(cv.opexBuffer||Z()));
    const hedging = Math.abs(rSum(cv.hedgingArr||Z()));
    const fin = Math.abs(rSum(cv.finCostArr||Z()));
    const total = baseCost + contingency + cb + ub + opexBuf + hedging + fin;
    const groups = [ ['Cost base',baseCost,'#0176D3'], ['Contingency',contingency,'#A56105'], ['Bond fees',cb+ub,'#BA0517'], ['OPEX Buffer',opexBuf,'#06A59A'], ['Hedging',hedging,'#5A2D82'], ['Financing cost',fin,'#7C3AED'] ];
    const pct = v => total ? (v/total*100) : 0;
    wrap.innerHTML = `
      <style>.csgrid{display:grid;grid-template-columns:repeat(auto-fit,minmax(170px,1fr));gap:12px;margin-bottom:16px}.cscard{background:#fff;border:1px solid #e5e7eb;border-radius:14px;padding:14px;box-shadow:0 8px 22px rgba(15,23,42,.06)}.csk{font-size:10px;font-weight:900;letter-spacing:.08em;text-transform:uppercase;color:#64748b}.csv{font-size:20px;font-weight:900;margin-top:4px}.csbar{height:7px;background:#eef2f7;border-radius:99px;margin-top:10px;overflow:hidden}.csfill{height:100%;border-radius:99px}.cstable{width:100%;border-collapse:separate;border-spacing:0;background:#fff;border:1px solid #e5e7eb;border-radius:14px;overflow:hidden;box-shadow:0 8px 22px rgba(15,23,42,.06)}.cstable th{background:#0f2235;color:#fff;text-align:left;font-size:10px;text-transform:uppercase;letter-spacing:.07em;padding:11px 12px}.cstable td{padding:10px 12px;border-bottom:1px solid #edf0f4;font-size:12px}.cstable tr:last-child td{border-bottom:0}.num{text-align:right;font-variant-numeric:tabular-nums}.pill{font-size:10px;font-weight:900;border-radius:99px;padding:3px 8px;display:inline-block}</style>
      <div style="display:flex;justify-content:space-between;align-items:flex-end;gap:12px;flex-wrap:wrap;margin-bottom:14px"><div><div style="font-size:20px;font-weight:900;color:#0f2235">Cost Summary</div><div style="font-size:12px;color:#64748b">Resumen por componente, peso relativo y detalle de líneas.</div></div><div style="background:#0f2235;color:white;border-radius:14px;padding:12px 16px"><div style="font-size:10px;text-transform:uppercase;opacity:.75;font-weight:900">Total cost</div><div style="font-size:22px;font-weight:900">$${fmt(total,false)}</div></div></div>
      <div class="csgrid">${groups.map(([k,v,c])=>`<div class="cscard"><div class="csk">${k}</div><div class="csv" style="color:${c}">$${fmt(v,false)}</div><div style="font-size:11px;color:#64748b;margin-top:2px">${pct(v).toFixed(1)}% del total</div><div class="csbar"><div class="csfill" style="width:${Math.min(pct(v),100)}%;background:${c}"></div></div></div>`).join('')}</div>
      <table class="cstable"><thead><tr><th>Tipo</th><th>Detalle</th><th class="num">Monto USD</th><th class="num">Peso</th><th>Referencia</th></tr></thead><tbody>
      ${items.map(it=>{ const v=Math.abs(getBase(it)); const type=(capex||[]).includes(it)?'CAPEX':'OPEX'; const c=type==='CAPEX'?'#0176D3':'#5A2D82'; return `<tr><td><span class="pill" style="background:${c}18;color:${c}">${type}</span></td><td><strong>${esc(it.name||'Unnamed')}</strong><div style="font-size:10px;color:#64748b">${esc(it.cat||'Other')} · ${esc(it.cur||'USD')}${it.igv?' · Incl. IGV':''}</div></td><td class="num"><strong>$${fmt(v,false)}</strong></td><td class="num">${pct(v).toFixed(1)}%</td><td>${it.refUrl?`<a href="${esc(it.refUrl)}" target="_blank">Open ref</a>`:esc(it.refNote||'—')}</td></tr>`; }).join('')}
      ${groups.slice(1).map(([k,v,c])=>`<tr><td><span class="pill" style="background:${c}18;color:${c}">Auto</span></td><td><strong>${k}</strong><div style="font-size:10px;color:#64748b">Calculated from model settings</div></td><td class="num"><strong>$${fmt(v,false)}</strong></td><td class="num">${pct(v).toFixed(1)}%</td><td>Formula-based</td></tr>`).join('')}
      </tbody></table>`;
  };

  // Refresh formula datalist after tab renders.
  const oldSetTab = setTab;
  setTab = function(tab){ oldSetTab(tab); setTimeout(ensureFormulaDatalist, 30); };
})();
