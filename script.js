...
function renderDashboard() {
  if (typeof Chart === 'undefined') { setTimeout(renderDashboard, 120); return; }
  const cv = compute();
  const tR = rSum(cv.totalIncome), tC = rSum(cv.totalCogsAdj);
  const tG = rSum(cv.grossProfit), tE = rSum(cv.ebit), tEB = rSum(cv.ebitda);
  const tN = rSum(cv.netCF);
  ...
  const kpiDelta = (a, b) => {
    ...
    return `<span style="font-size:9px;font-weight:700;color:${d>=0?'#2E844A':'#BA0517'}">${d>=0?'▲':'▼'} ${Math.abs(d).toFixed(1)}%</span>`;
  };
  document.getElementById('kpi-grid').innerHTML = [
    ...
    <div class="kpi" style="border-left:3px solid ${c};position:relative;overflow:hidden;background:linear-gradient(135deg,#fff 60%,${c}08)">
      <div style="position:absolute;top:-8px;right:-4px;font-size:32px;opacity:.07;line-height:1">${icon}</div>
      <div style="display:flex;align-items:center;gap:5px;margin-bottom:3px">
        <span style="font-size:16px">${icon}</span>
        <span class="kpi-lbl" style="color:var(--sub);font-size:10px;font-weight:600;text-transform:uppercase;letter-spacing:.04em">${l}</span>
      </div>
      <div class="kpi-val" style="color:${c};font-size:clamp(15px,2vw,21px);font-weight:800;letter-spacing:-.02em">${v}</div>
      <div style="font-size:9px;color:${c};opacity:.7;margin-top:2px;font-weight:600">${sub}</div>
    </div>`).join('');

  // View toggle
  const ctWrap = document.getElementById('chart-view-tog');
  ctWrap.innerHTML = '<span style="font-size:11px;color:var(--sub);font-weight:600">View:</span>';
  ...
  const GC = 'rgba(100,116,139,0.08)';
  const TC = '#94a3b8';

  function yAx(extra) {
    return { grid:{color:GC, drawBorder:false}, border:{display:false, dash:[4,3]},
  ...
  const TT = {
    backgroundColor:'rgba(15,23,42,0.94)', titleColor:'#f1f5f9', bodyColor:'#94a3b8',
    padding:12, cornerRadius:10, titleFont:{...F,weight:'700',size:11},
    borderColor:'rgba(255,255,255,.08)', borderWidth:1,
    callbacks:{ label: ctx => '  '+ctx.dataset.label+':  $'+fmt(ctx.raw) }
  };
  function grad(ctx, c1, c2, h) {
    const g = ctx.createLinearGradient(0,0,0,h||220);
    g.addColorStop(0,c1); g.addColorStop(1,c2); return g;
  }

  // ── Chart 1: Revenue vs COGS — grouped bars + GP line ──
  (() => {
      ...
      data: {
        ...
        datasets: [
          { label:'Revenue', data:dI,
            backgroundColor: grad(ctx,'#06A59Aee','#06A59A55'),
            borderColor:'#06A59A', borderWidth:1.5,
            borderRadius:4, borderSkipped:false, order:2 },
          { label:'COGS', data:dC.map(Math.abs),
            backgroundColor: grad(ctx,'#E05252dd','#E0525244'),
            borderColor:'#E05252', borderWidth:1.5,
            borderRadius:4, borderSkipped:false, order:3 },
          { label:'Gross Profit', data:dG, type:'line',
            borderColor:'#0176D3', borderWidth:2.5,
            backgroundColor: grad(ctx,'#0176D322','#0176D300'),
            fill:true, tension:.4, order:1,
            pointRadius: isA?5:2.5, pointBackgroundColor:'#fff',
            pointBorderColor:'#0176D3', pointBorderWidth:2, pointHoverRadius:7 },
        ]
      },
      options: {
        ...
        plugins:{ legend:{display:true, position:'bottom',
          labels:{font:F, color:TC, boxWidth:10, padding:14, usePointStyle:true}},
          tooltip:{...TT} },
        scales:{ x:xAx(), y:yAx() }
      }
    });
  ...
  // ── Chart 2: Margin % — smooth area chart ──
  (() => {
    const gpM  = dI.map((v,i) => v ? +(dG[i]/v*100).toFixed(2) : 0);
    const ebdM = dI.map((v,i) => v ? +(dEB[i]/v*100).toFixed(2) : 0);
    const ebiM = dI.map((v,i) => v ? +(dE[i]/v*100).toFixed(2) : 0);
    ...
    _mkChart('chart-margins', {
      ...
      data:{
        ...
        datasets:[
          { label:'GP Margin %', data:gpM,
            borderColor:'#0176D3', borderWidth:2.5,
            backgroundColor:grad(ctx,'#0176D330','#0176D305',200),
          ...
          { label:'EBITDA Margin %', data:ebdM,
            ...
            backgroundColor:grad(ctx,'#5A2D8222','#5A2D8204',200),
            fill:true, tension:.45, borderDash:[],
            pointRadius:isA?5:1.5, pointHoverRadius:7,
            pointBackgroundColor:'#fff', pointBorderColor:'#5A2D82', pointBorderWidth:2 },
          { label:'EBIT Margin %', data:ebiM,
            borderColor:'#2E844A', borderWidth:1.8,
            fill:false, tension:.45, borderDash:[5,3],
      ...
      options:{
        ...
        scales:{ x:xAx(), y:yAx({ticks:{color:TC, font:F, callback:v=>v.toFixed(0)+'%'}}) }
      }
    });
  })();

  // ── Chart 3: Net CF — signed color bars with threshold line ──
  (() => {
    const ctx = document.getElementById('chart-ncf').getContext('2d');
    _mkChart('chart-ncf', {
      type:'bar',
      data:{
        labels:dL,
        datasets:[{
          label:'Net CF',
          data:dN,
          backgroundColor: dN.map(v => v>=0
            ? grad(ctx,'#2E844Acc','#2E844A55')
            : grad(ctx,'#BA0517cc','#BA051755')),
          borderColor: dN.map(v => v>=0 ? '#2E844A' : '#BA0517'),
      ...
      options:{
        ...
        plugins:{ legend:{display:false},
          tooltip:{...TT, callbacks:{label:ctx=>'  Net CF:  $'+fmt(ctx.raw)+(ctx.raw>=0?' ✓':' ✗')}} },
        scales:{
          x:xAx(),
          y:yAx({ grid:{ color:ctx => ctx.tick.value===0 ? 'rgba(0,0,0,.25)':GC,
  ...
  // ── Chart 4: Accumulated CF — dual-colour area with break-even marker ──
  (() => {
    const ctx = document.getElementById('chart-accum').getContext('2d');
      ...
      data:{
        ...
        datasets:[
          { label:' ', data:posD,
            ...
            backgroundColor:grad(ctx,'#2E844A44','#2E844A08'), fill:true, tension:.4, pointRadius:0, spanGaps:false },
          { label:'  ', data:negD,
            borderColor:'transparent', borderWidth:0,
            backgroundColor:grad(ctx,'#BA051744','#BA051708'), fill:true, tension:.4, pointRadius:0, spanGaps:false },
          { label:'Accumulated CF', data:dA,
            borderColor:'#1c2b3a', borderWidth:2.8, fill:false, tension:.4,
            pointRadius:dA.map((_,i)=>i===beIdx?8:(isA?4:0)),
            pointHoverRadius:8,
            pointBackgroundColor:dA.map((_,i)=>i===beIdx?'#FFB300':(dA[i]>=0?'#2E844A':'#BA0517')),
            pointBorderColor:dA.map((_,i)=>i===beIdx?'#fff':'transparent'),
            pointBorderWidth:dA.map((_,i)=>i===beIdx?3:0) }
        ]
      },
      options:{
        responsive:true, maintainAspectRatio:false,
        ...
        plugins:{ legend:{display:false},
          tooltip:{...TT, callbacks:{label:ctx=>{
            if (ctx.dataset.label.trim()==='') return null;
        ...
        scales:{
          x:xAx(),
          y:yAx({ grid:{color:c=>c.tick.value===0?'rgba(0,0,0,.25)':GC,
  ...
  // ── Chart 5: P&L Waterfall — Revenue → COGS → GP → OPEX → EBITDA → Financial → EBIT ──
  (() => {
    const ctx  = document.getElementById('chart-waterfall').getContext('2d');
    const yrs  = yrLabels();
    // Use project totals for waterfall (cleaner than single year)
    const rev  = rSum(cv.totalIncome);
    const cogs = Math.abs(rSum(cv.totalCogsAdj));
    const gp   = rSum(cv.grossProfit);
    const opx  = rSum(cv.opexBuffer);
    const ebd  = rSum(cv.ebitda);
    const bonds= rSum(cv.bondFees);
    const hedg = rSum(cv.hedgingArr);
    const finC = rSum(cv.finCostArr);
    const ebt  = rSum(cv.ebit);

    // [start, end] for floating bar waterfall
    const cats  = ['Revenue','− COGS','Gross Profit','− OPEX Buffer','EBITDA','− Financial','EBIT'];
    const vals  = [
      [0,   rev],
      [gp,  rev - cogs],  // COGS deduction (visual: from GP down to Rev-COGS)
      [0,   gp],
      [ebd, gp - opx],    // OPEX deduction
      [0,   ebd],
      [ebt, ebd - bonds - hedg - finC],  // financial deductions
      [0,   ebt],
    ];
    const isTot = [true,false,true,false,true,false,true];
    const colors = vals.map(([a,b],i) => {
      if (!isTot[i]) return '#E0525288';
      return b>=0 ? (i===4?'#5A2D82bb':i===6?'#2E844Abb':'#0176D3bb') : '#BA0517bb';
    });
    const bords = vals.map(([a,b],i) => {
      if (!isTot[i]) return '#E05252';
      return b>=0 ? (i===4?'#5A2D82':i===6?'#2E844A':'#0176D3') : '#BA0517';
    });
    const dLabels = vals.map(([a,b]) => b >= 0 ? '+$'+fmtK(b) : '-$'+fmtK(Math.abs(b)));
    ...
    _mkChart('chart-waterfall', {
      ...
      plugins:[{
        ...
        afterDatasetsDraw(chart) {
          ...
          chart.getDatasetMeta(0).data.forEach((bar,i)=>{
            ...
            const above = b >= (a||0);
            c.save();
            c.font = 'bold 9.5px Inter,system-ui';
            c.fillStyle = bords[i];
            c.textAlign = 'center';
            c.fillText(lbl, bar.x, above ? bar.y - 7 : bar.y + bar.height + 15);
            c.restore();
          });
        }
      }],
      options:{
        ...
        plugins:{ legend:{display:false},
          tooltip:{...TT, callbacks:{label:ctx=>{
            const [a,b]=ctx.raw;
            const lbl=cats[ctx.dataIndex];
            return '  '+lbl+':  $'+fmt(b)+(a!==0?'  (Δ $'+fmt(b-a)+')':'');
          }}}
        },
        scales:{
