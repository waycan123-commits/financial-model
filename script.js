...
function getDistArr(item) {
 ...
 // Revenue accrual can follow a CAPEX/OPEX cost line.
 // It preserves the revenue total, but distributes it using the linked cost curve.
 if (item && item.linkedCostId && item.linkedCostType) {
  const linkedList = item.linkedCostType === 'capex' ? (capex || []) : (opex || []);
  const linked = linkedList.find(c => String(c.id) === String(item.linkedCostId));
  if (linked) {
   const linkedCopy = { ...linked, linkedCostId: null, linkedCostType: null };
   const costCurve = getDistArr(linkedCopy).map(v => Math.abs(v || 0));
   const totalCostCurve = costCurve.reduce((s, v) => s + v, 0);
   const revenueTotal = getBase(item);
   if (totalCostCurve > 0) {
    return costCurve.map(v => revenueTotal * v / totalCostCurve);
   }
  }
 }
...
function itemPctTotal(item) {
 if (item && item.linkedCostId && item.linkedCostType) return 100;
 if (item.progMode && item.progMonthly) {
...
const mkRev = () => ({ id: uid(), name:'', amount:'', cur:'USD', igv:false, cat:'', mos:{}, adv:'', advMo:1, payCond:'', refNote:'', refUrl:'', billingMode:'pattern', billingCuts:[], billingDelay:0, billingFirst:1, billingEvery:1, progMode:false, progMonthly:'', progCatRef:'', penOverride:'', penEnabled:false, linkedCostId:null, linkedCostType:null });
...
function buildItemCard(item, idx, type) {
 ...
 // Revenue accrual link: let a revenue line follow the accrual curve of a CAPEX/OPEX line.
 if (!isCost) {
  const linkWrap = document.createElement('div');
  linkWrap.style.cssText = 'margin-top:14px;border:1.5px solid #c7d2fe;border-radius:10px;overflow:hidden;background:#fff';
  linkWrap.innerHTML = '<div style="display:flex;align-items:center;gap:8px;padding:10px 14px;background:linear-gradient(90deg,#eef2ff,#f8fafc)">'
   + '<span style="font-size:15px">🔗</span>'
   + '<div style="flex:1"><div style="font-size:12px;font-weight:800;color:#1e3a8a">Revenue Accrual / Devengado</div>'
   + '<div style="font-size:10px;color:#64748b;margin-top:1px">Optional: make this revenue line follow the monthly curve of a cost line.</div></div></div>';

  const linkBody = document.createElement('div');
  linkBody.style.cssText = 'padding:12px 14px;display:grid;grid-template-columns:minmax(170px,.75fr) minmax(240px,1.25fr);gap:10px;align-items:end';

  function linkFld(lbl, el) {
  ...
  modeSel.innerHTML = '<option value="manual">Manual distribution</option><option value="linked">Follow CAPEX/OPEX line</option>';
  modeSel.value = item.linkedCostId ? 'linked' : 'manual';

  const costSel = document.createElement('select');
  function buildCostOptions() {
   const opts = [];
   (capex || []).forEach(c => opts.push({ type:'capex', id:c.id, label:'[CAPEX] ' + (c.name || 'Unnamed') + (c.cat ? ' — ' + c.cat : '') }));
   (opex || []).forEach(c => opts.push({ type:'opex', id:c.id, label:'[OPEX] ' + (c.name || 'Unnamed') + (c.cat ? ' — ' + c.cat : '') }));
   return opts;
  }
  function renderCostOptions() {
   const opts = buildCostOptions();
   costSel.innerHTML = '<option value="">Select cost line…</option>' + opts.map(o => {
    const value = o.type + ':' + o.id;
    return '<option value="' + esc(value) + '">' + esc(o.label) + '</option>';
   }).join('');
   if (item.linkedCostId && item.linkedCostType) costSel.value = item.linkedCostType + ':' + item.linkedCostId;
   costSel.disabled = modeSel.value !== 'linked' || !opts.length;
  }

  const note = document.createElement('div');
  note.style.cssText = 'grid-column:1 / -1;font-size:10.5px;line-height:1.6;color:#64748b;background:#f8fafc;border-radius:8px;padding:8px 10px';
  function updateLinkNote() {
   const opts = buildCostOptions();
   if (!opts.length) {
    note.innerHTML = 'Primero crea líneas en <strong>CAPEX</strong> u <strong>OPEX</strong> para poder asociarlas al devengado del revenue.';
    return;
   }
   if (modeSel.value === 'linked' && item.linkedCostId) {
    const selected = opts.find(o => o.type === item.linkedCostType && String(o.id) === String(item.linkedCostId));
    note.innerHTML = 'Este revenue mantiene su monto total, pero su devengado mensual seguirá la curva de: <strong>' + esc(selected ? selected.label : 'cost line') + '</strong>. La distribución manual queda como respaldo si desactivas el vínculo.';
   } else {
    note.innerHTML = 'Manual: usa el cronograma mensual de abajo. Asociado: el revenue se devenga proporcionalmente al CAPEX/OPEX elegido.';
   }
  }

  renderCostOptions();
  linkBody.appendChild(linkFld('Accrual mode', modeSel));
  linkBody.appendChild(linkFld('Linked cost line', costSel));
  linkBody.appendChild(note);
  linkWrap.appendChild(linkBody);
  body.appendChild(linkWrap);
  updateLinkNote();

  modeSel.addEventListener('change', () => {
   if (modeSel.value === 'manual') {
    item.linkedCostId = null;
    item.linkedCostType = null;
   } else if (!item.linkedCostId && costSel.value) {
    const [t, ...idParts] = costSel.value.split(':');
    item.linkedCostType = t;
    item.linkedCostId = idParts.join(':');
   }
   renderCostOptions();
   updateLinkNote();
   refresh();
 ...
 const linkedRevenue = !isCost && item.linkedCostId && item.linkedCostType;
 ...
 const linkedList = item.linkedCostType === 'capex' ? (capex || []) : (opex || []);
 const linked = linkedList.find(c => String(c.id) === String(item.linkedCostId));
 distLbl.innerHTML = 'Monthly Distribution  -  <span style="color:' + pc + '">linked to ' + esc(linked ? (linked.name || 'cost line') : 'cost line') + '</span>';
 } else if (item.progMode && item.progMonthly) {
 ...
 const linkedArr = linkedRevenue ? getDistArr(item) : null;
 AMs().forEach(m => {
 const v = evalPct((item.mos || {})[m]) || 0;
 const el = body.querySelector('[data-ma="' + m + '"]');
 if (el) {
  const a = linkedRevenue ? (linkedArr[m - 1] || 0) : v/100*rest;
  el.textContent = a===0?'':a.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2});
 }
 });
 for (let yi = 0; yi < nY; yi++) {
 const yms = AMs().filter(m => m > yi*12 && m <= (yi+1)*12);
 const yp = linkedRevenue
 ? (base ? yms.reduce((s, m) => s + ((linkedArr && linkedArr[m - 1]) || 0), 0) / base * 100 : 0)
 : yms.reduce((s, m) => s + (evalPct((item.mos || {})[m]) || 0), 0);
 const sp = body.querySelector('[data-yi="' + yi + '"]');
 if (sp) sp.textContent = yp.toFixed(4).replace(/\.?0+$/, '') + (linkedRevenue ? '% linked this year' : '% this year');
 }
 ...
 if (linkedRevenue) sumParts.push('linked accrual');
...
function renderDashboard() {
 ...
 const totalRevenue = rSum(cv.totalIncome);
 const totalCogs = rSum(cv.totalCogsAdj);
 const grossProfitTotal = rSum(cv.grossProfit);
 const ebitdaTotal = rSum(cv.ebitda || []);
 const ebitTotal = rSum(cv.ebit || []);
 const netCFTotal = rSum(cv.netCF);
 const gpMargin = totalRevenue ? (grossProfitTotal / totalRevenue * 100) : 0;
 const ebitdaMargin = totalRevenue ? (ebitdaTotal / totalRevenue * 100) : 0;
 const ebitMargin = totalRevenue ? (ebitTotal / totalRevenue * 100) : 0;
 const cogsRatio = totalRevenue ? (totalCogs / totalRevenue * 100) : 0;
 const peakNegCF = Math.min(...(cv.accumCF || [0]), 0);
 const breakEvenIdx = (cv.accumCF || []).findIndex(v => v >= 0);
 const pct = v => (isFinite(v) ? v : 0).toFixed(2) + '%';
 const moneyColor = v => v >= 0 ? '#2E844A' : '#BA0517';
 const safe = a => Array.isArray(a) ? a : Z();

 const kpiEl = document.getElementById('kpi-grid');
 if (kpiEl) {
  kpiEl.innerHTML = [
   ['Total Revenue', '$' + fmtK(totalRevenue), '#06A59A'],
   ['Total COGS', '$' + fmtK(totalCogs), '#BA0517'],
   ['Gross Profit', '$' + fmtK(grossProfitTotal), moneyColor(grossProfitTotal)],
   ['GP Margin', pct(gpMargin), moneyColor(gpMargin)],
   ['EBITDA', '$' + fmtK(ebitdaTotal), moneyColor(ebitdaTotal)],
   ['EBITDA Margin', pct(ebitdaMargin), moneyColor(ebitdaMargin)],
   ['EBIT', '$' + fmtK(ebitTotal), moneyColor(ebitTotal)],
   ['EBIT Margin', pct(ebitMargin), moneyColor(ebitMargin)],
   ['Net Cash Flow', '$' + fmtK(netCFTotal), moneyColor(netCFTotal)],
   ['NPV', '$' + fmtK(cv.npvCF), cv.npvCF >= 0 ? '#2E844A' : '#5A2D82'],
  ].map(([l, v, c]) =>
   '<div class="kpi exec-kpi">'
   + '<div class="kpi-lbl">' + l + '</div>'
   + '<div class="kpi-val" style="color:' + c + ';text-shadow:0 0 22px ' + c + '33">' + v + '</div></div>'
  ).join('');
 }
 ...
 if (ctWrap) {
  ...
  if (chartView === 'grouped') ctWrap.appendChild(_makeExecutiveYearSelect('dashboard', chartSelectedYear, v => { chartSelectedYear = v; renderDashboard(); }));
 }

 // Decision cards above charts
 let insight = document.getElementById('dashboard-decision-insights');
 const dashPanel = document.getElementById('panel-dashboard');
 if (!insight && dashPanel) {
  insight = document.createElement('div');
  insight.id = 'dashboard-decision-insights';
  const anchor = document.getElementById('chart-view-tog');
  if (anchor && anchor.parentNode) anchor.parentNode.insertBefore(insight, anchor.nextSibling);
 }
 if (insight) {
  const marginMsg = gpMargin < 0 ? 'Margen bruto negativo' : gpMargin < 15 ? 'Margen ajustado' : 'Margen saludable';
  const cfMsg = peakNegCF < 0 ? 'Requiere caja máxima de $' + fmtK(Math.abs(peakNegCF)) : 'Sin déficit acumulado';
  const beMsg = breakEvenIdx >= 0 ? 'Break-even en M' + (breakEvenIdx + 1) + ' (' + mInfo(breakEvenIdx + 1).label + ')' : 'No alcanza break-even';
  const costMsg = cogsRatio > 100 ? 'COGS supera ingresos' : 'COGS equivale a ' + pct(cogsRatio) + ' de ingresos';
  insight.innerHTML = [
   ['Rentabilidad', marginMsg, 'GP ' + pct(gpMargin) + ' · EBITDA ' + pct(ebitdaMargin), gpMargin >= 0 ? '#2E844A' : '#BA0517'],
   ['Presión de costos', costMsg, 'Total COGS $' + fmtK(totalCogs), cogsRatio <= 80 ? '#2E844A' : '#A56105'],
   ['Caja', cfMsg, 'Net CF $' + fmtK(netCFTotal), netCFTotal >= 0 ? '#2E844A' : '#BA0517'],
   ['Recuperación', beMsg, 'NPV $' + fmtK(cv.npvCF), cv.npvCF >= 0 ? '#2E844A' : '#5A2D82'],
  ].map(([t, v, s, c]) => '<div class="decision-card" style="border-left-color:' + c + '"><div class="decision-k">' + t + '</div><div class="decision-v" style="color:' + c + '">' + v + '</div><div class="decision-s">' + s + '</div></div>').join('');
 }

 function dataForView(arr, acc) {
  arr = safe(arr);
  if (chartView === 'annual') return acc ? aggAccY(arr) : aggY(arr);
  if (chartView === 'grouped') {
   const years = getYearRanges();
   let y = years.find(x => String(x.year) === String(chartSelectedYear));
   if (!y) y = years[0];
   if (!y) return arr;
   if (acc) return y.yms.map(m => arr[m - 1] || 0);
   return y.yms.map(m => arr[m - 1] || 0);
  }
  return arr;
 }
 function labelsForView() {
  if (chartView === 'annual') return yrLabels();
  if (chartView === 'grouped') {
   const years = getYearRanges();
   let y = years.find(x => String(x.year) === String(chartSelectedYear));
   if (!y) y = years[0];
   return y ? y.yms.map(m => mInfo(m).short + ' ' + String(mInfo(m).year).slice(2)) : AMs().map(m => mInfo(m).short);
  }
  return AMs().map(m => mInfo(m).short);
 }
 const labels = labelsForView();
 const incomeData = dataForView(cv.totalIncome);
 const cogsData = dataForView(cv.totalCogsAdj);
 const gpData = dataForView(cv.grossProfit);
 const ebitdaData = dataForView(cv.ebitda || Z());
 const ebitData = dataForView(cv.ebit || Z());
 const netCFData = dataForView(cv.netCF);
 const accumCFData = dataForView(cv.accumCF, true);
 const isA = chartView === 'annual';
 const isYear = chartView === 'grouped';

 const title = document.getElementById('rc-chart-title');
 if (title) title.textContent = 'Revenue vs COGS  -  ' + (isA ? 'Annual' : isYear ? ('Year ' + (chartSelectedYear || '')) : 'Monthly');

 function getCanvas(id, preferredHeight) {
  ...
  if ((el.tagName || '').toLowerCase() === 'canvas') {
   ...
   canvas = el.querySelector && el.querySelector('canvas');
   if (!canvas) {
  ...
  const parent = canvas.parentElement || el.parentElement || el;
  parent.style.minHeight = preferredHeight + 'px';
  canvas.style.display = 'block';
  canvas.style.width = '100%';
  canvas.style.height = preferredHeight + 'px';
  const rectW = Math.floor((parent.clientWidth || el.clientWidth || 620));
  const cssW = Math.max(360, rectW);
  const cssH = preferredHeight;
 ...
 function niceRange(values, forceZero) {
  const nums = (values || []).filter(v => isFinite(v));
  let min = Math.min(forceZero ? 0 : Infinity, ...nums);
  let max = Math.max(forceZero ? 0 : -Infinity, ...nums);
 ...
 function drawAxes(ctx, w, h, pad, min, max, percent) {
  const plotW = w - pad.l - pad.r, plotH = h - pad.t - pad.b;
  const toY = v => pad.t + plotH - ((v - min) / ((max - min) || 1)) * plotH;
  const labelCount = arguments.length > 6 ? arguments[6] : 12;
  ctx.save(); ctx.font = _chartFont ? _chartFont(labelCount, false) : '10px Arial, sans-serif'; ctx.textAlign = 'right'; ctx.textBaseline = 'middle'; ctx.lineWidth = 1;
  for (let i = 0; i <= 4; i++) {
   const v = min + ((max - min) / 4) * i; const y = toY(v);
   ctx.strokeStyle = '#E8EDF2'; ctx.beginPath(); ctx.moveTo(pad.l, y); ctx.lineTo(w - pad.r, y); ctx.stroke();
   ctx.fillStyle = '#8A96A3'; ctx.fillText(percent ? v.toFixed(0) + '%' : fmtK(v), pad.l - 8, y);
  }
  const zeroY = toY(0); ctx.strokeStyle = '#AEB7C2'; ctx.beginPath(); ctx.moveTo(pad.l, zeroY); ctx.lineTo(w - pad.r, zeroY); ctx.stroke(); ctx.restore();
  return { toY, zeroY, plotW, plotH };
 }
 function drawLegend(ctx, series, x, y) {
  ctx.save(); ctx.font = '10px Arial, sans-serif'; ctx.textBaseline = 'middle'; let cx = x; let cy = y;
  series.forEach(s => { const col = typeof s.color === 'function' ? s.color(1) : s.color; const label = String(s.label || ''); const wText = ctx.measureText(label).width; if (cx + wText + 42 > ctx.canvas.width / (window.devicePixelRatio || 1) - 20) { cx = x; cy += 16; } ctx.fillStyle = col; ctx.fillRect(cx, cy - 5, 10, 10); ctx.fillStyle = '#475569'; ctx.fillText(label, cx + 14, cy); cx += wText + 44; }); ctx.restore();
 }
 function _chartLabelStep(n) {
  if (n <= 8) return 1;
  if (n <= 12) return 2;
  if (n <= 18) return 3;
  if (n <= 30) return 5;
 ...
 function _chartFont(n, bold) {
  const size = n > 28 ? 8 : n > 16 ? 9 : 10;
  return (bold ? '700 ' : '') + size + 'px Arial, sans-serif';
 }
 function _drawXLabels(ctx, labels, toX, h, pad, rotate) {
  const n = labels.length;
  const step = _chartLabelStep(n);
  ctx.save();
  ctx.fillStyle = '#64748B';
  ctx.font = _chartFont(n, false);
  ctx.textAlign = 'center';
  ctx.textBaseline = 'top';
  labels.forEach((lbl, i) => {
   const shouldShow = (i % step === 0) || (n <= 10 && i === n - 1);
   ...
   if (rotate && n > 10) {
    ...
    ctx.fillText(String(lbl).slice(0, 11), x, y);
   }
  });
  ctx.restore();
 }
 function drawBarChart(id, series, labels, opts) {
  opts = opts || {};
  const surface = getCanvas(id, opts.height || 210);
  if (!surface) return;
  const { ctx, w, h } = surface;
  const n = Math.max(1, labels.length);
  const pad = { t: 34, r: 26, b: n > 10 ? 62 : 48, l: 78 };
  const allVals = series.flatMap(s => s.data || []);
  const rg = niceRange(allVals, true);
  const ax = drawAxes(ctx, w, h, pad, rg.min, rg.max, !!opts.percent, n);
  const groupW = ax.plotW / n;
  const barW = Math.max(3, Math.min(n > 18 ? 14 : 24, (groupW * 0.72) / Math.max(1, series.length)));
  const totalBarW = barW * series.length;
  labels.forEach((lbl, i) => {
   const groupX = pad.l + i * groupW + groupW / 2;
   series.forEach((s, si) => {
    const raw = (s.data || [])[i] || 0;
    const col = typeof s.color === 'function' ? s.color(raw) : s.color;
    const x = groupX - totalBarW / 2 + si * barW;
    const y = ax.toY(raw);
    const top = Math.min(y, ax.zeroY);
    const bh = Math.max(1, Math.abs(ax.zeroY - y));
    ctx.fillStyle = col;
    ctx.globalAlpha = s.opacity || 1;
    ctx.fillRect(x + 1, top, Math.max(2, barW - 2), bh);
    ctx.globalAlpha = 1;
   });
  });
  const toX = i => pad.l + i * groupW + groupW / 2;
  _drawXLabels(ctx, labels, toX, h, pad, !isA);
  drawLegend(ctx, series, pad.l, 17);
 }
 function drawLineChart(id, series, labels, opts) {
  opts = opts || {};
  const surface = getCanvas(id, opts.height || 185);
  if (!surface) return;
  const { ctx, w, h } = surface;
  const n = Math.max(1, labels.length);
  const pad = { t: 36, r: 28, b: n > 10 ? 58 : 46, l: 78 };
  const allVals = series.flatMap(s => s.data || []);
  const rg = niceRange(allVals, false);
  const ax = drawAxes(ctx, w, h, pad, rg.min, rg.max, !!opts.percent, n);
  const toX = i => pad.l + (n === 1 ? ax.plotW / 2 : i / (n - 1) * ax.plotW);
  series.forEach(s => {
   ctx.save();
   ctx.beginPath();
   (s.data || []).forEach((v, i) => {
    const x = toX(i), y = ax.toY(v || 0);
   ...
   const step = _chartLabelStep(n);
   (s.data || []).forEach((v, i) => {
    if (i % step === 0 || (n <= 10 && i === n - 1)) {
     ctx.beginPath();
     ctx.arc(toX(i), ax.toY(v || 0), 2.8, 0, Math.PI * 2);
  ...
  _drawXLabels(ctx, labels, toX, h, pad, false);
  drawLegend(ctx, series, pad.l, 17);
 }
 function drawWaterfall(id) {
  const surface = getCanvas(id, 225); if (!surface) return; const { ctx, w, h } = surface; const pad={t:30,r:20,b:44,l:78};
  const vals=[totalRevenue,-Math.abs(totalCogs),grossProfitTotal,-Math.abs(rSum(cv.opexBuffer||[])),ebitdaTotal,ebitTotal,netCFTotal];
  const labs=['Revenue','COGS','GP','OPEX','EBITDA','EBIT','Net CF']; const rg=niceRange(vals,true); const ax=drawAxes(ctx,w,h,pad,rg.min,rg.max,false); const n=labs.length, bw=Math.min(42,(ax.plotW/n)*0.58);
  labs.forEach((lab,i)=>{ const v=vals[i]||0; const x=pad.l+i*(ax.plotW/n)+(ax.plotW/n-bw)/2; const y=ax.toY(v); const top=Math.min(y,ax.zeroY); const bh=Math.max(1,Math.abs(ax.zeroY-y)); ctx.fillStyle=v>=0?'#0176D3':'#BA0517'; ctx.globalAlpha=.9; ctx.fillRect(x,top,bw,bh); ctx.globalAlpha=1; ctx.fillStyle='#475569'; ctx.font='10px Arial'; ctx.textAlign='center'; ctx.fillText(lab,x+bw/2,h-pad.b+12); ctx.fillStyle=v>=0?'#0176D3':'#BA0517'; ctx.font='700 10px Arial'; ctx.fillText(fmtK(v),x+bw/2,top-6); });
 }
 try {
  drawBarChart('chart-rc', [
   { label: 'Revenue', data: incomeData, color: '#06A59A' },
   { label: 'COGS', data: cogsData.map(v => Math.abs(v)), color: '#BA0517', opacity: 0.82 },
   { label: 'Gross Profit', data: gpData, color: v => v >= 0 ? '#0176D3' : '#A56105', opacity: 0.92 },
  ], labels, { height: 205 });
  const gpMarginData = incomeData.map((v, i) => v ? (gpData[i] / v * 100) : 0);
  const ebitdaMarginData = incomeData.map((v, i) => v ? (ebitdaData[i] / v * 100) : 0);
  const ebitMarginData = incomeData.map((v, i) => v ? (ebitData[i] / v * 100) : 0);
  drawLineChart('chart-margins', [
   { label: 'GP %', data: gpMarginData, color: '#0176D3' },
   { label: 'EBITDA %', data: ebitdaMarginData, color: '#2E844A' },
   { label: 'EBIT %', data: ebitMarginData, color: '#5A2D82' },
  ], labels, { height: 205, percent: true });
  drawBarChart('chart-ncf', [{ label: 'Net CF', data: netCFData, color: v => v >= 0 ? '#0176D3' : '#BA0517' }], labels, { height: 165 });
  drawLineChart('chart-accum', [{ label: 'Accum. CF', data: accumCFData, color: '#5A2D82' }], labels, { height: 165 });
  drawWaterfall('chart-waterfall');
 } catch (e) {
  console.error('[Dashboard charts] render error:', e);
 }
}
...
/* =====================================================================
   FINAL FIX — Custom formula chips, custom-line totals, CF logic and Know How
   ===================================================================== */
(function(){
  if (window.__customLinesKnowHowFixApplied) return;
  window.__customLinesKnowHowFixApplied = true;

  const _baseComputeForCustomFix = compute;
  const arr0 = () => new Array(PM()).fill(0);
  const cloneArr = a => (a || arr0()).map(v => Number(v) || 0);
  const sumArrs = (arrs) => {
    const out = arr0();
    (arrs || []).forEach(a => (a || []).forEach((v,i) => { out[i] += Number(v) || 0; }));
    return out;
  };
  const addArrs = (a,b) => (a || arr0()).map((v,i) => (Number(v)||0) + (Number((b||[])[i])||0));
  const subArrs = (a,b) => (a || arr0()).map((v,i) => (Number(v)||0) - (Number((b||[])[i])||0));
  const negArr = a => (a || arr0()).map(v => -(Number(v)||0));
  const isFormulaLine = l => l && String(l.formula || '').trim();

  function getLineArrs(baseCv) {
    const lines = (customLines || []).filter(isFormulaLine).map(line => {
      let arr;
      try { arr = evalCustomLineArr(line, baseCv); }
      catch(e) { console.warn('Custom line error:', line && line.name, e); arr = arr0(); }
      return { line, arr: cloneArr(arr) };
    });
    const by = sec => lines.filter(x => x.line.section === sec);
    return {
      all: lines,
      income: by('income_section'),
      cogs: by('cogs_section'),
      afterGP: by('after_gross_profit'),
      afterPBO: by('after_profit_b_opex'),
      afterEBIT: by('after_ebit'),
      cfIn: by('cf_after_inflows'),
      cfOut: by('cf_after_outflows'),
    };
  }

  compute = window.compute = function(){
    const cv = _baseComputeForCustomFix();
    const groups = getLineArrs(cv);
    const incomeCustom = sumArrs(groups.income.map(x => x.arr));
    const cogsCustom = sumArrs(groups.cogs.map(x => x.arr));
    const afterGP = sumArrs(groups.afterGP.map(x => x.arr));
    const afterPBO = sumArrs(groups.afterPBO.map(x => x.arr));
    const afterEBIT = sumArrs(groups.afterEBIT.map(x => x.arr));

    // P&L: custom income lines add to income. Custom COGS lines are signed visually;
    // a negative custom cost increases COGS, a positive one reduces COGS.
    cv.totalIncome = addArrs(cv.totalIncome, incomeCustom);
    cv.totalCogsAdj = subArrs(cv.totalCogsAdj, cogsCustom);
    cv.grossProfit = subArrs(cv.totalIncome, cv.totalCogsAdj);
    cv.bondFees = addArrs(cv.finExpCB || arr0(), cv.finExpUB || arr0());
    cv.profitBeforeOpex = addArrs(subArrs(cv.grossProfit, cv.bondFees), afterGP);
    cv.ebit = addArrs(subArrs(subArrs(subArrs(cv.profitBeforeOpex, cv.opexBuffer || arr0()), cv.hedgingArr || arr0()), cv.finCostArr || arr0()), afterPBO);
    cv.ebitda = addArrs(cv.ebit, afterEBIT);

    // CF: lines marked showInCF affect operational cash.
    const cfIncome = groups.income.filter(x => x.line.showInCF).map(x => x.arr);
    const cfCogs = groups.cogs.filter(x => x.line.showInCF).map(x => x.arr);
    const cfAfterGP = groups.afterGP.filter(x => x.line.showInCF).map(x => x.arr);
    const cfAfterPBO = groups.afterPBO.filter(x => x.line.showInCF).map(x => x.arr);
    const cfInExtra = sumArrs([...cfIncome, ...groups.cfIn.map(x => x.arr)]);
    const cfOutSigned = sumArrs([...cfCogs, ...groups.cfOut.map(x => x.arr)]);
    const cfNeutral = sumArrs([...cfAfterGP, ...cfAfterPBO]);

    cv.totalIncomeCF = addArrs(cv.totalIncomeCF, cfInExtra);
    cv.totalCogsCFAdj = subArrs(cv.totalCogsCFAdj, cfOutSigned);
    const outflows = sumArrs([cv.totalCogsCFAdj || arr0(), cv.opexBufferCF || arr0(), cv.finExpCB || arr0(), cv.finExpUB || arr0(), cv.hedgingArr || arr0(), cv.finCostArr || arr0(), cv.vatOut || arr0()]);
    const inflows = addArrs(cv.totalIncomeCF || arr0(), cv.vatIn || arr0());
    cv.totalInflowsVisible = inflows;
    cv.totalOutflowsVisible = outflows;
    cv.netCF = addArrs(subArrs(inflows, outflows), cfNeutral);
    let ac = 0;
    cv.accumCF = cv.netCF.map(v => { ac += Number(v)||0; return ac; });
    const mRate = cv.mRate || (Math.pow(1 + CFG.dis / 100, 1 / 12) - 1);
    cv.npvCF = cv.netCF.reduce((a,v,i) => a + (Number(v)||0) / Math.pow(1 + mRate, i + 1), 0);

    cv._customLineGroups = groups;
    return cv;
  };

  function lineRows(section, cv, mkRow, opts={}) {
    return (customLines || [])
      .filter(l => l.section === section && String(l.formula || '').trim())
      .map(line => {
        let arr = arr0();
        try { arr = evalCustomLineArr(line, _baseComputeForCustomFix()); }
        catch(e) { console.warn('custom row render error', line.name, e); }
        const editBtn = `<button onclick="openEditLineModal(customLines.find(l=>String(l.id)==='${String(line.id)}'))" title="Edit formula" style="margin-left:8px;font-size:9px;padding:1px 6px;border:1px solid var(--bdr2);border-radius:3px;background:var(--alt);color:var(--accent);cursor:pointer;vertical-align:middle">✎</button>`;
        const label = (line.name || 'Custom') + editBtn + (line.note ? ` <span style="font-size:9px;opacity:.6">(${line.note})</span>` : '');
        return mkRow(label, arr, Object.assign({ind:true, color:line.color || 'amber'}, opts));
      }).join('');
  }

  function renderFlexibleGrouped(which, cv) {
    const tbl = document.getElementById(which === 'pl' ? 'pl-tbl' : 'cf-tbl');
    if (!tbl) return;
    const yearOpen = which === 'pl' ? plYearOpen : cfYearOpen;
    const years = getYearRanges();
    years.forEach(({yi}) => { if (yearOpen[yi] === undefined) yearOpen[yi] = (yi === 0); });
    const cols = [];
    years.forEach(({yi,year,yms}) => { if (yearOpen[yi]) yms.forEach(m => cols.push({type:'month', yi, year, m, yms})); cols.push({type:'year', yi, year, yms, open:!!yearOpen[yi]}); });
    cols.push({type:'total'});
    const cellVal = (arr,col) => col.type === 'month' ? (arr[col.m-1] || 0) : col.type === 'year' ? col.yms.reduce((s,m)=>s+(arr[m-1]||0),0) : rSum(arr || []);
    let head = '<thead><tr><th style="min-width:220px;text-align:left;position:sticky;left:0;z-index:3;background:var(--navy)">Category</th>';
    cols.forEach(col => {
      if (col.type === 'month') { const mi=mInfo(col.m); head += `<th style="min-width:76px;background:var(--navy)"><div style="font-size:10px">${mi.short} ${String(mi.year).slice(2)}</div><div style="color:#5ba4f5;font-size:9px;font-weight:700">(M${col.m})</div></th>`; }
      else if (col.type === 'year') head += `<th class="yr-col-hdr" data-yi="${col.yi}" style="min-width:90px;background:#1a4a80;cursor:pointer;user-select:none"><div style="font-size:11px;font-weight:800">${col.year}</div><div style="font-size:8px;opacity:.75;margin-top:2px">${col.open?' collapse':' expand'}</div></th>`;
      else head += '<th style="min-width:90px;background:#0a2a50"><div style="font-size:10px">Total</div></th>';
    });
    head += '</tr></thead>';
    const row = (label, arr, opts={}) => {
      const bold=!!opts.bold, ind=!!opts.ind, col=opts.color||'';
      const bg = bold ? 'var(--alt)' : 'var(--surface)';
      let h = `<tr class="${bold?'bold-row':''}"><td class="lbl${ind?' ind':''}${bold?' bold':''}" style="position:sticky;left:0;z-index:1;background:${bg}">${label}</td>`;
      cols.forEach(c => { const v=cellVal(arr||arr0(), c); const special=c.type==='year'?'background:#f0f4f8;font-weight:700;':c.type==='total'?'background:var(--alt);':''; h += `<td class="num${bold?' bold':''}${v<0?' c-red':''}${col?' c-'+col:''}" style="${special}">${fmt(v)}</td>`; });
      return h + '</tr>';
    };
    const sec = (txt,color) => `<tr class="sec-row"><td colspan="999" style="background:${color}">${txt}</td></tr>`;
    const baseCv = _baseComputeForCustomFix();
    let body = '';
    if (which === 'pl') {
      body += sec('INCOME','var(--th-income)');
      Object.entries(baseCv.revCats || {}).forEach(([k,a]) => body += row(k,a,{ind:true}));
      body += row('Penalties', baseCv.penArr || arr0(), {ind:true,color:'amber'});
      (baseCv.adjIncomeRows || []).forEach(ac => body += row((ac.adj.name||'Adj')+' ('+(ac.adj.pct||0)+'%)', (ac.arr||arr0()).map(v=>ac.adj.sign==='add'?v:-v), {ind:true,color:'amber'}));
      body += lineRows('income_section', cv, row);
      body += row('Total Income', cv.totalIncome || arr0(), {bold:true,color:'teal'});

      body += sec('COGS','var(--th-cogs)');
      (baseCv.userCats || []).forEach(k => body += row(k, negArr((baseCv.costCat||{})[k] || arr0()), {ind:true}));
      body += row('Contingency ('+CFG.con+'%)', negArr((baseCv.costCat||{}).Contingency || arr0()), {ind:true});
      (baseCv.adjCostRows || []).forEach(ac => body += row((ac.adj.name||'Adj')+' ('+(ac.adj.pct||0)+'%)', (ac.arr||arr0()).map(v=>-(ac.adj.sign==='add'?v:-v)), {ind:true,color:'amber'}));
      body += lineRows('cogs_section', cv, row);
      body += row('Total COGS', negArr(cv.totalCogsAdj || arr0()), {bold:true,color:'red'});
      body += row('Gross Profit', cv.grossProfit || arr0(), {bold:true,color:'blue'});
      body += lineRows('after_gross_profit', cv, row);

      body += sec('BOND FEES','var(--th-bond)');
      body += row('Compliance Bond Fee', negArr(baseCv.finExpCB || arr0()), {ind:true,color:'amber'});
      body += row('Upfront Bond Fee', negArr(baseCv.finExpUB || arr0()), {ind:true,color:'amber'});
      body += row('Profit before OPEX / Interest', cv.profitBeforeOpex || arr0(), {bold:true,color:'blue'});
      body += lineRows('after_profit_b_opex', cv, row);

      body += sec('OPEX & INTEREST','var(--th-opex)');
      body += row('OPEX Buffer ('+(CFG.opxbuf??30)+'% of Revenue)', negArr(baseCv.opexBuffer || arr0()), {ind:true,color:'amber'});
      body += row('Hedging Adjustment (monthly)', negArr(baseCv.hedgingArr || arr0()), {ind:true});
      body += row('Financing Cost (on neg. CF)', negArr(baseCv.finCostArr || arr0()), {ind:true,color:'amber'});
      body += row('EBIT', cv.ebit || arr0(), {bold:true,color:'blue'});
      body += lineRows('after_ebit', cv, row);
      body += row('EBITDA', cv.ebitda || arr0(), {bold:true,color:'teal'});
    } else {
      body += sec('INFLOWS','var(--th-cf-in)');
      Object.entries(baseCv.revCatsCF || {}).forEach(([k,a]) => body += row(k,a,{ind:true}));
      body += row('Penalties', baseCv.penArrCF || arr0(), {ind:true,color:'amber'});
      body += row('VAT Inflow (IGV)', baseCv.vatIn || arr0(), {ind:true,color:'teal'});
      body += lineRows('income_section', cv, row);
      body += lineRows('cf_after_inflows', cv, row);
      body += row('Total Inflows', cv.totalInflowsVisible || cv.totalIncomeCF || arr0(), {bold:true,color:'teal'});

      body += sec('OUTFLOWS','var(--th-cf-out)');
      (baseCv.userCats || []).forEach(k => body += row(k, (baseCv.costCatCF||{})[k] || arr0(), {ind:true}));
      body += row('Contingency ('+CFG.con+'%)', (baseCv.costCatCF||{}).Contingency || arr0(), {ind:true});
      lineRows('cogs_section', cv, row).replace(/<\/tr>/g,'</tr>');
      body += lineRows('cogs_section', cv, row);
      body += row('OPEX Buffer ('+(CFG.opxbuf??30)+'%)', baseCv.opexBufferCF || arr0(), {ind:true,color:'amber'});
      body += row('Compliance Bond Fee', baseCv.finExpCB || arr0(), {ind:true,color:'amber'});
      body += row('Upfront Bond Fee', baseCv.finExpUB || arr0(), {ind:true,color:'amber'});
      body += row('Hedging', baseCv.hedgingArr || arr0(), {ind:true});
      body += row('Financing Cost', baseCv.finCostArr || arr0(), {ind:true,color:'amber'});
      body += row('VAT Outflow', baseCv.vatOut || arr0(), {ind:true,color:'red'});
      body += lineRows('cf_after_outflows', cv, row);
      body += row('Total Outflows', cv.totalOutflowsVisible || arr0(), {bold:true,color:'red'});

      body += sec('NET CASH FLOW','var(--th-bond)');
      body += lineRows('after_gross_profit', cv, row);
      body += lineRows('after_profit_b_opex', cv, row);
      body += row('Net Cash Flow', cv.netCF || arr0(), {bold:true,color:'blue'});
      body += row('Accumulated CF', cv.accumCF || arr0(), {bold:true,color:'purple'});
    }
    tbl.innerHTML = head + '<tbody>' + body + '</tbody>';
    tbl.querySelectorAll('.yr-col-hdr').forEach(th => th.addEventListener('click', () => {
      const yi = parseInt(th.dataset.yi,10);
      if (which === 'pl') plYearOpen[yi] = !plYearOpen[yi]; else cfYearOpen[yi] = !cfYearOpen[yi];
      if (which === 'pl') renderPL(); else renderCF();
    }));
  }

  renderGroupedPLCF = window.renderGroupedPLCF = renderFlexibleGrouped;

  renderPL = window.renderPL = function(){
    const wrap = document.getElementById('pl-view-tog');
    if (wrap) { wrap.innerHTML=''; const tog=buildToggle(plView, v=>{ plView=v; renderPL(); }); if (tog) wrap.appendChild(tog); }
    const cv = compute();
    if (plView === 'grouped') { renderFlexibleGrouped('pl', cv); return; }
    const baseCv = _baseComputeForCustomFix();
    const cols = plView === 'annual' ? yrLabels() : AMs().map(m=>mInfo(m).label);
    const mNums = plView === 'annual' ? null : AMs();
    const cw = plView === 'annual' ? 110 : 82;
    const nc = cols.length;
    const row = (lbl,arr,opts) => dataRow(lbl, arr, plView, opts || {});
    const sum = (lbl,arr,color,pctArr) => {
      const data = plView === 'annual' ? aggY(arr) : arr;
      const tot = rSum(arr);
      const badge = pctArr ? pctBadge(arr, pctArr) : '';
      return `<tr class="bold-row"><td class="lbl bold" style="${color?'color:'+color:''}">${lbl}${badge}</td>` + data.map(v=>`<td class="num bold${v<0?' c-red':''}" style="${color?'color:'+color:''}">${fmt(v)}</td>`).join('') + `<td class="num tot bold${tot<0?' c-red':''}" style="${color?'color:'+color:''}">${fmt(tot)}</td></tr>`;
    };
    let h = tHead(cols,cw,mNums) + '<tbody>';
    h += secRow('INCOME','var(--th-income)');
    Object.entries(baseCv.revCats || {}).forEach(([k,a]) => h += row(k,a,{ind:true}));
    h += row('Penalties', baseCv.penArr || arr0(), {ind:true,color:'amber'});
    (baseCv.adjIncomeRows || []).forEach(ac => h += row((ac.adj.name||'Adj')+' ('+(ac.adj.pct||0)+'%)', (ac.arr||arr0()).map(v=>ac.adj.sign==='add'?v:-v), {ind:true,color:'amber'}));
    h += lineRows('income_section', cv, row);
    h += addLineRow('income_section', nc);
    h += sum('Total Income', cv.totalIncome || arr0(), 'var(--th-income)');

    h += secRow('COGS','var(--th-cogs)');
    (baseCv.userCats || []).forEach(k => h += row(k, negArr((baseCv.costCat||{})[k] || arr0()), {ind:true}));
    h += row('Contingency ('+CFG.con+'%)', negArr((baseCv.costCat||{}).Contingency || arr0()), {ind:true});
    (baseCv.adjCostRows || []).forEach(ac => h += row((ac.adj.name||'Adj')+' ('+(ac.adj.pct||0)+'%)', (ac.arr||arr0()).map(v=>-(ac.adj.sign==='add'?v:-v)), {ind:true,color:'amber'}));
    h += lineRows('cogs_section', cv, row);
    h += addLineRow('cogs_section', nc);
    h += sum('Total COGS', negArr(cv.totalCogsAdj || arr0()), 'var(--th-cogs)');
    h += sum('Gross Profit', cv.grossProfit || arr0(), 'var(--accent)', cv.totalIncome || arr0());
    h += lineRows('after_gross_profit', cv, row);
    h += addLineRow('after_gross_profit', nc);

    h += secRow('BOND FEES','var(--th-bond)');
    h += row('Compliance Bond Fee', negArr(baseCv.finExpCB || arr0()), {ind:true,color:'amber'});
    h += row('Upfront Bond Fee', negArr(baseCv.finExpUB || arr0()), {ind:true,color:'amber'});
    h += sum('Profit before OPEX / Interest', cv.profitBeforeOpex || arr0(), 'var(--accent)', cv.totalIncome || arr0());
    h += lineRows('after_profit_b_opex', cv, row);
    h += addLineRow('after_profit_b_opex', nc);

    h += secRow('OPEX & INTEREST','var(--th-opex)');
    h += row('OPEX Buffer ('+(CFG.opxbuf??30)+'% of Revenue)', negArr(baseCv.opexBuffer || arr0()), {ind:true,color:'amber'});
    h += row('Hedging Adjustment (monthly)', negArr(baseCv.hedgingArr || arr0()), {ind:true});
    h += row('Financing Cost (on neg. CF)', negArr(baseCv.finCostArr || arr0()), {ind:true,color:'amber'});
    h += sum('EBIT', cv.ebit || arr0(), 'var(--accent)', cv.totalIncome || arr0());
    h += lineRows('after_ebit', cv, row);
    h += addLineRow('after_ebit', nc);
    h += sum('EBITDA', cv.ebitda || arr0(), 'var(--teal)', cv.totalIncome || arr0());
    h += '</tbody>';
    const tbl=document.getElementById('pl-tbl'); if (tbl) tbl.innerHTML = h;
  };

  renderCF = window.renderCF = function(){
    const wrap = document.getElementById('cf-view-tog');
    if (wrap) { wrap.innerHTML=''; const tog=buildToggle(cfView, v=>{ cfView=v; renderCF(); }); if (tog) wrap.appendChild(tog); }
    const cv = compute();
    if (cfView === 'grouped') { renderFlexibleGrouped('cf', cv); return; }
    const baseCv = _baseComputeForCustomFix();
    const cols = cfView === 'annual' ? yrLabels() : AMs().map(m=>mInfo(m).label);
    const mNums = cfView === 'annual' ? null : AMs();
    const cw = cfView === 'annual' ? 110 : 82;
    const nc = cols.length;
    const row = (lbl,arr,opts) => dataRow(lbl, arr, cfView, opts || {});
    let h = tHead(cols,cw,mNums) + '<tbody>';
    h += secRow('INFLOWS','var(--th-cf-in)');
    Object.entries(baseCv.revCatsCF || {}).forEach(([k,a]) => h += row(k,a,{ind:true}));
    h += row('Penalties', baseCv.penArrCF || arr0(), {ind:true,color:'amber'});
    h += row('VAT Inflow (IGV)', baseCv.vatIn || arr0(), {ind:true,color:'teal'});
    h += lineRows('income_section', cv, row);
    h += lineRows('cf_after_inflows', cv, row);
    h += addLineRow('cf_after_inflows', nc);
    h += row('Total Inflows', cv.totalInflowsVisible || cv.totalIncomeCF || arr0(), {bold:true});

    h += secRow('OUTFLOWS','var(--th-cf-out)');
    (baseCv.userCats || []).forEach(k => h += row(k, (baseCv.costCatCF||{})[k] || arr0(), {ind:true}));
    h += row('Contingency ('+CFG.con+'%)', (baseCv.costCatCF||{}).Contingency || arr0(), {ind:true});
    h += lineRows('cogs_section', cv, row);
    h += row('OPEX Buffer ('+(CFG.opxbuf??30)+'%)', baseCv.opexBufferCF || arr0(), {ind:true,color:'amber'});
    h += row('Compliance Bond Fee', baseCv.finExpCB || arr0(), {ind:true,color:'amber'});
    h += row('Upfront Bond Fee', baseCv.finExpUB || arr0(), {ind:true,color:'amber'});
    h += row('Hedging (monthly)', baseCv.hedgingArr || arr0(), {ind:true});
    h += row('Financing Cost', baseCv.finCostArr || arr0(), {ind:true,color:'amber'});
    h += row('VAT Outflow', baseCv.vatOut || arr0(), {ind:true,color:'red'});
    h += lineRows('cf_after_outflows', cv, row);
    h += addLineRow('cf_after_outflows', nc);
    h += row('Total Outflows', cv.totalOutflowsVisible || arr0(), {bold:true});

    h += secRow('NET CASH FLOW','var(--th-bond)');
    h += lineRows('after_gross_profit', cv, row);
    h += lineRows('after_profit_b_opex', cv, row);
    h += row('Net Cash Flow', cv.netCF || arr0(), {bold:true});
    h += row('Accumulated CF', cv.accumCF || arr0(), {bold:true, acc:true});
    h += secRow('NPV ANALYSIS','var(--th-bond)');
    h += infoRow('Discount Rate', CFG.dis + '% p.a.  ' + ((cv.mRate||0)*100).toFixed(3) + '% /mo', 'var(--sub)', nc);
    h += infoRow('NPV (Net CF)', 'USD ' + fmt(cv.npvCF), cv.npvCF >= 0 ? 'var(--green)' : 'var(--red)', nc);
    const peakNeg = Math.min(...(cv.accumCF || [0]), 0);
    const beIdx = (cv.accumCF || []).findIndex(v => v >= 0);
    h += infoRow('Peak Negative CF', 'USD ' + fmt(peakNeg), 'var(--sub)', nc);
    h += infoRow('Break-even Month', beIdx >= 0 ? 'M' + (beIdx+1) + ' (' + mInfo(beIdx+1).label + ')' : 'Not reached', 'var(--sub)', nc);
    h += '</tbody>';
    const tbl=document.getElementById('cf-tbl'); if (tbl) tbl.innerHTML = h;
  };
  ...
  function insertAtCursor(input, text) {
    ...
    const pos = (before + padBefore + text).length;
    input.focus(); input.setSelectionRange(pos,pos);
    input.dispatchEvent(new Event('input', {bubbles:true}));
  }
  function buildFormulaChips() {
    const wrap = document.getElementById('cl-formula-chips');
    if (!wrap) return;
    let baseCv; try { baseCv = _baseComputeForCustomFix(); } catch(e) { baseCv = {}; }
    const groups = [
      ['P&L Totals', ['Total Income','Total COGS','Gross Profit','Profit before OPEX / Interest','EBIT','EBITDA','OPEX Buffer','Hedging Adjustment','Financing Cost','Penalties']],
      ['Cash Flow', ['Total Inflows','Total Outflows','Net Cash Flow','Accumulated Cash Flow','totalIncomeCF','netCF','accumCF']],
      ['Revenue Categories', Object.keys(baseCv.revCats || {})],
      ['Cost Categories', Object.keys(baseCv.costCat || {}).filter(k => k !== 'Financial Exp')],
      ['Revenue Items', (revenues || []).map(x => x.name).filter(Boolean)],
      ['Cost Items', ([...(capex || []), ...(opex || [])]).map(x => x.name).filter(Boolean)],
      ['Parameters', ['FX','IGV','month','months']]
    ].filter(g => g[1].length);
    wrap.innerHTML = groups.map(([label, vars]) => `<div style="margin-bottom:10px"><div style="font-size:10px;font-weight:800;color:var(--navy);margin-bottom:6px">${label}</div><div style="display:flex;gap:6px;flex-wrap:wrap">${Array.from(new Set(vars)).map(v => `<button type="button" class="formula-chip" data-var="${String(v).replace(/"/g,'&quot;')}" style="border:1px solid var(--bdr2);background:#fff;border-radius:999px;padding:4px 9px;font-size:10px;color:var(--accent);cursor:pointer;font-weight:700">${v}</button>`).join('')}</div></div>`).join('');
    wrap.querySelectorAll('.formula-chip').forEach(btn => btn.addEventListener('click', () => insertAtCursor(document.getElementById('cl-m-formula'), btn.dataset.var)));
  }
  window._buildFormulaChips = buildFormulaChips;
  const oldOpenModal = openCLModal;
  openCLModal = window.openCLModal = function(section){ oldOpenModal(section); setTimeout(buildFormulaChips, 0); };
  const oldOpenEdit = openEditLineModal;
  openEditLineModal = window.openEditLineModal = function(line){ oldOpenEdit(line); setTimeout(buildFormulaChips, 0); };

  renderKnowHow = window.renderKnowHow = function(){
    const root = document.getElementById('knowhow-root');
    if (!root) return;
    root.innerHTML = `
      <div class="pg-hdr"><div><div class="pg-title">Know How</div><div class="pg-sub">Guía rápida para usar el modelo financiero y entender cómo se conectan P&L, Cash Flow, fórmulas y cargas masivas.</div></div></div>
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:14px;margin-bottom:18px">
        <div class="card"><div class="card-body"><div class="sec-lbl">1. P&L vs Cash Flow</div><p style="font-size:12px;line-height:1.7;color:var(--sub)"><strong>P&L</strong> muestra lo devengado: ingresos, COGS, gross profit, EBIT y EBITDA. <strong>Cash Flow</strong> muestra cuándo entra o sale el dinero, considerando condiciones de pago e IGV cuando corresponde.</p></div></div>
        <div class="card"><div class="card-body"><div class="sec-lbl">2. Moneda e IGV</div><p style="font-size:12px;line-height:1.7;color:var(--sub)">Carga Revenue, CAPEX u OPEX en <strong>PEN o USD</strong>. El modelo convierte todo a USD con el FX de Settings. Los montos se cargan sin IGV; si marcas “afecto a IGV”, el P&L se mantiene neto y el Cash Flow considera el movimiento con IGV.</p></div></div>
        <div class="card"><div class="card-body"><div class="sec-lbl">3. Líneas calculadas</div><p style="font-size:12px;line-height:1.7;color:var(--sub)">Usa <strong>+ Add calculated line</strong> para crear líneas como MTC, management fee o royalties. Los chips de fórmula insertan referencias como Gross Profit, Total COGS, EBIT o categorías específicas.</p></div></div>
      </div>
      <div class="card"><div class="card-body"><div class="sec-lbl">Variables útiles para fórmulas</div>
        <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:10px">
          ${['Total Income','Total COGS','Gross Profit','Profit before OPEX / Interest','EBIT','EBITDA','Net Cash Flow','Accumulated Cash Flow','FX','IGV'].map(v=>`<span style="background:#f8fafc;border:1px solid var(--bdr);border-radius:999px;padding:6px 10px;font-size:11px;font-weight:700;color:var(--navy)">${v}</span>`).join('')}
        </div>
        <div style="font-size:12px;color:var(--sub);line-height:1.7;margin-top:14px">Ejemplo: <code>2% of Total Income</code>, <code>Gross Profit * 0.05</code> o <code>IF(EBIT &gt; 0, EBIT * 0.1, 0)</code>.</div>
      </div></div>`;
  };

  const oldRenderTab = renderTab;
  renderTab = window.renderTab = function(t){
    if (t === 'knowhow') return renderKnowHow();
    return oldRenderTab(t);
  };
})();
...
/* =====================================================================
   UX / YEAR SELECTORS / AUTH LAYER
   NOTE: This block only changes interface, access and visualization behavior.
   It does not change compute(), formulas, P&L logic or Cash Flow formulas.
   ===================================================================== */
let chartSelectedYear = String(CFG.sy || new Date().getFullYear());
let plSelectedYear = 'all';
let cfSelectedYear = 'all';

function _injectExecutiveUiStyles() {
 if (document.getElementById('exec-ui-styles')) return;
 const st = document.createElement('style');
 st.id = 'exec-ui-styles';
 st.textContent = `
  header{position:sticky!important;top:0!important;z-index:80!important;box-shadow:0 6px 24px rgba(0,0,0,.10)!important}
  .hdr-inner{gap:10px!important;align-items:center!important}
  #main-nav{display:flex!important;align-items:center!important;gap:6px!important;overflow-x:auto!important;overflow-y:hidden!important;white-space:nowrap!important;max-width:min(56vw,820px)!important;padding:4px!important;border-radius:999px!important;background:rgba(255,255,255,.08)!important;scrollbar-width:thin!important}
  #main-nav button{flex:0 0 auto!important;border-radius:999px!important;padding:7px 12px!important;font-size:11px!important;letter-spacing:.01em!important}
  #main-nav button.active{box-shadow:0 6px 18px rgba(1,118,211,.25)!important}
  .exec-year-select{height:28px;border:1px solid var(--bdr2);border-radius:999px;background:#fff;color:var(--navy);font-size:11px;font-weight:700;padding:0 12px;margin-left:8px;outline:none;box-shadow:0 2px 8px rgba(0,0,0,.05)}
  #dashboard-decision-insights{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:12px;margin:10px 0 18px}
  .decision-card{background:#fff;border:1px solid var(--bdr);border-left:5px solid #0176D3;border-radius:14px;padding:12px 14px;box-shadow:0 10px 28px rgba(0,0,0,.06)}
  .decision-k{font-size:10px;text-transform:uppercase;letter-spacing:.08em;color:var(--muted);font-weight:800;margin-bottom:5px}
  .decision-v{font-size:14px;font-weight:900;margin-bottom:4px}
  .decision-s{font-size:11px;color:var(--sub);line-height:1.45}
  .exec-kpi{min-width:145px!important}
  .auth-screen{min-height:100vh;display:grid;place-items:center;background:linear-gradient(135deg,#08233f,#12385c 46%,#f8fafc 46%);padding:28px;font-family:inherit}
  .auth-card{width:min(920px,96vw);display:grid;grid-template-columns:1fr 1.05fr;background:#fff;border-radius:24px;box-shadow:0 30px 100px rgba(0,0,0,.28);overflow:hidden}
  .auth-brand{background:linear-gradient(160deg,#032D60,#0176D3);color:#fff;padding:36px;display:flex;flex-direction:column;justify-content:space-between;gap:24px}
  .auth-brand h1{font-size:28px;margin:0 0 10px;line-height:1.1}.auth-brand p{font-size:13px;line-height:1.7;opacity:.86}
  .auth-form{padding:34px;display:grid;gap:14px}.auth-tabs{display:flex;background:#f1f5f9;border-radius:999px;padding:4px;margin-bottom:6px}.auth-tabs button{flex:1;border:0;background:transparent;border-radius:999px;padding:8px;font-weight:800;color:#64748b;cursor:pointer}.auth-tabs button.active{background:#fff;color:#032D60;box-shadow:0 3px 12px rgba(0,0,0,.08)}
  .auth-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}.auth-field{display:grid;gap:5px}.auth-field label{font-size:10px;font-weight:800;text-transform:uppercase;color:#64748b}.auth-field input,.auth-field select{height:38px;border:1px solid #d8e0ea;border-radius:10px;padding:0 11px;font-size:13px;outline:none}.auth-field input:focus{border-color:#0176D3;box-shadow:0 0 0 3px rgba(1,118,211,.10)}
  .auth-btn{height:40px;border:0;border-radius:10px;background:#0176D3;color:#fff;font-weight:900;cursor:pointer}.auth-msg{font-size:12px;min-height:18px;color:#BA0517}.user-pill{display:flex;align-items:center;gap:8px;background:rgba(255,255,255,.14);color:#fff;border:1px solid rgba(255,255,255,.24);border-radius:999px;padding:6px 10px;font-size:11px;font-weight:800}.user-pill button{border:0;border-radius:999px;background:rgba(255,255,255,.16);color:#fff;font-size:10px;padding:4px 8px;cursor:pointer}
  .users-modal{position:fixed;inset:0;display:none;align-items:center;justify-content:center;background:rgba(0,0,0,.55);z-index:9999}.users-modal.show{display:flex}.users-box{width:min(760px,94vw);max-height:86vh;overflow:auto;background:#fff;border-radius:18px;box-shadow:0 30px 90px rgba(0,0,0,.35)}.users-head{display:flex;justify-content:space-between;align-items:center;background:#032D60;color:#fff;padding:14px 18px}.users-body{padding:18px}.users-row{display:grid;grid-template-columns:1.4fr 1fr 120px 80px;gap:8px;align-items:center;border-bottom:1px solid #edf2f7;padding:10px 0;font-size:12px}.users-row select{height:30px;border:1px solid #d8e0ea;border-radius:8px;padding:0 8px}.users-close{border:0;background:rgba(255,255,255,.15);color:#fff;border-radius:8px;padding:5px 9px;cursor:pointer}
  @media(max-width:920px){.auth-card{grid-template-columns:1fr}.auth-brand{padding:26px}.auth-grid{grid-template-columns:1fr}#main-nav{max-width:70vw!important}}
 `;
 document.head.appendChild(st);
}

function _makeExecutiveYearSelect(kind, value, onChange) {
 const sel = document.createElement('select');
 sel.className = 'exec-year-select';
 const years = getYearRanges();
 if (kind !== 'dashboard') {
  const opt = document.createElement('option'); opt.value = 'all'; opt.textContent = 'All years'; sel.appendChild(opt);
 }
 years.forEach(y => { const opt = document.createElement('option'); opt.value = String(y.year); opt.textContent = 'FY ' + y.year; sel.appendChild(opt); });
 const fallback = years[0] ? String(years[0].year) : 'all';
 sel.value = value && (value === 'all' || years.some(y => String(y.year) === String(value))) ? String(value) : (kind === 'dashboard' ? fallback : 'all');
 sel.addEventListener('change', e => onChange(e.target.value));
 return sel;
}
function _hideGroupedYearColumns(tblId, selectedYear) {
 if (!selectedYear || selectedYear === 'all') return;
 const tbl = document.getElementById(tblId); if (!tbl) return;
 const years = getYearRanges(); const target = years.find(y => String(y.year) === String(selectedYear)); if (!target) return;
 const headers = Array.from(tbl.querySelectorAll('thead th'));
 const hideIdx = [];
 headers.forEach((th, idx) => { if (idx === 0) return; const yi = th.dataset ? th.dataset.yi : null; if (yi !== undefined && yi !== null && yi !== '' && parseInt(yi) !== target.yi) hideIdx.push(idx + 1); });
 hideIdx.forEach(n => { tbl.querySelectorAll('tr > *:nth-child(' + n + ')').forEach(cell => cell.style.display = 'none'); });
}
function _prepareSingleYearOpen(which, selectedYear) {
 if (!selectedYear || selectedYear === 'all') return;
 const years = getYearRanges();
 const target = years.find(y => String(y.year) === String(selectedYear));
 if (!target) return;
 years.forEach(y => { if (which === 'pl') plYearOpen[y.yi] = (y.yi === target.yi); else cfYearOpen[y.yi] = (y.yi === target.yi); });
}

(function _wrapYearSelectorsAndNav(){
 _injectExecutiveUiStyles();
 const _renderPL0 = renderPL;
 renderPL = window.renderPL = function(){
  if (plView === 'grouped') _prepareSingleYearOpen('pl', plSelectedYear);
  _renderPL0();
  const wrap = document.getElementById('pl-view-tog');
  if (wrap && plView === 'grouped') wrap.appendChild(_makeExecutiveYearSelect('pl', plSelectedYear, v => { plSelectedYear = v; renderPL(); }));
  if (plView === 'grouped') _hideGroupedYearColumns('pl-tbl', plSelectedYear);
 };
 const _renderCF0 = renderCF;
 renderCF = window.renderCF = function(){
  if (cfView === 'grouped') _prepareSingleYearOpen('cf', cfSelectedYear);
  _renderCF0();
  const wrap = document.getElementById('cf-view-tog');
  if (wrap && cfView === 'grouped') wrap.appendChild(_makeExecutiveYearSelect('cf', cfSelectedYear, v => { cfSelectedYear = v; renderCF(); }));
  if (cfView === 'grouped') _hideGroupedYearColumns('cf-tbl', cfSelectedYear);
 };
 const _showProject0 = showProject;
 showProject = window.showProject = function(){ _showProject0(); _enhanceModernNav(); };
 const _setTab0 = setTab;
 setTab = window.setTab = function(tab){ _setTab0(tab); _enhanceModernNav(); };
})();
function _enhanceModernNav(){
 const nav = document.getElementById('main-nav'); if (!nav) return;
 nav.querySelectorAll('button').forEach(btn => { if (!btn.title) btn.title = btn.textContent.trim(); });
}

/* -------------------- Simple users / roles layer -------------------- */
const USERS_TABLE = 'app_users';
const LS_USERS = 'fm_app_users_v1';
const LS_SESSION = 'fm_current_user_v1';
function _safeJsonParse(s, fallback){ try { return JSON.parse(s || ''); } catch(e){ return fallback; } }
function _localUsers(){ return _safeJsonParse(localStorage.getItem(LS_USERS), []); }
function _setLocalUsers(users){ localStorage.setItem(LS_USERS, JSON.stringify(users || [])); }
function _currentUser(){ return _safeJsonParse(localStorage.getItem(LS_SESSION), null); }
function _setCurrentUser(user){ localStorage.setItem(LS_SESSION, JSON.stringify({ id:user.id, first_name:user.first_name, last_name:user.last_name, username:user.username, role:user.role || 'member' })); }
function _clearCurrentUser(){ localStorage.removeItem(LS_SESSION); }
function _normalizeName(s){ return String(s||'').trim().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-zA-Z0-9]/g,'').toLowerCase(); }
async function _sha256(txt){
 ...
 let h = 0; for (let i=0;i<txt.length;i++) h = Math.imul(31,h) + txt.charCodeAt(i) | 0; return 'fallback_' + Math.abs(h);
}
async function _passwordHash(password, salt){ return _sha256((salt || '') + '|' + password); }
async function _fetchUsers(){
 let local = _localUsers();
 if (!_sbReady || !supabaseClient) return local;
 try {
  const { data, error } = await supabaseClient.from(USERS_TABLE).select('id,first_name,last_name,username,role,password_hash,password_salt,created_at').order('created_at', { ascending:true });
  if (error) return local;
  const remote = data || [];
  if (remote.length) { _setLocalUsers(remote); return remote; }
  return local;
 } catch(e){ return local; }
}
async function _saveUserRecord(user){
 const users = _localUsers(); const i = users.findIndex(u => u.username === user.username || u.id === user.id); if (i >= 0) users[i] = user; else users.push(user); _setLocalUsers(users);
 if (!_sbReady || !supabaseClient) return user;
 try { await supabaseClient.from(USERS_TABLE).upsert([user], { onConflict:'username' }); } catch(e){}
 return user;
}
function _uniqueUsername(first, last, users){
 const base = ((_normalizeName(first).charAt(0) || 'u') + (_normalizeName(last) || 'user')).slice(0,28);
 const used = new Set((users||[]).map(u => String(u.username).toLowerCase()));
 let u = base, n = 2; while (used.has(u.toLowerCase())) { u = base + n; n++; }
 return u;
}
function _ensureAuthRoot(){
 let root = document.getElementById('auth-screen-root');
 if (!root) { root = document.createElement('div'); root.id = 'auth-screen-root'; document.body.prepend(root); }
 return root;
}
function _showPortfolioDom(){ const root=document.getElementById('auth-screen-root'); if(root) root.style.display='none'; const ps=document.getElementById('portfolio-screen'); if(ps) ps.style.display=''; const pr=document.getElementById('project-screen'); if(pr) pr.style.display='none'; }
function _renderAuthScreen(msg){
 _injectExecutiveUiStyles();
 const root = _ensureAuthRoot();
 root.style.display = 'block';
 const portfolio = document.getElementById('portfolio-screen'); if (portfolio) portfolio.style.display = 'none';
 const project = document.getElementById('project-screen'); if (project) project.style.display = 'none';
 root.innerHTML = `<div class="auth-screen"><div class="auth-card">
  <div class="auth-brand"><div><h1>Financial Model Manager</h1><p>Acceso por usuario, rol y contraseña. El username se genera automáticamente con la primera letra del nombre y el apellido.</p><div style="margin-top:14px;background:rgba(255,255,255,.13);border:1px solid rgba(255,255,255,.22);border-radius:14px;padding:12px 14px;font-size:12px;line-height:1.6"><strong>Ejemplo de registro:</strong><br>Nombre: Rodrigo · Apellido: Hernandez<br>Username generado: <strong>@rhernandez</strong></div></div><p style="font-size:11px;opacity:.72">Roles: Admin puede ver usuarios y todos los proyectos. Member trabaja con sus proyectos asignados.</p></div>
  <div class="auth-form">
   <div class="auth-tabs"><button id="auth-login-tab" class="active">Ingresar</button><button id="auth-reg-tab">Registrarse</button></div>
   <div id="auth-login-box">
    <div class="auth-field"><label>Username</label><input id="login-username" placeholder="ej. rhernandez"></div>
    <div class="auth-field"><label>Contraseña</label><input id="login-password" type="password" placeholder="••••••••"></div>
    <button class="auth-btn" id="login-btn">Ingresar</button>
   </div>
   <div id="auth-reg-box" style="display:none">
    <div class="auth-grid"><div class="auth-field"><label>Nombre</label><input id="reg-first" placeholder="Rodrigo"></div><div class="auth-field"><label>Apellido</label><input id="reg-last" placeholder="Hernandez"></div></div>
    <div id="reg-username-preview" style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:9px 11px;font-size:12px;color:#032D60">Ejemplo: Rodrigo Hernandez → <strong>@rhernandez</strong></div>
    <div class="auth-field"><label>Contraseña</label><input id="reg-password" type="password" placeholder="mínimo 6 caracteres"></div>
    <button class="auth-btn" id="register-btn">Crear usuario</button>
   </div>
   <div class="auth-msg" id="auth-msg">${msg ? esc(msg) : ''}</div>
   <details style="font-size:11px;color:#64748b;line-height:1.6"><summary style="cursor:pointer;font-weight:800;color:#0176D3">SQL para usuarios en Supabase</summary><pre style="white-space:pre-wrap;background:#0f2438;color:#7dd3fc;border-radius:10px;padding:12px;font-size:10px">CREATE TABLE IF NOT EXISTS app_users (\n  id text PRIMARY KEY,\n  first_name text,\n  last_name text,\n  username text UNIQUE NOT NULL,\n  role text CHECK (role IN ('admin','member')) DEFAULT 'member',\n  password_hash text NOT NULL,\n  password_salt text NOT NULL,\n  created_at timestamptz DEFAULT now(),\n  updated_at timestamptz DEFAULT now()\n);\nALTER TABLE app_users DISABLE ROW LEVEL SECURITY;</pre></details>
  </div></div></div>`;
 const loginTab = document.getElementById('auth-login-tab'), regTab = document.getElementById('auth-reg-tab');
 const loginBox = document.getElementById('auth-login-box'), regBox = document.getElementById('auth-reg-box'), m = document.getElementById('auth-msg');
 loginTab.onclick = () => { loginTab.classList.add('active'); regTab.classList.remove('active'); loginBox.style.display='grid'; loginBox.style.gap='10px'; regBox.style.display='none'; m.textContent=''; };
 regTab.onclick = () => { regTab.classList.add('active'); loginTab.classList.remove('active'); regBox.style.display='grid'; regBox.style.gap='10px'; loginBox.style.display='none'; m.textContent=''; };
 const _updateRegPreview = () => {
  const f = document.getElementById('reg-first')?.value.trim() || 'Rodrigo';
  const l = document.getElementById('reg-last')?.value.trim() || 'Hernandez';
  const preview = document.getElementById('reg-username-preview');
  if (preview) preview.innerHTML = 'Username generado: <strong>@' + esc(((_normalizeName(f).charAt(0)||'r') + (_normalizeName(l)||'hernandez')).slice(0,28)) + '</strong>';
 };
 ['reg-first','reg-last'].forEach(id => { const el = document.getElementById(id); if (el) el.addEventListener('input', _updateRegPreview); });
 _updateRegPreview();
 document.getElementById('register-btn').onclick = async () => {
  const first = document.getElementById('reg-first').value.trim(); const last = document.getElementById('reg-last').value.trim(); const password = document.getElementById('reg-password').value;
  if (!first || !last || password.length < 6) { m.textContent = 'Completa nombre, apellido y una contraseña de al menos 6 caracteres.'; return; }
  const users = await _fetchUsers(); const username = _uniqueUsername(first, last, users); const salt = 's_' + Date.now() + '_' + Math.random().toString(36).slice(2);
  const user = { id:'user_' + Date.now() + '_' + Math.random().toString(36).slice(2,8), first_name:first, last_name:last, username, role: users.length ? 'member' : 'admin', password_salt:salt, password_hash: await _passwordHash(password, salt), created_at:new Date().toISOString(), updated_at:new Date().toISOString() };
  await _saveUserRecord(user); _setCurrentUser(user); _showPortfolioDom(); await renderPortfolio(); _sbToast('Usuario creado: ' + username + ' (' + user.role + ')', '#2E844A');
 };
 document.getElementById('login-btn').onclick = async () => {
  const username = _normalizeName(document.getElementById('login-username').value); const password = document.getElementById('login-password').value;
  const users = await _fetchUsers(); const user = users.find(u => String(u.username).toLowerCase() === username.toLowerCase());
  if (!user) { m.textContent = 'Usuario no encontrado.'; return; }
  const h = await _passwordHash(password, user.password_salt); if (h !== user.password_hash) { m.textContent = 'Contraseña incorrecta.'; return; }
  _setCurrentUser(user); _showPortfolioDom(); await renderPortfolio();
 };
}
function _projectOwner(p){ return p && p.data && (p.data.__ownerUsername || p.data.ownerUsername); }
function _canSeeProject(p){ const u = _currentUser(); if (!u) return false; if (u.role === 'admin') return true; const owner = _projectOwner(p); return !owner || owner === u.username; }
(function _wrapAuthLayer(){
 const _projectPayload0 = projectPayload;
 projectPayload = window.projectPayload = function(){ const p = _projectPayload0(); const u = _currentUser(); if (u) { p.__ownerUsername = u.username; p.__ownerName = (u.first_name + ' ' + u.last_name).trim(); } return p; };
 const _loadPortfolio0 = loadPortfolio;
 loadPortfolio = window.loadPortfolio = async function(){ await _loadPortfolio0(); if (_currentUser()) portfolio = (portfolio || []).filter(_canSeeProject); };
 const _renderPortfolio0 = renderPortfolio;
 renderPortfolio = window.renderPortfolio = async function(){ if (!_currentUser()) { _renderAuthScreen(); return; } await _renderPortfolio0(); _decorateUserBar(); };
 const _showPortfolio0 = showPortfolio;
 showPortfolio = window.showPortfolio = function(){ if (!_currentUser()) { _renderAuthScreen(); return; } _showPortfolio0(); _decorateUserBar(); };
 const _newProject0 = newProject;
 newProject = window.newProject = function(){ if (!_currentUser()) { _renderAuthScreen(); return; } _newProject0(); };
})();
function _decorateUserBar(){
 const u = _currentUser(); if (!u) return;
 const header = document.querySelector('#portfolio-screen .port-header') || document.querySelector('header .hdr-inner'); if (!header) return;
 let pill = document.getElementById('current-user-pill');
 if (!pill) { pill = document.createElement('div'); pill.id = 'current-user-pill'; pill.className = 'user-pill'; }
 if (pill.parentElement !== header) header.appendChild(pill);
 pill.innerHTML = `<span>${esc(u.first_name||'')} ${esc(u.last_name||'')} · @${esc(u.username)} · ${esc(u.role)}</span>${u.role==='admin'?'<button id="manage-users-btn">Users</button>':''}<button id="logout-btn">Salir</button>`;
 const logout = document.getElementById('logout-btn'); if (logout) logout.onclick = () => { _clearCurrentUser(); location.reload(); };
 const usersBtn = document.getElementById('manage-users-btn'); if (usersBtn) usersBtn.onclick = _openUsersModal;
}
async function _openUsersModal(){
 let modal = document.getElementById('users-modal');
 if (!modal) { modal = document.createElement('div'); modal.id='users-modal'; modal.className='users-modal'; document.body.appendChild(modal); }
 const users = await _fetchUsers();
 modal.innerHTML = `<div class="users-box"><div class="users-head"><strong>Usuarios y roles</strong><button class="users-close" id="users-close">×</button></div><div class="users-body"><div style="font-size:12px;color:#64748b;margin-bottom:10px">El primer usuario se crea como admin. Desde aquí puedes cambiar el rol de cada persona.</div>${users.map(u=>`<div class="users-row"><div><strong>${esc((u.first_name||'')+' '+(u.last_name||''))}</strong><br><span style="color:#64748b">@${esc(u.username)}</span></div><div>${esc(u.created_at ? new Date(u.created_at).toLocaleDateString('es-PE') : '')}</div><select data-user-role="${esc(u.username)}"><option value="admin" ${u.role==='admin'?'selected':''}>admin</option><option value="member" ${u.role!=='admin'?'selected':''}>member</option></select><span style="font-weight:800;color:${u.role==='admin'?'#0176D3':'#64748b'}">${esc(u.role||'member')}</span></div>`).join('')}</div></div>`;
 modal.classList.add('show');
 document.getElementById('users-close').onclick = () => modal.classList.remove('show');
 modal.querySelectorAll('[data-user-role]').forEach(sel => sel.onchange = async () => { const username = sel.dataset.userRole; const users2 = await _fetchUsers(); const user = users2.find(x=>x.username===username); if (!user) return; user.role = sel.value; user.updated_at = new Date().toISOString(); await _saveUserRecord(user); const cur = _currentUser(); if (cur && cur.username === user.username) _setCurrentUser(user); _sbToast('Rol actualizado', '#2E844A'); _openUsersModal(); });
}


/* =====================================================================
   PROJECT ASSIGNMENT + COMMENTS PATCH
   Scope: UI/auth/project metadata only. Does not modify financial formulas,
   compute(), P&L logic, Cash Flow logic or import/export logic.
   ===================================================================== */
let projectComments = Array.isArray(window.projectComments) ? window.projectComments : [];
let _appUsersCache = [];

function _fullUserName(u){ return String(((u && u.first_name) || '') + ' ' + ((u && u.last_name) || '')).trim(); }
function _commentDate(iso){ try { return new Date(iso).toLocaleString('es-PE', { dateStyle:'medium', timeStyle:'short' }); } catch(e){ return ''; } }
function _projectAssignedUsername(p){ return p && (p.assignedUsername || p.assigned_username || (p.data && (p.data.__assignedUsername || p.data.assignedUsername))); }
function _projectAssignedName(p){ return p && (p.assignedName || p.assigned_name || (p.data && (p.data.__assignedName || p.data.assignedName))); }
function _getProjectCommentsFromPayload(p){ return Array.isArray(p && p.__comments) ? p.__comments : (Array.isArray(p && p.comments) ? p.comments : []); }

// Allow members to see projects they own or projects assigned to them.
_canSeeProject = function(p){
 const u = _currentUser();
 if (!u) return false;
 if (u.role === 'admin') return true;
 const owner = _projectOwner(p);
 const assigned = _projectAssignedUsername(p);
 return (!owner && !assigned) || owner === u.username || assigned === u.username;
};

async function _refreshUsersCache(){
 try { _appUsersCache = await _fetchUsers(); }
 catch(e){ _appUsersCache = _localUsers ? _localUsers() : []; }
 return _appUsersCache || [];
}
function _findCachedUser(username){
 const u = (_appUsersCache || []).find(x => String(x.username).toLowerCase() === String(username||'').toLowerCase());
 return u || null;
}

function _ensureAssigneeField(){
 const status = document.getElementById('save-status');
 if (!status) return;
 const statusFld = status.closest('.fld');
 let field = document.getElementById('save-assignee-field');
 if (!field) {
  field = document.createElement('div');
  field.className = 'fld';
  field.id = 'save-assignee-field';
  field.innerHTML = '<label>Asignado a</label><select id="save-assignee"><option value="">Cargando usuarios...</option></select><div style="font-size:10px;color:var(--muted);margin-top:4px">El proyecto será visible para el admin, el dueño y la persona asignada.</div>';
  if (statusFld && statusFld.parentElement) statusFld.after(field);
 }
 _populateAssigneeSelect();
}
async function _populateAssigneeSelect(){
 const sel = document.getElementById('save-assignee');
 if (!sel) return;
 const users = await _refreshUsersCache();
 const current = _currentUser();
 const proj = currentProjectId ? (portfolio || []).find(p => p.id === currentProjectId) : null;
 const assigned = _projectAssignedUsername(proj) || (current && current.username) || '';
 sel.innerHTML = users.length ? users.map(u => `<option value="${esc(u.username)}">${esc(_fullUserName(u) || u.username)} · @${esc(u.username)} · ${esc(u.role || 'member')}</option>`).join('') : '<option value="">Sin usuarios registrados</option>';
 if (assigned && Array.from(sel.options).some(o => o.value === assigned)) sel.value = assigned;
}

// The original save button listener opens the modal. This extra listener only injects the assignee field after it opens.
const _saveBtnForAssignee = document.getElementById('save-proj-btn');
if (_saveBtnForAssignee) _saveBtnForAssignee.addEventListener('click', () => setTimeout(_ensureAssigneeField, 0));

// Preserve comments and assignment inside the existing project JSON data.
const _projectPayloadAssignment0 = projectPayload;
projectPayload = window.projectPayload = function(){
 const p = _projectPayloadAssignment0();
 const current = _currentUser();
 const sel = document.getElementById('save-assignee');
 const fallbackAssigned = p.__assignedUsername || (current && current.username) || '';
 const assignedUsername = sel ? sel.value : fallbackAssigned;
 const assignedUser = _findCachedUser(assignedUsername);
 p.__assignedUsername = assignedUsername || '';
 p.__assignedName = assignedUser ? _fullUserName(assignedUser) : (p.__assignedName || assignedUsername || '');
 p.__comments = Array.isArray(projectComments) ? projectComments : [];
 return p;
};

const _loadProjectPayloadAssignment0 = loadProjectPayload;
loadProjectPayload = window.loadProjectPayload = function(p){
 projectComments = _getProjectCommentsFromPayload(p);
 window.projectComments = projectComments;
 _loadProjectPayloadAssignment0(p);
 setTimeout(() => { if (document.getElementById('panel-comments')?.classList.contains('show')) renderProjectComments(); }, 0);
};

function _ensureCommentsTab(){
 const nav = document.getElementById('main-nav');
 if (nav && !document.querySelector('[data-tab="comments"]')) {
  const btn = document.createElement('button');
  btn.dataset.tab = 'comments';
  btn.textContent = 'Comments';
  nav.appendChild(btn);
 }
 const main = document.querySelector('#project-screen main');
 if (main && !document.getElementById('panel-comments')) {
  const panel = document.createElement('div');
  panel.className = 'panel';
  panel.id = 'panel-comments';
  panel.innerHTML = '<div id="comments-root"></div>';
  main.appendChild(panel);
 }
 if (typeof TAB_TITLES !== 'undefined') TAB_TITLES.comments = 'Comments';
}

const _renderTabComments0 = renderTab;
renderTab = window.renderTab = function(t){
 _ensureCommentsTab();
 if (t === 'comments') return renderProjectComments();
 return _renderTabComments0(t);
};

function _commentAuthor(){
 const u = _currentUser();
 return u ? { username:u.username, name:_fullUserName(u), role:u.role || 'member' } : { username:'local', name:'Usuario local', role:'member' };
}
async function _persistProjectMetaOnly(){
 if (!currentProjectId) {
  _sbToast('Comentario guardado en borrador. Guarda el proyecto para conservarlo.', '#A56105');
  return;
 }
 const idx = (portfolio || []).findIndex(p => p.id === currentProjectId);
 if (idx < 0) return;
 const existing = portfolio[idx];
 const updated = {
  ...existing,
  data: projectPayload(),
  snapshot: existing.snapshot || buildSnapshot(),
  updatedAt: new Date().toISOString()
 };
 const saved = await saveProjectToSupabase(updated);
 portfolio[idx] = { ...updated, ...saved };
}
function renderProjectComments(){
 ...
 const current = _currentUser();
 const proj = currentProjectId ? (portfolio || []).find(p => p.id === currentProjectId) : null;
 const assignedName = _projectAssignedName(proj) || (current ? _fullUserName(current) : '');
 const assignedUsername = _projectAssignedUsername(proj) || (current && current.username) || '';
 const comments = Array.isArray(projectComments) ? projectComments : [];
 root.innerHTML = `
  <div class="comments-layout">
   <div class="comments-head">
    <div>
     <div class="comments-title">Comentarios del proyecto</div>
     <div class="comments-sub">Registra acuerdos, pendientes, decisiones y contexto sin alterar el modelo financiero.</div>
    </div>
    <div class="comments-assignee">Asignado a: <strong>${esc(assignedName || 'Sin asignar')}</strong>${assignedUsername ? ` · @${esc(assignedUsername)}` : ''}</div>
   </div>
   <div class="comments-compose">
    <textarea id="comment-text" placeholder="Escribe un comentario, pendiente o decisión del proyecto..."></textarea>
    <div style="display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap">
     <span style="font-size:11px;color:#64748b">Se guardará dentro del proyecto actual.</span>
     <button id="comment-add-btn" class="btn btn-primary" style="padding:7px 16px;font-size:12px">Agregar comentario</button>
    </div>
   </div>
   <div class="comments-list">
    ${comments.length ? comments.slice().reverse().map(c => `<div class="comment-card">
      <div class="comment-top"><strong>${esc(c.authorName || c.author || 'Usuario')}</strong><span>@${esc(c.authorUsername || '')} · ${esc(_commentDate(c.createdAt))}</span></div>
      <div class="comment-text">${esc(c.text || '').replace(/\n/g,'<br>')}</div>
     </div>`).join('') : '<div class="comments-empty">Aún no hay comentarios en este proyecto.</div>'}
   </div>
  </div>`;
 ...
 if (btn) btn.onclick = async () => {
  ...
  if (!txt) return;
  const a = _commentAuthor();
  projectComments.push({ id:'comment_' + Date.now() + '_' + Math.random().toString(36).slice(2,7), text:txt, authorName:a.name, authorUsername:a.username, authorRole:a.role, createdAt:new Date().toISOString() });
  window.projectComments = projectComments;
  await _persistProjectMetaOnly();
  renderProjectComments();
 };
}

function _decorateProjectAssigneeBadges(){
 const grid = document.getElementById('port-grid');
 if (!grid || !(portfolio || []).length) return;
 const cards = Array.from(grid.querySelectorAll('.proj-card'));
 if (!cards.length) return;
 const visible = activeFilter === 'All' ? portfolio : portfolio.filter(p => p.classification === activeFilter);
 const sorted = [...visible].sort((a,b) => (b.updatedAt||'') > (a.updatedAt||'') ? 1 : -1);
 sorted.forEach((proj, i) => {
  const card = cards[i]; if (!card) return;
  card.dataset.projectId = proj.id || '';
  const meta = card.querySelector('.proj-card-meta');
  if (!meta || meta.querySelector('.assigned-badge')) return;
  const assignedName = _projectAssignedName(proj);
  const assignedUsername = _projectAssignedUsername(proj);
  if (assignedName || assignedUsername) {
   const chip = document.createElement('span');
   chip.className = 'proj-badge assigned-badge';
   chip.style.background = '#eef2ff';
   chip.style.color = '#3730a3';
   chip.textContent = 'Asignado: ' + (assignedName || ('@' + assignedUsername));
   meta.appendChild(chip);
  }
 });
}

const _renderPortfolioAssign0 = renderPortfolio;
renderPortfolio = window.renderPortfolio = async function(){
 await _renderPortfolioAssign0();
 _decorateProjectAssigneeBadges();
 _ensureCommentsTab();
};

// Reset comments on the new-project action without touching the original financial setup.
const _newProjectBtnComments = document.getElementById('port-new-btn');
if (_newProjectBtnComments) _newProjectBtnComments.addEventListener('click', () => { projectComments = []; window.projectComments = projectComments; }, true);

(function _injectAssignmentCommentStyles(){
 if (document.getElementById('assignment-comments-styles')) return;
 const st = document.createElement('style');
 st.id = 'assignment-comments-styles';
 st.textContent = `
  .comments-layout{display:grid;gap:14px}.comments-head{display:flex;justify-content:space-between;align-items:flex-start;gap:14px;flex-wrap:wrap;background:#fff;border:1px solid var(--bdr);border-radius:16px;padding:16px 18px;box-shadow:0 10px 28px rgba(15,35,56,.06)}
  .comments-title{font-size:17px;font-weight:900;color:var(--navy)}.comments-sub{font-size:12px;color:#64748b;margin-top:3px}.comments-assignee{background:#eef2ff;color:#3730a3;border-radius:999px;padding:7px 12px;font-size:12px}
  .comments-compose{background:#fff;border:1px solid var(--bdr);border-radius:16px;padding:14px 16px;display:grid;gap:10px;box-shadow:0 10px 28px rgba(15,35,56,.05)}.comments-compose textarea{min-height:92px;border:1px solid #d8e0ea;border-radius:12px;padding:12px;font-size:13px;resize:vertical;outline:none;font-family:inherit}.comments-compose textarea:focus{border-color:#0176D3;box-shadow:0 0 0 3px rgba(1,118,211,.10)}
  .comments-list{display:grid;gap:10px}.comment-card{background:#fff;border:1px solid var(--bdr);border-radius:14px;padding:13px 15px}.comment-top{display:flex;justify-content:space-between;gap:10px;flex-wrap:wrap;font-size:12px;color:#032D60}.comment-top span{color:#64748b;font-size:11px}.comment-text{font-size:13px;color:#334155;line-height:1.55;margin-top:8px}.comments-empty{border:1px dashed #cbd5e1;border-radius:14px;padding:22px;text-align:center;color:#64748b;background:#f8fafc}
  #save-assignee-field select{height:36px;border:1px solid var(--bdr2);border-radius:8px;padding:0 10px;font-size:13px;background:#fff}
 `;
 document.head.appendChild(st);
})();
...
renderPortfolio();
/* =====================================================================
   PROFESSIONAL FORMULA-BASED EXCEL EXPORT
   Scope: Export only. Does not modify compute(), model logic, formulas or UI behavior.
   ===================================================================== */
(function _formulaBasedExcelExportOnly(){
  function _excelCol(n){
    let s='';
    while(n>=0){ s=String.fromCharCode((n%26)+65)+s; n=Math.floor(n/26)-1; }
    return s;
  }
  function _cellAddr(r,c){ return _excelCol(c) + String(r); }
  ...
  function _isVisibleCell(cell){
    if (!cell) return false;
    if (cell.style && cell.style.display === 'none') return false;
    try { if (window.getComputedStyle(cell).display === 'none') return false; } catch(e) {}
    return true;
  }
  function _visibleCells(tr){ return Array.from(tr.children || []).filter(_isVisibleCell); }
  function _parseNum(raw){
    raw = _cleanText(raw);
    if (!raw || raw === '-' || raw === '—') return '';
    // Keep business text rows as text
    if (/^(USD|PEN)\s+/i.test(raw)) raw = raw.replace(/^(USD|PEN)\s+/i,'');
    if (/%\s*(p\.a\.|\/mo)?/i.test(raw) && !/^\(?[-+]?\d[\d,.]*%\)?$/.test(raw)) return raw;
  ...
  function _headerText(th){
    let txt = _cleanText(th.innerText || th.textContent || '');
    const yi = th.dataset && th.dataset.yi;
    if (yi !== undefined && yi !== null && yi !== '') {
      const yr = (txt.match(/20\d{2}|19\d{2}/)||[])[0];
      return yr ? ('FY ' + yr) : txt.replace(/collapse|expand/ig,'').trim();
    }
    txt = txt.replace(/\s*collapse\s*$/i,'').replace(/\s*expand\s*$/i,'').trim();
    txt = txt.replace(/([A-Za-z]{3}\s*\d{2})\s*\(M(\d+)\)/, '$1 (M$2)');
    return txt || 'Column';
  }
  function _extractVisibleTable(tblId){
    const tbl = document.getElementById(tblId);
    if (!tbl) throw new Error('No encontré la tabla ' + tblId);
    const headerTr = tbl.querySelector('thead tr');
    let headers = headerTr ? _visibleCells(headerTr).map(_headerText) : [];
    if (!headers.length) headers = ['Category'];
    headers[0] = 'Category';
    const rows = [];
    Array.from(tbl.querySelectorAll('tbody tr')).forEach(tr => {
      if (tr.querySelector('button')) return; // skip Add line / reorder UI rows
      const secCell = tr.querySelector('td[colspan="999"], td[colspan="998"], td[colspan="997"]');
      if (tr.classList && tr.classList.contains('sec-row') || secCell) {
        const c = secCell || _visibleCells(tr)[0];
        const label = _cleanText(c ? (c.innerText || c.textContent) : '');
        if (label) rows.push({ type:'section', label });
        return;
      }
      const cells = _visibleCells(tr);
      if (!cells.length) return;
      const label = _cleanText(cells[0].innerText || cells[0].textContent || '');
      if (!label || /^\+/.test(label) || /add line/i.test(label)) return;
      const row = { type:'data', label, values:[], isBold: tr.classList.contains('bold-row') || cells[0].classList.contains('bold'), isInfo: false };
      const colspan = parseInt((cells[1] && cells[1].getAttribute('colspan')) || '1', 10);
      if (cells.length === 2 && colspan > 1) {
        row.isInfo = true;
        row.values = [_cleanText(cells[1].innerText || cells[1].textContent || '')];
      } else {
        for (let i=1;i<cells.length;i++) row.values.push(_parseNum(cells[i].innerText || cells[i].textContent || ''));
      }
      rows.push(row);
    });
    return { headers, rows };
  }
  function _loadScriptOnce(src, flagName){
    return new Promise(resolve => {
      if (flagName && window[flagName]) return resolve(true);
      const existing = Array.from(document.scripts || []).find(s => s.src === src);
      if (existing) {
        existing.addEventListener('load', () => resolve(true), { once:true });
        existing.addEventListener('error', () => resolve(false), { once:true });
        setTimeout(() => resolve(!!(flagName && window[flagName])), 3500);
        return;
      }
      const s = document.createElement('script');
      s.src = src;
      s.onload = () => { if (flagName) window[flagName] = true; resolve(true); };
      s.onerror = () => resolve(false);
      document.head.appendChild(s);
      setTimeout(() => resolve(!!(flagName && window[flagName])), 6000);
    });
  }
  async function _ensureXLSXExportLib(){
    if (typeof XLSX === 'undefined') {
      if (typeof ensureXLSXLoaded === 'function') await ensureXLSXLoaded();
      if (typeof XLSX === 'undefined') await _loadScriptOnce('https://cdn.jsdelivr.net/npm/xlsx@0.18.5/dist/xlsx.full.min.js', null);
    }
    // Load style-capable SheetJS-compatible bundle for better-looking xlsx. If it fails, normal XLSX still works.
    if (!window.__FM_XLSX_STYLE_TRIED__) {
      window.__FM_XLSX_STYLE_TRIED__ = true;
      await _loadScriptOnce('https://cdn.jsdelivr.net/npm/xlsx-js-style@1.2.0/dist/xlsx.bundle.js', '__FM_XLSX_STYLE_LOADED__');
    }
    return typeof XLSX !== 'undefined';
  }
  function _formulaForCustomLine(label, col, labelRows){
    const candidates = (customLines || []).filter(l => _labelKey(l.name) === _labelKey(label) && String(l.formula||'').trim());
    if (!candidates.length) return null;
    let expr = String(candidates[0].formula || '').trim().replace(/^=/,'');
    expr = expr.replace(/;/g, ',');
    expr = expr.replace(/(\d+(?:\.\d+)?)\s*%\s*(?:of|de)\s*([A-Za-zÁÉÍÓÚÜÑáéíóúüñ][A-Za-zÁÉÍÓÚÜÑáéíóúüñ0-9 _&/\-.]*?)(?=\s*(?:[+\-*/),]|$))/gi, function(_, pct, ref){
      const key = _bestLabelKey(ref, labelRows);
      return key ? '(' + (Number(pct)/100) + '*' + _cellAddr(labelRows[key], col) + ')' : '(' + (Number(pct)/100) + '*0)';
    });
    expr = expr.replace(/(\d+(?:\.\d+)?)\s*%/g, function(_, pct){ return String(Number(pct)/100); });
    const aliases = {
      totalincome:'Total Income', totalcogs:'Total COGS', grossprofit:'Gross Profit',
      profitbeforeopex:'Profit before OPEX / Interest', profitbeforeinterest:'Profit before OPEX / Interest',
      ebit:'EBIT', ebitda:'EBITDA', opexbuffer:'OPEX Buffer', hedging:'Hedging', hedgingadjustment:'Hedging Adjustment',
      financingcost:'Financing Cost', netcf:'Net Cash Flow', accumulatedcf:'Accumulated CF', totalinflows:'Total Inflows', totaloutflows:'Total Outflows', penalties:'Penalties'
    };
    Object.entries(aliases).sort((a,b)=>b[0].length-a[0].length).forEach(([alias, target]) => {
      const key = _bestLabelKey(target, labelRows);
      if (key) expr = expr.replace(new RegExp('(?<![A-Za-z0-9_])' + alias + '(?![A-Za-z0-9_])', 'gi'), _cellAddr(labelRows[key], col));
    });
    Object.keys(labelRows).sort((a,b)=>b.length-a.length).forEach(k => {
      const label = k.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      expr = expr.replace(new RegExp('(?<![A-Za-z0-9_])' + label + '(?![A-Za-z0-9_])', 'gi'), _cellAddr(labelRows[k], col));
    });
    expr = expr.replace(/\bIF\s*\(/gi, 'IF(').replace(/\bABS\s*\(/gi,'ABS(').replace(/\bMAX\s*\(/gi,'MAX(').replace(/\bMIN\s*\(/gi,'MIN(').replace(/\bSUM\s*\(/gi,'SUM(');
    // If unresolved words remain beyond Excel funcs/cell refs, use stored value instead.
    const unresolved = expr.replace(/\b(IF|ABS|MAX|MIN|SUM|ROUND|AND|OR|NOT|TRUE|FALSE)\b/gi,'').replace(/\$?[A-Z]{1,3}\$?\d+/g,'').match(/[A-Za-zÁÉÍÓÚÜÑáéíóúüñ_]{2,}/);
    return unresolved ? null : expr;
  }
  function _bestLabelKey(target, labelRows){
    const k = _labelKey(target);
    if (labelRows[k] != null) return k;
    const b = _baseLabelKey(target);
    if (labelRows[b] != null) return b;
    const found = Object.keys(labelRows).find(x => x.indexOf(b) >= 0 || b.indexOf(x) >= 0);
    return found || null;
  }
  function _makeCell(v, style){
    if (v && typeof v === 'object' && (v.f || v.t || v.v !== undefined)) {
      if (style) v.s = style;
      return v;
    }
    if (typeof v === 'number') return { t:'n', v, s: style };
    return { t:'s', v: v == null ? '' : String(v), s: style };
  }
  function _isResultLabel(label, type){
    const k = _labelKey(label);
    if (type === 'pl') return /^(total income|total cogs|gross profit|profit before opex|ebit|ebitda)/.test(k);
    return /^(total inflows|total outflows|net cash flow|net cf from operations|accumulated cf|accumulated cash flow)/.test(k);
  }
  function _sectionColor(label){
    const k = _labelKey(label);
    if (/income|inflow/.test(k)) return '0B7C78';
    if (/cogs|outflow/.test(k)) return '8F0000';
    if (/opex|interest/.test(k)) return '3B2A5A';
    if (/cash|npv|bond/.test(k)) return '14364B';
    return '032D60';
  }
  function _styles(){
    const border = { top:{style:'thin',color:{rgb:'D8E0EA'}}, bottom:{style:'thin',color:{rgb:'D8E0EA'}}, left:{style:'thin',color:{rgb:'D8E0EA'}}, right:{style:'thin',color:{rgb:'D8E0EA'}} };
    return {
      title:{ font:{bold:true, sz:15, color:{rgb:'FFFFFF'}}, fill:{fgColor:{rgb:'032D60'}}, alignment:{horizontal:'left', vertical:'center'} },
      meta:{ font:{sz:10, color:{rgb:'64748B'}}, fill:{fgColor:{rgb:'F8FAFC'}}, alignment:{horizontal:'left'} },
      head:{ font:{bold:true, sz:10, color:{rgb:'FFFFFF'}}, fill:{fgColor:{rgb:'032D60'}}, alignment:{horizontal:'center', vertical:'center', wrapText:true}, border },
      label:{ font:{sz:10, color:{rgb:'334155'}}, alignment:{horizontal:'left'}, border },
      labelBold:{ font:{bold:true, sz:10, color:{rgb:'032D60'}}, fill:{fgColor:{rgb:'F0F6FC'}}, alignment:{horizontal:'left'}, border },
      num:{ font:{sz:10, color:{rgb:'334155'}}, numFmt:'#,##0.00;(#,##0.00);-', alignment:{horizontal:'right'}, border },
      numBold:{ font:{bold:true, sz:10, color:{rgb:'032D60'}}, fill:{fgColor:{rgb:'F0F6FC'}}, numFmt:'#,##0.00;[Red](#,##0.00);-', alignment:{horizontal:'right'}, border },
      totalCol:{ font:{bold:true, sz:10, color:{rgb:'032D60'}}, fill:{fgColor:{rgb:'E8F4FD'}}, numFmt:'#,##0.00;[Red](#,##0.00);-', alignment:{horizontal:'right'}, border },
      formula:{ font:{bold:true, sz:10, color:{rgb:'003B71'}}, fill:{fgColor:{rgb:'EAF4FF'}}, numFmt:'#,##0.00;[Red](#,##0.00);-', alignment:{horizontal:'right'}, border },
      info:{ font:{bold:true, sz:10, color:{rgb:'475569'}}, fill:{fgColor:{rgb:'F8FAFC'}}, alignment:{horizontal:'left'}, border }
    };
  }
  function _buildFormulaWorkbook(parsed, type, sheetTitle){
    const S = _styles();
    const colCount = Math.max(parsed.headers.length, 2);
    const lastCol = colCount - 1;
    const aoa = [];
    const merges = [];
    aoa.push([_makeCell((document.getElementById('proj-name')?.value || 'Financial Model') + ' — ' + sheetTitle, S.title)]);
    merges.push({s:{r:0,c:0}, e:{r:0,c:lastCol}});
    aoa.push([_makeCell('Exportado en Excel con fórmulas · ' + new Date().toLocaleString('es-PE') + ' · Moneda: USD', S.meta)]);
    merges.push({s:{r:1,c:0}, e:{r:1,c:lastCol}});
    aoa.push(new Array(colCount).fill(''));
    aoa.push(parsed.headers.map(h => _makeCell(h, S.head)));

    const dataStartExcelRow = 5;
    const dataRowsMeta = [];
    const labelRows = {};
    const dataColStart = 1;
    const dataColEnd = colCount - 2; // excludes Total column
    const totalCol = colCount - 1;
    let currentSection = '';
    parsed.rows.forEach(src => {
      if (src.type === 'section') {
        const r = aoa.length;
        const style = Object.assign({}, S.title, { fill:{fgColor:{rgb:_sectionColor(src.label)}} });
        aoa.push([_makeCell(src.label.toUpperCase(), style)]);
        merges.push({s:{r,c:0}, e:{r,c:lastCol}});
        currentSection = _labelKey(src.label);
        return;
      }
      if (src.type !== 'data') return;
      const excelRow = aoa.length + 1;
      const rowStyle = src.isBold || _isResultLabel(src.label, type) ? S.labelBold : S.label;
      const cells = [_makeCell(src.label, rowStyle)];
      if (src.isInfo) {
        cells.push(_makeCell(src.values[0] || '', S.info));
        for (let i=2;i<colCount;i++) cells.push(_makeCell('', S.info));
      } else {
        for (let i=0;i<colCount-1;i++) {
          const v = src.values[i] === undefined ? '' : src.values[i];
          cells.push(_makeCell(v, (src.isBold || _isResultLabel(src.label, type)) ? (i === colCount-2 ? S.totalCol : S.numBold) : (i === colCount-2 ? S.totalCol : S.num)));
        }
      }
      aoa.push(cells);
      const meta = { src, excelRow, aoaIndex: aoa.length-1, section: currentSection };
      dataRowsMeta.push(meta);
      const k = _labelKey(src.label), b = _baseLabelKey(src.label);
      if (!labelRows[k]) labelRows[k] = excelRow;
      if (!labelRows[b]) labelRows[b] = excelRow;
    });

    function rowsBetween(startExclusive, endExclusive, filter){
      return dataRowsMeta.filter(m => m.excelRow > startExclusive && m.excelRow < endExclusive && (!filter || filter(m)));
    }
    function rowsInSection(sectionKey, endRow){
      return dataRowsMeta.filter(m => m.excelRow < endRow && (_labelKey(m.section) === sectionKey || _labelKey(m.section).indexOf(sectionKey) >= 0) && !_isResultLabel(m.src.label, type) && !m.src.isInfo);
    }
    function _sumRefs(rows, c){
      if (!rows.length) return '0';
      return rows.map(m => _cellAddr(m.excelRow, c)).join('+');
    }
    function setFormula(meta, c, formula){
      const cell = aoa[meta.aoaIndex][c];
      const cur = cell && typeof cell.v === 'number' ? cell.v : 0;
      aoa[meta.aoaIndex][c] = { t:'n', f: formula, v: cur, s: c === totalCol ? S.totalCol : S.formula };
    }
    const metaByKey = {};
    dataRowsMeta.forEach(m => { metaByKey[_labelKey(m.src.label)] = m; metaByKey[_baseLabelKey(m.src.label)] = m; });
    const getMeta = (name) => { const k = _bestLabelKey(name, metaByKey); return k ? metaByKey[k] : null; };
    function _pctFromLabel(label, fallback){
      const m = String(label||'').match(/(\d+(?:\.\d+)?)\s*%/);
      const v = m ? Number(m[1]) : Number(fallback || 0);
      return isFinite(v) ? v / 100 : 0;
    }
    function _sectionHas(meta, word){ return _labelKey(meta.section || '').indexOf(word) >= 0; }
    function _isCalcLike(label){
      const k = _labelKey(label);
      return /^(total|gross profit|profit before|ebit|ebitda|contingency|opex buffer|compliance bond|upfront bond|hedging|financing cost|vat|npv|peak negative|break-even)/.test(k);
    }
    function rowsBeforeSameSection(meta, filter){
      return dataRowsMeta.filter(m => m.excelRow < meta.excelRow && _labelKey(m.section) === _labelKey(meta.section) && !m.src.isInfo && (!filter || filter(m)));
    }
    function rowsInSectionFuzzy(word, endRow, filter){
      return dataRowsMeta.filter(m => m.excelRow < endRow && _sectionHas(m, word) && !m.src.isInfo && (!_isResultLabel(m.src.label, type)) && (!filter || filter(m)));
    }
    function _sumRowRange(rowMeta, c1, c2){ return 'SUM(' + _cellAddr(rowMeta.excelRow, c1) + ':' + _cellAddr(rowMeta.excelRow, c2) + ')'; }
    function _sumAllDataCols(rows){
      if (!rows.length) return '0';
      return rows.map(m => _sumRowRange(m, dataColStart, dataColEnd)).join('+');
    }

    dataRowsMeta.forEach(meta => {
      if (meta.src.isInfo) return;
      const k = _labelKey(meta.src.label);
      for (let c=dataColStart; c<=dataColEnd; c++) {
        let f = null;
        const customF = _formulaForCustomLine(meta.src.label, c, labelRows);
        if (customF) f = customF;
        // Calculated rows exported as live Excel formulas. Scope: export only.
        if (/^contingency/.test(k)) {
          const pct = _pctFromLabel(meta.src.label, CFG.con);
          const baseRows = rowsBeforeSameSection(meta, m => !_isCalcLike(m.src.label));
          if (baseRows.length) f = '(' + _sumRefs(baseRows, c) + ')*' + pct;
        } else if (/^penalties/.test(k)) {
          const pct = Number(CFG.pen || 0) / 100;
          const baseRows = rowsBeforeSameSection(meta, m => !_isCalcLike(m.src.label));
          if (baseRows.length) f = '(' + _sumRefs(baseRows, c) + ')*-' + pct;
        } else if (/^opex buffer/.test(k)) {
          const rate = Number(CFG.opxbuf || 0) / 100;
          const base = type === 'cf' ? (getMeta('Total Inflows') || getMeta('Total Income')) : getMeta('Total Income');
          if (base) f = (type === 'pl' ? '-ABS(' : 'ABS(') + _cellAddr(base.excelRow,c) + ')*' + rate;
        } else if (/^financing cost/.test(k) && type === 'cf') {
          const rate = Number(CFG.finRate || 0) / 100;
          const acc = getMeta('Accumulated CF') || getMeta('Accumulated Cash Flow');
          if (acc) f = c === dataColStart ? '0' : 'IF(' + _cellAddr(acc.excelRow,c-1) + '<0,ABS(' + _cellAddr(acc.excelRow,c-1) + ')*' + rate + ',0)';
        } else if (/^compliance bond fee/.test(k) && dataColEnd >= dataColStart) {
          const rate = (Number(CFG.fcbCost || 0) / 100) * (1 + (typeof IGV !== 'undefined' ? IGV : 0.18)) * (Number(CFG.fcb || 0) / 100);
          const revRows = dataRowsMeta.filter(m => _sectionHas(m, type === 'cf' ? 'inflow' : 'income') && !_isCalcLike(m.src.label) && !m.src.isInfo);
          if (revRows.length && rate) f = (type === 'pl' ? '-' : '') + '(' + _sumAllDataCols(revRows) + ')*' + rate;
        } else if (/^upfront bond fee/.test(k) && dataColEnd >= dataColStart) {
          const rate = Number(CFG.fub || 0) / 100;
          const periods = Math.max(1, dataColEnd - dataColStart + 1);
          const revRows = dataRowsMeta.filter(m => _sectionHas(m, type === 'cf' ? 'inflow' : 'income') && !_isCalcLike(m.src.label) && !m.src.isInfo);
          if (revRows.length && rate) f = (type === 'pl' ? '-' : '') + '((' + _sumAllDataCols(revRows) + ')*' + rate + ')/' + periods;
        }
        if (type === 'pl') {
          const totalIncome = getMeta('Total Income');
          const totalCogs = getMeta('Total COGS');
          const gp = getMeta('Gross Profit');
          const pbo = getMeta('Profit before OPEX / Interest');
          const ebit = getMeta('EBIT');
          if (/^total income$/.test(k)) f = _sumRefs(rowsInSection('income', meta.excelRow), c);
          else if (/^total cogs$/.test(k)) f = _sumRefs(rowsInSection('cogs', meta.excelRow), c);
          else if (/^gross profit/.test(k) && totalIncome && totalCogs) f = _cellAddr(totalIncome.excelRow,c) + '+' + _cellAddr(totalCogs.excelRow,c);
          else if (/^profit before opex/.test(k) && gp) f = _cellAddr(gp.excelRow,c) + '+' + _sumRefs(rowsBetween(gp.excelRow, meta.excelRow, m=>!m.src.isInfo), c);
          else if (/^ebit$/.test(k) && pbo) f = _cellAddr(pbo.excelRow,c) + '+' + _sumRefs(rowsBetween(pbo.excelRow, meta.excelRow, m=>!m.src.isInfo), c);
          else if (/^ebitda$/.test(k) && ebit) {
            const mids = rowsBetween(ebit.excelRow, meta.excelRow, m=>!m.src.isInfo);
            f = _cellAddr(ebit.excelRow,c) + (mids.length ? '+' + _sumRefs(mids, c) : '');
          }
        } else {
          const ti = getMeta('Total Inflows');
          const to = getMeta('Total Outflows');
          const ncf = getMeta('Net Cash Flow') || getMeta('NET CF from Operations');
          const accPrev = dataRowsMeta.find(m => m.excelRow === meta.excelRow - 1 && /^accumulated/.test(_labelKey(meta.src.label)));
          if (/^total inflows$/.test(k)) f = _sumRefs(rowsInSection('inflows', meta.excelRow), c);
          else if (/^total outflows$/.test(k)) f = _sumRefs(rowsInSection('outflows', meta.excelRow), c);
          else if (/^(net cash flow|net cf from operations)$/.test(k) && ti && to) f = _cellAddr(ti.excelRow,c) + '-' + _cellAddr(to.excelRow,c);
          else if (/^accumulated/.test(k) && ncf) {
            if (c === dataColStart) f = _cellAddr(ncf.excelRow,c);
            else f = _cellAddr(meta.excelRow,c-1) + '+' + _cellAddr(ncf.excelRow,c);
          }
        }
        if (f) setFormula(meta, c, f);
      }
      // Total column formula
      const totalCell = aoa[meta.aoaIndex][totalCol];
      if (totalCell && !meta.src.isInfo) {
        let ft = null;
        if (/^accumulated/.test(k)) ft = _cellAddr(meta.excelRow, Math.max(dataColStart, dataColEnd));
        else if (dataColEnd >= dataColStart) ft = 'SUM(' + _cellAddr(meta.excelRow,dataColStart) + ':' + _cellAddr(meta.excelRow,dataColEnd) + ')';
        if (ft) setFormula(meta, totalCol, ft);
      }
    });

    const ws = XLSX.utils.aoa_to_sheet(aoa);
    ws['!merges'] = merges;
    ws['!cols'] = [{wch:36}].concat(Array.from({length:colCount-2},()=>({wch:14})), [{wch:15}]);
    ws['!rows'] = aoa.map((_,i) => ({ hpt: i===0 ? 24 : i===3 ? 28 : 18 }));
    ws['!views'] = [{ state:'frozen', xSplit:1, ySplit:4 }];
    ws['!autofilter'] = { ref: 'A4:' + _cellAddr(Math.max(4, aoa.length), lastCol) };
    ws['!ref'] = 'A1:' + _cellAddr(aoa.length, lastCol);
    return ws;
  }
  function _buildSummaryFormulaSheet(type, mainSheetName, parsed){
    const S = _styles();
    const rows = [];
    rows.push([_makeCell('Executive summary — formulas linked to ' + mainSheetName, S.title), '', '', '']);
    rows.push([_makeCell('This sheet references the exported statement, so totals update if source rows change.', S.meta), '', '', '']);
    rows.push(['']);
    rows.push(['Metric','Formula / Value','Interpretation','']);
    const labelRows = {};
    let excelRow = 5;
    parsed.rows.forEach(r => { if (r.type === 'data') { const k=_labelKey(r.label); if(!labelRows[k]) labelRows[k]=excelRow; const b=_baseLabelKey(r.label); if(!labelRows[b]) labelRows[b]=excelRow; excelRow++; } else if (r.type === 'section') excelRow++; });
    const ref = label => {
      const k = _bestLabelKey(label, labelRows);
      return k ? "'" + mainSheetName.replace(/'/g,"''") + "'!" + _cellAddr(labelRows[k], Math.max(1, parsed.headers.length-1)) : null;
    };
    const totalIncome = ref('Total Income');
    if (type === 'pl') {
      const gp = ref('Gross Profit'), ebit = ref('EBIT'), ebitda = ref('EBITDA');
      rows.push(['Total Income', totalIncome ? {t:'n', f:totalIncome.replace(/^=/,''), v:0} : '', '', '']);
      rows.push(['Gross Profit', gp ? {t:'n', f:gp.replace(/^=/,''), v:0} : '', totalIncome && gp ? {t:'n', f:gp + '/' + totalIncome, v:0, z:'0.00%'} : '', '']);
      rows.push(['EBITDA', ebitda ? {t:'n', f:ebitda.replace(/^=/,''), v:0} : '', totalIncome && ebitda ? {t:'n', f:ebitda + '/' + totalIncome, v:0, z:'0.00%'} : '', '']);
      rows.push(['EBIT', ebit ? {t:'n', f:ebit.replace(/^=/,''), v:0} : '', totalIncome && ebit ? {t:'n', f:ebit + '/' + totalIncome, v:0, z:'0.00%'} : '', '']);
    } else {
      const ncf = ref('Net Cash Flow') || ref('NET CF from Operations'), acc = ref('Accumulated CF');
      rows.push(['Net Cash Flow', ncf ? {t:'n', f:ncf.replace(/^=/,''), v:0} : '', '', '']);
      rows.push(['Final Accumulated CF', acc ? {t:'n', f:acc.replace(/^=/,''), v:0} : '', '', '']);
    }
    const ws = XLSX.utils.aoa_to_sheet(rows.map(r => r.map((v,i)=>_makeCell(v, i===0 ? S.labelBold : S.num))));
    ws['!merges'] = [{s:{r:0,c:0},e:{r:0,c:3}}, {s:{r:1,c:0},e:{r:1,c:3}}];
    ws['!cols'] = [{wch:28},{wch:18},{wch:18},{wch:10}];
    return ws;
  }
  window.exportTableXLSX = async function(tblId, sheetName, filename){
    try {
      const ready = await _ensureXLSXExportLib();
      if (!ready) { alert('No se pudo cargar la librería de Excel. Revisa tu conexión e inténtalo nuevamente.'); return; }
      const type = tblId === 'cf-tbl' ? 'cf' : 'pl';
      const parsed = _extractVisibleTable(tblId);
      const cleanSheet = (sheetName || (type === 'pl' ? 'P&L' : 'Cash Flow')).replace(/[\\/?*\[\]:]/g,' ').slice(0,31) || 'Statement';
      const ws = _buildFormulaWorkbook(parsed, type, cleanSheet);
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, cleanSheet);
      XLSX.utils.book_append_sheet(wb, _buildSummaryFormulaSheet(type, cleanSheet, parsed), 'Executive Summary');
      wb.Workbook = wb.Workbook || {};
      wb.Workbook.CalcPr = { fullCalcOnLoad: true };
      XLSX.writeFile(wb, filename || ((document.getElementById('proj-name')?.value || 'model').replace(/\s+/g,'_') + '_' + cleanSheet + '.xlsx'), { cellStyles:true, bookType:'xlsx' });
    } catch(err) {
      console.error('Excel formula export error:', err);
      alert('No se pudo exportar el Excel: ' + (err.message || err));
    }
  };
})();

/* =====================================================================
   FINAL UI PATCH — Scrollable/expandable charts + cost detail rows in P&L/CF + Excel
   Scope: presentation/export only. Does not modify financial logic or compute formulas.
   ===================================================================== */
(function(){
  if (window.__scrollChartsCostBreakdownExcelPatch) return;
  window.__scrollChartsCostBreakdownExcelPatch = true;

  const _arr0 = () => new Array(PM()).fill(0);
  const _clone = a => (a || _arr0()).map(v => Number(v) || 0);
  const _neg = a => _clone(a).map(v => -v);
  const _sum = a => (a || []).reduce((s,v)=>s+(Number(v)||0),0);
  const _normCat = it => ((it && it.cat) || 'Other').trim() || 'Other';
  const _escHtml = s => String(s == null ? '' : s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;');
  const _plain = s => String(s == null ? '' : s).replace(/<[^>]*>/g,'').replace(/&nbsp;/g,' ').trim();

  function _customRowsFor(cv, section, opts={}) {
    const groups = cv && cv._customLineGroups;
    const all = groups && Array.isArray(groups.all) ? groups.all : [];
    return all.filter(x => x && x.line && x.line.section === section && (!opts.cfOnly || x.line.showInCF));
  }

  function _rowHtml(label, arr, view, opts={}) {
    const d = opts.acc ? aggAccY(arr) : (view === 'annual' ? aggY(arr) : arr);
    const tot = opts.acc ? (arr && arr.length ? arr[arr.length - 1] : 0) : _sum(arr);
    const cc = opts.color || '';
    const cls = [opts.bold ? 'bold-row' : '', opts.detail ? 'cost-detail-row' : '', opts.category ? 'cost-category-row' : ''].filter(Boolean).join(' ');
    const labelStyle = opts.detail ? 'padding-left:44px;font-size:11px;color:#64748b;background:#fbfdff' : '';
    const rowStyle = opts.detail ? 'background:#fbfdff' : '';
    return '<tr class="' + cls + '"' + (opts.category ? ' data-cost-category="1"' : '') + (rowStyle ? ' style="'+rowStyle+'"' : '') + '>'
      + '<td class="lbl' + (opts.ind || opts.detail ? ' ind' : '') + (opts.bold ? ' bold' : '') + '"' + (labelStyle ? ' style="'+labelStyle+'"' : '') + '>' + label + '</td>'
      + d.map(v => '<td class="num' + (opts.bold ? ' bold' : '') + (v < 0 ? ' c-red' : '') + (cc ? ' c-' + cc : '') + '"' + (opts.detail ? ' style="font-size:11px;background:#fbfdff"' : '') + '>' + fmt(v) + '</td>').join('')
      + '<td class="num tot' + (opts.bold ? ' bold' : '') + (tot < 0 ? ' c-red' : '') + (cc ? ' c-' + cc : '') + '"' + (opts.detail ? ' style="font-size:11px;background:#fbfdff"' : '') + '>' + fmt(tot) + '</td>'
      + '</tr>';
  }

  function _costBreakdownRows(cv, view, mode) {
    const isPL = mode === 'pl';
    const source = isPL ? (cv.costCat || {}) : (cv.costCatCF || {});
    const items = [...(capex || []), ...(opex || [])];
    let html = '';
    (cv.userCats || []).forEach(cat => {
      const catArr = source[cat] || _arr0();
      html += _rowHtml(cat, isPL ? _neg(catArr) : catArr, view, { ind:true, category:true });
      items.filter(it => _normCat(it) === cat).forEach(it => {
        let arr = _arr0();
        try { arr = isPL ? _neg(getDistArr(it)) : getCFDistArr(it); } catch(e) { arr = _arr0(); }
        const currency = it.cur ? ' · ' + it.cur : '';
        const igv = it.igv ? ' · IGV' : '';
        const label = '↳ ' + _escHtml(it.name || 'Unnamed cost') + '<span class="cost-detail-meta">' + _escHtml(currency + igv) + '</span>';
        html += _rowHtml(label, arr, view, { detail:true });
      });
    });
    return html;
  }

  function _customLineRow(x, cv, view, opts={}) {
    const btn = x.line ? `<button onclick="openEditLineModal(customLines.find(l=>String(l.id)==='${String(x.line.id)}'))" title="Edit formula" style="margin-left:8px;font-size:9px;padding:1px 6px;border:1px solid var(--bdr2);border-radius:3px;background:var(--alt);color:var(--accent);cursor:pointer;vertical-align:middle">✎</button>` : '';
    const note = x.line && x.line.note ? ` <span style="font-size:9px;opacity:.6">(${_escHtml(x.line.note)})</span>` : '';
    return _rowHtml(_escHtml((x.line && x.line.name) || 'Custom') + btn + note, x.arr || _arr0(), view, Object.assign({ind:true, color:(x.line && x.line.color) || 'amber'}, opts));
  }

  function _sumRow(label, arr, view, color, pctArr) {
    const data = view === 'annual' ? aggY(arr) : arr;
    const tot = _sum(arr);
    const badge = pctArr ? pctBadge(arr, pctArr) : '';
    return `<tr class="bold-row"><td class="lbl bold" style="${color?'color:'+color:''}">${label}${badge}</td>`
      + data.map(v=>`<td class="num bold${v<0?' c-red':''}" style="${color?'color:'+color:''}">${fmt(v)}</td>`).join('')
      + `<td class="num tot bold${tot<0?' c-red':''}" style="${color?'color:'+color:''}">${fmt(tot)}</td></tr>`;
  }

  function _renderFlatPLBreakdown(){
    const wrap = document.getElementById('pl-view-tog');
    if (wrap) { wrap.innerHTML=''; const tog=buildToggle(plView, v=>{ plView=v; renderPL(); }); if (tog) wrap.appendChild(tog); }
    const cv = compute();
    if (plView === 'grouped') { _renderGroupedBreakdown('pl', cv); return; }
    const view = plView;
    const cols = view === 'annual' ? yrLabels() : AMs().map(m=>mInfo(m).label);
    const mNums = view === 'annual' ? null : AMs();
    const cw = view === 'annual' ? 110 : 82;
    ...
    h += secRow('INCOME','var(--th-income)');
    Object.entries(cv.revCats || {}).forEach(([k,a]) => h += _rowHtml(_escHtml(k), a, view, {ind:true}));
    h += _rowHtml('Penalties', cv.penArr || _arr0(), view, {ind:true,color:'amber'});
    (cv.adjIncomeRows || []).forEach(ac => h += _rowHtml(_escHtml((ac.adj.name||'Adj')+' ('+(ac.adj.pct||0)+'%)'), (ac.arr||_arr0()).map(v=>ac.adj.sign==='add'?v:-v), view, {ind:true,color:'amber'}));
    _customRowsFor(cv,'income_section').forEach(x => h += _customLineRow(x, cv, view));
    h += addLineRow('income_section', nc);
    h += _sumRow('Total Income', cv.totalIncome || _arr0(), view, 'var(--th-income)');

    h += secRow('COGS','var(--th-cogs)');
    h += _costBreakdownRows(cv, view, 'pl');
    h += _rowHtml('Contingency ('+CFG.con+'%)', _neg((cv.costCat||{}).Contingency || _arr0()), view, {ind:true,color:'amber'});
    (cv.adjCostRows || []).forEach(ac => h += _rowHtml(_escHtml((ac.adj.name||'Adj')+' ('+(ac.adj.pct||0)+'%)'), (ac.arr||_arr0()).map(v=>-(ac.adj.sign==='add'?v:-v)), view, {ind:true,color:'amber'}));
    _customRowsFor(cv,'cogs_section').forEach(x => h += _customLineRow(x, cv, view));
    h += addLineRow('cogs_section', nc);
    h += _sumRow('Total COGS', _neg(cv.totalCogsAdj || _arr0()), view, 'var(--th-cogs)');
    h += _sumRow('Gross Profit', cv.grossProfit || _arr0(), view, 'var(--accent)', cv.totalIncome || _arr0());
    _customRowsFor(cv,'after_gross_profit').forEach(x => h += _customLineRow(x, cv, view));
    h += addLineRow('after_gross_profit', nc);

    h += secRow('BOND FEES','var(--th-bond)');
    h += _rowHtml('Compliance Bond Fee', _neg(cv.finExpCB || _arr0()), view, {ind:true,color:'amber'});
    h += _rowHtml('Upfront Bond Fee', _neg(cv.finExpUB || _arr0()), view, {ind:true,color:'amber'});
    h += _sumRow('Profit before OPEX / Interest', cv.profitBeforeOpex || _arr0(), view, 'var(--accent)', cv.totalIncome || _arr0());
    _customRowsFor(cv,'after_profit_b_opex').forEach(x => h += _customLineRow(x, cv, view));
    h += addLineRow('after_profit_b_opex', nc);

    h += secRow('OPEX & INTEREST','var(--th-opex)');
    h += _rowHtml('OPEX Buffer ('+(CFG.opxbuf??30)+'% of Revenue)', _neg(cv.opexBuffer || _arr0()), view, {ind:true,color:'amber'});
    h += _rowHtml('Hedging Adjustment (monthly)', _neg(cv.hedgingArr || _arr0()), view, {ind:true});
    h += _rowHtml('Financing Cost (on neg. CF)', _neg(cv.finCostArr || _arr0()), view, {ind:true,color:'amber'});
    h += _sumRow('EBIT', cv.ebit || _arr0(), view, 'var(--accent)', cv.totalIncome || _arr0());
    _customRowsFor(cv,'after_ebit').forEach(x => h += _customLineRow(x, cv, view));
    h += addLineRow('after_ebit', nc);
    h += _sumRow('EBITDA', cv.ebitda || _arr0(), view, 'var(--teal)', cv.totalIncome || _arr0());
    h += '</tbody>';
    const tbl=document.getElementById('pl-tbl'); if (tbl) tbl.innerHTML = h;
  }

  function _renderFlatCFBreakdown(){
    const wrap = document.getElementById('cf-view-tog');
    if (wrap) { wrap.innerHTML=''; const tog=buildToggle(cfView, v=>{ cfView=v; renderCF(); }); if (tog) wrap.appendChild(tog); }
    const cv = compute();
    if (cfView === 'grouped') { _renderGroupedBreakdown('cf', cv); return; }
    const view = cfView;
    const cols = view === 'annual' ? yrLabels() : AMs().map(m=>mInfo(m).label);
    const mNums = view === 'annual' ? null : AMs();
    const cw = view === 'annual' ? 110 : 82;
    ...
    let h = tHead(cols,cw,mNums) + '<tbody>';
    h += secRow('INFLOWS','var(--th-cf-in)');
    Object.entries(cv.revCatsCF || {}).forEach(([k,a]) => h += _rowHtml(_escHtml(k), a, view, {ind:true}));
    h += _rowHtml('Penalties', cv.penArrCF || _arr0(), view, {ind:true,color:'amber'});
    h += _rowHtml('VAT Inflow (IGV)', cv.vatIn || _arr0(), view, {ind:true,color:'teal'});
    _customRowsFor(cv,'income_section',{cfOnly:true}).forEach(x => h += _customLineRow(x, cv, view));
    _customRowsFor(cv,'cf_after_inflows').forEach(x => h += _customLineRow(x, cv, view));
    h += addLineRow('cf_after_inflows', nc);
    h += _rowHtml('Total Inflows', cv.totalInflowsVisible || cv.totalIncomeCF || _arr0(), view, {bold:true,color:'teal'});

    h += secRow('OUTFLOWS','var(--th-cf-out)');
    h += _costBreakdownRows(cv, view, 'cf');
    h += _rowHtml('Contingency ('+CFG.con+'%)', (cv.costCatCF||{}).Contingency || _arr0(), view, {ind:true,color:'amber'});
    _customRowsFor(cv,'cogs_section',{cfOnly:true}).forEach(x => h += _customLineRow(x, cv, view));
    h += _rowHtml('OPEX Buffer ('+(CFG.opxbuf??30)+'%)', cv.opexBufferCF || _arr0(), view, {ind:true,color:'amber'});
    h += _rowHtml('Compliance Bond Fee', cv.finExpCB || _arr0(), view, {ind:true,color:'amber'});
    h += _rowHtml('Upfront Bond Fee', cv.finExpUB || _arr0(), view, {ind:true,color:'amber'});
    h += _rowHtml('Hedging (monthly)', cv.hedgingArr || _arr0(), view, {ind:true});
    h += _rowHtml('Financing Cost', cv.finCostArr || _arr0(), view, {ind:true,color:'amber'});
    h += _rowHtml('VAT Outflow', cv.vatOut || _arr0(), view, {ind:true,color:'red'});
    _customRowsFor(cv,'cf_after_outflows').forEach(x => h += _customLineRow(x, cv, view));
    h += addLineRow('cf_after_outflows', nc);
    h += _rowHtml('Total Outflows', cv.totalOutflowsVisible || _arr0(), view, {bold:true,color:'red'});

    h += secRow('NET CASH FLOW','var(--th-bond)');
    _customRowsFor(cv,'after_gross_profit',{cfOnly:true}).forEach(x => h += _customLineRow(x, cv, view));
    _customRowsFor(cv,'after_profit_b_opex',{cfOnly:true}).forEach(x => h += _customLineRow(x, cv, view));
    h += _rowHtml('Net Cash Flow', cv.netCF || _arr0(), view, {bold:true,color:'blue'});
    h += _rowHtml('Accumulated CF', cv.accumCF || _arr0(), view, {bold:true,color:'purple', acc:true});
    h += secRow('NPV ANALYSIS','var(--th-bond)');
    h += infoRow('Discount Rate', CFG.dis + '% p.a.  ' + ((cv.mRate||0)*100).toFixed(3) + '% /mo', 'var(--sub)', nc);
    h += infoRow('NPV (Net CF)', 'USD ' + fmt(cv.npvCF), cv.npvCF >= 0 ? 'var(--green)' : 'var(--red)', nc);
    const peakNeg = Math.min(...(cv.accumCF || [0]), 0);
  ...
  function _renderGroupedBreakdown(which, cv){
    const tbl = document.getElementById(which === 'pl' ? 'pl-tbl' : 'cf-tbl');
    if (!tbl) return;
    const yearOpen = which === 'pl' ? plYearOpen : cfYearOpen;
    const years = getYearRanges();
    ...
    years.forEach(({yi,year,yms}) => { if (yearOpen[yi]) yms.forEach(m => cols.push({type:'month', yi, year, m, yms})); cols.push({type:'year', yi, year, yms, open:!!yearOpen[yi]}); });
    cols.push({type:'total'});
    const cellVal = (arr,col) => col.type === 'month' ? (arr[col.m-1] || 0) : col.type === 'year' ? col.yms.reduce((s,m)=>s+(arr[m-1]||0),0) : _sum(arr || []);
    let head = '<thead><tr><th style="min-width:240px;text-align:left;position:sticky;left:0;z-index:3;background:var(--navy)">Category</th>';
    cols.forEach(col => {
      if (col.type === 'month') { const mi=mInfo(col.m); head += `<th style="min-width:76px;background:var(--navy)"><div style="font-size:10px">${mi.short} ${String(mi.year).slice(2)}</div><div style="color:#5ba4f5;font-size:9px;font-weight:700">(M${col.m})</div></th>`; }
      else if (col.type === 'year') head += `<th class="yr-col-hdr" data-yi="${col.yi}" style="min-width:90px;background:#1a4a80;cursor:pointer;user-select:none"><div style="font-size:11px;font-weight:800">${col.year}</div><div style="font-size:8px;opacity:.75;margin-top:2px">${col.open?'collapse':'expand'}</div></th>`;
      else head += '<th style="min-width:90px;background:#0a2a50"><div style="font-size:10px">Total</div></th>';
    ...
    head += '</tr></thead>';
    const row = (label, arr, opts={}) => {
      const bold=!!opts.bold, ind=!!opts.ind, col=opts.color||'', detail=!!opts.detail, cat=!!opts.category;
      const bg = detail ? '#fbfdff' : (bold ? 'var(--alt)' : 'var(--surface)');
      let h = `<tr class="${bold?'bold-row ':''}${detail?'cost-detail-row ':''}${cat?'cost-category-row':''}"${cat?' data-cost-category="1"':''}><td class="lbl${ind||detail?' ind':''}${bold?' bold':''}" style="position:sticky;left:0;z-index:1;background:${bg};${detail?'padding-left:44px;font-size:11px;color:#64748b':''}">${label}</td>`;
      cols.forEach(c => { const v=cellVal(arr||_arr0(), c); const special=c.type==='year'?'background:#f0f4f8;font-weight:700;':c.type==='total'?'background:var(--alt);':''; h += `<td class="num${bold?' bold':''}${v<0?' c-red':''}${col?' c-'+col:''}" style="${special}${detail?'font-size:11px;background:#fbfdff;':''}">${fmt(v)}</td>`; });
      return h + '</tr>';
    };
    const sec = (txt,color) => `<tr class="sec-row"><td colspan="999" style="background:${color}">${txt}</td></tr>`;
    const custom = (section, opts={}) => _customRowsFor(cv, section, opts).map(x => _customLineRow(x, cv, 'monthly', opts).replace(/<td class="num[^>]*>[^<]*<\/td>/g, function(){ return ''; })).join('');
    // For grouped custom rows, use grouped row builder instead of flat HTML.
    const customG = (section, opts={}) => _customRowsFor(cv, section, opts).map(x => {
      const btn = `<button onclick="openEditLineModal(customLines.find(l=>String(l.id)==='${String(x.line.id)}'))" title="Edit formula" style="margin-left:8px;font-size:9px;padding:1px 6px;border:1px solid var(--bdr2);border-radius:3px;background:var(--alt);color:var(--accent);cursor:pointer;vertical-align:middle">✎</button>`;
      return row(_escHtml(x.line.name || 'Custom') + btn, x.arr || _arr0(), {ind:true,color:x.line.color||'amber'});
    }).join('');
    const costRowsG = (mode) => {
      const isPL = mode === 'pl';
      const source = isPL ? (cv.costCat||{}) : (cv.costCatCF||{});
      const items = [...(capex||[]),...(opex||[])];
      let out='';
      (cv.userCats||[]).forEach(cat=>{
        out += row(_escHtml(cat), isPL ? _neg(source[cat]||_arr0()) : (source[cat]||_arr0()), {ind:true,category:true});
        items.filter(it=>_normCat(it)===cat).forEach(it=>{
          const a = isPL ? _neg(getDistArr(it)) : getCFDistArr(it);
          out += row('↳ ' + _escHtml(it.name || 'Unnamed cost') + '<span class="cost-detail-meta">' + _escHtml((it.cur?' · '+it.cur:'') + (it.igv?' · IGV':'')) + '</span>', a, {detail:true});
        });
      });
      return out;
    };
    ...
    if (which === 'pl') {
      body += sec('INCOME','var(--th-income)');
      Object.entries(cv.revCats || {}).forEach(([k,a]) => body += row(_escHtml(k), a, {ind:true}));
      body += row('Penalties', cv.penArr || _arr0(), {ind:true,color:'amber'});
      (cv.adjIncomeRows || []).forEach(ac => body += row(_escHtml((ac.adj.name||'Adj')+' ('+(ac.adj.pct||0)+'%)'), (ac.arr||_arr0()).map(v=>ac.adj.sign==='add'?v:-v), {ind:true,color:'amber'}));
      body += customG('income_section');
      body += row('Total Income', cv.totalIncome || _arr0(), {bold:true,color:'teal'});
      body += sec('COGS','var(--th-cogs)');
      body += costRowsG('pl');
      body += row('Contingency ('+CFG.con+'%)', _neg((cv.costCat||{}).Contingency || _arr0()), {ind:true,color:'amber'});
      (cv.adjCostRows || []).forEach(ac => body += row(_escHtml((ac.adj.name||'Adj')+' ('+(ac.adj.pct||0)+'%)'), (ac.arr||_arr0()).map(v=>-(ac.adj.sign==='add'?v:-v)), {ind:true,color:'amber'}));
      body += customG('cogs_section');
      body += row('Total COGS', _neg(cv.totalCogsAdj || _arr0()), {bold:true,color:'red'});
      body += row('Gross Profit', cv.grossProfit || _arr0(), {bold:true,color:'blue'});
      body += customG('after_gross_profit');
      body += sec('BOND FEES','var(--th-bond)');
      body += row('Compliance Bond Fee', _neg(cv.finExpCB || _arr0()), {ind:true,color:'amber'});
      body += row('Upfront Bond Fee', _neg(cv.finExpUB || _arr0()), {ind:true,color:'amber'});
      body += row('Profit before OPEX / Interest', cv.profitBeforeOpex || _arr0(), {bold:true,color:'blue'});
      body += customG('after_profit_b_opex');
      body += sec('OPEX & INTEREST','var(--th-opex)');
      body += row('OPEX Buffer ('+(CFG.opxbuf??30)+'% of Revenue)', _neg(cv.opexBuffer || _arr0()), {ind:true,color:'amber'});
      body += row('Hedging Adjustment (monthly)', _neg(cv.hedgingArr || _arr0()), {ind:true});
      body += row('Financing Cost (on neg. CF)', _neg(cv.finCostArr || _arr0()), {ind:true,color:'amber'});
      body += row('EBIT', cv.ebit || _arr0(), {bold:true,color:'blue'});
      body += customG('after_ebit');
      body += row('EBITDA', cv.ebitda || _arr0(), {bold:true,color:'teal'});
    } else {
      body += sec('INFLOWS','var(--th-cf-in)');
      Object.entries(cv.revCatsCF || {}).forEach(([k,a]) => body += row(_escHtml(k), a, {ind:true}));
      body += row('Penalties', cv.penArrCF || _arr0(), {ind:true,color:'amber'});
      body += row('VAT Inflow (IGV)', cv.vatIn || _arr0(), {ind:true,color:'teal'});
      body += customG('income_section',{cfOnly:true}) + customG('cf_after_inflows');
      body += row('Total Inflows', cv.totalInflowsVisible || cv.totalIncomeCF || _arr0(), {bold:true,color:'teal'});
      body += sec('OUTFLOWS','var(--th-cf-out)');
      body += costRowsG('cf');
      body += row('Contingency ('+CFG.con+'%)', (cv.costCatCF||{}).Contingency || _arr0(), {ind:true,color:'amber'});
      body += customG('cogs_section',{cfOnly:true});
      body += row('OPEX Buffer ('+(CFG.opxbuf??30)+'%)', cv.opexBufferCF || _arr0(), {ind:true,color:'amber'});
      body += row('Compliance Bond Fee', cv.finExpCB || _arr0(), {ind:true,color:'amber'});
      body += row('Upfront Bond Fee', cv.finExpUB || _arr0(), {ind:true,color:'amber'});
      body += row('Hedging', cv.hedgingArr || _arr0(), {ind:true});
      body += row('Financing Cost', cv.finCostArr || _arr0(), {ind:true,color:'amber'});
      body += row('VAT Outflow', cv.vatOut || _arr0(), {ind:true,color:'red'});
      body += customG('cf_after_outflows');
      body += row('Total Outflows', cv.totalOutflowsVisible || _arr0(), {bold:true,color:'red'});
      body += sec('NET CASH FLOW','var(--th-bond)');
      body += customG('after_gross_profit',{cfOnly:true}) + customG('after_profit_b_opex',{cfOnly:true});
      body += row('Net Cash Flow', cv.netCF || _arr0(), {bold:true,color:'blue'});
      body += row('Accumulated CF', cv.accumCF || _arr0(), {bold:true,color:'purple'});
    }
    tbl.innerHTML = head + '<tbody>' + body + '</tbody>';
    tbl.querySelectorAll('.yr-col-hdr').forEach(th => th.addEventListener('click', () => {
      const yi = parseInt(th.dataset.yi,10);
  ...
  renderPL = window.renderPL = _renderFlatPLBreakdown;
  renderCF = window.renderCF = _renderFlatCFBreakdown;
  renderGroupedPLCF = window.renderGroupedPLCF = _renderGroupedBreakdown;

  const _oldRenderDashboard = renderDashboard;
  function _chartMinWidth(){
    const months = PM();
    if (chartView === 'annual') return Math.max(680, Math.ceil(PM()/12) * 160 + 240);
    if (chartView === 'grouped') {
      const years = typeof getYearRanges === 'function' ? getYearRanges() : [];
      const yr = years.find(x=>String(x.year) === String(window.chartSelectedYear || chartSelectedYear)) || years[0];
      return Math.max(760, ((yr && yr.yms ? yr.yms.length : Math.min(12, months)) * 92) + 180);
    }
    return Math.max(860, months * 72 + 180);
  }
  function _ensureScrollableCharts(){
    ['chart-rc','chart-margins','chart-ncf','chart-accum','chart-waterfall'].forEach(id=>{
      const canvas = document.getElementById(id);
      if (!canvas) return;
      const outer = canvas.closest('.chart-canvas-wrap') || canvas.parentElement;
      if (!outer) return;
      let inner = canvas.parentElement;
      if (!inner.classList || !inner.classList.contains('chart-scroll-inner')) {
        const scroll = document.createElement('div');
        scroll.className = 'chart-scrollbox';
        const newInner = document.createElement('div');
        newInner.className = 'chart-scroll-inner';
        outer.insertBefore(scroll, canvas);
        scroll.appendChild(newInner);
        newInner.appendChild(canvas);
        inner = newInner;
      }
      const wide = id === 'chart-waterfall' ? Math.max(760, outer.clientWidth || 760) : _chartMinWidth();
  ...
  function _ensureExpandModal(){
    ...
    modal.innerHTML = `<div class="chart-expand-shell"><div class="chart-expand-head"><strong id="chart-expand-title">Chart</strong><button id="chart-expand-close">×</button></div><div class="chart-expand-body"><img id="chart-expand-img" alt="Expanded chart"></div></div>`;
    document.body.appendChild(modal);
    modal.querySelector('#chart-expand-close').onclick = () => modal.classList.remove('show');
  ...
  function _addExpandButtons(){
    ['chart-rc','chart-margins','chart-ncf','chart-accum','chart-waterfall'].forEach(id=>{
      ...
      if (!card || card.querySelector('.chart-expand-btn[data-chart="'+id+'"]')) return;
      const title = card.querySelector('[id$="chart-title"]') || card.querySelector('div[style*="font-weight:700"]') || card.firstElementChild;
      const btn = document.createElement('button');
      btn.className = 'chart-expand-btn';
      btn.dataset.chart = id;
      btn.textContent = 'Expandir';
      btn.onclick = () => {
        const modal = _ensureExpandModal();
        const img = modal.querySelector('#chart-expand-img');
        const t = modal.querySelector('#chart-expand-title');
        try { img.src = canvas.toDataURL('image/png'); } catch(e) { img.removeAttribute('src'); }
        const cssW = Math.max(canvas.width / (window.devicePixelRatio || 1), 900);
        img.style.width = cssW + 'px';
        t.textContent = _plain((title && title.textContent) || id);
        modal.classList.add('show');
      };
      if (title && title.parentElement) {
        title.parentElement.style.display = 'flex';
        title.parentElement.style.justifyContent = 'space-between';
  ...
  renderDashboard = window.renderDashboard = function(){
    _ensureScrollableCharts();
    _oldRenderDashboard();
    _ensureScrollableCharts();
    _addExpandButtons();
  };

  function _injectPatchStyles(){
    if (document.getElementById('scroll-cost-breakdown-styles')) return;
    const st = document.createElement('style');
    st.id = 'scroll-cost-breakdown-styles';
    st.textContent = `
      .chart-scrollbox{width:100%;height:100%;overflow-x:auto;overflow-y:hidden;padding-bottom:6px;scrollbar-width:thin;scrollbar-color:#94a3b8 #eef2f7}
      .chart-scrollbox::-webkit-scrollbar{height:8px}.chart-scrollbox::-webkit-scrollbar-track{background:#eef2f7;border-radius:999px}.chart-scrollbox::-webkit-scrollbar-thumb{background:#94a3b8;border-radius:999px}
      .chart-scroll-inner{height:100%;min-height:inherit}.chart-scroll-inner canvas{max-width:none!important}
      .chart-expand-btn{border:1px solid #d8e0ea;background:#fff;color:#032D60;border-radius:999px;padding:4px 10px;font-size:10px;font-weight:800;cursor:pointer;box-shadow:0 4px 12px rgba(15,35,56,.08)}
      .chart-expand-btn:hover{border-color:#0176D3;color:#0176D3;background:#f8fbff}
      #chart-expand-modal{position:fixed;inset:0;z-index:99999;background:rgba(15,23,42,.72);display:none;align-items:center;justify-content:center;padding:22px;backdrop-filter:blur(3px)}
      #chart-expand-modal.show{display:flex}.chart-expand-shell{width:min(1200px,96vw);height:min(720px,92vh);background:#fff;border-radius:18px;box-shadow:0 28px 90px rgba(0,0,0,.35);overflow:hidden;display:flex;flex-direction:column}
      .chart-expand-head{display:flex;justify-content:space-between;align-items:center;padding:13px 18px;background:#032D60;color:#fff}.chart-expand-head button{background:rgba(255,255,255,.14);border:0;color:#fff;font-size:24px;line-height:1;border-radius:8px;padding:2px 10px;cursor:pointer}
      .chart-expand-body{flex:1;overflow:auto;padding:18px;background:#f8fafc}.chart-expand-body img{max-width:none;display:block;background:#fff;border-radius:12px;box-shadow:0 10px 30px rgba(15,35,56,.12)}
      tr.cost-detail-row td{border-top:0!important;color:#64748b!important}.cost-detail-meta{font-size:9px;color:#94a3b8;margin-left:6px;font-weight:600}tr.cost-category-row td.lbl{font-weight:700;color:#334155}
    `;
    document.head.appendChild(st);
  }

  // Excel export override: includes visible cost detail rows, while formulas ignore detail rows to avoid double counting.
  async function _ensureXlsx(){
    if (typeof XLSX !== 'undefined') return true;
    if (typeof ensureXLSXLoaded === 'function') await ensureXLSXLoaded();
    if (typeof XLSX !== 'undefined') return true;
    await new Promise(resolve=>{ const s=document.createElement('script'); s.src='https://cdn.jsdelivr.net/npm/xlsx@0.18.5/dist/xlsx.full.min.js'; s.onload=resolve; s.onerror=resolve; document.head.appendChild(s); setTimeout(resolve,5000); });
    return typeof XLSX !== 'undefined';
  }
  const _clean = s => String(s == null ? '' : s).replace(/\u00a0/g,' ').replace(/\s+/g,' ').trim();
  const _key = s => _clean(s).toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/↳/g,'').replace(/\([^)]*\)/g,'').replace(/\s+/g,' ').trim();
  const _parseNum = raw => {
    raw = _clean(raw).replace(/^(USD|PEN)\s+/i,'');
    if (!raw || raw === '-' || raw === '—') return '';
    const neg = /^\(.*\)$/.test(raw) || /^-/.test(raw);
    const cleaned = raw.replace(/[()$,%\s]/g,'').replace(/,/g,'');
    if (!/^[-+]?\d*\.?\d+$/.test(cleaned)) return raw;
    const n = Number(cleaned); return isFinite(n) ? (neg ? -Math.abs(n) : n) : raw;
  };
  const _col = n => { let s=''; while(n>=0){ s=String.fromCharCode((n%26)+65)+s; n=Math.floor(n/26)-1; } return s; };
  const _addr = (r,c) => _col(c)+r;
  function _visible(el){ if (!el) return false; try { return getComputedStyle(el).display !== 'none'; } catch(e) { return true; } }
  function _extractTableForXlsx(tblId){
    const tbl = document.getElementById(tblId); if (!tbl) throw new Error('No encontré la tabla ' + tblId);
    const hcells = Array.from((tbl.querySelector('thead tr')||{}).children || []).filter(_visible);
    const headers = hcells.map((th,i)=> i===0 ? 'Category' : _clean(th.innerText || th.textContent || '').replace(/collapse|expand/ig,'').trim() || ('Col '+i));
    const rows=[]; let sec='';
    Array.from(tbl.querySelectorAll('tbody tr')).forEach(tr=>{
      // Keep custom formula rows even though they include an edit button; skip only UI add-line rows below.
      const secCell = tr.querySelector('td[colspan="999"],td[colspan="998"],td[colspan="997"]');
      if (tr.classList.contains('sec-row') || secCell) { sec = _clean((secCell || tr.children[0]).innerText || (secCell || tr.children[0]).textContent || ''); if (sec) rows.push({type:'section',label:sec}); return; }
      const cells = Array.from(tr.children || []).filter(_visible); if (!cells.length) return;
      const label = _clean(cells[0].innerText || cells[0].textContent || ''); if (!label || /^\+/.test(label) || /add line/i.test(label)) return;
      const colspan = parseInt((cells[1] && cells[1].getAttribute('colspan')) || '1',10);
      const r = {type:'data', section:sec, label, isBold:tr.classList.contains('bold-row') || cells[0].classList.contains('bold'), isDetail:tr.classList.contains('cost-detail-row') || /^↳/.test(label), isCategory:tr.classList.contains('cost-category-row') || !!tr.dataset.costCategory, isInfo:false, values:[]};
      if (cells.length === 2 && colspan > 1) { r.isInfo=true; r.values=[_clean(cells[1].innerText || cells[1].textContent || '')]; }
      else { for(let i=1;i<cells.length;i++) r.values.push(_parseNum(cells[i].innerText || cells[i].textContent || '')); }
      rows.push(r);
    });
    return {headers, rows};
  }
  function _stylesX(){
    const border={top:{style:'thin',color:{rgb:'D8E0EA'}},bottom:{style:'thin',color:{rgb:'D8E0EA'}},left:{style:'thin',color:{rgb:'D8E0EA'}},right:{style:'thin',color:{rgb:'D8E0EA'}}};
    return { title:{font:{bold:true,sz:15,color:{rgb:'FFFFFF'}},fill:{fgColor:{rgb:'032D60'}}}, meta:{font:{sz:10,color:{rgb:'64748B'}},fill:{fgColor:{rgb:'F8FAFC'}}}, head:{font:{bold:true,sz:10,color:{rgb:'FFFFFF'}},fill:{fgColor:{rgb:'032D60'}},alignment:{horizontal:'center',wrapText:true},border}, label:{font:{sz:10,color:{rgb:'334155'}},border}, detail:{font:{sz:9,color:{rgb:'64748B'}},fill:{fgColor:{rgb:'FBFDFF'}},border}, bold:{font:{bold:true,sz:10,color:{rgb:'032D60'}},fill:{fgColor:{rgb:'F0F6FC'}},border}, num:{font:{sz:10,color:{rgb:'334155'}},numFmt:'#,##0.00;[Red](#,##0.00);-',alignment:{horizontal:'right'},border}, numD:{font:{sz:9,color:{rgb:'64748B'}},fill:{fgColor:{rgb:'FBFDFF'}},numFmt:'#,##0.00;[Red](#,##0.00);-',alignment:{horizontal:'right'},border}, numB:{font:{bold:true,sz:10,color:{rgb:'032D60'}},fill:{fgColor:{rgb:'F0F6FC'}},numFmt:'#,##0.00;[Red](#,##0.00);-',alignment:{horizontal:'right'},border}, formula:{font:{bold:true,sz:10,color:{rgb:'003B71'}},fill:{fgColor:{rgb:'EAF4FF'}},numFmt:'#,##0.00;[Red](#,##0.00);-',alignment:{horizontal:'right'},border} };
  }
  function _make(v,s){ if (v && typeof v==='object' && (v.f || v.t || v.v!==undefined)) { if(s) v.s=s; return v; } if (typeof v === 'number') return {t:'n',v,s}; return {t:'s',v:v == null ? '' : String(v),s}; }
  function _isResult(label,type){ const k=_key(label); return type==='pl' ? /^(total income|total cogs|gross profit|profit before opex|ebit|ebitda)/.test(k) : /^(total inflows|total outflows|net cash flow|accumulated cf|accumulated cash flow)/.test(k); }
  function _isCalc(label){ return /^(penalties|contingency|opex buffer|compliance bond|upfront bond|hedging|financing cost|vat|npv|peak negative|break-even)/.test(_key(label)); }
  function _buildXlsx(parsed,type,sheetTitle){
    const S=_stylesX(), colCount=Math.max(parsed.headers.length,2), lastCol=colCount-1, totalCol=colCount-1, dataStart=1, dataEnd=colCount-2;
    const aoa=[], merges=[]; const title=(document.getElementById('proj-name')?.value || 'Financial Model') + ' — ' + sheetTitle;
    aoa.push([_make(title,S.title)]); merges.push({s:{r:0,c:0},e:{r:0,c:lastCol}});
    aoa.push([_make('Exportado en Excel con fórmulas · Moneda: USD · Incluye desglose de categorías de costo',S.meta)]); merges.push({s:{r:1,c:0},e:{r:1,c:lastCol}});
    aoa.push(new Array(colCount).fill(''));
    aoa.push(parsed.headers.map(h=>_make(h,S.head)));
    const metas=[]; let section='';
    parsed.rows.forEach(src=>{
      if (src.type==='section') { const r=aoa.length; aoa.push([_make(String(src.label).toUpperCase(),S.title)]); merges.push({s:{r,c:0},e:{r,c:lastCol}}); section=_key(src.label); return; }
      if (src.type!=='data') return;
      const er=aoa.length+1;
      const labelStyle=src.isDetail ? S.detail : (src.isBold || _isResult(src.label,type) ? S.bold : S.label);
      const numStyle=src.isDetail ? S.numD : (src.isBold || _isResult(src.label,type) ? S.numB : S.num);
      const cells=[_make(src.label,labelStyle)];
      for (let i=0;i<colCount-1;i++) cells.push(_make(src.values[i]===undefined?'':src.values[i], i===colCount-2 && !src.isDetail ? S.numB : numStyle));
      aoa.push(cells); metas.push({src,excelRow:er,aoaIndex:aoa.length-1,section});
    });
    const labelMeta={}; metas.forEach(m=>{ const k=_key(m.src.label); if(!labelMeta[k]) labelMeta[k]=m; });
    const find = name => labelMeta[_key(name)] || metas.find(m=>_key(m.src.label).includes(_key(name)) || _key(name).includes(_key(m.src.label)));
    const refs = (rows,c) => rows.length ? rows.map(m=>_addr(m.excelRow,c)).join('+') : '0';
    const setF=(m,c,f)=>{ const cur=aoa[m.aoaIndex][c] && typeof aoa[m.aoaIndex][c].v==='number'?aoa[m.aoaIndex][c].v:0; aoa[m.aoaIndex][c]={t:'n',f,v:cur,s:c===totalCol?S.formula:S.formula}; };
    const sectionRows=(word,end,filter)=>metas.filter(m=>m.excelRow<end && m.section.includes(word) && !m.src.isInfo && !m.src.isDetail && !_isResult(m.src.label,type) && (!filter || filter(m)));
    const between=(a,b,filter)=>metas.filter(m=>m.excelRow>a && m.excelRow<b && !m.src.isInfo && !m.src.isDetail && (!_isResult(m.src.label,type)) && (!filter || filter(m)));
    const isYearCol = c => /^\s*(FY\s*)?\d{4}\s*$/i.test(parsed.headers[c]||'');
    const yearCols=[]; for(let c=dataStart;c<=dataEnd;c++) if(isYearCol(c)) yearCols.push(c);
    function monthColsBeforeYear(c){ let start=dataStart; for(let j=c-1;j>=dataStart;j--){ if(isYearCol(j)){start=j+1;break;} } const out=[]; for(let j=start;j<c;j++) if(!isYearCol(j)) out.push(j); return out; }
    metas.forEach((m,idx)=>{
      if (m.src.isInfo) return;
      const k=_key(m.src.label);
      const detailRows=[];
      for(let j=idx+1;j<metas.length && metas[j].src.isDetail;j++) detailRows.push(metas[j]);
      for(let c=dataStart;c<=dataEnd;c++){
        let f=null;
        if (isYearCol(c)) { const ms=monthColsBeforeYear(c); if(ms.length) f='SUM('+_addr(m.excelRow,ms[0])+':'+_addr(m.excelRow,ms[ms.length-1])+')'; }
        if (m.src.isCategory && detailRows.length) f=refs(detailRows,c);
        else if (/^contingency/.test(k)) { const bases=sectionRows(type==='pl'?'cogs':'outflows',m.excelRow,x=>x.src.isCategory || (!_isCalc(x.src.label) && !x.src.isDetail)); if(bases.length) f='('+refs(bases,c)+')*'+(Number(CFG.con||0)/100); }
        else if (/^penalties/.test(k)) { const bases=sectionRows(type==='pl'?'income':'inflows',m.excelRow,x=>!_isCalc(x.src.label)); if(bases.length) f='('+refs(bases,c)+')*-'+(Number(CFG.pen||0)/100); }
        else if (/^opex buffer/.test(k)) { const base=find(type==='cf'?'Total Inflows':'Total Income'); if(base) f=(type==='pl'?'-ABS(':'ABS(')+_addr(base.excelRow,c)+')*'+(Number(CFG.opxbuf||0)/100); }
        if (type==='pl') {
          const ti=find('Total Income'), tc=find('Total COGS'), gp=find('Gross Profit'), pbo=find('Profit before OPEX / Interest'), ebit=find('EBIT');
          if (/^total income$/.test(k)) f=refs(sectionRows('income',m.excelRow),c);
          else if (/^total cogs$/.test(k)) f=refs(sectionRows('cogs',m.excelRow),c);
          else if (/^gross profit/.test(k) && ti && tc) f=_addr(ti.excelRow,c)+'+'+_addr(tc.excelRow,c);
          else if (/^profit before opex/.test(k) && gp) f=_addr(gp.excelRow,c)+'+'+refs(between(gp.excelRow,m.excelRow),c);
          else if (/^ebit$/.test(k) && pbo) f=_addr(pbo.excelRow,c)+'+'+refs(between(pbo.excelRow,m.excelRow),c);
          else if (/^ebitda$/.test(k) && ebit) f=_addr(ebit.excelRow,c)+'+'+refs(between(ebit.excelRow,m.excelRow),c);
        } else {
          const ti=find('Total Inflows'), to=find('Total Outflows'), ncf=find('Net Cash Flow');
          if (/^total inflows$/.test(k)) f=refs(sectionRows('inflows',m.excelRow),c);
          else if (/^total outflows$/.test(k)) f=refs(sectionRows('outflows',m.excelRow),c);
          else if (/^net cash flow$/.test(k) && ti && to) f=_addr(ti.excelRow,c)+'-'+_addr(to.excelRow,c);
          else if (/^accumulated/.test(k) && ncf) f=c===dataStart ? _addr(ncf.excelRow,c) : _addr(m.excelRow,c-1)+'+'+_addr(ncf.excelRow,c);
        }
        if (f) setF(m,c,f);
      }
      if (!m.src.isInfo) {
        let ft=null;
        if (/^accumulated/.test(k)) ft=_addr(m.excelRow, dataEnd);
        else if (yearCols.length) ft=refs(yearCols.map(c=>({excelRow:m.excelRow, _c:c})), totalCol).replace(new RegExp(_addr(m.excelRow,totalCol),'g'),'');
        if (yearCols.length) ft=yearCols.map(c=>_addr(m.excelRow,c)).join('+');
        else if (dataEnd>=dataStart) ft='SUM('+_addr(m.excelRow,dataStart)+':'+_addr(m.excelRow,dataEnd)+')';
        if (ft) setF(m,totalCol,ft);
      }
    });
    const ws=XLSX.utils.aoa_to_sheet(aoa); ws['!merges']=merges; ws['!cols']=[{wch:42}].concat(Array.from({length:colCount-2},()=>({wch:14})),[{wch:16}]); ws['!views']=[{state:'frozen',xSplit:1,ySplit:4}]; ws['!autofilter']={ref:'A4:'+_addr(Math.max(4,aoa.length),lastCol)}; ws['!ref']='A1:'+_addr(aoa.length,lastCol); return ws;
  }
  window.exportTableXLSX = async function(tblId, sheetName, filename){
    try{
      if (!(await _ensureXlsx())) { alert('No se pudo cargar la librería de Excel.'); return; }
      const type = tblId === 'cf-tbl' ? 'cf' : 'pl';
      const parsed = _extractTableForXlsx(tblId);
      const cleanSheet = (sheetName || (type==='pl'?'P&L':'Cash Flow')).replace(/[\\/?*\[\]:]/g,' ').slice(0,31) || 'Statement';
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, _buildXlsx(parsed,type,cleanSheet), cleanSheet);
      wb.Workbook = wb.Workbook || {}; wb.Workbook.CalcPr = { fullCalcOnLoad:true };
      XLSX.writeFile(wb, filename || ((document.getElementById('proj-name')?.value || 'model').replace(/\s+/g,'_') + '_' + cleanSheet + '.xlsx'), {cellStyles:true, bookType:'xlsx'});
    } catch(e){ console.error('Excel export breakdown error:', e); alert('No se pudo exportar el Excel: ' + (e.message || e)); }
  };

  _injectPatchStyles();
  // Refresh visible views if the project screen is already open.
  setTimeout(()=>{
    try { if (document.getElementById('panel-dashboard')?.classList.contains('show')) renderDashboard(); } catch(e) {}
    try { if (document.getElementById('panel-pl')?.classList.contains('show')) renderPL(); } catch(e) {}
    try { if (document.getElementById('panel-cf')?.classList.contains('show')) renderCF(); } catch(e) {}
  }, 0);
})();

/* =====================================================================
 FINAL PATCH — Keep dashboard charts at original size + expand/scroll modal
 and add expandable/collapsible cost category rows in P&L / Cash Flow.
 Scope: UI only. Does not modify compute(), model logic, formulas or exports.
 ===================================================================== */
(function _finalChartSizeAndCostCollapsePatch(){
  if (window.__finalChartSizeAndCostCollapsePatch) return;
  window.__finalChartSizeAndCostCollapsePatch = true;

  const COST_TOGGLE_STATE_KEY = 'fm_cost_category_expand_state_v1';
  function _loadCostToggleState(){
    try { return JSON.parse(localStorage.getItem(COST_TOGGLE_STATE_KEY) || '{}') || {}; }
    catch(e){ return {}; }
  }
  function _saveCostToggleState(state){
    try { localStorage.setItem(COST_TOGGLE_STATE_KEY, JSON.stringify(state || {})); } catch(e){}
  }
  const _costToggleState = _loadCostToggleState();

  function _stripHtml(s){ return String(s == null ? '' : s).replace(/<[^>]*>/g,'').replace(/\s+/g,' ').trim(); }
  function _cleanCostKey(txt){
    return _stripHtml(txt).replace(/^[▾▸+\-\s]+/, '').replace(/\s*\(.*?\)\s*$/,'').trim() || 'Cost category';
  }
  function _stateKey(which, label){ return String(which || 'pl') + '::' + _cleanCostKey(label).toLowerCase(); }

  function _applyCostCategoryToggles(which){
    const tbl = document.getElementById(which === 'cf' ? 'cf-tbl' : 'pl-tbl');
    if (!tbl) return;
    const catRows = Array.from(tbl.querySelectorAll('tr.cost-category-row'));
    catRows.forEach((row, idx) => {
      const first = row.cells && row.cells[0];
      if (!first) return;
      if (!row.dataset.costKey) row.dataset.costKey = _cleanCostKey(first.textContent || first.innerText || ('category_' + idx));
      const key = _stateKey(which, row.dataset.costKey);
      if (_costToggleState[key] === undefined) _costToggleState[key] = false; // default compressed
      const expanded = !!_costToggleState[key];

      let btn = first.querySelector('.cost-cat-toggle-btn');
      if (!btn) {
        btn = document.createElement('button');
        btn.type = 'button';
        btn.className = 'cost-cat-toggle-btn';
        btn.title = 'Desplegar / comprimir categoría';
        first.insertBefore(btn, first.firstChild);
        btn.addEventListener('click', ev => {
          ev.preventDefault(); ev.stopPropagation();
          const k = _stateKey(which, row.dataset.costKey || first.textContent);
          _costToggleState[k] = !_costToggleState[k];
          _saveCostToggleState(_costToggleState);
          _applyCostCategoryToggles(which);
        });
      }
      ...
      while (next && next.classList && next.classList.contains('cost-detail-row')) {
        next.style.display = expanded ? '' : 'none';
        next.setAttribute('aria-hidden', expanded ? 'false' : 'true');
  ...
  function _installCostToggleStyles(){
    if (document.getElementById('final-cost-toggle-styles')) return;
    const st = document.createElement('style');
    st.id = 'final-cost-toggle-styles';
    st.textContent = `
      .cost-cat-toggle-btn{width:20px;height:20px;margin-right:7px;border:1px solid #cbd5e1;border-radius:6px;background:#fff;color:#032D60;font-size:12px;font-weight:900;line-height:17px;cursor:pointer;vertical-align:middle;box-shadow:0 2px 6px rgba(15,35,56,.05)}
      .cost-cat-toggle-btn:hover{border-color:#0176D3;color:#0176D3;background:#f8fbff}
      tr.cost-category-row td.lbl{font-weight:800!important;color:#263b53!important;background:#f8fafc!important}
      tr.cost-category-row.is-expanded td.lbl{color:#032D60!important}
      tr.cost-detail-row td{background:#fbfdff!important;color:#64748b!important}
      .chart-expand-btn{white-space:nowrap}
      #chart-expand-modal .chart-expand-body{overflow:auto!important}
      #chart-expanded-canvas{display:block;background:#fff;border-radius:12px;box-shadow:0 10px 30px rgba(15,35,56,.12);max-width:none!important}
    `;
    document.head.appendChild(st);
  }

  function _unwrapNormalChartCanvas(id){
    const canvas = document.getElementById(id);
    if (!canvas) return null;
    const scroll = canvas.closest && canvas.closest('.chart-scrollbox');
    const outer = scroll ? scroll.parentElement : (canvas.closest('.chart-canvas-wrap') || canvas.parentElement);
    if (scroll && outer) {
      outer.insertBefore(canvas, scroll);
      try { scroll.remove(); } catch(e) { if (scroll.parentNode) scroll.parentNode.removeChild(scroll); }
    }
    canvas.style.width = '100%';
  ...
  function _chartLabels(){
    try {
      if (chartView === 'annual') return yrLabels();
      if (chartView === 'grouped') {
        const years = typeof getYearRanges === 'function' ? getYearRanges() : [];
        let y = years.find(x => String(x.year) === String(chartSelectedYear)) || years[0];
        return y && y.yms ? y.yms.map(m => mInfo(m).short + ' ' + String(mInfo(m).year).slice(2)) : AMs().map(m => mInfo(m).short);
      }
      return AMs().map(m => mInfo(m).short);
    } catch(e) { return []; }
  }
  function _chartArr(arr, acc){
    arr = Array.isArray(arr) ? arr : new Array(PM()).fill(0);
    try {
      if (chartView === 'annual') return acc ? aggAccY(arr) : aggY(arr);
      if (chartView === 'grouped') {
        const years = typeof getYearRanges === 'function' ? getYearRanges() : [];
        let y = years.find(x => String(x.year) === String(chartSelectedYear)) || years[0];
        return y && y.yms ? y.yms.map(m => arr[m-1] || 0) : arr;
      }
    } catch(e) {}
    return arr;
  }
  function _fmtAxis(v, pct){
  ...
  function _range(series, zero){
    const vals = [];
    (series||[]).forEach(s => (s.data||[]).forEach(v => { if (isFinite(v)) vals.push(Number(v)); }));
  ...
  function _axes(ctx, w, h, pad, rg, pct){
    const plotW = w-pad.l-pad.r, plotH = h-pad.t-pad.b;
    const toY = v => pad.t + plotH - ((v-rg.min)/((rg.max-rg.min)||1))*plotH;
    ctx.save();
    ctx.font = '10px Arial, sans-serif';
    ctx.textAlign = 'right'; ctx.textBaseline = 'middle';
    for (let i=0;i<=4;i++){
      const v = rg.min + (rg.max-rg.min)*i/4;
      const y = toY(v);
      ctx.strokeStyle = '#E8EDF2'; ctx.beginPath(); ctx.moveTo(pad.l,y); ctx.lineTo(w-pad.r,y); ctx.stroke();
      ctx.fillStyle = '#7b8794'; ctx.fillText(_fmtAxis(v,pct), pad.l-8, y);
    }
    const zy = toY(0);
    ctx.strokeStyle = '#B7C1CC'; ctx.beginPath(); ctx.moveTo(pad.l,zy); ctx.lineTo(w-pad.r,zy); ctx.stroke();
    ctx.restore();
    return {plotW, plotH, toY, zeroY:zy};
  }
  function _legend(ctx, series, x, y, w){
    ctx.save(); ctx.font = '10px Arial, sans-serif'; ctx.textBaseline = 'middle';
    let cx=x, cy=y;
    (series||[]).forEach(s=>{
      const col = typeof s.color === 'function' ? s.color(1) : s.color;
      const label = String(s.label||''); const tw = ctx.measureText(label).width;
      if (cx + tw + 42 > w - 20) { cx=x; cy += 16; }
      ctx.fillStyle = col; ctx.fillRect(cx, cy-5, 10, 10); ctx.fillStyle = '#475569'; ctx.fillText(label, cx+14, cy); cx += tw + 44;
    });
    ctx.restore();
  }
  function _xLabels(ctx, labels, toX, h, pad, expanded){
    const n = labels.length;
    const step = expanded ? 1 : (n <= 8 ? 1 : n <= 12 ? 2 : n <= 18 ? 3 : Math.ceil(n/6));
    ctx.save(); ctx.fillStyle = '#64748B'; ctx.font = expanded ? '10px Arial, sans-serif' : '9px Arial, sans-serif'; ctx.textAlign='center'; ctx.textBaseline='top';
    labels.forEach((lbl,i)=>{
      if (i % step !== 0 && i !== n-1) return;
      const x = toX(i), y = h - pad.b + 12;
      if (!expanded && n > 10) { ctx.save(); ctx.translate(x,y); ctx.rotate(-Math.PI/5.2); ctx.fillText(String(lbl).slice(0,10),0,0); ctx.restore(); }
      else ctx.fillText(String(lbl).slice(0,11), x, y);
    });
    ctx.restore();
  }
  function _drawBars(canvas, series, labels, opts){
    opts = opts || {};
    const parent = canvas.parentElement;
    const normalW = Math.max(360, Math.floor((parent && parent.clientWidth) || 620));
    const w = opts.expanded ? Math.max(980, labels.length * 92 + 160) : normalW;
    const h = opts.expanded ? 440 : (opts.height || Math.max(160, Math.floor((parent && parent.clientHeight) || 190)));
    const ctx = _setupCanvas(canvas, w, h);
    const pad = {t:34,r:24,b:(labels.length>10&&!opts.expanded)?58:46,l:78};
    const rg = _range(series, true);
    const ax = _axes(ctx,w,h,pad,rg,!!opts.percent);
    _legend(ctx, series, pad.l, 18, w);
    const n = Math.max(1, labels.length), groupW = ax.plotW/n;
    const barW = Math.max(3, Math.min(opts.expanded ? 18 : 13, groupW / Math.max(1, series.length) * .68));
    const toX = i => pad.l + groupW*i + groupW/2;
    (series||[]).forEach((s,si)=>{
      (s.data||[]).forEach((v,i)=>{
        const col = typeof s.color === 'function' ? s.color(v) : s.color;
        const x = toX(i) - (series.length*barW)/2 + si*barW + barW*.12;
        const y = ax.toY(Math.max(0,v)), y0 = ax.toY(0);
        ctx.fillStyle = col; ctx.fillRect(x, Math.min(y,y0), barW*.76, Math.max(1, Math.abs(y0-y)));
      });
    });
    _xLabels(ctx, labels, toX, h, pad, !!opts.expanded);
  }
  function _drawLines(canvas, series, labels, opts){
    opts = opts || {};
    const parent = canvas.parentElement;
    const normalW = Math.max(360, Math.floor((parent && parent.clientWidth) || 620));
    const w = opts.expanded ? Math.max(980, labels.length * 92 + 160) : normalW;
    ...
    const ctx = _setupCanvas(canvas, w, h);
    const pad = {t:34,r:24,b:(labels.length>10&&!opts.expanded)?58:46,l:78};
    const rg = _range(series, !!opts.forceZero);
    const ax = _axes(ctx,w,h,pad,rg,!!opts.percent);
    _legend(ctx, series, pad.l, 18, w);
    const n = Math.max(1, labels.length), stepX = n <= 1 ? 0 : ax.plotW/(n-1);
    const toX = i => pad.l + stepX*i;
    (series||[]).forEach(s=>{
      ctx.strokeStyle = s.color || '#0176D3'; ctx.lineWidth = 2.5; ctx.beginPath();
      (s.data||[]).forEach((v,i)=>{ const x=toX(i), y=ax.toY(v); if(i===0) ctx.moveTo(x,y); else ctx.lineTo(x,y); });
      ctx.stroke();
      ctx.fillStyle = s.color || '#0176D3';
      (s.data||[]).forEach((v,i)=>{ const x=toX(i), y=ax.toY(v); ctx.beginPath(); ctx.arc(x,y, opts.expanded?3:2.5,0,Math.PI*2); ctx.fill(); });
    });
    _xLabels(ctx, labels, toX, h, pad, !!opts.expanded);
  }
  function _drawWaterfall(canvas, labels, values, opts){
    opts = opts || {};
    const parent = canvas.parentElement;
    const normalW = Math.max(520, Math.floor((parent && parent.clientWidth) || 980));
    const w = opts.expanded ? Math.max(1100, labels.length * 125 + 180) : normalW;
    const h = opts.expanded ? 460 : (opts.height || Math.max(200, Math.floor((parent && parent.clientHeight) || 220)));
    const series = [{data: values}];
    const ctx = _setupCanvas(canvas,w,h);
    const pad = {t:28,r:24,b:56,l:82};
    const rg = _range(series,true); const ax = _axes(ctx,w,h,pad,rg,false);
    const n = labels.length, groupW = ax.plotW/Math.max(1,n), barW=Math.min(64, groupW*.5);
    const toX = i => pad.l + groupW*i + groupW/2;
    values.forEach((v,i)=>{
      const x=toX(i)-barW/2, y=ax.toY(Math.max(0,v)), y0=ax.toY(0);
      ctx.fillStyle = v >= 0 ? '#06A59A' : '#BA0517';
      ctx.fillRect(x, Math.min(y,y0), barW, Math.max(1, Math.abs(y0-y)));
    });
    _xLabels(ctx, labels, toX, h, pad, !!opts.expanded);
  }
  function _chartPayload(id){
    const cv = compute();
    const labels = _chartLabels();
    const arr = a => _chartArr(a || [], false);
    const acc = a => _chartArr(a || [], true);
    if (id === 'chart-rc') return {type:'bar', labels, series:[
      {label:'Revenue', data:arr(cv.totalIncome), color:'#06A59A'},
      {label:'COGS', data:arr(cv.totalCogsAdj).map(v=>-Math.abs(v)), color:'#BA0517'},
      {label:'Gross Profit', data:arr(cv.grossProfit), color:'#0176D3'}
    ], height:190};
    if (id === 'chart-margins') {
      const rev = arr(cv.totalIncome);
      const pct = a => arr(a).map((v,i)=> rev[i] ? v/rev[i]*100 : 0);
      return {type:'line', labels, percent:true, series:[
        {label:'GP %', data:pct(cv.grossProfit), color:'#0176D3'},
        {label:'EBITDA %', data:pct(cv.ebitda || []), color:'#06A59A'},
        {label:'EBIT %', data:pct(cv.ebit || []), color:'#5A2D82'}
      ], height:190};
    }
    if (id === 'chart-ncf') return {type:'bar', labels, series:[{label:'Net CF', data:arr(cv.netCF), color:v=>v>=0?'#0176D3':'#BA0517'}], height:170};
    if (id === 'chart-accum') return {type:'line', labels, series:[{label:'Accum. CF', data:acc(cv.accumCF), color:'#5A2D82'}], height:170};
    const wLabels = ['Revenue','COGS','OPEX','Bond','Hedging','Financing','EBIT'];
    const vals = [rSum(cv.totalIncome||[]), -Math.abs(rSum(cv.totalCogsAdj||[])), -Math.abs(rSum(cv.opexBuffer||[])), -Math.abs(rSum(cv.bondFees||[])), -Math.abs(rSum(cv.hedgingArr||[])), -Math.abs(rSum(cv.finCostArr||[])), rSum(cv.ebit||[])];
    return {type:'waterfall', labels:wLabels, values:vals, height:220};
  }
  function _drawDashboardChart(id, expanded, targetCanvas){
    const canvas = targetCanvas || document.getElementById(id);
    if (!canvas) return;
    const p = _chartPayload(id);
    if (!p) return;
    if (p.type === 'line') _drawLines(canvas, p.series, p.labels, {expanded, height:p.height, percent:p.percent, forceZero:false});
    else if (p.type === 'waterfall') _drawWaterfall(canvas, p.labels, p.values, {expanded, height:p.height});
    else _drawBars(canvas, p.series, p.labels, {expanded, height:p.height});
  }
  function _redrawNormalDashboardCharts(){
    ['chart-rc','chart-margins','chart-ncf','chart-accum','chart-waterfall'].forEach(id=>{
      const c = _unwrapNormalChartCanvas(id);
      if (!c) return;
      _drawDashboardChart(id, false, c);
    });
  }
  function _ensureFinalExpandModal(){
    let modal = document.getElementById('chart-expand-modal');
    if (!modal) {
      modal = document.createElement('div');
      modal.id = 'chart-expand-modal';
    ...
    modal.innerHTML = `<div class="chart-expand-shell"><div class="chart-expand-head"><strong id="chart-expand-title">Chart</strong><button id="chart-expand-close">×</button></div><div class="chart-expand-body"><canvas id="chart-expanded-canvas"></canvas></div></div>`;
    modal.querySelector('#chart-expand-close').onclick = () => modal.classList.remove('show');
    modal.onclick = e => { if (e.target === modal) modal.classList.remove('show'); };
    return modal;
  }
  function _wireFinalExpandButtons(){
    ['chart-rc','chart-margins','chart-ncf','chart-accum','chart-waterfall'].forEach(id=>{
      const canvas = document.getElementById(id);
      if (!canvas) return;
      const card = canvas.closest('.chart-card');
      if (!card) return;
      let btn = card.querySelector('.chart-expand-btn[data-chart="'+id+'"]');
      if (!btn) {
        const title = card.querySelector('[id$="chart-title"]') || card.querySelector('div[style*="font-weight:700"]') || card.firstElementChild;
        btn = document.createElement('button'); btn.className = 'chart-expand-btn'; btn.dataset.chart = id; btn.textContent = 'Expandir';
        if (title && title.parentElement) { title.parentElement.style.display='flex'; title.parentElement.style.justifyContent='space-between'; title.parentElement.style.alignItems='center'; title.parentElement.appendChild(btn); }
        else card.insertBefore(btn, card.firstChild);
      }
      btn.onclick = ev => {
        ev.preventDefault(); ev.stopPropagation();
        const modal = _ensureFinalExpandModal();
        const titleEl = modal.querySelector('#chart-expand-title');
        const title = card.querySelector('[id$="chart-title"]') || card.querySelector('div[style*="font-weight:700"]');
        titleEl.textContent = _stripHtml((title && title.textContent) || 'Chart');
        modal.classList.add('show');
        const expCanvas = modal.querySelector('#chart-expanded-canvas');
        setTimeout(()=>_drawDashboardChart(id, true, expCanvas), 0);
      };
    });
  }

  const _prevRenderDashboard = window.renderDashboard || renderDashboard;
  renderDashboard = window.renderDashboard = function(){
    _prevRenderDashboard.apply(this, arguments);
    setTimeout(()=>{
      _redrawNormalDashboardCharts();
      _wireFinalExpandButtons();
    }, 0);
  };

  const _prevRenderPL = window.renderPL || renderPL;
  renderPL = window.renderPL = function(){
    _prevRenderPL.apply(this, arguments);
    _applyCostCategoryToggles('pl');
  };
  const _prevRenderCF = window.renderCF || renderCF;
  renderCF = window.renderCF = function(){
    _prevRenderCF.apply(this, arguments);
    _applyCostCategoryToggles('cf');
  };

  _installCostToggleStyles();
  document.addEventListener('DOMContentLoaded', () => {
    _installCostToggleStyles();
    setTimeout(()=>{ try { _applyCostCategoryToggles('pl'); _applyCostCategoryToggles('cf'); _redrawNormalDashboardCharts(); _wireFinalExpandButtons(); } catch(e){} }, 300);
  });
})();
