...
function fmt(v, blankZero) {
 // Show real 0.00 values instead of hidden/control characters or blank dashes.
 // Use fmt(value, true) only when a specific UI row should intentionally hide zeros.
 const dash = '–';
 const n = Number(v);
 if (v === undefined || v === null || Number.isNaN(n)) return dash;
 if (blankZero === true && Math.abs(n) < 0.005) return dash;
...
function renderGroupedPLCF(which, cv) {
 ...
 if (which === 'pl') {
 const plTotalCogs = cv.totalCogsAdj.map(v => -v);
 const plGrossProfit = cv.totalIncome.map((v, i) => v + plTotalCogs[i]);
 const plProfitBeforeOpex = plGrossProfit.map((v, i) => v - (cv.finExpCB[i] || 0) - (cv.finExpUB[i] || 0));
 const plEBIT = plProfitBeforeOpex.map((v, i) => v - (cv.opexBuffer[i] || 0) - (cv.hedgingArr[i] || 0) - (cv.finCostArr[i] || 0));
 const plEBITDA = plGrossProfit.slice();
 ...
 body += mkRow('Total COGS', plTotalCogs, {bold:true});

 //  GROSS PROFIT — Excel logic: Total Income + Total COGS (COGS displayed as negative)
 body += mkRow('Gross Profit', plGrossProfit, {bold:true, color:'blue'});
 ...
 body += mkRow('Profit before OPEX / Interest', plProfitBeforeOpex, {bold:true, color:'blue'});
 ...
 body += mkRow('EBIT', plEBIT, {bold:true, color:'blue'});
 body += mkRow('EBITDA', plEBITDA, {bold:true, color:'teal'});
 ...
 const totInflow = sumArrs(inflowRowsForTotal);
 const totOutflow = sumArrs(outflowRowsForTotal);
 ...
 adjIncomeRowsCF.forEach(({ac, arr}) => {
   body += mkRow((ac.adj.name||'Adj')+' ('+ac.adj.pct+'%)', arr, {ind:true,color:'amber'});
 });
 ...
 outflowCustomRows.forEach(({line, arr}) => body += mkRow(line.name||'Custom', arr, {ind:true,color:line.color||'amber'}));
...
function renderPL() {
 ...
 return sumRow('Gross Profit', cv.totalIncome.map((v, i) => v - (cv.totalCogsAdj[i] || 0)), 'var(--accent)', cv.totalIncome)
 ...
 return sumRow('Profit before OPEX / Interest', cv.totalIncome.map((v, i) => v - (cv.totalCogsAdj[i] || 0) - (cv.finExpCB[i] || 0) - (cv.finExpUB[i] || 0)), 'var(--accent)', cv.totalIncome)
 ...
 + sumRow('EBIT', cv.totalIncome.map((v, i) => v - (cv.totalCogsAdj[i] || 0) - (cv.finExpCB[i] || 0) - (cv.finExpUB[i] || 0) - (cv.opexBuffer[i] || 0) - (cv.hedgingArr[i] || 0) - (cv.finCostArr[i] || 0)), 'var(--accent)', cv.totalIncome);
 case 'pl_ebitda':
 return sumRow('EBITDA', cv.totalIncome.map((v, i) => v - (cv.totalCogsAdj[i] || 0)), 'var(--teal)', cv.totalIncome);
 default: return '';
 }
 }
