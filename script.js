...
function getDistArr(item) {
 ...
 if (!item.progMode && item.linkedCostId) {
  ...
  if (linked) {
   const costDist = getDistArr(linked);
...
function buildItemCard(item, idx, type) {
 ...
 if (!isCost) {
  ...
  // ── P&L RECOGNITION LINK ──────────────────────────────────────────────
  const recSec = document.createElement('div');
  recSec.style.cssText = 'margin-top:14px;border:1px solid #c7ddf5;border-radius:10px;overflow:hidden';
  const recHdr = document.createElement('div');
  recHdr.style.cssText = 'display:flex;align-items:center;gap:8px;padding:10px 14px;background:#eaf3fd';
  recHdr.innerHTML = '<span style="font-size:13px">📊</span>'
   + '<span style="font-size:12px;font-weight:700;color:#032D60;flex:1">P&L Recognition</span>'
   + '<span style="font-size:10px;color:#5a7fa0">Link to CAPEX/OPEX for % completion method</span>';
  recSec.appendChild(recHdr);
  const recBody = document.createElement('div');
  recBody.style.cssText = 'padding:12px 14px;display:grid;gap:10px';

  const recModeRow = document.createElement('div'); recModeRow.style.cssText = 'display:flex;gap:8px;flex-wrap:wrap';
  ['manual','linked'].forEach(m => {
   const lbl = document.createElement('label');
   const isActive = (item.linkedCostId ? 'linked' : 'manual') === m;
   lbl.style.cssText = 'display:flex;align-items:center;gap:5px;cursor:pointer;font-size:11px;font-weight:600;padding:5px 12px;border-radius:20px;border:1.5px solid ' + (isActive ? '#0176D3;background:#e8f4fd;color:#0176D3' : '#d0d5dd;color:#666');
   const rb = document.createElement('input'); rb.type='radio'; rb.name='recmode_'+item.id; rb.value=m; rb.checked=isActive;
   lbl.appendChild(rb);
   lbl.appendChild(document.createTextNode(m==='manual'?'✏ Manual':'🔗 Linked to CAPEX/OPEX'));
   recModeRow.appendChild(lbl);
  });
  recBody.appendChild(recModeRow);

  const recLinkRow = document.createElement('div');
  recLinkRow.style.cssText = 'display:'+(item.linkedCostId?'grid':'none')+';grid-template-columns:1fr 1fr;gap:8px;align-items:end';
  const recTypeFld = document.createElement('div'); recTypeFld.className='fld'; recTypeFld.style.margin='0';
  const recTypeLbl = document.createElement('label'); recTypeLbl.textContent='Cost type'; recTypeLbl.style.fontSize='10px';
  const recTypeSel = document.createElement('select');
  ['capex','opex'].forEach(t => {
   const o = document.createElement('option'); o.value=t; o.textContent=t.toUpperCase();
   if ((item.linkedCostType||'capex')===t) o.selected=true; recTypeSel.appendChild(o);
  });
  recTypeFld.appendChild(recTypeLbl); recTypeFld.appendChild(recTypeSel);
  const recItemFld = document.createElement('div'); recItemFld.className='fld'; recItemFld.style.margin='0';
  const recItemLbl = document.createElement('label'); recItemLbl.textContent='Line item'; recItemLbl.style.fontSize='10px';
  const recItemSel = document.createElement('select');
  function populateRecItems(tp) {
   recItemSel.innerHTML='<option value="">— Select item —</option>';
   (tp==='capex'?capex:opex).forEach(c => {
    const o=document.createElement('option'); o.value=c.id;
    o.textContent=(c.name||'Unnamed')+(c.cat?' ['+c.cat+']':'');
    if(c.id===item.linkedCostId) o.selected=true; recItemSel.appendChild(o);
   });
  }
  populateRecItems(item.linkedCostType||'capex');
  recItemFld.appendChild(recItemLbl); recItemFld.appendChild(recItemSel);
  recLinkRow.appendChild(recTypeFld); recLinkRow.appendChild(recItemFld);
  recBody.appendChild(recLinkRow);

  const recNote = document.createElement('div');
  recNote.style.cssText='font-size:10px;color:#2E844A;padding:6px 10px;background:#eafaea;border-radius:6px;display:'+(item.linkedCostId?'block':'none');
  recNote.textContent='✓ P&L recognition follows the linked item\'s monthly curve (% completion method). Distribution grid below is ignored.';
  recBody.appendChild(recNote);
  recSec.appendChild(recBody); body.appendChild(recSec);

  recModeRow.querySelectorAll('input[type=radio]').forEach(rb => {
   rb.addEventListener('change', () => {
    const isLinked = rb.value==='linked';
    recLinkRow.style.display = isLinked?'grid':'none';
    recNote.style.display = isLinked?'block':'none';
    if(!isLinked){item.linkedCostId=null;item.linkedCostType=null;refresh();}
    recModeRow.querySelectorAll('label').forEach(l => {
     const v=l.querySelector('input').value;
     l.style.borderColor=v===rb.value?'#0176D3':'#d0d5dd';
     l.style.background=v===rb.value?'#e8f4fd':'';
     l.style.color=v===rb.value?'#0176D3':'#666';
    });
   });
  });
  recTypeSel.addEventListener('change',()=>{item.linkedCostType=recTypeSel.value;item.linkedCostId=null;populateRecItems(recTypeSel.value);});
  recItemSel.addEventListener('change',()=>{item.linkedCostId=recItemSel.value||null;item.linkedCostType=recTypeSel.value;recNote.style.display=item.linkedCostId?'block':'none';refresh();});

  // ── CF COLLECTION SCHEDULE ────────────────────────────────────────────
  const cfSec = document.createElement('div');
  cfSec.style.cssText = 'margin-top:10px;border:1px solid #c7e0c7;border-radius:10px;overflow:hidden';
  const cfHdr = document.createElement('div');
  cfHdr.style.cssText = 'display:flex;align-items:center;gap:8px;padding:10px 14px;background:#eafaea';
  cfHdr.innerHTML='<span style="font-size:13px">💵</span><span style="font-size:12px;font-weight:700;color:#064A0C;flex:1">Cash Flow Collection</span><span style="font-size:10px;color:#4a7a4a">How & when cash is received</span>';
  cfSec.appendChild(cfHdr);
  const cfBody2 = document.createElement('div');
  cfBody2.style.cssText = 'padding:12px 14px;display:grid;gap:10px';

  const cfModeRow = document.createElement('div'); cfModeRow.style.cssText='display:flex;gap:6px;flex-wrap:wrap';
  [{v:'lag',l:'📅 Lag Schedule'},{v:'map',l:'🗓 Month Map'},{v:'terms',l:'📆 Payment Terms'}].forEach(({v,l}) => {
   const btn=document.createElement('button');
   const cur2=item.cfScheduleMode||'terms';
   btn.style.cssText='font-size:10px;font-weight:700;padding:4px 12px;border-radius:16px;cursor:pointer;transition:all .15s;border:1.5px solid '+(cur2===v?'#2E844A;background:#2E844A;color:#fff':'#d0d5dd;background:#fff;color:#555');
   btn.textContent=l; btn.dataset.cfMode=v; cfModeRow.appendChild(btn);
  });
  cfBody2.appendChild(cfModeRow);

  // Lag panel
  const lagPanel = document.createElement('div');
  lagPanel.style.cssText='display:'+((item.cfScheduleMode||'terms')==='lag'?'block':'none');
  function renderLagPanel() {
   lagPanel.innerHTML='';
   const sched=item.lagSchedule||[{lag:0,pct:100}];
   const total=sched.reduce((s,e)=>s+(parseFloat(e.pct)||0),0);
   const hdr2=document.createElement('div');
   hdr2.style.cssText='display:grid;grid-template-columns:90px 80px 32px;gap:6px;margin-bottom:4px';
   hdr2.innerHTML='<span style="font-size:9px;font-weight:700;color:#555;text-transform:uppercase">Lag (months)</span><span style="font-size:9px;font-weight:700;color:#555;text-transform:uppercase">% recognized</span><span></span>';
   lagPanel.appendChild(hdr2);
   sched.forEach((entry,ei) => {
    const row=document.createElement('div'); row.style.cssText='display:grid;grid-template-columns:90px 80px 32px;gap:6px;align-items:center';
    const lagI=document.createElement('input'); lagI.type='number'; lagI.min='0'; lagI.step='1'; lagI.value=entry.lag??0; lagI.style.cssText='font-size:12px;text-align:center';
    const pctI=document.createElement('input'); pctI.type='number'; pctI.min='0'; pctI.max='100'; pctI.step='1'; pctI.value=entry.pct??0; pctI.style.cssText='font-size:12px;text-align:center';
    const delBtn=document.createElement('button'); delBtn.textContent='✕'; delBtn.style.cssText='font-size:11px;background:none;border:none;color:#BA0517;cursor:pointer;padding:2px 4px';
    lagI.addEventListener('input',()=>{sched[ei].lag=parseInt(lagI.value)||0;item.lagSchedule=sched;});
    pctI.addEventListener('input',()=>{sched[ei].pct=parseFloat(pctI.value)||0;item.lagSchedule=sched;updateLagTotal();});
    delBtn.addEventListener('click',()=>{sched.splice(ei,1);item.lagSchedule=sched;renderLagPanel();});
    row.appendChild(lagI); row.appendChild(pctI); row.appendChild(delBtn); lagPanel.appendChild(row);
   });
   const totalLine=document.createElement('div');
   totalLine.id='lag-total-'+item.id;
   totalLine.style.cssText='font-size:10px;font-weight:700;margin-top:4px;padding:4px 8px;border-radius:4px;color:'+(Math.abs(total-100)<0.01?'#2E844A':'#BA0517')+';background:'+(Math.abs(total-100)<0.01?'#eafaea':'#fef1f1');
   totalLine.textContent='Total: '+total.toFixed(1)+'% '+(Math.abs(total-100)<0.01?'✓':'← must equal 100%');
   lagPanel.appendChild(totalLine);
   const addBtn=document.createElement('button'); addBtn.textContent='+ Add row'; addBtn.style.cssText='font-size:10px;margin-top:6px;color:#2E844A;background:none;border:1px dashed #2E844A;border-radius:4px;padding:3px 10px;cursor:pointer';
   addBtn.addEventListener('click',()=>{sched.push({lag:sched.length,pct:0});item.lagSchedule=sched;renderLagPanel();});
   lagPanel.appendChild(addBtn);
   const hint=document.createElement('div'); hint.style.cssText='font-size:10px;color:#888;margin-top:6px;line-height:1.5';
   hint.innerHTML='<strong>Example:</strong> lag=0 50%, lag=1 30%, lag=2 20% → In month 4: receives 50% of month 4 revenue + 30% of month 3 + 20% of month 2.';
   lagPanel.appendChild(hint);
  }
  function updateLagTotal(){const el=document.getElementById('lag-total-'+item.id);if(!el)return;const t=(item.lagSchedule||[]).reduce((s,e)=>s+(parseFloat(e.pct)||0),0);el.textContent='Total: '+t.toFixed(1)+'% '+(Math.abs(t-100)<0.01?'✓':'← must equal 100%');el.style.color=Math.abs(t-100)<0.01?'#2E844A':'#BA0517';el.style.background=Math.abs(t-100)<0.01?'#eafaea':'#fef1f1';}
  renderLagPanel();
  cfBody2.appendChild(lagPanel);

  // Terms panel
  const termsPanel=document.createElement('div');
  termsPanel.style.cssText='display:'+((item.cfScheduleMode||'terms')==='terms'?'grid':'none')+';grid-template-columns:1fr 1fr;gap:8px';
  const tFld1=document.createElement('div'); tFld1.className='fld'; tFld1.style.margin='0';
  const tLbl1=document.createElement('label'); tLbl1.textContent='Payment terms (days)'; tLbl1.style.fontSize='10px';
  const tInp1=document.createElement('input'); tInp1.type='number'; tInp1.min='0'; tInp1.placeholder='e.g. 30, 60, 90'; tInp1.value=item.payCond||'';
  tFld1.appendChild(tLbl1); tFld1.appendChild(tInp1);
  const tFld2=document.createElement('div'); tFld2.className='fld'; tFld2.style.margin='0';
  const tLbl2=document.createElement('label'); tLbl2.textContent='Advance % (on month 1)'; tLbl2.style.fontSize='10px';
  const tInp2=document.createElement('input'); tInp2.type='number'; tInp2.min='0'; tInp2.max='100'; tInp2.placeholder='e.g. 30'; tInp2.value=item.adv||'';
  tFld2.appendChild(tLbl2); tFld2.appendChild(tInp2);
  termsPanel.appendChild(tFld1); termsPanel.appendChild(tFld2);
  tInp1.addEventListener('input',()=>{item.payCond=tInp1.value;});
  tInp2.addEventListener('input',()=>{item.adv=tInp2.value;});
  cfBody2.appendChild(termsPanel);

  // IGV note
  const igvNote=document.createElement('div');
  igvNote.style.cssText='font-size:10px;padding:6px 10px;border-radius:6px;background:#fff8e8;border-left:3px solid #A56105;color:#7a5000;display:'+(item.igv?'block':'none');
  igvNote.innerHTML='⚠ IGV marked → CF amounts × 1.18 (18% IGV on cash collected). P&L stays ex-VAT.';
  cfBody2.appendChild(igvNote);
  cfSec.appendChild(cfBody2); body.appendChild(cfSec);

  cfModeRow.querySelectorAll('button[data-cf-mode]').forEach(btn => {
   btn.addEventListener('click',()=>{
    item.cfScheduleMode=btn.dataset.cfMode;
    cfModeRow.querySelectorAll('button[data-cf-mode]').forEach(b=>{const a=b.dataset.cfMode===item.cfScheduleMode;b.style.borderColor=a?'#2E844A':'#d0d5dd';b.style.background=a?'#2E844A':'#fff';b.style.color=a?'#fff':'#555';});
    lagPanel.style.display=item.cfScheduleMode==='lag'?'block':'none';
    termsPanel.style.display=item.cfScheduleMode==='terms'?'grid':'none';
   });
  });
  igvChk.addEventListener('change',()=>{igvNote.style.display=igvChk.checked?'block':'none';});
