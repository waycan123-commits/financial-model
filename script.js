...
function fmt(v, blankZero) {
 // Use a real en dash for empty/zero values. The previous file had a
 // hidden control character here, which is why zeros looked like odd marks.
 const dash = '–';
 const n = Number(v);
 if (v === undefined || v === null || Number.isNaN(n)) return dash;
 if ((blankZero !== false) && Math.abs(n) < 0.005) return dash;
