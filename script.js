...
function buildItemCard(item, idx, type) {
 ...
 if (!isCost) {
  ...
  // Helper: get linked item name for display
  function getLinkedName() {
   if (!item.linkedCostId) return '';
   const list = item.linkedCostType==='capex' ? (capex||[]) : (opex||[]);
   const found = list.find(c=>c.id===item.linkedCostId);
   return found ? (found.name||'Unnamed') + (found.cat?' ['+found.cat+']':'') : '(item deleted)';
  }
  ...
  const recHdrDiv = document.createElement('div');
  recHdrDiv.style.cssText = 'display:flex;align-items:center;gap:8px;padding:10px 14px;background:linear-gradient(90deg,#eaf3fd,#f5f9ff)';
  recHdrDiv.innerHTML = '<span style="font-size:15px">📊</span>'
   + '<div style="flex:1">'
   + '<div style="font-size:12px;font-weight:700;color:#032D60">P&L Recognition</div>'
   + '<div style="font-size:10px;color:#5a7fa0;margin-top:1px">When is revenue recognized in the income statement?</div>'
   + '</div>';
  recSec.appendChild(recHdrDiv);

  const recBodyDiv = document.createElement('div');
  recBodyDiv.style.cssText = 'padding:12px 14px;display:grid;gap:10px';

  // Mode toggle pills
  const modeWrap = document.createElement('div');
  modeWrap.style.cssText = 'display:grid;grid-template-columns:1fr 1fr;gap:8px';

  function makeModePill(label, desc, value, active) {
   const pill = document.createElement('div');
   pill.style.cssText = 'padding:10px 12px;border-radius:8px;cursor:pointer;border:2px solid '
    + (active?'#0176D3;background:#e8f4fd':'#e2e8f0;background:#fff')
    + ';transition:all .15s';
   pill.dataset.mode = value;
   pill.innerHTML = '<div style="font-size:11px;font-weight:700;color:'+(active?'#0176D3':'#374151')+'">'
    + label + '</div>'
    + '<div style="font-size:9px;color:'+(active?'#0176D3':'#9ca3af')+';margin-top:2px;line-height:1.4">'
    + desc + '</div>';
   pill.addEventListener('click', () => {
    const toLinked = value === 'linked';
    if (!toLinked) {
     item.linkedCostId = null; item.linkedCostType = null;
    }
    modeWrap.querySelectorAll('[data-mode]').forEach(p => {
     const a = p.dataset.mode === value;
     p.style.borderColor = a?'#0176D3':'#e2e8f0';
     p.style.background = a?'#e8f4fd':'#fff';
     p.querySelector('div').style.color = a?'#0176D3':'#374151';
    });
    linkedPanel.style.display = toLinked ? 'grid' : 'none';
    manualNote.style.display = toLinked ? 'none' : 'block';
    linkedPreview.style.display = 'none';
    if (!toLinked) refresh();
   });
   return pill;
  }
  modeWrap.appendChild(makeModePill('✏️ Manual', 'You define % per month in the distribution grid', 'manual', !isLinkedMode));
  modeWrap.appendChild(makeModePill('🔗 Follow a Cost Item', 'Revenue recognized as % of a CAPEX/OPEX executes', 'linked', isLinkedMode));
  recBodyDiv.appendChild(modeWrap);

  // Manual note
  const manualNote = document.createElement('div');
  manualNote.style.cssText = 'font-size:10px;color:#64748b;background:#f8fafc;padding:8px 10px;border-radius:6px;border-left:3px solid #94a3b8;display:'+(isLinkedMode?'none':'block');
  manualNote.innerHTML = '📝 Set the <strong>% per month</strong> in the monthly distribution grid below. Make sure totals sum to 100%.';
  recBodyDiv.appendChild(manualNote);

  // Linked picker panel
  const linkedPanel = document.createElement('div');
  linkedPanel.style.cssText = 'display:'+(isLinkedMode?'grid':'none')+';grid-template-columns:1fr 1fr;gap:8px;align-items:end';

  const typeFld = document.createElement('div'); typeFld.className='fld'; typeFld.style.margin='0';
  const typeLbl = document.createElement('label'); typeLbl.textContent='Cost list'; typeLbl.style.fontSize='10px';
  const typeSel = document.createElement('select');
  typeSel.style.cssText = 'font-weight:600';
  ['capex','opex'].forEach(t=>{
   const o=document.createElement('option'); o.value=t; o.textContent=t==='capex'?'CAPEX Items':'OPEX Items';
   if((item.linkedCostType||'capex')===t) o.selected=true; typeSel.appendChild(o);
  });
  typeFld.appendChild(typeLbl); typeFld.appendChild(typeSel);

  const itemFld = document.createElement('div'); itemFld.className='fld'; itemFld.style.margin='0';
  const itemLbl = document.createElement('label'); itemLbl.textContent='Cost item to follow'; itemLbl.style.fontSize='10px';
  const itemSel = document.createElement('select');

  function fillItems(tp) {
   itemSel.innerHTML='<option value="">— pick a cost item —</option>';
   const list = tp==='capex'?capex:opex;
   if (!list || !list.length) {
    const o=document.createElement('option'); o.value=''; o.textContent='No '+tp.toUpperCase()+' items yet';
    itemSel.appendChild(o); return;
   }
   list.forEach(c=>{
    const o=document.createElement('option'); o.value=c.id;
    o.textContent=(c.name||'Unnamed') + (c.cat?' ['+c.cat+']':'');
    if(c.id===item.linkedCostId) o.selected=true;
    itemSel.appendChild(o);
   });
  }
  fillItems(item.linkedCostType||'capex');
  itemFld.appendChild(itemLbl); itemFld.appendChild(itemSel);
  linkedPanel.appendChild(typeFld); linkedPanel.appendChild(itemFld);
  recBodyDiv.appendChild(linkedPanel);

  // Preview banner (shown after selecting a linked item)
  const linkedPreview = document.createElement('div');
  linkedPreview.style.cssText = 'display:none;font-size:10px;border-radius:6px;overflow:hidden';

  function updateLinkedPreview() {
   if (!item.linkedCostId) { linkedPreview.style.display='none'; return; }
   const tp = item.linkedCostType||'capex';
   const list = tp==='capex'?capex:opex;
   const linked = (list||[]).find(c=>c.id===item.linkedCostId);
   if (!linked) { linkedPreview.style.display='none'; return; }
   const tmpLinked = Object.assign({},linked,{linkedCostId:null});
   // getDistArr might not be callable here during render, use mos
   const hasDist = linked.progMode || Object.values(linked.mos||{}).some(v=>v&&parseFloat(v)>0);
   linkedPreview.style.display = 'block';
   linkedPreview.innerHTML = hasDist
    ? '<div style="background:#eafaea;padding:8px 12px;border-left:3px solid #2E844A;color:#064A0C">'
     + '✅ <strong>Linked to: '+(linked.name||'Unnamed')+'</strong>'
     + '<div style="margin-top:3px;opacity:.8">Revenue will be recognized proportionally to this cost\'s monthly curve. The distribution grid below is driven by this link.</div>'
     + '</div>'
    : '<div style="background:#fff8e8;padding:8px 12px;border-left:3px solid #A56105;color:#7a5000">'
     + '⚠️ <strong>'+(linked.name||'Unnamed')+'</strong> has no monthly distribution yet.'
     + '<div style="margin-top:3px">Go to CAPEX/OPEX and set the monthly % for that item first. Until then, revenue will be spread evenly.</div>'
     + '</div>';
  }
  updateLinkedPreview();
  recBodyDiv.appendChild(linkedPreview);
  recSec.appendChild(recBodyDiv);
  body.appendChild(recSec);

  // Wire type selector
  typeSel.addEventListener('change', ()=>{
   item.linkedCostType = typeSel.value;
   item.linkedCostId = null;
   itemSel.value = '';
   fillItems(typeSel.value);
   updateLinkedPreview();
   refresh();
  });
  // Wire item selector
  itemSel.addEventListener('change', ()=>{
   item.linkedCostId = itemSel.value || null;
   item.linkedCostType = typeSel.value;
   updateLinkedPreview();
   refresh();
  });
  ...
  const cfHdrDiv = document.createElement('div');
  cfHdrDiv.style.cssText = 'display:flex;align-items:center;gap:8px;padding:10px 14px;background:linear-gradient(90deg,#eafaea,#f5fff5)';
  cfHdrDiv.innerHTML = '<span style="font-size:15px">💵</span>'
   + '<div style="flex:1">'
   + '<div style="font-size:12px;font-weight:700;color:#064A0C">Cash Flow Collection</div>'
   + '<div style="font-size:10px;color:#4a7a4a;margin-top:1px">When does cash actually hit your bank account?</div>'
   + '</div>';
  cfSec.appendChild(cfHdrDiv);

  const cfBodyDiv = document.createElement('div');
  cfBodyDiv.style.cssText = 'padding:12px 14px;display:grid;gap:10px';

  // Mode buttons
  const cfModeWrap = document.createElement('div'); cfModeWrap.style.cssText='display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px';
  const cfModes = [
   {v:'terms', icon:'📆', label:'Payment Terms', desc:'e.g. 30/60/90 days credit'},
   {v:'lag',   icon:'📅', label:'Lag Schedule',  desc:'Split by month lag %'},
   {v:'map',   icon:'🗓', label:'Month Map',     desc:'Map accrual → exact pay month'},
  ];
  const curCFMode = item.cfScheduleMode||'terms';
  cfModes.forEach(({v,icon,label,desc})=>{
   const btn=document.createElement('div');
   const a=curCFMode===v;
   btn.style.cssText='padding:8px 10px;border-radius:8px;cursor:pointer;border:2px solid '+(a?'#2E844A;background:#eafaea':'#e2e8f0;background:#fff')+';transition:all .15s;text-align:center';
   btn.innerHTML='<div style="font-size:14px">'+icon+'</div>'
    +'<div style="font-size:10px;font-weight:700;color:'+(a?'#064A0C':'#374151')+'">'+label+'</div>'
    +'<div style="font-size:9px;color:#9ca3af;margin-top:1px">'+desc+'</div>';
   btn.addEventListener('click',()=>{
    item.cfScheduleMode=v;
    cfModeWrap.querySelectorAll('[data-cfmode]').forEach(b=>{
     const ba=b.dataset.cfmode===v;
     b.style.borderColor=ba?'#2E844A':'#e2e8f0';
     b.style.background=ba?'#eafaea':'#fff';
    });
    termsP.style.display=v==='terms'?'grid':'none';
    lagP.style.display=v==='lag'?'block':'none';
    mapNote.style.display=v==='map'?'block':'none';
   });
   btn.dataset.cfmode=v;
   cfModeWrap.appendChild(btn);
  });
  cfBodyDiv.appendChild(cfModeWrap);

  // ── Terms panel
  const termsP = document.createElement('div');
  termsP.style.cssText='display:'+(curCFMode==='terms'?'grid':'none')+';grid-template-columns:1fr 1fr;gap:8px';
  function termsFld(lbl,hint,inp) {
   const d=document.createElement('div'); d.className='fld'; d.style.margin='0';
   const l=document.createElement('label'); l.style.cssText='font-size:10px;display:block';
   l.innerHTML='<strong>'+lbl+'</strong><span style="font-weight:400;color:#9ca3af;display:block;margin-top:1px">'+hint+'</span>';
  ...
  termsP.appendChild(termsFld('Credit Days','30=1mo, 60=2mo, 90=3mo',t1));
  termsP.appendChild(termsFld('Advance %','Paid upfront on month 1',t2));
  t1.addEventListener('input',()=>{item.payCond=t1.value;});
  t2.addEventListener('input',()=>{item.adv=t2.value;});
  cfBodyDiv.appendChild(termsP);

  // ── Lag panel
  const lagP = document.createElement('div');
  lagP.style.cssText='display:'+(curCFMode==='lag'?'block':'none');
  function buildLagUI() {
   lagP.innerHTML='';
   const sched = item.lagSchedule && item.lagSchedule.length ? item.lagSchedule : [{lag:0,pct:100}];
   item.lagSchedule = sched;
   const colHdr=document.createElement('div');
   colHdr.style.cssText='display:grid;grid-template-columns:1fr 1fr 32px;gap:6px;margin-bottom:6px;padding:0 2px';
   colHdr.innerHTML='<span style="font-size:9px;font-weight:700;color:#64748b;text-transform:uppercase">Lag (months after recognition)</span>'
    +'<span style="font-size:9px;font-weight:700;color:#64748b;text-transform:uppercase">% of recognized amount</span><span></span>';
   lagP.appendChild(colHdr);
   sched.forEach((entry,ei)=>{
    const row=document.createElement('div');
    row.style.cssText='display:grid;grid-template-columns:1fr 1fr 32px;gap:6px;align-items:center;margin-bottom:4px';
    const lagI=document.createElement('input'); lagI.type='number'; lagI.min='0'; lagI.max='24'; lagI.value=entry.lag??0;
    lagI.style.cssText='text-align:center;font-weight:600';
    lagI.placeholder='0=same month';
    const pctI=document.createElement('input'); pctI.type='number'; pctI.min='0'; pctI.max='100'; pctI.value=entry.pct??0;
    pctI.style.cssText='text-align:center;font-weight:600';
    pctI.placeholder='e.g. 50';
    const del=document.createElement('button'); del.textContent='✕';
    del.style.cssText='background:none;border:none;color:#BA0517;cursor:pointer;font-size:13px;padding:2px';
    del.title='Remove row';
    lagI.addEventListener('input',()=>{entry.lag=parseInt(lagI.value)||0;checkLagTotal();});
    pctI.addEventListener('input',()=>{entry.pct=parseFloat(pctI.value)||0;checkLagTotal();});
    del.addEventListener('click',()=>{sched.splice(ei,1);buildLagUI();});
    row.appendChild(lagI); row.appendChild(pctI); row.appendChild(del);
    lagP.appendChild(row);
   });
   const total=sched.reduce((s,e)=>s+(parseFloat(e.pct)||0),0);
   const ok=Math.abs(total-100)<0.01;
   const totalBar=document.createElement('div');
   totalBar.id='lag-total-'+item.id;
   totalBar.style.cssText='padding:5px 10px;border-radius:5px;font-size:10px;font-weight:700;margin-top:4px;'+(ok?'background:#eafaea;color:#2E844A':'background:#fef1f1;color:#BA0517');
   totalBar.textContent='Total: '+total.toFixed(1)+'% '+(ok?'✓':'← needs to equal 100%');
   lagP.appendChild(totalBar);
   const addR=document.createElement('button'); addR.textContent='+ Add row';
   addR.style.cssText='font-size:10px;margin-top:8px;color:#0176D3;background:none;border:1px dashed #0176D3;border-radius:4px;padding:3px 12px;cursor:pointer';
   addR.addEventListener('click',()=>{sched.push({lag:sched.length,pct:0});buildLagUI();});
   lagP.appendChild(addR);
   const ex=document.createElement('div');
   ex.style.cssText='margin-top:8px;font-size:10px;color:#64748b;background:#f8fafc;padding:8px 10px;border-radius:6px;line-height:1.6';
   ex.innerHTML='<strong>💡 Example:</strong> lag=0→50%, lag=1→30%, lag=2→20%<br>'
    +'In month 4: you collect 50% of month-4 revenue + 30% of month-3 + 20% of month-2.';
   lagP.appendChild(ex);
  }
  function checkLagTotal(){
   const el=document.getElementById('lag-total-'+item.id); if(!el)return;
   const t=(item.lagSchedule||[]).reduce((s,e)=>s+(parseFloat(e.pct)||0),0);
   const ok=Math.abs(t-100)<0.01;
   el.textContent='Total: '+t.toFixed(1)+'% '+(ok?'✓':'← needs to equal 100%');
   el.style.background=ok?'#eafaea':'#fef1f1'; el.style.color=ok?'#2E844A':'#BA0517';
  }
  buildLagUI();
  cfBodyDiv.appendChild(lagP);

  // ── Month map note
  const mapNote=document.createElement('div');
  mapNote.style.cssText='display:'+(curCFMode==='map'?'block':'none')+';font-size:10px;background:#f8fafc;padding:8px 10px;border-radius:6px;color:#374151;line-height:1.6';
  mapNote.innerHTML='<strong>🗓 Month Map mode</strong><br>Set the collection schedule in the distribution grid below. '
   +'Each month\'s row will let you specify which accrual months get paid. This mode is for complex billing schedules.';
  cfBodyDiv.appendChild(mapNote);

  // IGV note
  const igvCFNote=document.createElement('div');
  igvCFNote.style.cssText='font-size:10px;padding:7px 10px;border-radius:6px;background:#fff8e8;border-left:3px solid #A56105;color:#7a5000;line-height:1.5;display:'+(item.igv?'block':'none');
  igvCFNote.innerHTML='⚠️ <strong>IGV applied to CF:</strong> Cash collections will include 18% IGV (× 1.18). P&L always stays ex-VAT.';
  cfBodyDiv.appendChild(igvCFNote);

  cfSec.appendChild(cfBodyDiv);
  body.appendChild(cfSec);
  igvChk.addEventListener('change',()=>{igvCFNote.style.display=igvChk.checked?'block':'none';});
