...
function signedAdjArr(ac) {
 const arr = (ac && ac.arrCF) ? ac.arrCF : Z();
 return ac && ac.adj && ac.adj.sign === 'add' ? arr : arr.map(v => -v);
}

function cfCustomLineRows(cv, sections) {
 return customLines
...
// Excel-style row builders for P&L and Cash Flow totals.
// P&L displays costs as negative values, so totals are calculated from the same displayed rows.
function plIncomeRows(cv) {
  return [
    ...Object.values(cv.revCats || {}),
    cv.penArr || Z(),
    ...(cv.adjIncomeRows || []).map(ac => ac.arr.map(v => ac.adj.sign === 'add' ? v : -v)),
    ...customLines
      .filter(l => l.section === 'income_section' && l.formula && l.formula.trim())
      .map(l => evalCustomLineArr(l, cv))
  ];
}
function plCogsRows(cv) {
  return [
    ...(cv.userCats || []).map(c => (cv.costCat[c] || Z()).map(v => -v)),
    (cv.costCat['Contingency'] || Z()).map(v => -v),
    ...(cv.adjCostRows || []).map(ac => ac.arr.map(v => -(ac.adj.sign === 'add' ? v : -v))),
    ...customLines
      .filter(l => l.section === 'cogs_section' && l.formula && l.formula.trim())
      .map(l => evalCustomLineArr(l, cv))
  ];
}
function plBondRows(cv) {
  return [
    (cv.finExpCB || Z()).map(v => -v),
    (cv.finExpUB || Z()).map(v => -v),
    ...customLines
      .filter(l => l.section === 'after_gross_profit' && l.formula && l.formula.trim())
      .map(l => evalCustomLineArr(l, cv))
  ];
}
function plOpexRows(cv) {
  return [
    (cv.opexBuffer || Z()).map(v => -v),
    (cv.hedgingArr || Z()).map(v => -v),
    (cv.finCostArr || Z()).map(v => -v),
    ...customLines
      .filter(l => ['after_profit_b_opex','after_ebit'].includes(l.section) && l.formula && l.formula.trim())
      .map(l => evalCustomLineArr(l, cv))
  ];
}
function cfInflowRows(cv) {
  const inflowCustomRows = cfCustomLineRows(cv, ['income_section','cf_after_inflows']);
  return [
    ...Object.values(cv.revCatsCF || {}),
    cv.penArrCF || Z(),
    ...(cv.adjIncomeRows || []).map(ac => signedAdjArr(ac)),
    cv.vatIn || Z(),
    ...inflowCustomRows.map(r => r.arr)
  ];
}
function cfOutflowRows(cv) {
  const outflowCustomRows = cfCustomLineRows(cv, ['cogs_section','cf_after_outflows']);
  const vatNet = (cv.vatIn || Z()).map((v, i) => v - ((cv.vatOut || Z())[i] || 0));
  return [
    ...(cv.userCats || []).map(c => (cv.costCatCF[c] || Z())),
    cv.costCatCF['Contingency'] || Z(),
    ...(cv.adjCostRows || []).map(ac => signedAdjArr(ac)),
    ...outflowCustomRows.map(r => r.arr),
    cv.finExpCB || Z(),
    cv.finExpUB || Z(),
    cv.opexBufferCF || Z(),
    cv.hedgingArr || Z(),
...
function renderGroupedPLCF(which, cv) {
 ...
 if (which === 'pl') {
   const totalIncome = sumArrs(plIncomeRows(cv));
   const totalCogs = sumArrs(plCogsRows(cv));
   const grossProfit = totalIncome.map((v, i) => v + totalCogs[i]);
   const bondFees = sumArrs(plBondRows(cv));
   const profitBeforeOpex = grossProfit.map((v, i) => v + bondFees[i]);
   const opexInterest = sumArrs(plOpexRows(cv));
   const ebit = profitBeforeOpex.map((v, i) => v + opexInterest[i]);
   ...
   body += mkRow('Penalties', cv.penArr || Z(), {ind:true, color:'amber'});
   (cv.adjIncomeRows || []).forEach(ac => {
     body += mkRow((ac.adj.name||'Adj')+' ('+ac.adj.pct+'%)', ac.arr.map(v=>ac.adj.sign==='add'?v:-v), {ind:true,color:'amber'});
   });
   customLinesFor('income_section', cv, mkRow).trim() && (body += customLinesFor('income_section', cv, mkRow));
   body += mkRow('Total Income', totalIncome, {bold:true, color:'teal'});

   // COGS
   body += mkSec('COGS', 'var(--th-cogs)');
   (cv.userCats || []).forEach(cat => body += mkRow(cat, (cv.costCat[cat]||Z()).map(v=>-v), {ind:true}));
   body += mkRow('Contingency ('+CFG.con+'%)', (cv.costCat['Contingency']||Z()).map(v=>-v), {ind:true});
   (cv.adjCostRows || []).forEach(ac => {
     body += mkRow((ac.adj.name||'Adj')+' ('+ac.adj.pct+'%)', ac.arr.map(v=>-(ac.adj.sign==='add'?v:-v)), {ind:true,color:'amber'});
   });
   customLinesFor('cogs_section', cv, mkRow).trim() && (body += customLinesFor('cogs_section', cv, mkRow));
   body += mkRow('Total COGS', totalCogs, {bold:true});

   body += mkRow('Gross Profit', grossProfit, {bold:true, color:'blue'});
   customLinesFor('after_gross_profit', cv, mkRow).trim() && (body += customLinesFor('after_gross_profit', cv, mkRow));

   // BOND FEES
   body += mkSec('BOND FEES', 'var(--th-bond)');
   body += mkRow('Compliance Bond Fee', (cv.finExpCB||Z()).map(v=>-v), {ind:true, color:'amber'});
   body += mkRow('Upfront Bond Fee', (cv.finExpUB||Z()).map(v=>-v), {ind:true, color:'amber'});
   body += mkRow('Profit before OPEX / Interest', profitBeforeOpex, {bold:true, color:'blue'});
   customLinesFor('after_profit_b_opex', cv, mkRow).trim() && (body += customLinesFor('after_profit_b_opex', cv, mkRow));

   // OPEX & INTEREST
   body += mkSec('OPEX & INTEREST', 'var(--th-opex)');
   body += mkRow('OPEX Buffer ('+(CFG.opxbuf??30)+'% of Revenue)', (cv.opexBuffer||Z()).map(v=>-v), {ind:true, color:'amber'});
   body += mkRow('Hedging Adjustment (monthly)', (cv.hedgingArr||Z()).map(v=>-v), {ind:true});
   body += mkRow('Financing Cost (on neg. CF)', (cv.finCostArr||Z()).map(v=>-v), {ind:true, color:'amber'});
   customLinesFor('after_ebit', cv, mkRow).trim() && (body += customLinesFor('after_ebit', cv, mkRow));
   body += mkRow('EBIT', ebit, {bold:true, color:'blue'});
   body += mkRow('EBITDA', ebitda, {bold:true, color:'teal'});

 } else {
   // CASH FLOW — same structure as the Excel reference
   const inflowCustomRows = cfCustomLineRows(cv, ['income_section','cf_after_inflows']);
   const outflowCustomRows = cfCustomLineRows(cv, ['cogs_section','cf_after_outflows']);
   const adjIncomeRowsCF = (cv.adjIncomeRows || []).map(ac => ({ ac, arr: signedAdjArr(ac) }));
   const adjCostRowsCF = (cv.adjCostRows || []).map(ac => ({ ac, arr: signedAdjArr(ac) }));
   const vatNet = (cv.vatIn || Z()).map((v, i) => v - ((cv.vatOut || Z())[i] || 0));
   const totInflow = sumArrs(cfInflowRows(cv));
   const totOutflow = sumArrs(cfOutflowRows(cv));
   ...
   body += mkRow('Penalties', cv.penArrCF || Z(), {ind:true,color:'amber'});
   adjIncomeRowsCF.forEach(({ac, arr}) => body += mkRow((ac.adj.name||'Adj')+' ('+ac.adj.pct+'%)', arr, {ind:true,color:'amber'}));
   inflowCustomRows.forEach(({line, arr}) => body += mkRow(line.name||'Custom', arr, {ind:true,color:line.color||'amber'}));
   body += mkRow('VAT Inflow', cv.vatIn || Z(), {ind:true,color:'teal'});
   ...
   adjCostRowsCF.forEach(({ac, arr}) => body += mkRow((ac.adj.name||'Adj')+' ('+ac.adj.pct+'%)', arr, {ind:true,color:'amber'}));
   outflowCustomRows.filter(r => r.line.section === 'cogs_section').forEach(({line, arr}) => body += mkRow(line.name||'Custom', arr, {ind:true,color:line.color||'amber'}));
   body += mkRow('Compliance Bond Fee', cv.finExpCB || Z(), {ind:true,color:'amber'});
   body += mkRow('Upfront Bond Fee', cv.finExpUB || Z(), {ind:true,color:'amber'});
   body += mkRow('OPEX Buffer ('+(CFG.opxbuf??30)+'%)', cv.opexBufferCF || Z(), {ind:true,color:'amber'});
   body += mkRow('Hedging', cv.hedgingArr || Z(), {ind:true,color:'purple'});
   body += mkRow('Financing Cost', cv.finCostArr || Z(), {ind:true,color:'amber'});
   outflowCustomRows.filter(r => r.line.section === 'cf_after_outflows').forEach(({line, arr}) => body += mkRow(line.name||'Custom', arr, {ind:true,color:line.color||'amber'}));
   body += mkRow('VAT Outflow', cv.vatOut || Z(), {ind:true,color:'red'});
   body += mkRow('VAT NET to pay to SUNAT', vatNet, {ind:true});
   body += mkRow('Total Outflows', totOutflow, {bold:true});

   body += mkSec('NET CASH FLOW From OPERATIONS', 'var(--th-bond)');
   body += mkRow('NET CASH FLOW From OPERATIONS', netCFcheck, {bold:true,color:'blue'});
   body += mkRow('Accumulated cash flow', accumCFcheck, {bold:true,color:'purple'});
 }
...
function renderPL() {
 ...
 case 'pl_total_cogs':
 return sumRow('Total COGS', sumArrs(plCogsRows(cv)), 'var(--th-cogs)')
 + addLineRow('cogs_section', nc)
 + customLinesFor('cogs_section', cv, (lbl,arr,opts) => dataRow(lbl, arr, view, opts));
 case 'pl_gross_profit':
 { const totalIncome = sumArrs(plIncomeRows(cv)); const totalCogs = sumArrs(plCogsRows(cv)); return sumRow('Gross Profit', totalIncome.map((v,i)=>v+totalCogs[i]), 'var(--accent)', totalIncome) }
 ...
 { const totalIncome=sumArrs(plIncomeRows(cv)); const totalCogs=sumArrs(plCogsRows(cv)); const gp=totalIncome.map((v,i)=>v+totalCogs[i]); const bonds=sumArrs(plBondRows(cv)); return sumRow('Profit before OPEX / Interest', gp.map((v,i)=>v+bonds[i]), 'var(--accent)', totalIncome) }
 ...
 + (() => { const totalIncome=sumArrs(plIncomeRows(cv)); const totalCogs=sumArrs(plCogsRows(cv)); const gp=totalIncome.map((v,i)=>v+totalCogs[i]); const bonds=sumArrs(plBondRows(cv)); const pbo=gp.map((v,i)=>v+bonds[i]); const opex=sumArrs(plOpexRows(cv)); return sumRow('EBIT', pbo.map((v,i)=>v+opex[i]), 'var(--accent)', totalIncome); })();
 case 'pl_ebitda':
 { const totalIncome=sumArrs(plIncomeRows(cv)); const totalCogs=sumArrs(plCogsRows(cv)); const gp=totalIncome.map((v,i)=>v+totalCogs[i]); const bonds=sumArrs(plBondRows(cv)); const pbo=gp.map((v,i)=>v+bonds[i]); const opex=sumArrs(plOpexRows(cv)); return sumRow('EBITDA', pbo.map((v,i)=>v+opex[i]), 'var(--teal)', totalIncome); }
 default: return '';
 }
 }
...
function renderCF() {
 ...
 // Totals are now calculated from the exact rows displayed in the CF table.
 // This keeps Total Outflows in sync when adjustments/custom lines are used.
 const totInflow = sumArrs(cfInflowRows(cv));
 const totOutflow = sumArrs(cfOutflowRows(cv));
 const netCFcheck = totInflow.map((v, i) => v - totOutflow[i]);
 ...
 adjIncomeRowsCF.forEach(({ac, arr}) => {
 h += cfDataRow((ac.adj.name||'Adj') + ' (' + (ac.adj.pct||0) + '%)', arr, {ind:true, color:'amber'});
 });
 inflowCustomRows.forEach(({line, arr}) => {
 h += cfDataRow(line.name||'Custom', arr, {ind:true, color:line.color||'amber'});
 });
 h += cfDataRow('VAT Inflow', cv.vatIn, {ind:true, color:'teal'});
 ...
 adjCostRowsCF.forEach(({ac, arr}) => {
 h += cfDataRow((ac.adj.name||'Adj') + ' (' + (ac.adj.pct||0) + '%)', arr, {ind:true, color:'amber'});
 });
 outflowCustomRows.filter(r => r.line.section === 'cogs_section').forEach(({line, arr}) => {
 h += cfDataRow(line.name||'Custom', arr, {ind:true, color:line.color||'amber'});
 });
 ...
 outflowCustomRows.filter(r => r.line.section === 'cf_after_outflows').forEach(({line, arr}) => {
 h += cfDataRow(line.name||'Custom', arr, {ind:true, color:line.color||'amber'});
 });
 ...
 //  NET CF 
 h += secRow('NET CASH FLOW From OPERATIONS', 'var(--th-bond)');
 ...
 h += cfDataRow('NET CASH FLOW From OPERATIONS', netCFcheck, {bold:true});
 h += cfDataRow('Accumulated cash flow', accumCFcheck, {bold:true, acc:true});
...
document.getElementById('exp-btn').addEventListener('click', () => {
 ...
 ['Total Outflows', cv.totalCogsCFAdj.map((v,i)=>v + (cv.opexBufferCF[i]||0) + (cv.finExpCB[i]||0) + (cv.finExpUB[i]||0) + (cv.hedgingArr[i]||0) + (cv.finCostArr[i]||0) + (cv.vatOut[i]||0))],
