...
function renderPL() {
  ...
  function cogsExpandRow(cat, catArr, catItems) {
    ...
    const dispArr = catArr.map(v => -v);   // flip sign for display: cost→negative, credit→positive
    ...
    if (catItems.length > 0) {
      h += '<tr class="pl-expandable" data-expand="' + rowId + '">'
        ...
        + '</tr>';
      catItems.forEach(it => {
        // Individual item: negate for display (cost→negative, credit stays positive)
...
const STORAGE_KEY = 'fm_portfolio_v1';
let portfolio = [];       // array of saved project snapshots
...
function loadPortfolio() {
  try { portfolio = JSON.parse(localStorage.getItem(STORAGE_KEY) || '[]'); }
  catch(e) { portfolio = []; }
}
...
document.getElementById('save-confirm').addEventListener('click', () => {
  ...
  if (currentProjectId) {
    ...
    if (idx >= 0) {
      portfolio[idx] = { ...portfolio[idx], name, classification:cls, description:desc, status, updatedAt:now, snapshot:snap, data:payload };
    }
  } else {
    currentProjectId = Date.now();
    portfolio.push({ id:currentProjectId, name, classification:cls, description:desc, status, createdAt:now, updatedAt:now, snapshot:snap, data:payload });
  }
  savePortfolio();
  closeSaveModal();
  ...
  reader.onload = ev => {
    try {
      ...
      if (proj.data && proj.name) {
        ...
        proj.id = proj.id || Date.now();
        proj.updatedAt = new Date().toISOString();
        if (!proj.snapshot && proj.data) {
          // Reconstruct snapshot by loading and computing
          loadProjectPayload(proj.data);
          proj.snapshot = buildSnapshot();
          loadPortfolio(); // restore
        }
        loadPortfolio();
        const existing = portfolio.findIndex(p => p.id === proj.id);
        if (existing >= 0) portfolio[existing] = proj; else portfolio.push(proj);
        savePortfolio();
        renderPortfolio();
