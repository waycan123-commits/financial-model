...
const CUSTOM_LINE_SECTIONS = [
 { value: 'income_section', label: 'P&L + CF — Income / Inflows (before Total Income)', pl: true, cf: true },
 { value: 'cogs_section', label: 'P&L + CF — COGS / Outflows (before Total COGS)', pl: true, cf: true },
 { value: 'after_gross_profit', label: 'P&L + CF — After Gross Profit / Outflow link', pl: true, cf: true },
 { value: 'after_profit_b_opex', label: 'P&L + CF — After Profit before OPEX / Outflow link', pl: true, cf: true },
 { value: 'after_ebit', label: 'P&L + CF — After EBIT / Outflow link', pl: true, cf: true },
 { value: 'cf_after_inflows', label: 'CF only — After Total Inflows', pl: false, cf: true },
 { value: 'cf_after_outflows', label: 'CF only — After Total Outflows', pl: false, cf: true },
];
...
function buildReportFigures(cv) {
 const zeroArr = () => new Array(PM()).fill(0);
 const sumRows = rows => {
   const out = zeroArr();
   rows.filter(Boolean).forEach(arr => (arr || zeroArr()).forEach((v,i)=>{ out[i] += Number(v) || 0; }));
   return out;
 };
 const customRows = (sections, onlyCF=false) => customLines
   .filter(l => (!onlyCF || l.showInCF) && sections.includes(l.section) && l.formula && l.formula.trim())
   .map(line => ({ line, arr: evalCustomLineArr(line, cv) }));
 const signedAdj = ac => {
   const sign = ac.adj && ac.adj.sign === 'add' ? 1 : -1;
   return (ac.arr || zeroArr()).map(v => sign * (Number(v) || 0));
 };
 const signedAdjCFIncome = ac => {
   const sign = ac.adj && ac.adj.sign === 'add' ? 1 : -1;
   return (ac.arrCF || ac.arr || zeroArr()).map(v => sign * (Number(v) || 0));
 };
 const adjCostPL = ac => (ac.arr || zeroArr()).map(v => -(ac.adj && ac.adj.sign === 'add' ? (Number(v)||0) : -(Number(v)||0)));
 const adjCostCF = ac => (ac.arrCF || ac.arr || zeroArr()).map(v => Math.abs(Number(v) || 0));
 const toOutflow = arr => (arr || zeroArr()).map(v => Math.abs(Number(v) || 0));

 const plIncomeCustom = customRows(['income_section']);
 const plCogsCustom = customRows(['cogs_section']);
 const plAfterGPCustom = customRows(['after_gross_profit']);
 const plAfterProfitCustom = customRows(['after_profit_b_opex']);
 const plAfterEbitCustom = customRows(['after_ebit']);

 const totalIncomePL = sumRows([
   ...Object.values(cv.revCats || {}),
   cv.penArr || zeroArr(),
   ...(cv.adjIncomeRows || []).map(signedAdj),
   ...plIncomeCustom.map(r => r.arr)
 ]);
 const totalCogsPL = sumRows([
   ...((cv.userCats || []).map(c => ((cv.costCat || {})[c] || zeroArr()).map(v => -(Number(v)||0)))),
   ((cv.costCat || {})['Contingency'] || zeroArr()).map(v => -(Number(v)||0)),
   ...(cv.adjCostRows || []).map(adjCostPL),
   ...plCogsCustom.map(r => r.arr)
 ]);
 const grossProfitPL = totalIncomePL.map((v,i) => v + totalCogsPL[i]);
 const bondFeesPL = sumRows([
   (cv.finExpCB || zeroArr()).map(v => -(Number(v)||0)),
   (cv.finExpUB || zeroArr()).map(v => -(Number(v)||0)),
   ...plAfterGPCustom.map(r => r.arr)
 ]);
 const profitBeforeOpexPL = grossProfitPL.map((v,i) => v + bondFeesPL[i]);
 const opexInterestPL = sumRows([
   (cv.opexBuffer || zeroArr()).map(v => -(Number(v)||0)),
   (cv.hedgingArr || zeroArr()).map(v => -(Number(v)||0)),
   (cv.finCostArr || zeroArr()).map(v => -(Number(v)||0)),
   ...plAfterProfitCustom.map(r => r.arr)
 ]);
 const ebitPL = profitBeforeOpexPL.map((v,i) => v + opexInterestPL[i]);
 const ebitdaPL = sumRows([ebitPL, ...plAfterEbitCustom.map(r => r.arr)]);

 const cfAdjIncomeRows = (cv.adjIncomeRows || []).map(ac => ({ ac, arr: signedAdjCFIncome(ac) }));
 const cfAdjCostRows = (cv.adjCostRows || []).map(ac => ({ ac, arr: adjCostCF(ac) }));
 const cfInCustomRows = customRows(['income_section','cf_after_inflows'], true).map(r => ({ ...r, arr: r.arr }));
 const cfOutCustomRows = customRows(['cogs_section','after_gross_profit','after_profit_b_opex','after_ebit','cf_after_outflows'], true)
   .map(r => ({ ...r, arr: toOutflow(r.arr) }));
 const totalInflowArr = sumRows([
   ...Object.values(cv.revCatsCF || {}),
   cv.penArrCF || zeroArr(),
   ...cfAdjIncomeRows.map(r => r.arr),
   cv.vatIn || zeroArr(),
   ...cfInCustomRows.map(r => r.arr)
 ]);
 const totalOutflowArr = sumRows([
   ...((cv.userCats || []).map(c => (cv.costCatCF || {})[c] || zeroArr())),
   (cv.costCatCF || {})['Contingency'] || zeroArr(),
   ...cfAdjCostRows.map(r => r.arr),
   cv.finExpCB || zeroArr(),
   cv.finExpUB || zeroArr(),
   cv.opexBufferCF || zeroArr(),
   cv.hedgingArr || zeroArr(),
   cv.finCostArr || zeroArr(),
   cv.vatOut || zeroArr(),
   ...cfOutCustomRows.map(r => r.arr)
 ]);
 const netCFArr = totalInflowArr.map((v,i) => v - totalOutflowArr[i]);
 const accumCFArr = (() => { let a = 0; return netCFArr.map(v => { a += v; return a; }); })();
 return {
   zeroArr, sumRows,
   plIncomeCustom, plCogsCustom, plAfterGPCustom, plAfterProfitCustom, plAfterEbitCustom,
   totalIncomePL, totalCogsPL, grossProfitPL, bondFeesPL, profitBeforeOpexPL, opexInterestPL, ebitPL, ebitdaPL,
   cfAdjIncomeRows, cfAdjCostRows, cfInCustomRows, cfOutCustomRows,
   totalInflowArr, totalOutflowArr, netCFArr, accumCFArr
 };
}
...
function renderGroupedPLCF(which, cv) {
 ...
 const rpt = buildReportFigures(cv);
 const zeroArr = rpt.zeroArr;
 years.forEach(({yi}) => { if (yearOpen[yi] === undefined) yearOpen[yi] = (yi === 0); });
 function aggYear(arr, yms) { return yms.reduce((s,m) => s + (arr[m-1]||0), 0); }
 function lastYearValue(arr, yms) { return arr[(yms[yms.length - 1] || 1) - 1] || 0; }
 const cols = [];
 years.forEach(({yi, year, yms}) => {
 ...
 function cellVal(arr, col, opts) {
   if (col.type === 'month') return arr[col.m - 1] || 0;
   if (opts && opts.acc) return col.type === 'year' ? lastYearValue(arr, col.yms) : (arr[arr.length - 1] || 0);
   if (col.type === 'year') return aggYear(arr, col.yms);
   return (opts && opts.acc) ? (arr[arr.length - 1] || 0) : rSum(arr);
 }
 function makeHead() {
   let h = '<thead><tr>'
   + '<th style="min-width:220px;text-align:left;position:sticky;left:0;z-index:3;background:var(--navy)">Category</th>';
   cols.forEach(col => {
     if (col.type === 'month') {
       ...
       h += `<th style="min-width:76px;background:var(--navy)"><div style="font-size:10px">${mi.short} ${String(mi.year).slice(2)}</div><div style="color:#5ba4f5;font-size:9px;font-weight:700">(M${col.m})</div></th>`;
     } else if (col.type === 'year') {
       h += `<th class="yr-col-hdr" data-yi="${col.yi}" style="min-width:90px;background:#1a4a80;cursor:pointer;user-select:none" title="Click to ${col.open?'collapse':'expand'} months"><div style="font-size:11px;font-weight:800">${col.year}</div><div style="font-size:8px;opacity:.75;margin-top:2px">${col.open?'collapse':'expand'}</div></th>`;
     } else {
 ...
 function mkRow(label, arr, opts) {
   opts = opts || {};
   const bold = !!opts.bold, cc = opts.color || '', ind = !!opts.ind;
   const stickyBg = bold ? 'var(--alt)' : 'var(--surface)';
   let r = `<tr class="${bold?'bold-row':''}"><td class="lbl${ind?' ind':''}${bold?' bold':''}" style="position:sticky;left:0;z-index:1;background:${stickyBg}">${label}</td>`;
   cols.forEach(col => {
     const v = cellVal(arr || zeroArr(), col, opts);
     const bg = col.type === 'year' ? 'background:#f0f4f8;' : col.type === 'total' ? 'background:var(--alt);' : '';
     const fw = col.type === 'year' ? 'font-weight:700;' : '';
     r += `<td class="num${bold?' bold':''}${v<0?' c-red':''}${cc?' c-'+cc:''}" style="${bg}${fw}">${fmt(v, false)}</td>`;
   });
   r += '</tr>';
   return r;
 }
 function mkSec(label, color) { return `<tr class="sec-row"><td colspan="999" style="background:${color||'#032D60'}">${label}</td></tr>`; }
 function addLineGrouped(section) {
   return `<tr class="add-line-row" data-section="${section}"><td colspan="999" style="padding:2px 8px;border:none;background:transparent"><button class="add-line-btn" data-section="${section}" style="font-size:10px;color:var(--accent);background:none;border:1px dashed var(--bdr2);border-radius:4px;padding:3px 10px;cursor:pointer;width:100%;text-align:left;opacity:.72">+ Add calculated line here</button></td></tr>`;
 }
 const lineRow = (row, asOutflow=false) => mkRow(row.line.name || 'Custom', asOutflow ? row.arr.map(v => Math.abs(Number(v)||0)) : row.arr, { ind:true, color:row.line.color || 'amber' });
 const rowOrder = getRowOrder(which);
 let body = '';
 function plSeg(id) {
   switch(id) {
     case 'pl_income_sec': return mkSec('INCOME', 'var(--th-income)');
     case 'pl_rev_cats': return Object.entries(cv.revCats || {}).map(([cat, arr]) => mkRow(cat, arr, {ind:true})).join('');
     case 'pl_penalties': return mkRow('Penalties', cv.penArr || zeroArr(), {ind:true, color:'amber'});
     case 'pl_adj_inc': return (cv.adjIncomeRows || []).map(ac => {
       const sign = ac.adj && ac.adj.sign === 'add' ? 1 : -1;
       return mkRow((ac.adj.name||'Adj')+' ('+ac.adj.pct+'%)', (ac.arr||zeroArr()).map(v=>sign*(Number(v)||0)), {ind:true,color:'amber'});
     }).join('');
     case 'pl_total_income': return rpt.plIncomeCustom.map(r => lineRow(r)).join('') + addLineGrouped('income_section') + mkRow('Total Income', rpt.totalIncomePL, {bold:true, color:'teal'});
     case 'pl_cogs_sec': return mkSec('COGS', 'var(--th-cogs)');
     case 'pl_cost_cats': return (cv.userCats || []).map(cat => mkRow(cat, ((cv.costCat || {})[cat] || zeroArr()).map(v=>-(Number(v)||0)), {ind:true})).join('');
     case 'pl_contingency': return mkRow('Contingency ('+CFG.con+'%)', ((cv.costCat || {})['Contingency'] || zeroArr()).map(v=>-(Number(v)||0)), {ind:true});
     case 'pl_adj_cost': return (cv.adjCostRows || []).map(ac => mkRow((ac.adj.name||'Adj')+' ('+ac.adj.pct+'%)', (ac.arr || zeroArr()).map(v=>-(ac.adj.sign==='add'?(Number(v)||0):-(Number(v)||0))), {ind:true,color:'amber'})).join('');
     case 'pl_total_cogs': return rpt.plCogsCustom.map(r => lineRow(r)).join('') + addLineGrouped('cogs_section') + mkRow('Total COGS', rpt.totalCogsPL, {bold:true});
     case 'pl_gross_profit': return mkRow('Gross Profit', rpt.grossProfitPL, {bold:true, color:'blue'}) + rpt.plAfterGPCustom.map(r => lineRow(r)).join('') + addLineGrouped('after_gross_profit');
     case 'pl_bond_sec': return mkSec('BOND FEES', 'var(--th-bond)');
     case 'pl_cb_fee': return mkRow('Compliance Bond Fee', (cv.finExpCB || zeroArr()).map(v=>-(Number(v)||0)), {ind:true, color:'amber'});
     case 'pl_ub_fee': return mkRow('Upfront Bond Fee', (cv.finExpUB || zeroArr()).map(v=>-(Number(v)||0)), {ind:true, color:'amber'});
     case 'pl_profit_bopex': return mkRow('Profit before OPEX / Interest', rpt.profitBeforeOpexPL, {bold:true, color:'blue'}) + rpt.plAfterProfitCustom.map(r => lineRow(r)).join('') + addLineGrouped('after_profit_b_opex');
     case 'pl_opex_sec': return mkSec('OPEX & INTEREST', 'var(--th-opex)');
     case 'pl_opex_buf': return mkRow('OPEX Buffer ('+(CFG.opxbuf??30)+'% of Revenue)', (cv.opexBuffer || zeroArr()).map(v=>-(Number(v)||0)), {ind:true, color:'amber'});
     case 'pl_hedging': return mkRow('Hedging Adjustment (monthly)', (cv.hedgingArr || zeroArr()).map(v=>-(Number(v)||0)), {ind:true});
     case 'pl_fin_cost': return mkRow('Financing Cost (on neg. CF)', (cv.finCostArr || zeroArr()).map(v=>-(Number(v)||0)), {ind:true, color:'amber'});
     case 'pl_ebit': return mkRow('EBIT', rpt.ebitPL, {bold:true, color:'blue'}) + rpt.plAfterEbitCustom.map(r => lineRow(r)).join('') + addLineGrouped('after_ebit');
     case 'pl_ebitda': return mkRow('EBITDA', rpt.ebitdaPL, {bold:true, color:'teal'});
     default: return '';
   }
 }
 function cfSeg(id) {
   switch(id) {
     case 'cf_in_sec': return mkSec('INFLOWS', 'var(--th-cf-in)');
     case 'cf_rev_cats': return Object.entries(cv.revCatsCF || {}).map(([cat, arr]) => mkRow(cat, arr, {ind:true})).join('');
     case 'cf_penalties': return mkRow('Penalties', cv.penArrCF || zeroArr(), {ind:true,color:'amber'});
     case 'cf_adj_inc': return rpt.cfAdjIncomeRows.map(({ac,arr}) => mkRow((ac.adj.name||'Adj')+' ('+ac.adj.pct+'%)', arr, {ind:true,color:'amber'})).join('');
     case 'cf_total_in': return rpt.cfInCustomRows.map(r => lineRow(r)).join('') + addLineGrouped('cf_after_inflows') + mkRow('Total Inflows', rpt.totalInflowArr, {bold:true});
     case 'cf_out_sec': return mkSec('OUTFLOWS', 'var(--th-cf-out)');
     case 'cf_cost_cats': return (cv.userCats || []).map(c => mkRow(c, ((cv.costCatCF || {})[c] || zeroArr()), {ind:true})).join('');
     case 'cf_contingency': return mkRow('Contingency', ((cv.costCatCF || {})['Contingency'] || zeroArr()), {ind:true});
     case 'cf_adj_cost': return rpt.cfAdjCostRows.map(({ac,arr}) => mkRow((ac.adj.name||'Adj')+' ('+ac.adj.pct+'%)', arr, {ind:true,color:'amber'})).join('');
     case 'cf_opex_buf': return mkRow('OPEX Buffer ('+(CFG.opxbuf??30)+'%)', cv.opexBufferCF || zeroArr(), {ind:true,color:'amber'});
     case 'cf_cb_fee': return mkRow('Compliance Bond Fee', cv.finExpCB || zeroArr(), {ind:true,color:'amber'});
     case 'cf_ub_fee': return mkRow('Upfront Bond Fee', cv.finExpUB || zeroArr(), {ind:true,color:'amber'});
     case 'cf_hedging': return mkRow('Hedging', cv.hedgingArr || zeroArr(), {ind:true,color:'purple'});
     case 'cf_fin_cost': return mkRow('Financing Cost', cv.finCostArr || zeroArr(), {ind:true,color:'amber'});
     case 'cf_total_out': return rpt.cfOutCustomRows.map(r => lineRow(r, true)).join('') + addLineGrouped('cf_after_outflows') + mkRow('Total Outflows', rpt.totalOutflowArr, {bold:true});
     case 'cf_net_sec': return mkSec('NET CASH FLOW FROM OPERATIONS', 'var(--th-bond)');
     case 'cf_net_cf': return mkRow('NET CASH FLOW From OPERATIONS', rpt.netCFArr, {bold:true,color:'blue'});
     case 'cf_accum': return mkRow('Accumulated cash flow', rpt.accumCFArr, {bold:true,color:'purple', acc:true});
     case 'cf_npv_sec': return '';
     case 'cf_npv': return '';
     default: return '';
   }
 }
 rowOrder.forEach(id => { body += which === 'pl' ? plSeg(id) : cfSeg(id); });
 document.getElementById(tblId).innerHTML = makeHead() + '<tbody>' + body + '</tbody>';
...
function renderDashboard() {
 const cv = compute();
 const rpt = buildReportFigures(cv);
 const tR = rSum(rpt.totalIncomePL), tC = -rSum(rpt.totalCogsPL);
 const tG = rSum(rpt.grossProfitPL);

 // Dashboard now follows the same Cash Flow logic used in the CF table.
 // This keeps the dashboard in sync with visible inflows/outflows, including VAT,
 // adjustments, bond fees, hedging, financing cost and CF custom lines.
 const zeroArr = () => new Array(PM()).fill(0);
 const sumRows = rows => {
   const out = zeroArr();
   rows.filter(Boolean).forEach(arr => arr.forEach((v,i)=>{ out[i] += Number(v)||0; }));
   return out;
 };
 const cfCustom = sections => customLines
   .filter(l => l.showInCF && sections.includes(l.section) && l.formula && l.formula.trim())
   .map(l => evalCustomLineArr(l, cv));
 const cfAdjIncomeRows = (cv.adjIncomeRows||[]).map(ac => (ac.arrCF || ac.arr || zeroArr()).map(v => (ac.adj.sign === 'add' ? 1 : -1) * (Number(v)||0)));
 const cfAdjCostRows = (cv.adjCostRows||[]).map(ac => (ac.arrCF || ac.arr || zeroArr()).map(v => Math.abs(Number(v)||0)));
 const dashInflow = sumRows([
   ...Object.values(cv.revCatsCF || {}),
   cv.penArrCF || zeroArr(),
   ...cfAdjIncomeRows,
   cv.vatIn || zeroArr(),
   ...cfCustom(['income_section','cf_after_inflows'])
 ]);
 const dashOutflow = sumRows([
   ...((cv.userCats||[]).map(c => (cv.costCatCF||{})[c] || zeroArr())),
   (cv.costCatCF||{})['Contingency'] || zeroArr(),
   ...cfAdjCostRows,
   cv.opexBufferCF || zeroArr(),
   ...
   cv.finCostArr || zeroArr(),
   cv.vatOut || zeroArr(),
   ...cfCustom(['cogs_section','cf_after_outflows'])
 ]);
 const dashNetCF = rpt.netCFArr;
 const dashAccumCF = rpt.accumCFArr;
 const tN = rSum(dashNetCF);
 ...
 ['EBIT', '$'+fmtK(rSum(rpt.ebitPL)), rSum(rpt.ebitPL) >= 0 ? '#2E844A' : '#BA0517'],
 ...
 const dI = isA ? aggY(rpt.totalIncomePL) : rpt.totalIncomePL;
 const dC = isA ? aggY(rpt.totalCogsPL.map(v=>-v)) : rpt.totalCogsPL.map(v=>-v);
 const dN = isA ? aggY(dashNetCF) : dashNetCF;
 const dA = isA ? aggAccY(dashAccumCF) : dashAccumCF;
 ...
 const dGP = isA ? aggY(rpt.grossProfitPL) : rpt.grossProfitPL;
 const dEB = isA ? aggY(rpt.ebitPL) : rpt.ebitPL;
