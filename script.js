...
function renderDashboard() {
 ...
 function drawAxes(ctx, w, h, pad, min, max, percent) {
  ...
  ctx.save(); ctx.font = '10px Arial, sans-serif'; ctx.textAlign = 'right'; ctx.textBaseline = 'middle'; ctx.lineWidth = 1;
  for (let i = 0; i <= 4; i++) {
 ...
 function drawLegend(ctx, series, x, y) {
  ctx.save(); ctx.font = '11px Arial, sans-serif'; ctx.textBaseline = 'middle'; let cx = x;
  series.forEach(s => { const col = typeof s.color === 'function' ? s.color(1) : s.color; ctx.fillStyle = col; ctx.fillRect(cx, y - 5, 10, 10); ctx.fillStyle = '#475569'; ctx.fillText(s.label, cx + 14, y); cx += ctx.measureText(s.label).width + 44; }); ctx.restore();
 }
 function drawBarChart(id, series, labels, opts) {
  opts = opts || {}; const surface = getCanvas(id, opts.height || 200); if (!surface) return; const { ctx, w, h } = surface;
  const pad = { t: 28, r: 18, b: 44, l: 70 };
  const allVals = series.flatMap(s => s.data || []); const rg = niceRange(allVals, true); const ax = drawAxes(ctx, w, h, pad, rg.min, rg.max, !!opts.percent);
  const n = Math.max(1, labels.length), groupW = ax.plotW / n; const barW = Math.max(5, Math.min(24, (groupW * 0.72) / Math.max(1, series.length))); const totalBarW = barW * series.length;
  labels.forEach((lbl, i) => { const groupX = pad.l + i * groupW + groupW / 2; series.forEach((s, si) => { const raw = (s.data || [])[i] || 0; const col = typeof s.color === 'function' ? s.color(raw) : s.color; const x = groupX - totalBarW / 2 + si * barW; const y = ax.toY(raw); const top = Math.min(y, ax.zeroY); const bh = Math.max(1, Math.abs(ax.zeroY - y)); ctx.fillStyle = col; ctx.globalAlpha = s.opacity || 1; ctx.fillRect(x + 1, top, Math.max(2, barW - 2), bh); ctx.globalAlpha = 1; }); const step = isA ? 1 : Math.max(1, Math.ceil(labels.length / 8)); if (i % step === 0 || i === labels.length - 1) { ctx.save(); ctx.fillStyle = '#6B7280'; ctx.font = '10px Arial, sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'top'; if (!isA && labels.length > 10) { ctx.translate(groupX, h - pad.b + 13); ctx.rotate(-Math.PI / 7); ctx.fillText(lbl, 0, 0); } else ctx.fillText(lbl, groupX, h - pad.b + 12); ctx.restore(); } });
  drawLegend(ctx, series, pad.l, 13);
 }
 function drawLineChart(id, series, labels, opts) {
  opts = opts || {}; const surface = getCanvas(id, opts.height || 175); if (!surface) return; const { ctx, w, h } = surface; const pad = { t: 28, r: 20, b: 42, l: 70 };
  const allVals = series.flatMap(s => s.data || []); const rg = niceRange(allVals, false); const ax = drawAxes(ctx, w, h, pad, rg.min, rg.max, !!opts.percent); const n = Math.max(1, labels.length); const toX = i => pad.l + (n === 1 ? ax.plotW / 2 : i / (n - 1) * ax.plotW);
  series.forEach(s => { ctx.save(); ctx.beginPath(); (s.data || []).forEach((v,i)=>{ const x=toX(i), y=ax.toY(v||0); if(i===0) ctx.moveTo(x,y); else ctx.lineTo(x,y); }); ctx.strokeStyle=s.color; ctx.lineWidth=s.width||2.5; ctx.stroke(); const step=Math.max(1,Math.ceil(n/8)); (s.data||[]).forEach((v,i)=>{ if(i%step===0||i===n-1){ ctx.beginPath(); ctx.arc(toX(i), ax.toY(v||0), 3,0,Math.PI*2); ctx.fillStyle=s.color; ctx.fill(); }}); ctx.restore(); });
  const step=Math.max(1,Math.ceil(n/8)); ctx.fillStyle='#6B7280'; ctx.font='10px Arial, sans-serif'; ctx.textAlign='center'; ctx.textBaseline='top'; labels.forEach((lbl,i)=>{ if(i%step===0||i===n-1) ctx.fillText(lbl,toX(i),h-pad.b+12); }); drawLegend(ctx, series, pad.l, 13);
 }
...
(function _formulaBasedExcelExportOnly(){
  ...
  function _buildFormulaWorkbook(parsed, type, sheetTitle){
    ...
    function rowsInSection(sectionKey, endRow){
      return dataRowsMeta.filter(m => m.excelRow < endRow && _labelKey(m.section) === sectionKey && !_isResultLabel(m.src.label, type) && !m.src.isInfo);
    }
